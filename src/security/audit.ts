...
import type { ConfigFileSnapshot, UAGENTConfig } from "../config/config.js";
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your UAGENT state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your UAGENT state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.UAGENT_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.UAGENT_GATEWAY_PASSWORD);
...
export function collectLoggingFindings(cfg: UAGENTConfig): SecurityAuditFinding[] {
...
export function collectElevatedFindings(cfg: UAGENTConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExecRuntimeFindings(cfg: UAGENTConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: UAGENTConfig): string[] {
