...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("uagent status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("uagent security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set UAGENT_GATEWAY_TOKEN/UAGENT_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("uagent security audit --deep")}".`,
    });
  }
  return findings;
}
