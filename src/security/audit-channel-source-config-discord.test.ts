...
function stubDiscordPlugin(params: {
  resolveAccount: (cfg: UAGENTConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: UAGENTConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: UAGENTConfig) => boolean;
}): ChannelPlugin {
  return {
    id: "discord",
    meta: {
...
describe("security audit channel source-config fallback discord", () => {
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    ...
    const resolvedConfig: UAGENTConfig = {
      commands: { native: true },
      channels: {
        discord: {
          enabled: true,
