...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "UAGENT_ENV_TEST_A";
    const keyB = "UAGENT_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "UAGENT_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "UAGENT_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "UAGENT_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "UAGENT_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "UAGENT_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "UAGENT_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "UAGENT_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "uagent-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousUAGENTHome = process.env.UAGENT_HOME;
    const previousStateDir = process.env.UAGENT_STATE_DIR;
    const previousBundledDir = process.env.UAGENT_BUNDLED_PLUGINS_DIR;
    process.env.UAGENT_HOME = "/srv/uagent-home";
    process.env.UAGENT_STATE_DIR = "/srv/uagent-state";
    process.env.UAGENT_BUNDLED_PLUGINS_DIR = "/srv/uagent-bundled";
    ...
    try {
      ...
      expect(env.UAGENT_HOME).toBeUndefined();
      expect(env.UAGENT_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.UAGENT_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("UAGENT_HOME", previousUAGENTHome);
      restoreEnvKey("UAGENT_STATE_DIR", previousStateDir);
      restoreEnvKey("UAGENT_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { UAGENT_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processUAGENTHome: process.env.UAGENT_HOME,
          processBundledDir: process.env.UAGENT_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.UAGENT_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processUAGENTHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.UAGENT_HOME).toBe("/srv/uagent-home");
    } finally {
      restoreEnvKey("UAGENT_HOME", previousUAGENTHome);
    }
  });
