...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".uagent");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.UAGENT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$UAGENT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via UAGENT_STATE_DIR.
