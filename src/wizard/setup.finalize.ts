...
export async function finalizeSetupWizard(
    ...
    if (gatewayHealth.healthError) {
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("uagent gateway run")}`,
          `Or rerun with: ${formatCliCommand("uagent onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("uagent onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.uagent.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: $UAGENT_CONFIG_PATH (default: ~/.uagent/uagent.json) under gateway.auth.token, or in UAGENT_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("uagent config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("uagent doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("uagent dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control UAGENT."
            : "Copy/paste this URL in a browser on this machine to control UAGENT.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("uagent dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.uagent.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.uagent.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("uagent configure --section web")}`,
          "",
          "Docs: https://docs.uagent.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("uagent configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.uagent.ai/tools/web"}`,
          "Docs: https://docs.uagent.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("uagent configure --section web")}`,
          "",
          "Docs: https://docs.uagent.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://uagent.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control UAGENT."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control UAGENT.",
  );
