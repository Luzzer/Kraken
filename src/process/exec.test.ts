...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        UAGENT_BASE_ENV: "base",
        UAGENT_TO_REMOVE: undefined,
      },
      env: {
        UAGENT_TEST_ENV: "ok",
      },
    ...
    expect(resolved.UAGENT_BASE_ENV).toBe("base");
    expect(resolved.UAGENT_TEST_ENV).toBe("ok");
    expect(resolved.UAGENT_TO_REMOVE).toBeUndefined();
    expect(resolved.UAGENT_CLI).toBe(UAGENT_CLI_ENV_VALUE);
  });
