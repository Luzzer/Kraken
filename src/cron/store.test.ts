...
const { makeStorePath } = createCronStoreHarness({ prefix: "uagent-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses UAGENT_HOME for tilde expansion", () => {
    vi.stubEnv("UAGENT_HOME", "/srv/uagent-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/uagent-home", "cron", "jobs.json"));
  });
});
