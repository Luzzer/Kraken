...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<UAGENTConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<UAGENTConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: UAGENTConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: UAGENTConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: UAGENTConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: UAGENTConfig, result: UAGENTConfig): UAGENTConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: UAGENTConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as UAGENTConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with uagent configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "UAGENT will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.uagent.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — UAGENT will store a reference instead of the API key.",
