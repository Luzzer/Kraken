...
export type PromptDefaultModelResult = { model?: string; config?: UAGENTConfig };
...
function resolveConfiguredModelRaw(cfg: UAGENTConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: UAGENTConfig): string[] {
...
export function applyModelAllowlist(cfg: UAGENTConfig, models: string[]): UAGENTConfig {
