...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.uagentCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.uagentCanvasA2UIAction &&
        typeof window.uagentCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.uagentSendUserAction === "function";
  ...
  window.addEventListener("uagent:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android UAGENT node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.uagentSendUserAction === "function"
        ? window.uagentSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.UAGENT_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.UAGENT_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>UAGENT Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
