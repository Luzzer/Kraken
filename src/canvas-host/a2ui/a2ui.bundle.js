...
var UAGENTA2UIHost = class extends i$6 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      box-sizing: border-box;
      padding:
        var(--uagent-a2ui-inset-top, 0px)
        var(--uagent-a2ui-inset-right, 0px)
        var(--uagent-a2ui-inset-bottom, 0px)
        var(--uagent-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--uagent-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--uagent-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--uagent-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--uagent-a2ui-empty-top, var(--uagent-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["uagent:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["uagent:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		globalThis.__uagentLastA2UIAction = userAction;
		const handler = globalThis.webkit?.messageHandlers?.uagentCanvasA2UIAction ?? globalThis.uagentCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.uagentCanvasA2UIAction) handler.postMessage(JSON.stringify({ userAction }));
...
if (!customElements.get("uagent-a2ui-host")) customElements.define("uagent-a2ui-host", UAGENTA2UIHost);
