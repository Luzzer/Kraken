...
describe("canvas host state dir defaults", () => {
  ...
  it("uses UAGENT_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("uagent-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("UAGENT Canvas");
      } finally {
        await handler.close();
      }
    });
