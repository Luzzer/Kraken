...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.UAGENT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*uagent-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "uagent";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "uagent");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "uagent-gw");
  ...
  records.push(`_uagent-gw._tcp IN PTR ${instanceLabel}._uagent-gw._tcp`);
  records.push(`${instanceLabel}._uagent-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._uagent-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; uagent-content-hash: ${contentHash}\n${contentBody}`;
}
