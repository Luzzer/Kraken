...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred UAGENT tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideUAGENT = path.join(os.tmpdir(), "outside-uagent", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
