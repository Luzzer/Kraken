...
import { resolvePreferredUAGENTTmpDir } from "../tmp-uagent-dir.js";
...
const whatsappChunkConfig: UAGENTConfig = {
  channels: { whatsapp: { textChunkLimit: 4000 } },
};

const expectedPreferredTmpRoot = resolvePreferredUAGENTTmpDir();
...
describe("deliverOutboundPayloads", () => {
  ...
  it("chunks direct adapter text and preserves delivery overrides across sends", async () => {
    ...
    const results = await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 2 } } } as UAGENTConfig,
      channel: "matrix",
      to: "!room",
      accountId: "default",
  ...
  it("uses adapter-provided formatted senders and scoped media roots when available", async () => {
    ...
    const textResults = await deliverOutboundPayloads({
      cfg: { channels: { line: {} } } as UAGENTConfig,
      channel: "line",
      to: "U123",
      accountId: "default",
    ...
    expect(
      sendFormattedMediaCall?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".uagent", "workspace-work")),
      ),
    ).toBe(true);
    expect(sendMedia).not.toHaveBeenCalled();
  });

  it("includes UAGENT tmp root in signal mediaLocalRoots", async () => {
  ...
  it("forwards audioAsVoice through generic plugin media delivery", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as UAGENTConfig,
      channel: "matrix",
  ...
  it("includes UAGENT tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes UAGENT tmp root in imessage mediaLocalRoots", async () => {
