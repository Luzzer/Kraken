...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.UAGENT_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-current-bindings-"));
    process.env.UAGENT_STATE_DIR = testStateDir;
