...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: UAGENTConfig | null = null;
  ...
  get(key: string, cfg: UAGENTConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: UAGENTConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: UAGENTConfig): void {
