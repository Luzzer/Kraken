...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-dotenv-test-"));
  const cwdDir = path.join(base, "cwd");
  const stateDir = path.join(base, "state");
  process.env.UAGENT_STATE_DIR = stateDir;
...
describe("loadDotEnv", () => {
  it("loads ~/.uagent/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.uagent/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".uagent");
        process.env.UAGENT_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "uagent", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "UAGENT_STATE_DIR=./evil-state",
            "UAGENT_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.UAGENT_STATE_DIR).toBe(stateDir);
        expect(process.env.UAGENT_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "UAGENT_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "UAGENT_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "UAGENT_LIVE_GEMINI_KEY=sk-gemini-live",
            "UAGENT_LIVE_OPENAI_KEY=sk-openai-live",
            "UAGENT_GATEWAY_TOKEN=attacker-token",
            "UAGENT_GATEWAY_PASSWORD=attacker-password",
            "UAGENT_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
        ...
        delete process.env.UAGENT_LIVE_ANTHROPIC_KEY;
        delete process.env.UAGENT_LIVE_ANTHROPIC_KEYS;
        delete process.env.UAGENT_LIVE_GEMINI_KEY;
        delete process.env.UAGENT_LIVE_OPENAI_KEY;
        delete process.env.UAGENT_GATEWAY_TOKEN;
        delete process.env.UAGENT_GATEWAY_PASSWORD;
        delete process.env.UAGENT_GATEWAY_SECRET;
        ...
        expect(process.env.UAGENT_LIVE_ANTHROPIC_KEY).toBeUndefined();
        expect(process.env.UAGENT_LIVE_ANTHROPIC_KEYS).toBeUndefined();
        expect(process.env.UAGENT_LIVE_GEMINI_KEY).toBeUndefined();
        expect(process.env.UAGENT_LIVE_OPENAI_KEY).toBeUndefined();
        expect(process.env.UAGENT_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.UAGENT_GATEWAY_PASSWORD).toBeUndefined();
        expect(process.env.UAGENT_GATEWAY_SECRET).toBeUndefined();
      });
    });
  });

  it("blocks UAGENT_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.UAGENT_STATE_DIR).toBeUndefined();
        expect(process.env.UAGENT_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (UAGENT_AGENT_DIR, UAGENT_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, UAGENT_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "UAGENT_AGENT_DIR=./evil-agent",
            `UAGENT_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "UAGENT_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );

        delete process.env.UAGENT_AGENT_DIR;
        delete process.env.UAGENT_BUNDLED_PLUGINS_DIR;
        delete process.env.PI_CODING_AGENT_DIR;
        delete process.env.UAGENT_OAUTH_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.UAGENT_AGENT_DIR).toBeUndefined();
        expect(process.env.UAGENT_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.UAGENT_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks UAGENT_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "UAGENT_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.UAGENT_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.UAGENT_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "UAGENT_PINNED_PYTHON=./attacker-python",
            "UAGENT_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.UAGENT_PINNED_PYTHON;
        delete process.env.UAGENT_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.UAGENT_PINNED_PYTHON).toBeUndefined();
        expect(process.env.UAGENT_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "UAGENT_BUNDLED_HOOKS_DIR=./attacker-hooks",
            "UAGENT_BUNDLED_PLUGINS_DIR=./attacker-plugins",
            "UAGENT_BUNDLED_SKILLS_DIR=./attacker-skills",
          ].join("\n"),
        );

        delete process.env.UAGENT_BUNDLED_HOOKS_DIR;
        delete process.env.UAGENT_BUNDLED_PLUGINS_DIR;
        delete process.env.UAGENT_BUNDLED_SKILLS_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.UAGENT_BUNDLED_HOOKS_DIR).toBeUndefined();
        expect(process.env.UAGENT_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.UAGENT_BUNDLED_SKILLS_DIR).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "UAGENT_PINNED_PYTHON=/trusted/python",
            "UAGENT_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.UAGENT_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.UAGENT_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "UAGENT_LIVE_GEMINI_KEY=sk-gemini-live",
            "UAGENT_LIVE_OPENAI_KEY=sk-openai-live",
            "UAGENT_GATEWAY_TOKEN=trusted-token",
            "UAGENT_GATEWAY_PASSWORD=trusted-password",
            "UAGENT_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_API_KEY;
        ...
        expect(process.env.UAGENT_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.UAGENT_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.UAGENT_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.UAGENT_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.UAGENT_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.UAGENT_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.UAGENT_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "UAGENT_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  ...
  it("does not load gateway.env when UAGENT_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.UAGENT_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "uagent", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.UAGENT_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
      const legacyStateDir = path.join(base, ".clawdbot");
      process.env.HOME = base;
      delete process.env.UAGENT_STATE_DIR;
      delete process.env.UAGENT_TEST_FAST;
      await fs.mkdir(cwdDir, { recursive: true });
