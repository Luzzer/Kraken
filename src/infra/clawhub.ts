...
// Keep aligned with @uagent/plugin-package-contract ExternalPluginCompatibility.
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    process.env.UAGENT_CLAWHUB_CONFIG_PATH?.trim() ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    process.env.UAGENT_CLAWHUB_TOKEN?.trim() ||
