...
const GATEWAY_DEVICE_ID_HEADER = "x-uagent-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-uagent-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-uagent-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.UAGENT_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "UAGENT_APNS_RELAY_BASE_URL"
    : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or UAGENT_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.UAGENT_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require UAGENT_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.UAGENT_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
