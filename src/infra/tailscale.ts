...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.UAGENT_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for UAGENT. You can keep running the web gateway without it: \`${formatCliCommand("uagent gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
