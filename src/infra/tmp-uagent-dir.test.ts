...
import { POSIX_UAGENT_TMP_DIR, resolvePreferredUAGENTTmpDir } from "./tmp-uagent-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredUAGENTTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `uagent-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredUAGENTTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_UAGENT_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredUAGENTTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredUAGENTTmpDir", () => {
  it("prefers /tmp/uagent when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_UAGENT_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/uagent when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_UAGENT_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it.each([
    {
      name: "falls back to os.tmpdir()/uagent when /tmp/uagent is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/uagent when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/uagent exists but is not writable",
    ...
    {
      name: "falls back when /tmp/uagent is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/uagent is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/uagent is group/other writable",
  ...
  it("repairs existing /tmp/uagent permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_UAGENT_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_UAGENT_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_UAGENT_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/uagent after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_UAGENT_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_UAGENT_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_UAGENT_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_UAGENT_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback UAGENT temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "uagent");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredUAGENTTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_UAGENT_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback UAGENT temp dir/);
  });
});
