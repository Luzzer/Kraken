...
export const POSIX_UAGENT_TMP_DIR = "/tmp/uagent";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredUAGENTTmpDirOptions = {
...
export function resolvePreferredUAGENTTmpDir(
  options: ResolvePreferredUAGENTTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "uagent" : `uagent-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[uagent] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback UAGENT temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback UAGENT temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_UAGENT_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_UAGENT_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_UAGENT_TMP_DIR)) {
      return POSIX_UAGENT_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_UAGENT_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_UAGENT_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_UAGENT_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_UAGENT_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_UAGENT_TMP_DIR;
