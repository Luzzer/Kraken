 ...
 * returning control to the caller (typically `triggerUAGENTRestart` →
...
/**
 * Parse uagent gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("uagent")) {
...
/**
 * Windows: find listening PIDs on the port, then verify each is an uagent
 ...
 * Returns only PIDs that belong to uagent gateway processes (not the current process).
 */
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only uagent gateway processes.
