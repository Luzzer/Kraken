...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers UAGENT_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for UAGENT_HOME",
      env: { UAGENT_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when UAGENT_HOME is tilde-only and no fallback home exists",
      env: { UAGENT_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores UAGENT_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          UAGENT_HOME: "/srv/uagent-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { UAGENT_HOME: "/srv/uagent-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/uagent-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/uagent-home " },
      expected: "/srv/uagent-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/uagent-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of UAGENT_HOME", () => {
    expect(
