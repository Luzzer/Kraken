...
const DEFAULT_PACKAGE_NAME = "uagent";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["uagent", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-update-preflight-"));
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "uagent.mjs");
      ...
      const doctorStep = await runStep(
        step("uagent doctor", doctorArgv, gitRoot, { UAGENT_UPDATE_IN_PROGRESS: "1" }),
      );
