...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@uagent/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@uagent/matrix", id: "matrix", expected: true },
    { packageName: "@uagent/matrix", id: "@uagent/matrix", expected: true },
    { packageName: "@uagent/matrix", id: "signal", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@uagent/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@uagent__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "uagent-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "uagent-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "uagent-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked base directories", async () => {
    await withTempDir({ prefix: "uagent-install-safe-" }, async (parentDir) => {
