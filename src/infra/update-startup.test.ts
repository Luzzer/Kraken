...
vi.mock("./uagent-root.js", async () => {
  const actual = await vi.importActual<typeof import("./uagent-root.js")>("./uagent-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "uagent-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["UAGENT_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.UAGENT_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveUAGENTPackageRoot } = await import("./uagent-root.js"));
    ...
    vi.mocked(resolveUAGENTPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveUAGENTPackageRoot).mockResolvedValue("/opt/uagent");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/uagent/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
