...
import { ensureUAGENTCliOnPath } from "./path-env.js";
...
describe("ensureUAGENTCliOnPath", () => {
  const envKeys = [
    "PATH",
    "UAGENT_PATH_BOOTSTRAPPED",
    "UAGENT_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/uagent-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "uagent");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureUAGENTCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.UAGENT_PATH_BOOTSTRAPPED;
    delete process.env.UAGENT_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling uagent exists", () => {
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "uagent");
      ...
      if (envValue === undefined) {
        ...
        process.env.UAGENT_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/uagent-path/case-linuxbrew");
