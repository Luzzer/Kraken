...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/uagent-state", "devices")).toEqual({
      dir: path.join("/tmp/uagent-state", "devices"),
      pendingPath: path.join("/tmp/uagent-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/uagent-state", "devices", "paired.json"),
    });
  });
