...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevUAGENTHome = process.env.UAGENT_HOME;
    const prevHome = process.env.HOME;
    process.env.UAGENT_HOME = "/srv/uagent-home";
    process.env.HOME = "/home/other";
    const openClawHome = path.join(path.resolve("/srv/uagent-home"), "bin", "tool");
    ...
    try {
      ...
      if (prevUAGENTHome === undefined) {
        delete process.env.UAGENT_HOME;
      } else {
        process.env.UAGENT_HOME = prevUAGENTHome;
      }
