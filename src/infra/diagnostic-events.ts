...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __uagentDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__uagentDiagnosticEventsState) {
    globalStore.__uagentDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__uagentDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: UAGENTConfig): boolean {
