...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: UAGENTConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: UAGENTConfig) {
...
function resolveHeartbeatAgents(cfg: UAGENTConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: UAGENTConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: UAGENTConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: UAGENTConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
