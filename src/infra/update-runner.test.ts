...
const fixtureRootTracker = createSuiteTempRootTracker({ prefix: "uagent-update-" });
...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "uagent.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "uagent.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options?: { packageManager?: string }) {
    ...
    const pkg: Record<string, string> = { name: "uagent", version: "1.0.0" };
  ...
  async function seedGlobalPackageRoot(pkgRoot: string, version = "1.0.0") {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "uagent", version }),
      "utf-8",
    );
  ...
  async function createGlobalPackageFixture(rootDir: string) {
    ...
    const pkgRoot = path.join(nodeModules, "uagent");
  ...
  function createGlobalNpmUpdateRunner(params: {
    ...
    const baseInstallKey = "npm i -g uagent@latest --no-fund --no-audit --loglevel=error";
    const omitOptionalInstallKey =
      "npm i -g uagent@latest --omit=optional --no-fund --no-audit --loglevel=error";
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "uagent.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("bootstraps pnpm via npm when pnpm and corepack are unavailable", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${process.execPath} ${path.join(tempDir, "uagent.mjs")} doctor --non-interactive`,
      onCommand: (key, options) => {
        if (key === "pnpm --version") {
          const envPath = options?.env?.PATH ?? options?.env?.Path ?? "";
          if (envPath.includes("uagent-update-pnpm-")) {
  ...
  it("uses npm-bootstrapped pnpm for dev preflight when pnpm and corepack are missing", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "uagent.mjs")} doctor --non-interactive --fix`;
    ...
    const runCommand = async (
      ...
      if (
        key.startsWith(`git -C ${tempDir} worktree add --detach /tmp/uagent-update-preflight-`) &&
        key.endsWith(` /worktree ${upstreamSha}`)
      ) {
        return { stdout: `HEAD is now at ${upstreamSha}`, stderr: "", code: 0 };
      }
      if (
        key.startsWith("git -C /tmp/uagent-update-preflight-") &&
      ...
      if (
        key.startsWith(`git -C ${tempDir} worktree remove --force /tmp/uagent-update-preflight-`)
      ) {
        return { stdout: "", stderr: "", code: 0 };
      }
    ...
    expect(pnpmEnvPaths.some((value) => value.includes("uagent-update-pnpm-"))).toBe(true);
  });
  ...
  it("retries windows pnpm git installs with --ignore-scripts for dev updates", async () => {
    ...
    try {
      const runCommand = async (
        ...
        if (
          key.startsWith(
            `git -C ${tempDir} worktree add --detach /tmp/uagent-update-preflight-`,
          ) &&
          key.endsWith(` /worktree ${upstreamSha}`)
        ) {
          return { stdout: `HEAD is now at ${upstreamSha}`, stderr: "", code: 0 };
        ...
        if (key === "pnpm install") {
          if (options?.cwd?.includes(`${path.sep}uagent-update-preflight-`)) {
        ...
        if (
          key.startsWith(
            `git -C ${tempDir} worktree remove --force /tmp/uagent-update-preflight-`,
          )
        ) {
          return { stdout: "", stderr: "", code: 0 };
        }
  ...
  it("does not fall back to npm scripts when a pnpm repo cannot bootstrap pnpm", async () => {
    ...
    expect(calls.some((call) => call.startsWith("git -C /tmp/uagent-update-preflight-"))).toBe(
      false,
    );
  });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "uagent", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "uagent", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g uagent@latest --no-fund --no-audit --loglevel=error",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g uagent@beta --no-fund --no-audit --loglevel=error",
  ...
  it("updates global npm installs from the GitHub main package spec", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand:
        "npm i -g github:uagent/uagent#main --no-fund --no-audit --loglevel=error",
      tag: "main",
    });
  ...
  it("falls back to global npm update when git is missing from PATH", async () => {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: "npm i -g uagent@latest --no-fund --no-audit --loglevel=error",
      gitRootMode: "missing",
    ...
    expect(result.status).toBe("ok");
    expect(result.mode).toBe("npm");
    expect(calls).toContain("npm i -g uagent@latest --no-fund --no-audit --loglevel=error");
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "uagent");
    const staleDir = path.join(nodeModules, ".uagent-stale");
  ...
  it("fails global npm update when the installed version misses the requested correction", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand: "npm i -g uagent@2026.3.23-2 --no-fund --no-audit --loglevel=error",
      tag: "2026.3.23-2",
    });
    ...
    expect(calls).toContain("npm i -g uagent@2026.3.23-2 --no-fund --no-audit --loglevel=error");
  });

  it("fails global npm update when bundled runtime sidecars are missing after install", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand = "npm i -g uagent@latest --no-fund --no-audit --loglevel=error";
  ...
  it("uses UAGENT_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand =
      "npm i -g http://10.211.55.2:8138/uagent-next.tgz --no-fund --no-audit --loglevel=error";
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: expectedInstallCommand,
    ...
    await withEnvAsync(
      { UAGENT_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/uagent-next.tgz" },
      async () => {
        const result = await runWithCommand(runCommand, { cwd: pkgRoot });
        expect(result.status).toBe("ok");
      },
  ...
  it("updates global bun installs when detected", async () => {
    ...
    await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
      ...
      const { calls, runCommand } = createGlobalInstallHarness({
        pkgRoot,
        installCommand: "bun add -g uagent@latest",
        onInstall: async () => {
          await writeGlobalPackageVersion(pkgRoot);
        },
      });
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g uagent@latest")).toBe(true);
    });
  });

  it("rejects git roots that are not a uagent checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-uagent-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when uagent.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "uagent.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("uagent doctor entry");
  });
