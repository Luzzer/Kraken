...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.UAGENT_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("uagent-gw");
  ...
  it("suppresses ciao self-probe retry console noise while advertising", async () => {
    ...
    try {
      ...
      console.log(
        "[test._uagent-gw._tcp.local.] failed probing with reason: Error: Can't probe for a service which is announced already. Received announcing for service test._uagent-gw._tcp.local.. Trying again in 2 seconds!",
      );
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("uagent (UAGENT)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("uagent");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("uagent.local");
