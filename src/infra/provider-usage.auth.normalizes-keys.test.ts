...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "uagent-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".uagent");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".uagent", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      UAGENT_STATE_DIR: path.join(home, ".uagent"),
      ...env,
    };
