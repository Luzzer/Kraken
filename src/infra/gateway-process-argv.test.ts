...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\UAGENT\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/uagent/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/uagent/uagent.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/uagent/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/uagent/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the uagent executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\uagent.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/uagent-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/uagent-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\uagent-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/uagent/custom.js", "gateway"])).toBe(false);
