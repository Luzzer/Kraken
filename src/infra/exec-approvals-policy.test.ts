...
describe("exec approvals policy helpers", () => {
  ...
  it("explains stricter host security and ask precedence", () => {
    ...
    expect(summary.security).toMatchObject({
      ...
      host: "allowlist",
      effective: "allowlist",
      hostSource: "~/.uagent/exec-approvals.json defaults.security",
    ...
    expect(summary.ask).toMatchObject({
      ...
      host: "always",
      effective: "always",
      hostSource: "~/.uagent/exec-approvals.json defaults.ask",
      note: "more aggressive ask wins",
    });
    expect(summary.askFallback).toEqual({
      effective: "deny",
      source: "~/.uagent/exec-approvals.json defaults.askFallback",
    });
  });
  ...
  it("attributes host policy to wildcard agent entries before defaults", () => {
    ...
    expect(summary.security).toMatchObject({
      host: "allowlist",
      hostSource: "~/.uagent/exec-approvals.json agents.*.security",
    });
    expect(summary.ask).toMatchObject({
      host: "always",
      hostSource: "~/.uagent/exec-approvals.json agents.*.ask",
    });
    expect(summary.askFallback).toEqual({
      effective: "deny",
      source: "~/.uagent/exec-approvals.json agents.*.askFallback",
    });
  });
  ...
  it("reports askFallback from the UAGENT default when approvals omit it", () => {
    const summary = resolveExecPolicyScopeSummary({
      approvals: {
        version: 1,
        agents: {},
  ...
  it("avoids a duplicate default-agent scope when main only appears in approvals", () => {
    ...
    expect(snapshots[0]?.security).toMatchObject({
      host: "allowlist",
      hostSource: "~/.uagent/exec-approvals.json agents.main.security",
    });
    expect(snapshots[0]?.ask).toMatchObject({
      host: "always",
      hostSource: "~/.uagent/exec-approvals.json agents.main.ask",
    });
  });
