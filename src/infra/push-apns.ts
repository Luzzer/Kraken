...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open UAGENT to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "uagent.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.UAGENT_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.UAGENT_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set UAGENT_APNS_TEAM_ID and UAGENT_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.UAGENT_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.UAGENT_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.UAGENT_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set UAGENT_APNS_PRIVATE_KEY_P8 or UAGENT_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading UAGENT_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
