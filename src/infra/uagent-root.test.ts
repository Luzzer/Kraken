...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__uagent_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "uagent-root");
...
function setPackageRoot(root: string, name = "uagent") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./uagent-root.js").resolveUAGENTPackageRootSync,
  asyncResolver: typeof import("./uagent-root.js").resolveUAGENTPackageRoot,
  opts: Parameters<typeof import("./uagent-root.js").resolveUAGENTPackageRootSync>[0],
...
vi.mock("./uagent-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveUAGENTPackageRoot", () => {
  let resolveUAGENTPackageRoot: typeof import("./uagent-root.js").resolveUAGENTPackageRoot;
  let resolveUAGENTPackageRootSync: typeof import("./uagent-root.js").resolveUAGENTPackageRootSync;
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ resolveUAGENTPackageRoot, resolveUAGENTPackageRootSync } =
      await import("./uagent-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "uagent");
        const pkgRoot = path.join(project, "node_modules", "uagent");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "uagent");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "uagent.mjs")));
    ...
    {
      name: "falls through from a non-uagent moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-uagent");
    ...
    {
      name: "returns null for non-uagent package roots",
      setup: () => {
        const pkgRoot = fx("not-uagent");
        setPackageRoot(pkgRoot, "not-uagent");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "uagent.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "uagent");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveUAGENTPackageRootSync,
      resolveUAGENTPackageRoot,
      opts,
      expected,
    );
  });
