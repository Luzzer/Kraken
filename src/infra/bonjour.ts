...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.UAGENT_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "UAGENT";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(UAGENT\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.UAGENT_MDNS_HOSTNAME?.trim() || "uagent";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
