...
function candidateBinDirs(opts: EnsureUAGENTPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `uagent` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "uagent");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.UAGENT_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "uagent"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `uagent`
...
/**
 * Best-effort PATH bootstrap so skills that require the `uagent` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureUAGENTCliOnPath(opts: EnsureUAGENTPathOpts = {}) {
  if (isTruthyEnvValue(process.env.UAGENT_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.UAGENT_PATH_BOOTSTRAPPED = "1";
