...
export function triggerUAGENTRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.UAGENT_SYSTEMD_UNIT,
      process.env.UAGENT_PROFILE,
    );
  ...
  const label =
    process.env.UAGENT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.UAGENT_PROFILE);
