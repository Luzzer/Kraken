...
This directory contains hooks that ship with UAGENT. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.uagent/workspace`)
...
homepage: https://docs.uagent.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.uagent/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.uagent/uagent.json`:
...
2. Restart gateway: `pkill -9 -f 'uagent.*gateway' && pnpm uagent gateway`
3. Enable the hook: `uagent hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.uagent.ai/automation/hooks
