...
homepage: https://docs.uagent.ai/automation/hooks#command-logger
metadata:
  {
    "uagent":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.uagent/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.uagent/logs/commands.log ~/.uagent/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/uagent`:
   ```
   /home/username/.uagent/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
