...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["UAGENT_BUNDLED_HOOKS_DIR"]);
  ...
  describe("loadInternalHooks", () => {
    ...
    const expectNoCommandHookRegistration = async (cfg: UAGENTConfig) => {
    ...
    it("should treat missing hooks.internal.enabled as enabled (default-on)", async () => {
      ...
      for (const cfg of [
        {} satisfies UAGENTConfig,
        { hooks: {} } satisfies UAGENTConfig,
        { hooks: { internal: {} } } satisfies UAGENTConfig,
      ]) {
        const count = await loadInternalHooks(cfg, tmpDir);
        expect(count).toBe(0);
      }
