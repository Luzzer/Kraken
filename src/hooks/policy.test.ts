...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "uagent-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "uagent-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "uagent-bundled");
      const plugin = makeHookEntry("shared", "uagent-plugin");
      const managed = makeHookEntry("shared", "uagent-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("uagent-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "uagent-managed");
      const workspace = makeHookEntry("shared", "uagent-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "uagent-workspace");
      const second = makeHookEntry("shared", "uagent-workspace");
      second.hook.handlerPath = "/tmp/uagent-workspace/shared/handler-2.js";
