...
`;
    ...
    expect(parsed.uagent.emoji).toBe("💾");
    expect(parsed.uagent.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.uagent.emoji).toBe("📝");
    expect(parsed.uagent.events).toEqual(["command"]);
    expect(parsed.uagent.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.uagent.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"uagent": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"uagent": {"events": ["test"]}}');
  });
...
describe("resolveUAGENTMetadata", () => {
  it("extracts uagent metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveUAGENTMetadata(frontmatter);
  ...
  it("returns undefined when uagent key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        uagent: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with UAGENT" },
            { id: "npm", kind: "npm", package: "@uagent/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@uagent/hook");
  });
    ...
    "uagent":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with UAGENT" }],
      },
  }
---
...
`;
    ...
    const uagent = resolveUAGENTMetadata(frontmatter);
    expect(uagent).toBeDefined();
    expect(uagent?.emoji).toBe("💾");
    expect(uagent?.events).toEqual(["command:new", "command:reset"]);
    expect(uagent?.requires?.config).toEqual(["workspace.dir"]);
    expect(uagent?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const uagent = resolveUAGENTMetadata(frontmatter);
    expect(uagent?.emoji).toBe("disk");
    expect(uagent?.events).toEqual(["command:new"]);
  });
});
