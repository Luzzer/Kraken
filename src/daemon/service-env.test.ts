...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.UAGENT_GATEWAY_PORT).toBe("18789");
    expect(env.UAGENT_GATEWAY_TOKEN).toBeUndefined();
    expect(env.UAGENT_SERVICE_MARKER).toBe("uagent");
    expect(env.UAGENT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.UAGENT_SERVICE_VERSION).toBe("string");
    expect(env.UAGENT_SYSTEMD_UNIT).toBe("uagent-gateway.service");
    expect(env.UAGENT_WINDOWS_TASK_NAME).toBe("UAGENT Gateway");
    if (process.platform === "darwin") {
      expect(env.UAGENT_LAUNCHD_LABEL).toBe("ai.uagent.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", UAGENT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.UAGENT_SYSTEMD_UNIT).toBe("uagent-gateway-work.service");
    expect(env.UAGENT_WINDOWS_TASK_NAME).toBe("UAGENT Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.UAGENT_LAUNCHD_LABEL).toBe("ai.uagent.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through UAGENT_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", UAGENT_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.UAGENT_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits UAGENT_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".uagent"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", UAGENT_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".uagent-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", UAGENT_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".uagent"));
  });

  it("uses UAGENT_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", UAGENT_STATE_DIR: "/var/lib/uagent" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/uagent"));
  });

  it("expands ~ in UAGENT_STATE_DIR", () => {
    const env = { HOME: "/Users/test", UAGENT_STATE_DIR: "~/uagent-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/uagent-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { UAGENT_STATE_DIR: "C:\\State\\uagent" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\uagent");
  });
});
