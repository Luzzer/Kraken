...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.UAGENT_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    UAGENT_PROFILE: profile,
    UAGENT_GATEWAY_PORT: String(port),
    UAGENT_LAUNCHD_LABEL: resolvedLaunchdLabel,
    UAGENT_SYSTEMD_UNIT: systemdUnit,
    UAGENT_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    UAGENT_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    UAGENT_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    UAGENT_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = env.UAGENT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    UAGENT_GATEWAY_TOKEN: gatewayToken,
    UAGENT_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    UAGENT_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    UAGENT_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    UAGENT_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    UAGENT_LOG_PREFIX: "node",
    UAGENT_SERVICE_MARKER: NODE_SERVICE_MARKER,
    UAGENT_SERVICE_KIND: NODE_SERVICE_KIND,
    UAGENT_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    UAGENT_STATE_DIR: sharedEnv.stateDir,
    UAGENT_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.UAGENT_STATE_DIR;
  const configPath = env.UAGENT_CONFIG_PATH;
