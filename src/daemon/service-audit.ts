...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds UAGENT_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `uagent gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service UAGENT_GATEWAY_TOKEN does not match gateway.auth.token in uagent.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.UAGENT_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.UAGENT_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `uagent gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
