...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when UAGENT_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".uagent", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when UAGENT_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", UAGENT_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".uagent-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers UAGENT_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        UAGENT_PROFILE: "rescue",
        UAGENT_STATE_DIR: "C:\\State\\uagent",
      },
      expected: path.join("C:\\State\\uagent", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", UAGENT_PROFILE: "default" },
      expected: path.join("/home/test", ".uagent", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem UAGENT Gateway",
          "cd /d C:\\Projects\\uagent",
          "set NODE_ENV=production",
          "set UAGENT_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\uagent",
          environment: {
            NODE_ENV: "production",
            UAGENT_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\uagent\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\uagent\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\UAGENT Share\\node.exe" "\\\\fileserver\\UAGENT Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\UAGENT Share\\node.exe",
            "\\\\fileserver\\UAGENT Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from UAGENT_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ UAGENT_STATE_DIR: path.join(tmpDir, "custom-state") }),
