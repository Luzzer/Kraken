...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `uagent-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      UAGENT_LAUNCHD_LABEL: `ai.uagent.launchd-int-${testId}`,
      UAGENT_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
