...
describe("local media roots", () => {
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "uagent-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "uagent-agent-media-roots-state"),
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "uagent-flexible-media-roots-state"),
      cfg: {},
      shouldContainPictures: false,
    },
    {
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "uagent-messaging-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "uagent-messaging-fs-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
  ...
  it("keeps agent-scoped defaults even when mediaSources include file URLs and top-level paths", () => {
    const stateDir = path.join("/tmp", "uagent-file-url-media-roots-state");
    const picturesDir =
  ...
  it("includes the config media root when legacy state and config dirs diverge", () => {
    const homeRoot = path.join(os.tmpdir(), "uagent-legacy-home-test");
    const roots = buildMediaLocalRoots(
      path.join(homeRoot, ".clawdbot"),
      path.join(homeRoot, ".uagent"),
    );
    ...
    expectNormalizedRootsContain(roots, [
      ...
      path.join(homeRoot, ".uagent", "media"),
    ]);
  });
});
