...
describe("plugin activation boundary", () => {
  ...
  let browserHelpersPromise:
    | Promise<{
        ...
        DEFAULT_UAGENT_BROWSER_COLOR: typeof import("./plugin-sdk/browser-config.js").DEFAULT_UAGENT_BROWSER_COLOR;
        DEFAULT_UAGENT_BROWSER_PROFILE_NAME: typeof import("./plugin-sdk/browser-config.js").DEFAULT_UAGENT_BROWSER_PROFILE_NAME;
  ...
  function importBrowserHelpers() {
    browserHelpersPromise ??= Promise.all([
      ...
      DEFAULT_UAGENT_BROWSER_COLOR: config.DEFAULT_UAGENT_BROWSER_COLOR,
      DEFAULT_UAGENT_BROWSER_PROFILE_NAME: config.DEFAULT_UAGENT_BROWSER_PROFILE_NAME,
  ...
  it("does not load bundled plugins for config and env detection helpers", async () => {
    ...
    expect(
      isChannelConfigured({}, "irc", { IRC_HOST: "irc.example.com", IRC_NICK: "uagent" }),
    ).toBe(true);
  ...
  it("keeps browser helper imports cold and loads only narrow browser helper surfaces on use", async () => {
    ...
    expect(browser.DEFAULT_UAGENT_BROWSER_COLOR).toBe("#FF4500");
    expect(browser.DEFAULT_UAGENT_BROWSER_PROFILE_NAME).toBe("uagent");
    ...
    expect(browser.resolveProfile(resolved, "uagent")).toEqual(
