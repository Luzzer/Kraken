...
const createTempDir = () => tempDirs.make("uagent-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets UAGENT_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "uagent",
    });

    expect(env.UAGENT_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("uagent");
  });

  it("overrides pre-existing UAGENT_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      UAGENT_SHELL: "wrong",
    });
    expect(env.UAGENT_SHELL).toBe("acp-client");
  ...
  it("preserves UAGENT_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        UAGENT_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["UAGENT_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default UAGENT bridge", () => {
    ...
    expect(env.UAGENT_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.UAGENT_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "uagent",
        serverArgs: ["acp"],
        defaultServerCommand: "uagent",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("UAGENT_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "uagent", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "uagent", "dist", "entry.js");
    const shimPath = path.join(dir, "uagent.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\uagent\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/uagent-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/uagent-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/uagent-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
