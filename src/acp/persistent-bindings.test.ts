...
type ConfiguredBinding = NonNullable<UAGENTConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<UAGENTConfig>,
): UAGENTConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: UAGENTConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/uagent" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
