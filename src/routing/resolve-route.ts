...
function listAgents(cfg: UAGENTConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: UAGENTConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<UAGENTConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: UAGENTConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: UAGENTConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: UAGENTConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<UAGENTConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  UAGENTConfig,
  {
    bindingsRef: UAGENTConfig["bindings"];
    agentsRef: UAGENTConfig["agents"];
    sessionRef: UAGENTConfig["session"];
...
function resolveRouteCacheForConfig(cfg: UAGENTConfig): Map<string, ResolvedAgentRoute> {
