...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://uagent.ai");
  });
  ...
  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    ...
    startupRegistry.channels = [
      {
        pluginId: "uagent-weixin",
        plugin: { id: "uagent-weixin", meta: { aliases: ["weixin"] } },
        source: "test",
      },
    ] as never;
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["uagent-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("uagent-weixin");
  });
