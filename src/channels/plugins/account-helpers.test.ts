...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): UAGENTConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as UAGENTConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: UAGENTConfig) => string[];
  input: UAGENTConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: UAGENTConfig, expected: string) {
