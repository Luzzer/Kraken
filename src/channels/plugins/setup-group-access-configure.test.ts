...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: UAGENTConfig, policy: ChannelAccessPolicy) => UAGENTConfig;
  resolveAllowlist: (params: { cfg: UAGENTConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: UAGENTConfig; resolved: TResolved }) => UAGENTConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: UAGENTConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: UAGENTConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: UAGENTConfig, policy: ChannelAccessPolicy): UAGENTConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: UAGENTConfig, policy: ChannelAccessPolicy): UAGENTConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: UAGENTConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: UAGENTConfig; resolved: string[] }) => {
      calls.push("apply");
