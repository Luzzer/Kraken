...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir({ prefix: "uagent-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "uagent", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir({ prefix: "uagent-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "uagent", version: "2.3.4" });
  ...
  it("ignores non-uagent package and blank build-info versions", async () => {
    await withTempDir({ prefix: "uagent-version-" }, async (root) => {
  ...
  it("prefers UAGENT_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        UAGENT_VERSION: "9.9.9",
        UAGENT_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale UAGENT_VERSION for compatibility checks", () => {
    const previous = process.env.UAGENT_VERSION;
    const previousService = process.env.UAGENT_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.UAGENT_VERSION = "2026.3.25";
      process.env.UAGENT_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.UAGENT_VERSION = previous;
      process.env.UAGENT_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        UAGENT_VERSION: "2026.3.99",
        UAGENT_SERVICE_VERSION: "2026.3.98",
