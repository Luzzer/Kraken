...
import type { ConfigFileSnapshot } from "./types.uagent.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/uagent/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/uagent/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/uagent/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/uagent/nickserv-password.txt");
