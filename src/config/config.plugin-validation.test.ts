...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "uagent.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      UAGENT_STATE_DIR: path.join(suiteHome, ".uagent"),
      UAGENT_PLUGIN_MANIFEST_CACHE_MS: "10000",
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-config-plugin-validation-"));
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", async () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          UAGENT_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );
