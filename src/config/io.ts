...
import type { UAGENTConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  uagent config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  uagent config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: UAGENTConfig,
  pathSegments: string[],
): { changed: boolean; next: UAGENTConfig } {
...
function coerceConfig(value: unknown): UAGENTConfig {
...
function stampConfigVersion(cfg: UAGENTConfig): UAGENTConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: UAGENTConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer UAGENT (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as UAGENTConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): UAGENTConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as UAGENTConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(
    ...
    let cfgToWrite = persistCandidate as UAGENTConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as UAGENTConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.UAGENT_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.UAGENT_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.UAGENT_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.UAGENT_WATCH_SESSION === "string" &&
        deps.env.UAGENT_WATCH_SESSION.trim().length > 0
          ? deps.env.UAGENT_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.UAGENT_WATCH_COMMAND === "string" &&
        deps.env.UAGENT_WATCH_COMMAND.trim().length > 0
          ? deps.env.UAGENT_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `UAGENT_CONFIG_PATH` (and friends) are expected to work even
...
function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: UAGENTConfig;
  candidate: UAGENTConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: UAGENTConfig): UAGENTConfig {
...
export function loadConfig(): UAGENTConfig {
  const runtimeConfigSnapshot = getRuntimeConfigSnapshotState();
  ...
  // reparsing uagent.json on hot code paths.
  setRuntimeConfigSnapshotState(config);
  return getRuntimeConfigSnapshotState() ?? config;
}

export function getRuntimeConfig(): UAGENTConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<UAGENTConfig> {
