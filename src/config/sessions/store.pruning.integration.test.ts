...
// Keep integration tests deterministic: never read a real uagent.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "uagent-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.UAGENT_SESSION_CACHE_TTL_MS;
    process.env.UAGENT_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.UAGENT_SESSION_CACHE_TTL_MS;
    } else {
      process.env.UAGENT_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
