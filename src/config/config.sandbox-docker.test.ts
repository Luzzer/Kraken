...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("uagent-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "uagent-sandbox-browser-global" },
      agentBrowser: { network: "uagent-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("uagent-sandbox-browser-agent");
  });
