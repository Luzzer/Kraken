...
import { buildWebSearchProviderConfig, withTempHome, writeUAGENTConfig } from "./test-helpers.js";
import { UAGENTSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = UAGENTSchema.safeParse({
      $schema: "https://uagent.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://uagent.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = UAGENTSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = UAGENTSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not treat resolved-only gateway.bind aliases as source-literal legacy or invalid", async () => {
    await withTempHome(async (home) => {
      await writeUAGENTConfig(home, {
        gateway: { bind: "${UAGENT_BIND}" },
      });

      const prev = process.env.UAGENT_BIND;
      process.env.UAGENT_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(true);
        ...
        if (prev === undefined) {
          delete process.env.UAGENT_BIND;
        } else {
          process.env.UAGENT_BIND = prev;
        }
      }
    });
  });
