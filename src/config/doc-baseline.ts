...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.UAGENT_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveUAGENTPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    UAGENT_STATE_DIR: path.join(os.tmpdir(), "uagent-config-doc-baseline-state"),
    UAGENT_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
