...
function collectConfigEnvVarsByTarget(cfg?: UAGENTConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: UAGENTConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: UAGENTConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: UAGENTConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.UAGENT_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
