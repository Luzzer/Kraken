...
import { resetConfigRuntimeState, type UAGENTConfig } from "./config.js";

export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  resetConfigRuntimeState();
  try {
    return await withTempHomeBase(fn, { prefix: "uagent-config-" });
  } finally {
    resetConfigRuntimeState();
  }
}

export async function writeUAGENTConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.UAGENT_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected UAGENT_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeUAGENTConfig(home, config);
