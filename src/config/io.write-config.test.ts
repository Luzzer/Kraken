...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "uagent-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  function createBlueBubblesManifestRecord(): PluginManifestRecord {
    return {
      ...
      source: "/virtual/plugins/bluebubbles/uagent.plugin.json",
      manifestPath: "/virtual/plugins/bluebubbles/uagent.plugin.json",
      channelCatalogMeta: {
        id: "bluebubbles",
        label: "BlueBubbles",
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".uagent", "uagent.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".uagent", "logs", "config-audit.jsonl");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withSuiteHome(async (home) => {
        const stateDir = path.join(home, ".uagent");
  ...
  it("keeps writes inside an UAGENT_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".uagent", "uagent.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "uagent.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "uagent.json"), "utf-8"),
      ) as {
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withSuiteHome(async (home) => {
      ...
      const invalidConfig: UAGENTConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "uagent config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'uagent config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it("does not mutate caller config when unsetPaths is applied on first write", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".uagent", "uagent.json");
      const io = createConfigIO({
        env: {} as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("keeps env refs in arrays when appending entries", async () => {
    await withSuiteHome(async (home) => {
      ...
      await io.writeConfigFile(next as UAGENTConfig);
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(lines.length).toBeGreaterThan(0);
      await expect(
        fs.stat(path.join(home, ".uagent", "logs", "config-audit.jsonl")),
      ).resolves.toBeDefined();
      await expect(
        fs.stat(path.resolve("undefined", ".uagent", "logs", "config-audit.jsonl")),
      ).rejects.toThrow();
    });
  });
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withSuiteHome(async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        env: {
          UAGENT_WATCH_MODE: "1",
          UAGENT_WATCH_SESSION: "watch-session-1",
          UAGENT_WATCH_COMMAND: "gateway --force",
        } as NodeJS.ProcessEnv,
      });
