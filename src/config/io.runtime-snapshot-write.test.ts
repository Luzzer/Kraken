...
function createSourceConfig(): UAGENTConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): UAGENTConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  ...
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("uagent-config-runtime-source-", async () => {
  ...
  it("skips source projection for non-runtime-derived configs", async () => {
    await withTempHome("uagent-config-runtime-projection-shape-", async () => {
      ...
      const independentConfig: UAGENTConfig = {
        models: {
          providers: {
            openai: {
              baseUrl: "https://api.openai.com/v1",
  ...
  it("preserves source secret refs when writeConfigFile receives runtime-resolved config", async () => {
    await withTempHome("uagent-config-runtime-write-", async (home) => {
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("uagent-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".uagent", "uagent.json");
      const sourceConfig: UAGENTConfig = {
        models: {
          providers: {
      ...
      const nextRuntimeConfig: UAGENTConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("uagent-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".uagent", "uagent.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: UAGENTConfig = {
  ...
  it("notifies in-process write listeners with the refreshed runtime snapshot", async () => {
    await withTempHome("uagent-config-runtime-write-listener-", async (home) => {
      const configPath = path.join(home, ".uagent", "uagent.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, `${JSON.stringify({ gateway: { port: 18789 } }, null, 2)}\n`);

      const seen: Array<{ configPath: string; runtimeConfig: UAGENTConfig }> = [];
