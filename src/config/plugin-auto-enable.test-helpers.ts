...
export function makeTempDir(): string {
  return makeTrackedTempDir("uagent-plugin-auto-enable", tempDirs);
}

export function makeIsolatedEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  const rootDir = makeTempDir();
  return {
    UAGENT_STATE_DIR: path.join(rootDir, "state"),
    ...overrides,
  };
}
...
export function writePluginManifestFixture(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "uagent.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function makeRegistry(
  ...
  return {
    plugins: plugins.map((plugin) => ({
      ...
      manifestPath: `/fake/${plugin.id}/uagent.plugin.json`,
    })),
    diagnostics: [],
  };
}
