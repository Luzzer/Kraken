...
import { parseUAGENTVersion } from "./version.js";

async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-config-"));
  try {
    await run(home);
  } finally {
...
async function expectNoNewerVersionWarning(touchedVersion: string) {
  await withTempHome(async (home) => {
    ...
    expect(logger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining("Config was last written by a newer UAGENT"),
    );
...
describe("config io paths", () => {
  it("uses ~/.uagent/uagent.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".uagent", 19001);
  ...
  it("defaults to ~/.uagent/uagent.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".uagent", "uagent.json"));
    });
  });

  it("uses UAGENT_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { UAGENT_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".uagent", "uagent.json"));
    });
  });

  it("honors explicit UAGENT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".uagent", 20002, "custom.json");
      const io = createIoForHome(home, { UAGENT_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("does not warn when config was last touched by a same-base correction publish", async () => {
    const parsedVersion = parseUAGENTVersion(VERSION);
    if (!parsedVersion) {
