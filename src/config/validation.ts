...
import type { UAGENTConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.uagent.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: UAGENTConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: UAGENTConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: UAGENTConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = UAGENTSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as UAGENTConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: UAGENTConfig | null | undefined;
  ...
  const ensureCompatConfig = (): UAGENTConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native UAGENT config schema.
