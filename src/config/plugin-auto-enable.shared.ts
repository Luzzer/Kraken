...
function collectModelRefs(cfg: UAGENTConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: UAGENTConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: UAGENTConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: UAGENTConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: UAGENTConfig, plugin: PluginManifestRecord): boolean {
...
function collectCandidateChannelIds(cfg: UAGENTConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: UAGENTConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: UAGENTConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: UAGENTConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function configMayNeedPluginManifestRegistry(cfg: UAGENTConfig): boolean {
...
function isPluginExplicitlyDisabled(cfg: UAGENTConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: UAGENTConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isBuiltInChannelAlreadyEnabled(cfg: UAGENTConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
function registerPluginEntry(cfg: UAGENTConfig, pluginId: string): UAGENTConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
