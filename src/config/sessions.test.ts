...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ UAGENT_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-uagent",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-uagent#general");
  });
  ...
  it("derives session transcripts dir from UAGENT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { UAGENT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when UAGENT_STATE_DIR differs from stored paths", () => {
