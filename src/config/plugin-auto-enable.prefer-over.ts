...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.uagent) || !isRecord(entry.uagent.channel)) {
      continue;
    }
    const channel = entry.uagent.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: UAGENTConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: UAGENTConfig, pluginId: string) => boolean;
}): boolean {
