...
export function parseUAGENTVersion(raw: string | null | undefined): UAGENTVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeUAGENTVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseUAGENTVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameUAGENTStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseUAGENTVersion(a);
  const parsedB = parseUAGENTVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareUAGENTVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseUAGENTVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseUAGENTVersion(current);
  const parsedTouched = parseUAGENTVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameUAGENTStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareUAGENTVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: UAGENTVersion): number {
