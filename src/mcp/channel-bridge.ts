...
export class UAGENTChannelBridge {
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`uagent mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `uagent mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
