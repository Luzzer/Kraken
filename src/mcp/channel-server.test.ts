...
import { z } from "zod";
import { createUAGENTChannelMcpServer, UAGENTChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createUAGENTChannelMcpServer({
...
describe("uagent channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new UAGENTChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
