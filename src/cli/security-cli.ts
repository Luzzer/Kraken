...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["uagent security audit", "Run a local security audit."],
          ["uagent security audit --deep", "Include best-effort live Gateway probe checks."],
          ["uagent security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "uagent security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["uagent security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["uagent security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.uagent.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("UAGENT security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("uagent security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("uagent security audit --fix")}`));
        if (!fixResult) {
