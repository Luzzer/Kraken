...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as UAGENTConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: UAGENTConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
export const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/uagent-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: UAGENTConfig) =>
    invokeMock<
      [UAGENTConfig],
...
vi.mock("../plugins/enable.js", () => ({
  enablePluginInConfig: ((cfg: UAGENTConfig, pluginId: string) =>
    invokeMock<[UAGENTConfig, string], unknown>(
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as UAGENTConfig);
  readConfigFileSnapshot.mockImplementation(async () => {
    const config = loadConfig();
    return {
      path: "/tmp/uagent-config.json5",
      exists: true,
      raw: "{}",
      parsed: config,
      resolved: config,
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: UAGENTConfig }) => ({
    config,
    warnings: [],
