    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "uagent node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["uagent node status", "Check node host service status."],
          ["uagent node install", "Install the node host service."],
          ["uagent node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.uagent.ai/cli/node")}\n`,
    );
