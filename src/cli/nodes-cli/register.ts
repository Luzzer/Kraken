    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["uagent nodes status", "List known nodes with live status."],
          ["uagent nodes pairing pending", "Show pending node pairing requests."],
          [
            'uagent nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["uagent nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.uagent.ai/cli/nodes")}\n`,
    );
