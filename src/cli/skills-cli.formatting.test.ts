...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-bundled-skills-test-"));
    process.env.UAGENT_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
