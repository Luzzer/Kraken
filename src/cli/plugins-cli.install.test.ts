...
const CLI_STATE_ROOT = "/tmp/uagent-state";

function cliInstallPath(pluginId: string): string {
  return installedPluginRoot(CLI_STATE_ROOT, pluginId);
}

function createEnabledPluginConfig(pluginId: string): UAGENTConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
describe("plugins cli install", () => {
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `uagent doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    ...
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-hook-link-"));
    const installedCfg = {
      hooks: {
        internal: {
          installs: {
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const cfg = {} as UAGENTConfig;
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-hook-install-"));
    const installedCfg = {
      hooks: {
        internal: {
          installs: {
  ...
  it("does not fall back to npm when ClawHub rejects a real package", async () => {
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "uagent skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "uagent skills install demo" instead.');
  });
  ...
  it("falls back to installing hook packs from npm specs", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing uagent.plugin.json",
    });
