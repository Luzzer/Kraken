...
const serveUAGENTChannelMcp = mocks.serveUAGENTChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveUAGENTChannelMcp: mocks.serveUAGENTChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("uagent-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("uagent-cli-mcp-home-", async () => {
      ...
      expect(serveUAGENTChannelMcp).toHaveBeenCalledWith({
