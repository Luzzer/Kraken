...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    UAGENT_STATE_DIR: "/tmp/uagent-daemon",
    UAGENT_CONFIG_PATH: "/tmp/uagent-daemon/uagent.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.UAGENT_STATE_DIR ?? "/tmp/uagent-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.UAGENT_CONFIG_PATH ?? `${stateDir}/uagent.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/uagent-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.UAGENT_STATE_DIR = "/tmp/uagent-cli";
    process.env.UAGENT_CONFIG_PATH = "/tmp/uagent-cli/uagent.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/uagent-daemon/uagent.json",
      }),
    );
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/uagent-cli/uagent.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.UAGENT_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.UAGENT_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.UAGENT_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "uagent-gateway" }],
        hints: [],
      },
      healthy: false,
