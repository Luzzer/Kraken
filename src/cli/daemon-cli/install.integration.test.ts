...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("uagent-daemon-install-int-");
    configPath = path.join(tempHome, "uagent.json");
    process.env.HOME = tempHome;
    process.env.UAGENT_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.UAGENT_GATEWAY_TOKEN).toBeUndefined();
  });
});
