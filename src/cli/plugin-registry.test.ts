...
const mocks = vi.hoisted(() => ({
  loadUAGENTPlugins: vi.fn<typeof import("../plugins/loader.js").loadUAGENTPlugins>(),
...
vi.mock("../plugins/loader.js", () => ({
  loadUAGENTPlugins: (...args: Parameters<typeof mocks.loadUAGENTPlugins>) =>
    mocks.loadUAGENTPlugins(...args),
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("reloads when escalating from configured-channels to channels", () => {
    ...
    expect(mocks.loadUAGENTPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadUAGENTPlugins).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
  ...
  it("does not treat a pre-seeded partial registry as all scope", () => {
    ...
    expect(mocks.loadUAGENTPlugins).toHaveBeenCalledTimes(1);
    expect(mocks.loadUAGENTPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        config,
        throwOnLoadError: true,
