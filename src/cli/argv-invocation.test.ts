...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "uagent", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "uagent", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "uagent", "--profile", "work", "gateway", "status"],
