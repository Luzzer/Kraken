...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      UAGENT_GATEWAY_PORT: String(params.port),
      ...(params.token ? { UAGENT_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.UAGENT_STATE_DIR = "/tmp/uagent-cli-state";
    process.env.UAGENT_CONFIG_PATH = "/tmp/uagent-cli-state/uagent.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        UAGENT_PROFILE: "dev",
        UAGENT_STATE_DIR: "/tmp/uagent-daemon-state",
        UAGENT_CONFIG_PATH: "/tmp/uagent-daemon-state/uagent.json",
        UAGENT_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.uagent.gateway.plist",
    });
