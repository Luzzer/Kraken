...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.uagent${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.UAGENT_PROFILE = profile;

  const stateDir = env.UAGENT_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.UAGENT_STATE_DIR?.trim()) {
    env.UAGENT_STATE_DIR = stateDir;
  }

  if (!env.UAGENT_CONFIG_PATH?.trim()) {
    env.UAGENT_CONFIG_PATH = path.join(stateDir, "uagent.json");
  }

  if (profile === "dev" && !env.UAGENT_GATEWAY_PORT?.trim()) {
    env.UAGENT_GATEWAY_PORT = "19001";
  }
}
