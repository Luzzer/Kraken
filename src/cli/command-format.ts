...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+uagent\b|^uagent\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE =
  /^(?:pnpm|npm|bunx|npx)\s+uagent\b.*(?:^|\s)update(?:\s|$)|^uagent\b.*(?:^|\s)update(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const container = env.UAGENT_CONTAINER_HINT?.trim();
  const profile = normalizeProfileName(env.UAGENT_PROFILE);
