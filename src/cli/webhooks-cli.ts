...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.uagent.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + UAGENT hooks")
    ...
    .option("--hook-url <url>", "UAGENT hook URL")
    .option("--hook-token <token>", "UAGENT hook token")
