...
${formatHelpExamples([
  ['uagent agent --to +15555550123 --message "status update"', "Start a new session."],
  ['uagent agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'uagent agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'uagent agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['uagent agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'uagent agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.uagent.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.uagent.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['uagent agents set-identity --agent main --name "UAGENT" --emoji "🦞"', "Set name + emoji."],
  ["uagent agents set-identity --agent main --avatar avatars/uagent.png", "Set avatar path."],
  [
    "uagent agents set-identity --workspace ~/.uagent/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "uagent agents set-identity --identity-file ~/.uagent/workspace/IDENTITY.md --agent main",
