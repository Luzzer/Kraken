...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["uagent status", "Show channel health + session summary."],
          ["uagent status --all", "Full diagnosis (read-only)."],
          ["uagent status --json", "Machine-readable output."],
          ["uagent status --usage", "Show model provider usage/quota snapshots."],
          [
            "uagent status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["uagent status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.uagent.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.uagent.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["uagent sessions", "List all sessions."],
          ["uagent sessions --agent work", "List sessions for one agent."],
          ["uagent sessions --all-agents", "Aggregate sessions across agents."],
          ["uagent sessions --active 120", "Only last 2 hours."],
          ["uagent sessions --json", "Machine-readable output."],
          ["uagent sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.uagent.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["uagent sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "uagent sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["uagent sessions cleanup --enforce", "Apply maintenance now."],
          ["uagent sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["uagent sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "uagent sessions cleanup --enforce --store ./tmp/sessions.json",
