...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update UAGENT and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["uagent update", "Update a source checkout (git)"],
        ["uagent update --channel beta", "Switch to beta channel (git + npm)"],
        ["uagent update --channel dev", "Switch to dev channel (git + npm)"],
        ["uagent update --tag beta", "One-off update to a dist-tag or version"],
        ["uagent update --tag main", "One-off package install from GitHub main"],
        ["uagent update --dry-run", "Preview actions without changing anything"],
        ["uagent update --no-restart", "Update without restarting the service"],
        ["uagent update --json", "Output result as JSON"],
        ["uagent update --yes", "Non-interactive (accept downgrade prompts)"],
        ["uagent update wizard", "Interactive update wizard"],
        ["uagent --update", "Shorthand for uagent update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run uagent update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.uagent.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["uagent update status", "Show channel + version status."],
          ["uagent update status --json", "JSON output."],
          ["uagent update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.uagent.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
