...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`uagent pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`uagent pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends UAGENTConfig = UAGENTConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends UAGENTConfig = UAGENTConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends UAGENTConfig = UAGENTConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: UAGENTConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends UAGENTConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends UAGENTConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends UAGENTConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
