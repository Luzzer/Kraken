...
const originalBundledPluginsDir = process.env.UAGENT_BUNDLED_PLUGINS_DIR;
...
afterEach(() => {
  ...
  if (originalBundledPluginsDir === undefined) {
    ...
    process.env.UAGENT_BUNDLED_PLUGINS_DIR = originalBundledPluginsDir;
  }
...
describe("plugin-sdk facade runtime", () => {
  it("honors bundled plugin dir overrides outside the package root", () => {
    const overrideA = createBundledPluginDir("uagent-facade-runtime-a-", "override-a");
    const overrideB = createBundledPluginDir("uagent-facade-runtime-b-", "override-b");

    process.env.UAGENT_BUNDLED_PLUGINS_DIR = overrideA;
    ...
    process.env.UAGENT_BUNDLED_PLUGINS_DIR = overrideB;
  ...
  it("returns the same object identity on repeated calls (sentinel consistency)", () => {
    const dir = createBundledPluginDir("uagent-facade-identity-", "identity-check");
    process.env.UAGENT_BUNDLED_PLUGINS_DIR = dir;
    const location = {
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const dir = createBundledPluginDir("uagent-facade-circular-", "circular-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("back-fills the sentinel before post-load facade tracking re-enters", () => {
    const dir = createBundledPluginDir("uagent-facade-post-load-", "post-load-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const dir = createThrowingPluginDir("uagent-facade-throw-");
    process.env.UAGENT_BUNDLED_PLUGINS_DIR = dir;
  ...
  it("allows runtime-api facade loads when the bundled plugin is explicitly enabled", () => {
    const dir = createTempDirSync("uagent-facade-runtime-enabled-");
  ...
  it("resolves a globally-installed plugin whose rootDir basename matches the dirName", () => {
    const lineDir = createTempDirSync("uagent-facade-global-line-");
    ...
    fs.writeFileSync(
      path.join(lineDir, "uagent.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
  ...
  it("resolves a globally-installed plugin with an encoded scoped rootDir basename", () => {
    const encodedDir = createTempDirSync("uagent-facade-encoded-line-");
    ...
    fs.writeFileSync(
      path.join(encodedDir, "uagent.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
        configSchema: { type: "object", additionalProperties: false, properties: {} },
