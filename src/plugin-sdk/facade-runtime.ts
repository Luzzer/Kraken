...
const EMPTY_FACADE_BOUNDARY_CONFIG: UAGENTConfig = {};
const jitiLoaders = new Map<string, ReturnType<typeof createJiti>>();
const loadedFacadeModules = new Map<string, unknown>();
const loadedFacadePluginIds = new Set<string>();
const UAGENT_SOURCE_EXTENSIONS_ROOT = path.resolve(UAGENT_PACKAGE_ROOT, "extensions");
let cachedBoundaryRawConfig: UAGENTConfig | undefined;
...
function resolveSourceFirstPublicSurfacePath(params: {
  ...
  const sourceRoot = params.bundledPluginsDir ?? path.resolve(UAGENT_PACKAGE_ROOT, "extensions");
...
function resolveBundledMetadataManifestRecord(params: {
  ...
  if (location.modulePath.startsWith(`${UAGENT_SOURCE_EXTENSIONS_ROOT}${path.sep}`)) {
    const relativeToExtensions = path.relative(
      UAGENT_SOURCE_EXTENSIONS_ROOT,
      location.modulePath,
    );
    ...
    return {
      ...
      rootDir: path.resolve(UAGENT_SOURCE_EXTENSIONS_ROOT, metadata.dirName),
...
function loadFacadeModuleAtLocationSync<T extends object>(params: {
  ...
  const opened = openBoundaryFileSync({
    ...
    boundaryLabel:
      location.boundaryRoot === UAGENT_PACKAGE_ROOT
        ? "UAGENT package root"
        : (() => {
