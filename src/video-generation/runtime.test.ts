...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    getVideoGenerationProvider: vi.fn<
      (providerId: string, config?: UAGENTConfig) => VideoGenerationProvider | undefined
    >(() => undefined),
    isFailoverError: vi.fn<(err: unknown) => boolean>(() => false),
    listVideoGenerationProviders: vi.fn<(config?: UAGENTConfig) => VideoGenerationProvider[]>(
      () => [],
    ),
...
describe("video-generation runtime", () => {
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeVideoGenerationProviders({ config: {} as UAGENTConfig })).toEqual(
      providers,
    );
    expect(mocks.listVideoGenerationProviders).toHaveBeenCalledWith({} as UAGENTConfig);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    const promise = generateVideo({ cfg: {} as UAGENTConfig, prompt: "animate a cat" });
