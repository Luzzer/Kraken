...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = makeTempDir(tempRoots, "uagent-install-sh-");
  const cliPath = path.join(root, "uagent");
...
resolve_uagent_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_UAGENT_BIN: cliPath,
        UAGENT_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("UAGENT 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("UAGENT dev's build");

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("UAGENT dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = makeTempDir(tempRoots, "uagent-install-stdin-");
