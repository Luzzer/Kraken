...
const ORIGINAL_STATE_DIR = process.env.UAGENT_STATE_DIR;
...
afterEach(() => {
  ...
  if (ORIGINAL_STATE_DIR == null) {
    ...
    process.env.UAGENT_STATE_DIR = ORIGINAL_STATE_DIR;
  }
});

async function withTaskRegistryTempDir<T>(run: () => Promise<T> | T): Promise<T> {
  return await withTempDir({ prefix: "uagent-task-owner-access-" }, async (root) => {
