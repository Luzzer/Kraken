...
describe("backup commands", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("uagent-backup-test-");
  });
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-workspace-link-"));
    ...
    try {
      ...
      await fs.writeFile(
        path.join(stateDir, "uagent.json"),
        JSON.stringify({
          agents: {
            defaults: {
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    ...
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-backups-"));
    try {
      process.env.UAGENT_CONFIG_PATH = configPath;
      await fs.writeFile(
        configPath,
      ...
      const extractDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-backup-extract-"));
      try {
  ...
  it("optionally verifies the archive after writing it", async () => {
    ...
    const archiveDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "uagent-backup-verify-on-create-"),
    );
    try {
      await fs.writeFile(path.join(stateDir, "uagent.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".uagent");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-backup-link-"));
  ...
  it("falls back to the home directory when cwd is a symlink into a backed-up source tree", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".uagent");
    const workspaceDir = path.join(stateDir, "workspace");
    const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-backup-cwd-link-"));
  ...
  it("backs up only the active config file when --only-config is requested", async () => {
    const stateDir = path.join(tempHome.home, ".uagent");
    const configPath = path.join(stateDir, "uagent.json");
    await fs.mkdir(path.join(stateDir, "credentials"), { recursive: true });
