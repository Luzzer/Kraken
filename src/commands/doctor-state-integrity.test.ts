...
type EnvSnapshot = {
  HOME?: string;
  UAGENT_HOME?: string;
  UAGENT_STATE_DIR?: string;
  UAGENT_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    UAGENT_HOME: process.env.UAGENT_HOME,
    UAGENT_STATE_DIR: process.env.UAGENT_STATE_DIR,
    UAGENT_OAUTH_DIR: process.env.UAGENT_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: UAGENTConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: UAGENTConfig) {
...
async function runStateIntegrityText(cfg: UAGENTConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.UAGENT_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when UAGENT_OAUTH_DIR is explicitly configured", async () => {
    process.env.UAGENT_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: UAGENTConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("prints uagent-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: UAGENTConfig = {};
    ...
    expect(text).toMatch(/uagent sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/uagent sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /uagent sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
