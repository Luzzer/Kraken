...
import type { UAGENTConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: UAGENTConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: UAGENTConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "UAGENT default (full)";
  const defaultRequestedAskSource = "UAGENT default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("uagent approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: UAGENTConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: UAGENTConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("uagent security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.uagent/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("uagent approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.uagent.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("uagent configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("uagent config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("uagent doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("uagent config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('uagent config set session.dmScope "per-channel-peer"') +
