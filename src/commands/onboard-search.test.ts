...
function createPerplexityConfig(apiKey: string, enabled?: boolean): UAGENTConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: UAGENTConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): UAGENTConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: UAGENTConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: UAGENTConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
