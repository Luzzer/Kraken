...
async function generateCompletionCache(): Promise<boolean> {
  const root = await resolveUAGENTPackageRoot({
...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(uagent completion ...)` */
 ...
 */
export async function ensureCompletionCacheExists(binName = "uagent"): Promise<boolean> {
  const shell = resolveShellFromEnv() as CompletionShell;
  const cacheExists = await completionCacheExists(shell, binName);
