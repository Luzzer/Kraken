...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("uagent doctor --fix")} or ${formatCliCommand("uagent memory status --fix")}`
    : `Verify: ${formatCliCommand("uagent memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: UAGENTConfig): Promise<void> {
  try {
 ...
 * Runs as part of `uagent doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("uagent config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("uagent config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("uagent memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`uagent config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("uagent configure --section model")}`,
        `- To disable: ${formatCliCommand("uagent config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("uagent memory status --deep")}`,
      ].join("\n"),
      "Memory search",
