...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:uagent security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:uagent security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:uagent status --all",
      "  Need to debug live? cmd:uagent logs --follow",
      "  Fix reachability first: cmd:uagent gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:uagent devices approve req-123)",
      "muted(Fallback: cmd:uagent devices approve --latest)",
      "muted(Inspect: cmd:uagent devices list)",
    ]);
  });
