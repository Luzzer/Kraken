...
function resolveAgentName(cfg: UAGENTConfig, agentId: string) {
...
function resolveAgentModel(cfg: UAGENTConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: UAGENTConfig): AgentSummary[] {
