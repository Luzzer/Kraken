...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/uagent-workspace";
  return {
    DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/uagent-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "uagent-agent-" });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<UAGENTConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<UAGENTConfig["channels"]>["telegram"]>>,
  ...
  const cfg = {
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "uagent"),
        ...agentOverrides,
      },
      list: agentsList,
    },
...
async function withCrossAgentResumeFixture(
  run: (params: { sessionId: string; sessionKey: string; cfg: UAGENTConfig }) => Promise<void>,
): Promise<void> {
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as UAGENTConfig },
    writeOptions: {},
