...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "UAGENT", ".uagent");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/uagent-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".uagent");
  ...
  it("anchors cloud detection to OS homedir when UAGENT_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".uagent");
    const originalUAGENTHome = process.env.UAGENT_HOME;
    process.env.UAGENT_HOME = "/tmp/uagent-home-override";
    ...
    try {
      ...
      if (originalUAGENTHome === undefined) {
