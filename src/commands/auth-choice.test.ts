...
function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<UAGENTConfig>;
}): ProviderPlugin {
  return {
    ...
    auth: [
      createProviderApiKeyAuthMethod({
        ...
        ...(params.applyConfig ? { applyConfig: () => params.applyConfig as UAGENTConfig } : {}),
        wizard: {
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as UAGENTConfig,
...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    const env = await setupAuthTestEnv("uagent-auth-");
      ...
      config: {
        secrets: {
          providers: {
            filemain: {
              source: "file",
              path: "/tmp/uagent-missing-secrets.json",
              mode: "json",
            },
          },
        },
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireUAGENTAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
