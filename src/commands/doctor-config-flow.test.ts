...
describe("doctor config flow", () => {
  ...
  it("creates a Matrix migration snapshot before doctor repair mutates Matrix state", async () => {
    await withTempHome(async (home) => {
      ...
      const snapshotDir = path.join(home, "Backups", "uagent-migrations");
      ...
      expect(marker.archivePath).toContain(path.join("Backups", "uagent-migrations"));
    });
  });
  ...
  it("warns when Matrix is installed from a stale custom path", async () => {
    ...
    expect(
      doctorWarnings.some(
        (line) => line.includes("custom path") && line.includes("/tmp/uagent-matrix-missing"),
      ),
    ).toBe(true);
  });
    ...
    try {
      ...
      expect(
        noteSpy.mock.calls.some(
          ([message, title]) =>
            title === "Doctor" &&
            String(message).includes('Run "uagent doctor --fix" to migrate legacy config keys.'),
        ),
      ).toBe(true);
    } finally {
      noteSpy.mockRestore();
