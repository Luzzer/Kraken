...
async function runGatewayHealthCheck(params: {
  ...
  const gatewayHealth = await waitForGatewayHealthy({
    ...
    probeTimeoutMs: params.installDaemon
      ...
      : undefined,
    retryAttempts: params.installDaemon ? INSTALL_DAEMON_HEALTH_RETRY_ATTEMPTS : 1,
    retryDelayMs: params.installDaemon ? INSTALL_DAEMON_HEALTH_RETRY_DELAY_MS : 0,
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from uagent.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<UAGENTConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "UAGENT update wizard" : "UAGENT configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    let currentBaseHash = snapshot.hash;
    const baseConfig: UAGENTConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("uagent doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.UAGENT_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.UAGENT_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
