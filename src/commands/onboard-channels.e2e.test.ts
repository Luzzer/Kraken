...
function createTelegramCfg(botToken: string, enabled?: boolean): UAGENTConfig {
  return {
    channels: {
      telegram: {
        botToken,
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveConfigured: ({ cfg }: { cfg: UAGENTConfig }) =>
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveSelectionHint: async ({ cfg }: { cfg: UAGENTConfig }) =>
            ...
            textInputs: [
              {
                ...
                required: true,
                applySet: ({ cfg, value }: { cfg: UAGENTConfig; value: string }) =>
                  ({
                    ...cfg,
                    channels: {
                      ...cfg.channels,
...
function patchTelegramAdapter(overrides: ChannelSetupWizardAdapterPatch) {
  ...
  const patch = {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: UAGENTConfig }) => ({
        channel: "telegram",
...
async function expectQuickstartPickerSkipsWithoutRuntime() {
  ...
  await expect(
    runSetupChannels({} as UAGENTConfig, prompter, {
      quickstartDefaults: true,
    }),
  ).resolves.toEqual({} as UAGENTConfig);
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as UAGENTConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("./channel-setup/plugin-install.js", async () => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: UAGENTConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    await runSetupChannels({} as UAGENTConfig, prompter, {
      quickstartDefaults: true,
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@uagent/msteams-plugin": { enabled: true },
          },
        },
      } as UAGENTConfig,
      prompter,
        ...
        if (channel === "msteams") {
          registry.channels.push({
            ...
            plugin: {
              ...
              config: {
                listAccountIds: (cfg: UAGENTConfig) =>
                  Object.keys(
                    (cfg.channels?.msteams as { accounts?: Record<string, unknown> } | undefined)
                      ?.accounts ?? {},
                  ),
                resolveAccount: (cfg: UAGENTConfig, accountId: string) =>
                  (
                    cfg.channels?.msteams as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: UAGENTConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: UAGENTConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
