...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "UAGENT_GATEWAY_TOKEN",
          placeholder: "UAGENT_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like UAGENT_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. UAGENT will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: UAGENT trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.uagent.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
