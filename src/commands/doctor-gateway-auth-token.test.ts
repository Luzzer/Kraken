...
describe("resolveGatewayAuthTokenForService", () => {
  ...
  it("falls back to UAGENT_GATEWAY_TOKEN when SecretRef is unresolved", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
      ...
      {
        UAGENT_GATEWAY_TOKEN: "env-fallback-token",
      } as NodeJS.ProcessEnv,
    );
  ...
  it("falls back to UAGENT_GATEWAY_TOKEN when SecretRef resolves to empty", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
...
describe("shouldRequireGatewayTokenForInstall", () => {
  ...
  it("requires token in inferred mode when password env exists only in shell", async () => {
    await withEnvAsync(
      { [envVar("UAGENT", "GATEWAY", "PASSWORD")]: "password-from-env" },
      async () => {
    ...
    const required = shouldRequireGatewayTokenForInstall(
      {
        ...
        env: {
          vars: {
            UAGENT_GATEWAY_PASSWORD: "configured-password", // pragma: allowlist secret
          },
        },
      } as UAGENTConfig,
      {} as NodeJS.ProcessEnv,
  ...
  it("does not require token in inferred mode when password env exists in state-dir .env", async () => {
    await withTempHome(async (_home) => {
      await writeStateDirDotEnv("UAGENT_GATEWAY_PASSWORD=dotenv-password\n", {
        env: process.env,
      });
