...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as UAGENTConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as UAGENTConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<UAGENTConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<UAGENTConfig["models"]>>;
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<UAGENTConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as UAGENTConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyTalkConfig(cfg: UAGENTConfig, changes: string[]): UAGENTConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as UAGENTConfig["talk"]) ?? {};
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<UAGENTConfig["tools"]>["media"],
    },
  };
}
...
export function normalizeLegacyMistralModelMaxTokens(
  ...
  return {
    ...
    models: {
      ...cfg.models,
      providers: nextProviders as NonNullable<UAGENTConfig["models"]>["providers"],
    },
  };
}
