...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/uagent-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = UAGENTConfig>(
  overrides: T = {} as T,
): UAGENTConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): UAGENTConfig {
