...
beforeAll(() => {
  envSnapshot = captureEnv(["UAGENT_PROFILE"]);
  process.env.UAGENT_PROFILE = "isolated";
});
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.uagent.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.uagent.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.UAGENT_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/uagent-root.js", () => ({
  resolveUAGENTPackageRoot: vi.fn().mockResolvedValue("/tmp/uagent"),
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/uagent/pnpm-lock.yaml",
      markerPath: "/tmp/uagent/node_modules/.modules.yaml",
    },
...
vi.mock("./status.daemon.js", () => ({
  getDaemonStatusSummary: vi.fn(async () => {
    ...
    return {
      ...
      loaded,
      managedByUAGENT: Boolean(command),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("uagent status --all") ||
          line.includes("uagent --profile isolated status --all"),
      ),
    ).toBe(true);
  });
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("uagent --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("UAGENT_GATEWAY_TOKEN", "abcd1234", async () => {
