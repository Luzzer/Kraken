...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveUAGENTAgentDir: vi.fn().mockReturnValue("/tmp/uagent-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/uagent-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/uagent-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/uagent-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/uagent-dev/uagent.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveUAGENTAgentDir: mocks.resolveUAGENTAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/uagent-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveUAGENTAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.configPath).toBe("/tmp/uagent-dev/uagent.json");
    expect(payload.auth.storePath).toBe("/tmp/uagent-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/uagent-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/uagent-agent-custom");
