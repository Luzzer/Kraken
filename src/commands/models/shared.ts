...
export async function loadValidConfigOrThrow(): Promise<UAGENTConfig> {
...
export async function updateConfig(
  mutator: (cfg: UAGENTConfig) => UAGENTConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: UAGENTConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: UAGENTConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("uagent agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
