...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under UAGENT_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("uagent-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-oauth-sync-"));
    ...
    process.env.UAGENT_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-oauth-nosync-"));
    process.env.UAGENT_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside UAGENT_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-oauth-external-"));
    process.env.UAGENT_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* UAGENT_STATE_DIR
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to UAGENT_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("uagent-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
