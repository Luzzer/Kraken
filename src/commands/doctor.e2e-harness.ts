...
export const resolveUAGENTPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadUAGENTPlugins: () => createEmptyPluginRegistry(),
}));
...
vi.mock("../infra/uagent-root.js", () => ({
  ...
  resolveUAGENTPackageRootSync: vi.fn(() => "/tmp/uagent"),
}));
...
beforeEach(() => {
  ...
  resolveUAGENTPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.UAGENT_STATE_DIR;
  originalUpdateInProgress = process.env.UAGENT_UPDATE_IN_PROGRESS;
  process.env.UAGENT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-doctor-state-"));
