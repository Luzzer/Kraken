...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-uagent-backup";
...
describe("backupVerifyCommand", () => {
  ...
  async function resetTempHome() {
    ...
    await fs.mkdir(path.join(tempHome.home, ".uagent"), { recursive: true });
    delete process.env.UAGENT_CONFIG_PATH;
  }

  beforeAll(async () => {
    tempHome = await createTempHomeEnv("uagent-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".uagent");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "uagent.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-uagent-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.uagent",
            archivePath: `${rootName}/payload/posix/tmp/.uagent`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "uagent-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "uagent-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".uagent");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-workspace-"));
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.uagent/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "uagent-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "uagent-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [
