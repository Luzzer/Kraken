...
const ensureUAGENTModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveUAGENTAgentDir = vi.fn().mockReturnValue("/tmp/uagent-agent");
...
beforeEach(() => {
  ...
  ensureUAGENTModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureUAGENTModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the source snapshot config when provided", async () => {
    ...
    expect(ensureUAGENTModelsJson).toHaveBeenCalled();
    expect(ensureUAGENTModelsJson.mock.calls[0]?.[0]).toEqual(sourceConfig);
  });
