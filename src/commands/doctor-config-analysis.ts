...
import { UAGENTSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: UAGENTConfig): {
  config: UAGENTConfig;
  removed: string[];
} {
  const parsed = UAGENTSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: UAGENTConfig): void {
