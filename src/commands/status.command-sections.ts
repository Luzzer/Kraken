...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("uagent security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("uagent security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.uagent.ai/faq",
    "Troubleshooting: https://docs.uagent.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("uagent status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("uagent logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("uagent node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("uagent status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("uagent gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`uagent devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("uagent devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("uagent devices list")}`),
  ];
}
