...
const SEARCH_TOOL = "https://docs.uagent.ai/mcp.SearchUAGENT";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.uagent.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('uagent docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.uagent.ai/");
      runtime.log(`Search: ${formatCliCommand('uagent docs "your query"')}`);
    }
    return;
  }
