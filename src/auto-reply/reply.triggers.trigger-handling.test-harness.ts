...
const piEmbeddedMocks = getSharedMocks("uagent.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("uagent.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("uagent.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("uagent.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    uagentHome: process.env.UAGENT_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("UAGENT_HOME", snapshot.uagentHome);
  restoreKey("UAGENT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.UAGENT_STATE_DIR = join(home, ".uagent");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "uagent-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".uagent", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): UAGENTConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "uagent"),
