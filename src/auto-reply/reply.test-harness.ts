...
type HomeEnvSnapshot = {
  ...
  UAGENT_STATE_DIR: string | undefined;
  UAGENT_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    UAGENT_AGENT_DIR: process.env.UAGENT_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".uagent", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.UAGENT_STATE_DIR = path.join(home, ".uagent");
    process.env.UAGENT_AGENT_DIR = path.join(home, ".uagent", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".uagent", "agent");
