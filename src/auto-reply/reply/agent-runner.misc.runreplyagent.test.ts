...
vi.mock("../../cron/store.js", () => {
  return {
    ...
    resolveCronStorePath: (storePath?: string) => storePath ?? "/tmp/uagent-cron-store.json",
  };
});
...
describe("runReplyAgent authProfileId fallback scoping", () => {
  it("drops authProfileId when provider changes during fallback", async () => {
    ...
    const followupRun = {
      ...
      run: {
        ...
        provider: "anthropic",
        model: "claude-opus",
        authProfileId: "anthropic:uagent",
  ...
  it("persists same-provider fallback model while keeping the scoped auth profile", async () => {
    ...
    const sessionEntry: SessionEntry = {
      ...
      totalTokens: 1,
      compactionCount: 0,
      authProfileOverride: "anthropic:uagent",
    ...
    expect(call.authProfileId).toBe("anthropic:uagent");
    ...
    expect(sessionEntry.authProfileOverride).toBe("anthropic:uagent");
...
describe("runReplyAgent auto-compaction token update", () => {
  ...
  it("lets /stop abort a run that is still in preflight compaction", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-preflight-stop-"));
  ...
  it("surfaces the restart notice when gateway shutdown aborts preflight compaction", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-preflight-restart-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionFile = "session-relative.jsonl";
    const workspaceDir = tmp;
  ...
  it("updates totalTokens after auto-compaction using lastCallUsage", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-compact-tokens-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("tracks auto-compaction from embedded result metadata even when no compaction event is emitted", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-compact-meta-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("accumulates compactions across fallback attempts without double-counting a single attempt", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-compact-fallback-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("does not count failed compaction end events from earlier fallback attempts", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-compact-fallback-failed-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("updates totalTokens from lastCallUsage even without compaction", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-usage-last-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("does not enqueue legacy post-compaction audit warnings", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-no-audit-warning-"));
...
describe("runReplyAgent messaging tool suppression", () => {
  ...
  it("persists usage fields even when replies are suppressed", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(os.tmpdir(), "uagent-session-store-")),
      "sessions.json",
    );
    const sessionKey = "main";
