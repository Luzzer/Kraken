...
export function resolveConfiguredAcpBackendId(cfg: UAGENTConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: UAGENTConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (workspaceLocalPath && existsSync(workspaceLocalPath)) {
      return `uagent plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^uagent plugins install /u, "");
    ...
    return "uagent plugins install acpx";
  }
