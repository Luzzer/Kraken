...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("uagent.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("uagent.reply.full-runtime");

type ReplyConfigWithMarker = UAGENTConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.UAGENT_ALLOW_SLOW_REPLY_TESTS === "1" || env.UAGENT_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends UAGENTConfig>(
  config: T,
...
export function withFastReplyConfig<T extends UAGENTConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends UAGENTConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

export function isCompleteReplyConfig(config: unknown): config is UAGENTConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  loadConfig: () => UAGENTConfig;
  isFastTestEnv: boolean;
  configOverride?: UAGENTConfig;
}): UAGENTConfig {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set UAGENT_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  if (params.isFastTestEnv && isCompleteReplyConfig(configOverride)) {
    return configOverride;
  }
  return applyMergePatch(params.loadConfig(), configOverride) as UAGENTConfig;
}
...
export function initFastReplySessionState(params: {
  ...
  const now = Date.now();
  const sessionFile = path.join(workspaceDir, ".uagent", "sessions", `${sessionId}.jsonl`);
