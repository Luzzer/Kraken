...
import { scheduleGatewaySigusr1Restart, triggerUAGENTRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting UAGENT in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerUAGENTRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting UAGENT via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
