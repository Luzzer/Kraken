...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop uagent!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@uagent_bot", { botUsername: "uagent_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@uagent_bot", { botUsername: "uagent_bot" })).toBe(true);
