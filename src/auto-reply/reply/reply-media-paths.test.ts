...
describe("createReplyMediaPathNormalizer", () => {
  ...
  it("drops arbitrary host-local media paths when sandbox exists", async () => {
    ...
    const result = await normalize({
      mediaUrls: ["/Users/peter/.uagent/media/inbound/photo.png"],
    });
  ...
  it("keeps managed generated media under the shared media root", async () => {
    vi.stubEnv("UAGENT_STATE_DIR", "/Users/peter/.uagent");
    ensureSandboxWorkspaceForSession.mockResolvedValue({
      workspaceDir: "/tmp/sandboxes/session-1",
      containerWorkdir: "/workspace",
    });
    ...
    const result = await normalize({
      mediaUrls: ["/Users/peter/.uagent/media/tool-image-generation/generated.png"],
    });

    expect(result).toMatchObject({
      mediaUrl: "/Users/peter/.uagent/media/tool-image-generation/generated.png",
      mediaUrls: ["/Users/peter/.uagent/media/tool-image-generation/generated.png"],
    });
    expect(saveMediaSource).not.toHaveBeenCalled();
  });
  ...
  it("drops absolute file URLs outside managed reply media roots", async () => {
    ...
    const result = await normalize({
      mediaUrls: ["file:///Users/peter/.uagent/media/inbound/photo.png"],
    });
  ...
  it("persists volatile agent-state media from the workspace into host outbound media", async () => {
    saveMediaSource.mockResolvedValue({
      path: "/Users/peter/.uagent/media/outbound/persisted.png",
    });
    const normalize = createReplyMediaPathNormalizer({
      cfg: {},
      sessionKey: "session-key",
      workspaceDir: "/Users/peter/.uagent/workspace",
    });

    const result = await normalize({
      mediaUrls: [
        "/Users/peter/.uagent/workspace/.uagent/media/tool-image-generation/generated.png",
      ],
    });
    ...
    expect(result).toMatchObject({
      mediaUrl: "/Users/peter/.uagent/media/outbound/persisted.png",
      mediaUrls: ["/Users/peter/.uagent/media/outbound/persisted.png"],
    });
  });
});
