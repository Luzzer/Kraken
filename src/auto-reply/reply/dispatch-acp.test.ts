...
const policyMocks = vi.hoisted(() => ({
  resolveAcpDispatchPolicyError: vi.fn<(cfg: UAGENTConfig) => AcpRuntimeError | null>(() => null),
  resolveAcpAgentPolicyError: vi.fn<(cfg: UAGENTConfig, agent: string) => AcpRuntimeError | null>(
    () => null,
  ),
}));
...
const sessionMetaMocks = vi.hoisted(() => ({
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: UAGENTConfig }) => AcpSessionStoreEntry | null
  >(() => null),
}));
...
function createAcpConfigWithVisibleToolTags(): UAGENTConfig {
...
describe("tryDispatchAcpReply", () => {
  beforeEach(async () => {
    ...
    vi.doMock("../../acp/policy.js", () => ({
      resolveAcpDispatchPolicyError: (cfg: UAGENTConfig) =>
        policyMocks.resolveAcpDispatchPolicyError(cfg),
      resolveAcpAgentPolicyError: (cfg: UAGENTConfig, agent: string) =>
  ...
  it("uses canonical session keys for bound-session identity notices", async () => {
    ...
    sessionMetaMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: UAGENTConfig }) =>
        params.sessionKey === canonicalSessionKey
          ? {
              cfg: params.cfg ?? createAcpTestConfig(),
              storePath: "/tmp/uagent-session-store.json",
              sessionKey: canonicalSessionKey,
