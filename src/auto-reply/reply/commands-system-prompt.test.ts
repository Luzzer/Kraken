...
const { createUAGENTCodingToolsMock } = vi.hoisted(() => ({
  createUAGENTCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(async () => {
    vi.restoreAllMocks();
    vi.resetModules();
    createUAGENTCodingToolsMock.mockClear();
    createUAGENTCodingToolsMock.mockReturnValue([]);
    const piTools = await import("../../agents/pi-tools.js");
    vi.spyOn(piTools, "createUAGENTCodingTools").mockImplementation(
      createUAGENTCodingToolsMock,
    );
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createUAGENTCodingToolsMock).toHaveBeenCalledWith(
