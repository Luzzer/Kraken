...
const createUAGENTToolsMock = vi.fn();
...
async function loadFreshInlineActionsModuleForTest() {
  ...
  vi.doMock("../../agents/uagent-tools.runtime.js", () => ({
    createUAGENTTools: (...args: unknown[]) => createUAGENTToolsMock(...args),
  }));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createUAGENTToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
