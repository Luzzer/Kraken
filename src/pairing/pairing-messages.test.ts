...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["UAGENT_CONTAINER_HINT", "UAGENT_PROFILE"]);
    delete process.env.UAGENT_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:uagent|uagent) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
