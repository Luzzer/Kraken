...
function isLikelyUAGENTCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = path
    .basename(argv[1] ?? "")
    .trim()
    .toLowerCase();
  return (
    entryBasename === "uagent" ||
    entryBasename === "uagent.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real UAGENT CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureUAGENTModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyUAGENTCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): UAGENTConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureUAGENTModelsJson(cfg);
    } catch {
