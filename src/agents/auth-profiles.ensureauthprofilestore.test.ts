...
describe("ensureAuthProfileStore", () => {
  ...
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-auth-merge-"));
    const previousAgentDir = process.env.UAGENT_AGENT_DIR;
    ...
    try {
      ...
      process.env.UAGENT_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.UAGENT_AGENT_DIR = previousAgentDir;
      }
  ...
  it.each([
    ...
    ({ name, profile, expected }) => {
      withTempAgentDir("uagent-auth-alias-", (agentDir) => {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    withTempAgentDir("uagent-auth-legacy-alias-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("merges legacy oauth.json into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-oauth-migrate-"));
    ...
    try {
      ...
      process.env.UAGENT_STATE_DIR = root;
      process.env.UAGENT_AGENT_DIR = agentDir;
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.UAGENT_STATE_DIR = previousStateDir;
      }
      if (previousAgentDir === undefined) {
        delete process.env.UAGENT_AGENT_DIR;
      } else {
  ...
  it("exposes Codex CLI auth without persisting copied tokens into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-codex-external-sync-"));
  ...
  it("does not write inherited auth stores during secrets runtime reads", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-secrets-runtime-"));
    const previousStateDir = process.env.UAGENT_STATE_DIR;
    try {
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    ...
    try {
      withTempAgentDir("uagent-auth-invalid-", (agentDir) => {
        const invalidStore = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it.each([
    ...
    },
    {
      ...
      prefix: "uagent-nonstr-key-malformed-ref-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "uagent-nonstr-token-ref-",
    ...
    },
    {
      ...
      prefix: "uagent-nonstr-token-num-",
      profileId: "anthropic:default",
      profile: {
        type: "token",
        provider: "anthropic",
