...
function resolveAttachmentLimits(config: UAGENTConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".uagent", "attachments");
  const relDir = path.posix.join(".uagent", "attachments", attachmentId);
