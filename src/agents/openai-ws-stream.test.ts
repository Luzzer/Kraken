...
describe("createOpenAIWebSocketStreamFn", () => {
  ...
  it("keeps native turn metadata stable across websocket retries and increments attempt", async () => {
    ...
    expect(firstPayload.metadata?.uagent_session_id).toBe("sess-turn-metadata-retry");
    expect(firstPayload.metadata?.uagent_transport).toBe("websocket");
    expect(firstPayload.metadata?.uagent_turn_id).toBeTruthy();
    expect(secondPayload.metadata?.uagent_turn_id).toBe(firstPayload.metadata?.uagent_turn_id);
    expect(firstPayload.metadata?.uagent_turn_attempt).toBe("1");
    expect(secondPayload.metadata?.uagent_turn_attempt).toBe("2");
  });
