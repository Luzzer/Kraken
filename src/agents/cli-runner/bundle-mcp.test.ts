...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a strict empty --mcp-config overlay for bundle-MCP-enabled backends without servers", async () => {
    const workspaceDir = await tempHarness.createTempDir("uagent-cli-bundle-mcp-empty-");
  ...
  it("injects a merged --mcp-config overlay for bundle-MCP-enabled backends", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("uagent-cli-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("uagent-cli-bundle-mcp-workspace-");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    try {
      ...
      expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual(["bundleProbe", "uagent"]);
      expect(raw.mcpServers?.uagent?.url).toBe("http://127.0.0.1:23119/mcp");
      expect(raw.mcpServers?.uagent?.headers?.Authorization).toBe("Bearer ${UAGENT_MCP_TOKEN}");
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    const workspaceDir = await tempHarness.createTempDir("uagent-cli-bundle-mcp-env-");
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: {},
      env: {
        UAGENT_MCP_TOKEN: "loopback-token-123",
        UAGENT_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/uagent-bundle-mcp-disabled",
    });
