...
const prepareDeps = {
  ...
  resolveUAGENTDocsPath: async (
    params: Parameters<typeof import("../docs-path.js").resolveUAGENTDocsPath>[0],
  ) => (await import("../docs-path.js")).resolveUAGENTDocsPath(params),
};
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          UAGENT_MCP_TOKEN: mcpLoopbackRuntime.token,
          UAGENT_MCP_AGENT_ID: sessionAgentId ?? "",
          UAGENT_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          UAGENT_MCP_SESSION_KEY: params.sessionKey ?? "",
          UAGENT_MCP_MESSAGE_CHANNEL: params.messageProvider ?? "",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  const heartbeatPrompt =
    sessionAgentId === defaultAgentId
      ...
      : undefined;
  const docsPath = await prepareDeps.resolveUAGENTDocsPath({
    workspaceDir,
