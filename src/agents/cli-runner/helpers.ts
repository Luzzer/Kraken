...
import { KeyedAsyncQueue } from "uagent/plugin-sdk/keyed-async-queue";
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredUAGENTTmpDir(), "uagent-cli-images", `${digest}${ext}`);
}
...
export async function writeCliImages(
  ...
  const imageRoot = path.join(resolvePreferredUAGENTTmpDir(), "uagent-cli-images");
