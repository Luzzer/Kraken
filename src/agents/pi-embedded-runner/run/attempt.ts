...
import { resolveUAGENTDocsPath } from "../../docs-path.js";
...
import { createUAGENTCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveUAGENTAgentDir();
    ...
    const toolsRaw = params.disableTools
      ? []
      : (() => {
          const allTools = createUAGENTCodingTools({
            agentId: sessionAgentId,
    ...
    const docsPath = await resolveUAGENTDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/uagent/uagent/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("uagent:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/uagent/uagent/issues/8643
