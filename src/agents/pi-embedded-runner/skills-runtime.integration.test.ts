...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("uagent-bundled-");
  const workspaceDir = await createTempDir("uagent-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: UAGENTConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.UAGENT_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
