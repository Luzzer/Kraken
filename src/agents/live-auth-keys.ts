...
const GOOGLE_LIVE_SINGLE_KEY = "UAGENT_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "UAGENT_LIVE_ANTHROPIC_KEY",
    listVar: "UAGENT_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "UAGENT_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `UAGENT_LIVE_${base}_KEY`;
