...
function createUAGENTModelRegistry(
  authStorage: PiAuthStorage,
  modelsJsonPath: string,
  agentDir: string,
): PiModelRegistry {
...
function scrubLegacyStaticAuthJsonEntries(pathname: string): void {
  if (process.env.UAGENT_AUTH_STORE_READONLY === "1") {
    return;
  }
...
export function discoverModels(authStorage: PiAuthStorage, agentDir: string): PiModelRegistry {
  return createUAGENTModelRegistry(authStorage, path.join(agentDir, "models.json"), agentDir);
}
