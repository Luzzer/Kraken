...
describe("runCliAgent spawn path", () => {
  ...
  it("loads workspace bootstrap files into the Claude CLI system prompt", async () => {
    ...
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "uagent-cli-bootstrap-context-"),
    );
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
  ...
  it("hydrates prompt media refs into CLI image args", async () => {
    ...
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredUAGENTTmpDir(), "uagent-cli-prompt-image-"),
    );
    ...
    expect(argv[imageArgIndex + 1]).toContain("uagent-cli-images");
  ...
  it("appends hydrated prompt media refs for stdin backends", async () => {
    ...
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredUAGENTTmpDir(), "uagent-cli-prompt-image-generic-"),
    );
    ...
    const appendedPath = promptCarrier
      .split("\n")
      .find((value) => value.includes("uagent-cli-images"));
  ...
  it("prefers explicit images over prompt refs", async () => {
    ...
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredUAGENTTmpDir(), "uagent-cli-explicit-images-"),
    );
