...
function mockContextDeps(params: {
  ...
  const ensureUAGENTModelsJson = vi.fn(async () => {});
  vi.doMock("../config/config.js", () => ({
    loadConfig: params.loadConfig,
  }));
  vi.doMock("./models-config.js", () => ({
    ensureUAGENTModelsJson,
  }));
  vi.doMock("./agent-paths.js", () => ({
    resolveUAGENTAgentDir: () => "/tmp/uagent-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real uagent startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "uagent", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "uagent", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "uagent", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureUAGENTModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await importContextModule();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureUAGENTModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
