...
type AgentEntry = NonNullable<NonNullable<UAGENTConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: UAGENTConfig): AgentEntry[] {
...
export function listAgentIds(cfg: UAGENTConfig): string[] {
...
export function resolveDefaultAgentId(cfg: UAGENTConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: UAGENTConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: UAGENTConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: UAGENTConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
