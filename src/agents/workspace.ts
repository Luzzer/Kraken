...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.UAGENT_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".uagent", `workspace-${profile}`);
  }
  return path.join(home, ".uagent", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".uagent";
