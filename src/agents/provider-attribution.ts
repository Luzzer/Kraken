...
const UAGENT_ATTRIBUTION_PRODUCT = "UAGENT";
const UAGENT_ATTRIBUTION_ORIGINATOR = "uagent";
...
function formatUAGENTUserAgent(version: string): string {
  return `${UAGENT_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: UAGENT_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: UAGENT_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatUAGENTUserAgent(identity.version),
    },
  };
}
