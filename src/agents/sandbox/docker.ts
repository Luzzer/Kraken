...
import { markUAGENTExecEnv } from "../../infra/uagent-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "uagent.sandbox=1");
  args.push("--label", `uagent.sessionKey=${params.scopeKey}`);
  args.push("--label", `uagent.createdAtMs=${createdAtMs}`);
  args.push("--label", `uagent.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `uagent.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markUAGENTExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "uagent.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`uagent sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`uagent sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("uagent sandbox recreate --all");
}
