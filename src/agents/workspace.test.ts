...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses UAGENT_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      UAGENT_HOME: "/srv/uagent-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/uagent-home"), ".uagent", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".uagent", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("uagent-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("uagent-workspace-");
    await fs.mkdir(path.join(tempDir, ".uagent"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-workspace-hardlink-"));
    try {
