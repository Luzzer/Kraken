...
describe("Ollama provider", () => {
  const createAgentDir = () => mkdtempSync(join(tmpdir(), "uagent-test-"));
  ...
  async function resolveProvidersWithOllamaOnly(params: {
    ...
    const env = {
      ...process.env,
      UAGENT_TEST_ONLY_PROVIDER_PLUGIN_IDS: "ollama",
      VITEST: "1",
      NODE_ENV: "test",
      ...params.env,
  ...
  async function loadOllamaCatalogProvider(): Promise<ProviderPlugin | undefined> {
    ollamaCatalogProviderPromise ??= resolvePluginDiscoveryProviders({
      env: { ...process.env, UAGENT_TEST_ONLY_PROVIDER_PLUGIN_IDS: "ollama", VITEST: "1" },
