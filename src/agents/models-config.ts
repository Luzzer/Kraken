...
import { planUAGENTModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  config: UAGENTConfig;
  sourceConfigForSecrets: UAGENTConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: UAGENTConfig): {
  config: UAGENTConfig;
  sourceConfigForSecrets: UAGENTConfig;
} {
...
export async function ensureUAGENTModelsJson(
  config?: UAGENTConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveUAGENTAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planUAGENTModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
