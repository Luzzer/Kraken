...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["UAGENT_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-subagent-"));
  ...
  it("uses isolated temp state when UAGENT_STATE_DIR is unset in tests", async () => {
    delete process.env.UAGENT_STATE_DIR;
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "uagent-test-state"));
  });
});
