...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-pi-tools-"));
    ...
    try {
      ...
      const tools = createUAGENTCodingTools({
        config: cfg,
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: UAGENTConfig) {
    return createUAGENTCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<UAGENTConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<UAGENTConfig["agents"]>["list"]>[number]["tools"];
  }): UAGENTConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createUAGENTCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createUAGENTCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createUAGENTCodingTools({
      config: cfg,
    ...
    const defaultTools = createUAGENTCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createUAGENTCodingTools({
      config: cfg,
    ...
    const bobTools = createUAGENTCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createUAGENTCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createUAGENTCodingTools({
      config: cfg,
