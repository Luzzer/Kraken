...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: UAGENTConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: UAGENTConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: UAGENTConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: UAGENTConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: UAGENTConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: UAGENTConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/uagent");
  });

  it("uses UAGENT_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "uagent-home");
    vi.stubEnv("UAGENT_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as UAGENTConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".uagent", "workspace"));
  });

  it("uses UAGENT_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "uagent-home");
    vi.stubEnv("UAGENT_HOME", home);
    // Clear state dir so it falls back to UAGENT_HOME
    vi.stubEnv("UAGENT_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as UAGENTConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".uagent", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/uagent-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/uagent-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-agent-scope-"));
