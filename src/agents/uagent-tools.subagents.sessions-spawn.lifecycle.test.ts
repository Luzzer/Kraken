...
} from "./uagent-tools.subagents.sessions-spawn.test-harness.js";
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.UAGENT_TEST_FAST;
...
describe("uagent-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnAnnounceFlowOverride();
    resetSessionsSpawnHookRunnerOverride();
    resetSessionsSpawnConfigOverride();
  ...
  afterAll(() => {
    ...
    process.env.UAGENT_TEST_FAST = fastModeEnv.previous;
  });
