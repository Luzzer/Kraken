...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["UAGENT_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns UAGENT_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-bundled-override-"));
    process.env.UAGENT_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "uagent" }));
