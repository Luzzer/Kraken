...
function resolveSkillsLimits(config?: UAGENTConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveUAGENTMetadata(frontmatter),
      invocation,
      exposure: {
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: UAGENTConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`uagent skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`uagent skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
