    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "uagent.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "uagent.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "uagent.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("uagent-");
  const acpxRoot = await tempDirs.make("uagent-acpx-plugin-");
  const helperRoot = await tempDirs.make("uagent-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("uagent-");
  const pluginRoot = await tempDirs.make("uagent-plugin-");
  const outsideDir = await tempDirs.make("uagent-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("uagent-");
    const pluginRoot = await tempDirs.make("uagent-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("uagent-");
    const pluginRoot = await tempDirs.make("uagent-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
