...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    uagent: process.env.UAGENT_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.uagent === undefined) {
      delete process.env.UAGENT_GATEWAY_TOKEN;
    } else {
      process.env.UAGENT_GATEWAY_TOKEN = envSnapshot.uagent;
    }
  });
  ...
  it("uses UAGENT_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.UAGENT_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
