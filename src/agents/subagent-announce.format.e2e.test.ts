...
let configOverride: UAGENTConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: UAGENTConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/uagent/uagent/issues/31298
    previousFastTestEnv = process.env.UAGENT_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.UAGENT_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // UAGENT_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("UAGENT runtime context (internal):");
