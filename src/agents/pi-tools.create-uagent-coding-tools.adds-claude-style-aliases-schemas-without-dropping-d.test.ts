...
describe("createUAGENTCodingTools", () => {
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-read-"));
    try {
      ...
      const contents = "Hello from uagent read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "uagent-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "uagent-workspace"),
    ...
    const tools = createUAGENTCodingTools({ sandbox });
