...
vi.mock("../plugins/provider-runtime.js", async () => {
  ...
  return {
    ...actual,
    applyProviderConfigDefaultsWithPlugin: (config: UAGENTConfig) => config,
...
let ensureUAGENTModelsJson: typeof import("./models-config.js").ensureUAGENTModelsJson;
...
beforeAll(async () => {
  ...
  ({ ensureUAGENTModelsJson, resetModelsJsonReadyCacheForTest } =
...
function createOpenAiApiKeySourceConfig(): UAGENTConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): UAGENTConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderSourceConfig(): UAGENTConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): UAGENTConfig {
  return {
    models: {
      providers: {
        openai: {
...
function withGatewayTokenMode(config: UAGENTConfig): UAGENTConfig {
  return {
    ...config,
    gateway: {
      auth: {
  ...
  await withTempHome(async () => {
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      try {
        ...
        await ensureUAGENTModelsJson(params.candidateConfig ?? loadConfig());
        await runAssertions();
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
...
describe("models-config runtime source snapshot", () => {
  ...
  it("uses non-env marker from runtime source snapshot for file refs", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        unsetEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS);
        const sourceConfig: UAGENTConfig = {
          models: {
            providers: {
              moonshot: {
        ...
        const runtimeConfig: UAGENTConfig = {
          models: {
            providers: {
              moonshot: {
                baseUrl: "https://api.moonshot.ai/v1",
        ...
        try {
          ...
          await ensureUAGENTModelsJson(loadConfig());
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const clonedRuntimeConfig: UAGENTConfig = {
          ...runtimeConfig,
          agents: {
            defaults: {
        ...
        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureUAGENTModelsJson(clonedRuntimeConfig);
  ...
  it("invalidates cached readiness when projected config changes under the same runtime snapshot", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const sourceConfig = createOpenAiApiKeySourceConfig();
        const runtimeConfig = createOpenAiApiKeyRuntimeConfig();
        const firstCandidate: UAGENTConfig = {
          ...runtimeConfig,
          models: {
            providers: {
              openai: {
        ...
        const secondCandidate: UAGENTConfig = {
          ...runtimeConfig,
          models: {
            providers: {
              openai: {
        ...
        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureUAGENTModelsJson(firstCandidate);
          ...
          await ensureUAGENTModelsJson(secondCandidate);
  ...
  it("keeps source markers when runtime projection is skipped for incompatible top-level shape", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const incompatibleCandidate: UAGENTConfig = {
          ...createOpenAiApiKeyRuntimeConfig(),
        };

        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureUAGENTModelsJson(incompatibleCandidate);
          await expectGeneratedProviderApiKey("openai", "OPENAI_API_KEY"); // pragma: allowlist secret
        } finally {
          clearRuntimeConfigSnapshot();
          clearConfigCache();
