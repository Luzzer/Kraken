...
function buildCoreToolGroupMap() {
  ...
  const uagentTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInUAGENTGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:uagent": uagentTools,
