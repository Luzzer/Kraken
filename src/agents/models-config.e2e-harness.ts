...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousUAGENTAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousUAGENTAgentDir = process.env.UAGENT_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousUAGENTAgentDir === undefined) {
      delete process.env.UAGENT_AGENT_DIR;
    } else {
      process.env.UAGENT_AGENT_DIR = previousUAGENTAgentDir;
    }
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "UAGENT_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.UAGENT_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.UAGENT_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: UAGENTConfig = {
  models: {
    providers: {
      "custom-proxy": {
