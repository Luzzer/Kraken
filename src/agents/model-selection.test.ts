...
function resolveAnthropicOpusThinking(cfg: UAGENTConfig) {
...
function createProviderWithModelsConfig(provider: string, models: Array<Record<string, unknown>>) {
  ...
  } as Partial<UAGENTConfig>;
}

function resolveConfiguredRefForTest(cfg: Partial<UAGENTConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as UAGENTConfig,
...
describe("model-selection", () => {
  ...
  describe("isCliProvider", () => {
    it("returns true for setup-registered cli backends", () => {
      expect(isCliProvider("claude-cli", {} as UAGENTConfig)).toBe(true);
    });

    it("returns false for provider ids", () => {
      expect(isCliProvider("example-cli", {} as UAGENTConfig)).toBe(false);
    });
  });
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<UAGENTConfig> = {
        agents: {
          defaults: {
            models: {
