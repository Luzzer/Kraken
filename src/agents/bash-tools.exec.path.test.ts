...
describe("exec PATH login shell merge", () => {
  ...
  it("sets UAGENT_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-uagent-shell", {
      command: 'printf "%s" "${UAGENT_SHELL:-}"',
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/uagent-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      ...
      expect(output).not.toContain("/tmp/uagent-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a uagent /approve abc123 deny",
