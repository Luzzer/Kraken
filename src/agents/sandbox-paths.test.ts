...
async function withOutsideHardlinkInUAGENTTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside uagent tmp root",
      media: path.join(os.tmpdir(), "outside-uagent", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked UAGENT tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked UAGENT tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInUAGENTTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked UAGENT tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInUAGENTTmp(
