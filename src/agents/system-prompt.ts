...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; UAGENT handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `UAGENT docs: ${docsPath}`,
    "Mirror: https://docs.uagent.ai",
    "Source: https://github.com/uagent/uagent",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.ai",
    "For UAGENT behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `uagent status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  if (promptMode === "none") {
    return "You are a personal assistant operating inside UAGENT.";
  }

  const lines = [
    "You are a personal assistant operating inside UAGENT.",
    "",
    "## Tooling",
    ...
    ...safetySection,
    "## UAGENT CLI Quick Reference",
    "UAGENT is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- uagent gateway status",
    "- uagent gateway start",
    "- uagent gateway stop",
    "- uagent gateway restart",
    "If unsure, ask the user to run `uagent help` (or `uagent gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## UAGENT Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, UAGENT pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by UAGENT and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'UAGENT treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
