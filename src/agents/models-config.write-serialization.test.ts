...
const { planUAGENTModelsJsonMock } = vi.hoisted(() => ({
  planUAGENTModelsJsonMock: vi.fn(),
}));

vi.mock("./models-config.plan.js", () => ({
  planUAGENTModelsJson: (...args: unknown[]) => planUAGENTModelsJsonMock(...args),
}));
...
describe("models-config write serialization", () => {
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      try {
        await Promise.all([ensureUAGENTModelsJson(first), ensureUAGENTModelsJson(second)]);
      } finally {
