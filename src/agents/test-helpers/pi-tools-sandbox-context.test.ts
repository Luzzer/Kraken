...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("uagent-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("uagent-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("uagent-sbx-");
