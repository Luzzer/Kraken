...
describe("provider attribution", () => {
  it("resolves the canonical UAGENT product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in UAGENT runtime wrapper.",
      product: "UAGENT",
      version: "2026.3.22",
      headers: {
        "HTTP-Referer": "https://uagent.ai",
        "X-OpenRouter-Title": "UAGENT",
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { UAGENT_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "uagent",
        version: "2026.3.22",
        "User-Agent": "uagent/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { UAGENT_VERSION: "2026.3.22" })).toEqual({
      originator: "uagent",
      version: "2026.3.22",
      "User-Agent": "uagent/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { UAGENT_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ UAGENT_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
