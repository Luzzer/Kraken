...
import "./test-helpers/fast-uagent-tools.js";
...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("uagent-ws-", async (workspaceDir) => {
      await withTempDir("uagent-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createUAGENTCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "uagent" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("uagent-ws-", async (workspaceDir) => {
      await withTempDir("uagent-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("uagent-ws-", async (workspaceDir) => {
      const cfg: UAGENTConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createUAGENTCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-uagent.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("uagent-sandbox-", async (sandboxDir) => {
      await withTempDir("uagent-workspace-", async (workspaceDir) => {
        ...
        const tools = createUAGENTCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
