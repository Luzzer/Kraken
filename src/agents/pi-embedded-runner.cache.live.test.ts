...
describeCacheLive("pi embedded runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.UAGENT_CACHE_TRACE,
      file: process.env.UAGENT_CACHE_TRACE_FILE,
      messages: process.env.UAGENT_CACHE_TRACE_MESSAGES,
      prompt: process.env.UAGENT_CACHE_TRACE_PROMPT,
      system: process.env.UAGENT_CACHE_TRACE_SYSTEM,
    };
    process.env.UAGENT_CACHE_TRACE = "1";
    process.env.UAGENT_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.UAGENT_CACHE_TRACE_MESSAGES = "0";
    process.env.UAGENT_CACHE_TRACE_PROMPT = "0";
    process.env.UAGENT_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      const restore = (
        key:
          | "UAGENT_CACHE_TRACE"
          | "UAGENT_CACHE_TRACE_FILE"
          | "UAGENT_CACHE_TRACE_MESSAGES"
          | "UAGENT_CACHE_TRACE_PROMPT"
          | "UAGENT_CACHE_TRACE_SYSTEM",
      ...
      restore("UAGENT_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("UAGENT_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("UAGENT_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("UAGENT_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("UAGENT_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "UAGENT_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "UAGENT_LIVE_ANTHROPIC_CACHE_MODEL",
