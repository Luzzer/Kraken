...
const DEBUG_COVERAGE_BATCHES = process.env.UAGENT_DEBUG_RUNTIME_COVERAGE === "1";
...
function applyConfigForUAGENTTarget(
  config: UAGENTConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every uagent.json registry target when configured as active", async () => {
    const entries = COVERAGE_REGISTRY_ENTRIES.filter(
      (entry) => entry.configFile === "uagent.json",
    );
    for (const batch of buildCoverageBatches(entries)) {
      logCoverageBatch("uagent.json", batch);
      const config = {} as UAGENTConfig;
      const env: Record<string, string> = {};
      for (const [index, entry] of batch.entries()) {
        const envId = `UAGENT_SECRET_TARGET_${entry.id}`;
        ...
        applyConfigForUAGENTTarget(config, entry, envId, wildcardToken);
      }
      const snapshot = await prepareCoverageSnapshot({
        config,
        env,
        agentDirs: ["/tmp/uagent-agent-main"],
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `UAGENT_AUTH_SECRET_TARGET_${entry.id}`;
