...
let compiledCoreUAGENTTargetState: {
  knownTargetIds: Set<string>;
  openClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  openClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;
...
function getCompiledCoreUAGENTTargetState() {
  if (compiledCoreUAGENTTargetState) {
    return compiledCoreUAGENTTargetState;
  }
  const openClawCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "uagent.json")
    .map(compileTargetRegistryEntry);
  compiledCoreUAGENTTargetState = {
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreUAGENTTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreUAGENTTargetState()
