...
function setConfiguredFetchProviderKey(configTarget: UAGENTConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: UAGENTConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: UAGENTConfig, provider: ProviderUnderTest): unknown {
