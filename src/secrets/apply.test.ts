...
function buildFixturePaths(rootDir: string) {
  ...
  return {
    rootDir,
    stateDir,
    configPath: path.join(stateDir, "uagent.json"),
...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(os.tmpdir(), "uagent-secrets-apply-")),
  );
  ...
  return {
    ...paths,
    env: {
      UAGENT_STATE_DIR: paths.stateDir,
      UAGENT_CONFIG_PATH: paths.configPath,
