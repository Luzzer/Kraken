...
describe("secrets runtime snapshot core lanes", () => {
  ...
  it("resolves env-backed auth profile SecretRefs", async () => {
    ...
    expect(snapshot.warnings.map((warning) => warning.path)).toEqual(
      expect.arrayContaining([
        "/tmp/uagent-agent-main.auth-profiles.openai:default.key",
        "/tmp/uagent-agent-main.auth-profiles.github-copilot:default.token",
      ]),
    );
  ...
  it("activates runtime snapshots for ensureAuthProfileStore", async () => {
    ...
    expect(
      ensureAuthProfileStore("/tmp/uagent-agent-main").profiles["openai:default"],
    ).toMatchObject({
      type: "api_key",
      key: "sk-runtime",
    });
