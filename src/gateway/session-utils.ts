...
import { type UAGENTConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: UAGENTConfig): string[] {
...
export function listAgentsForGateway(cfg: UAGENTConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: UAGENTConfig): string {
...
function resolveSessionStoreAgentId(cfg: UAGENTConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: UAGENTConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: UAGENTConfig): GatewaySessionsDefaults {
