...
export const UAGENT_MODEL_ID = "uagent";
export const UAGENT_DEFAULT_MODEL_ID = "uagent/default";
...
export function resolveTrustedHttpOperatorScopes(
  ...
  const headerValue = getHeader(req, "x-uagent-scopes");
...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    getHeader(req, "x-uagent-agent-id")?.trim() ||
    getHeader(req, "x-uagent-agent")?.trim() ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === UAGENT_MODEL_ID || lowered === UAGENT_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^uagent[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `uagent` or `uagent/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-uagent-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-uagent-model`." };
  }
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-uagent-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-uagent-message-channel")) ??
