...
const ORIGINAL_GATEWAY_TOKEN_ENV = process.env.UAGENT_GATEWAY_TOKEN;
const OLD_TOKEN = "shared-token-old";
const NEW_TOKEN = "shared-token-new";
const DEFERRED_RESTART_DELAY_MS = 1_000;
const SECRET_REF_TOKEN_ID = "UAGENT_SHARED_AUTH_ROTATION_SECRET_REF";
...
afterAll(async () => {
  ...
  if (ORIGINAL_GATEWAY_TOKEN_ENV === undefined) {
    ...
    process.env.UAGENT_GATEWAY_TOKEN = ORIGINAL_GATEWAY_TOKEN_ENV;
  }
  await server.close();
});
...
async function openDeviceTokenWs(): Promise<WebSocket> {
  const identityPath = path.join(os.tmpdir(), `uagent-shared-auth-${process.pid}-${port}.json`);
