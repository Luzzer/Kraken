...
let lastCreateUAGENTToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/uagent-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateUAGENTToolsContext?.agentTo,
          agentThreadId: lastCreateUAGENTToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createUAGENTTools: (ctx: Record<string, unknown>) => {
      lastCreateUAGENTToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateUAGENTToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-uagent-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-uagent-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateUAGENTToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateUAGENTToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateUAGENTToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateUAGENTToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-uagent-message-to": "channel:24514",
        "x-uagent-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("treats shared-secret bearer auth as owner on /tools/invoke", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        authorization: "Bearer secret",
        "x-uagent-scopes": "operator.approvals",
      },
      tool: "owner_only_test",
      sessionKey: "main",
    });
