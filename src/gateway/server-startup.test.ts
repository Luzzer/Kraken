...
const ensureUAGENTModelsJsonMock = vi.fn<
...
vi.mock("../agents/agent-paths.js", () => ({
  resolveUAGENTAgentDir: () => "/tmp/agent",
}));

vi.mock("../agents/models-config.js", () => ({
  ensureUAGENTModelsJson: (config: unknown, agentDir: unknown) =>
    ensureUAGENTModelsJsonMock(config, agentDir),
}));
...
describe("gateway startup primary model warmup", () => {
  ...
  beforeEach(() => {
    ensureUAGENTModelsJsonMock.mockClear();
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureUAGENTModelsJsonMock).toHaveBeenCalledWith(cfg, "/tmp/agent");
  ...
  it("skips warmup when no explicit primary model is configured", async () => {
    ...
    expect(ensureUAGENTModelsJsonMock).not.toHaveBeenCalled();
