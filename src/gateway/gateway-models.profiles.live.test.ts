...
import { resolveUAGENTAgentDir } from "../agents/agent-paths.js";
...
import { ensureUAGENTModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/config.js";
import type { ModelsConfig, UAGENTConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.UAGENT_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.UAGENT_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.UAGENT_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.UAGENT_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.UAGENT_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.UAGENT_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["UAGENT_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.UAGENT_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.UAGENT_LIVE_MAX_MODELS, 0));
}
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.UAGENT_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.UAGENT_LIVE_MODEL_TIMEOUT_MS,
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<UAGENTConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.UAGENT_CONFIG_PATH,
    token: process.env.UAGENT_GATEWAY_TOKEN,
    skipChannels: process.env.UAGENT_SKIP_CHANNELS,
    skipGmail: process.env.UAGENT_SKIP_GMAIL_WATCHER,
    skipCron: process.env.UAGENT_SKIP_CRON,
    skipCanvas: process.env.UAGENT_SKIP_CANVAS_HOST,
    disableBonjour: process.env.UAGENT_DISABLE_BONJOUR,
    logLevel: process.env.UAGENT_LOG_LEVEL,
    agentDir: process.env.UAGENT_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.UAGENT_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.UAGENT_SKIP_CHANNELS = "1";
  process.env.UAGENT_SKIP_GMAIL_WATCHER = "1";
  process.env.UAGENT_SKIP_CRON = "1";
  process.env.UAGENT_SKIP_CANVAS_HOST = "1";
  if (QUIET_LIVE_LOGS) {
    process.env.UAGENT_DISABLE_BONJOUR = "1";
    process.env.UAGENT_LOG_LEVEL = "silent";
  }

  const token = `test-${randomUUID()}`;
  process.env.UAGENT_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveUAGENTAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-live-state-"));
  process.env.UAGENT_STATE_DIR = tempStateDir;
  ...
  process.env.UAGENT_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.uagent-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveUAGENTAgentDir();
  const sanitizedCfg: UAGENTConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-live-"));
  const tempConfigPath = path.join(tempDir, "uagent.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.UAGENT_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "UAGENT live tool probe (local, safe): " +
    ...
    process.env.UAGENT_CONFIG_PATH = previous.configPath;
    process.env.UAGENT_GATEWAY_TOKEN = previous.token;
    process.env.UAGENT_SKIP_CHANNELS = previous.skipChannels;
    process.env.UAGENT_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.UAGENT_SKIP_CRON = previous.skipCron;
    process.env.UAGENT_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.UAGENT_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.UAGENT_LOG_LEVEL = previous.logLevel;
    process.env.UAGENT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.UAGENT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        await ensureUAGENTModelsJson(cfg);
        ...
        const rawModels = process.env.UAGENT_LIVE_GATEWAY_MODELS?.trim();
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via UAGENT_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.uagent-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
