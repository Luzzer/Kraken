...
import { clearRuntimeConfigSnapshot, type UAGENTConfig } from "../config/config.js";
...
const CLI_LIVE = isTruthyEnvValue(process.env.UAGENT_LIVE_CLI_BACKEND);
const CLI_RESUME = isTruthyEnvValue(process.env.UAGENT_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("UAGENT_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.UAGENT_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "UAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.UAGENT_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const rawModel = process.env.UAGENT_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
      const parsed = parseModelRef(rawModel, "claude-cli");
      if (!parsed) {
        throw new Error(
          `UAGENT_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
        );
      }
      ...
      const cliCommand = process.env.UAGENT_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
      if (!cliCommand) {
        throw new Error(
          `UAGENT_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
        );
      }
      const baseCliArgs =
        parseJsonStringArray(
          "UAGENT_LIVE_CLI_BACKEND_ARGS",
          process.env.UAGENT_LIVE_CLI_BACKEND_ARGS,
        ) ?? providerDefaults?.args;
      if (!baseCliArgs || baseCliArgs.length === 0) {
        throw new Error(`UAGENT_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
      }
      const cliClearEnv =
        parseJsonStringArray(
          "UAGENT_LIVE_CLI_BACKEND_CLEAR_ENV",
          process.env.UAGENT_LIVE_CLI_BACKEND_CLEAR_ENV,
      ...
      const cliImageArg =
        process.env.UAGENT_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.UAGENT_LIVE_CLI_BACKEND_IMAGE_MODE) ??
        providerDefaults?.imageMode;

      if (cliImageMode && !cliImageArg) {
        throw new Error(
          "UAGENT_LIVE_CLI_BACKEND_IMAGE_MODE requires UAGENT_LIVE_CLI_BACKEND_IMAGE_ARG.",
        );
      }

      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-live-cli-"));
      const bootstrapWorkspace =
        providerId === "claude-cli" ? await createBootstrapWorkspace(tempDir) : null;
      const disableMcpConfig = process.env.UAGENT_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
      ...
      const cfgWithCliBackends = cfg as UAGENTConfig & {
        agents?: {
          defaults?: {
