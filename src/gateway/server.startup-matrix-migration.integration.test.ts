...
describe("gateway startup channel maintenance wiring", () => {
  it("runs startup channel maintenance with the resolved startup config", async () => {
    const previousBundledPluginsDir = process.env.UAGENT_BUNDLED_PLUGINS_DIR;
    const previousSkipChannels = process.env.UAGENT_SKIP_CHANNELS;
    process.env.UAGENT_BUNDLED_PLUGINS_DIR = path.resolve(process.cwd(), "extensions");
    ...
    try {
      ...
      if (previousSkipChannels === undefined) {
        ...
        process.env.UAGENT_SKIP_CHANNELS = previousSkipChannels;
      }
      clearPluginDiscoveryCache();
      clearPluginManifestRegistryCache();
    }
