...
const { createUAGENTTools } = await import("../agents/uagent-tools.js");
...
type SessionSendTool = ReturnType<typeof createUAGENTTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createUAGENTTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["UAGENT_GATEWAY_PORT", "UAGENT_GATEWAY_TOKEN"]);
  ...
  process.env.UAGENT_GATEWAY_PORT = String(gatewayPort);
  process.env.UAGENT_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("UAGENT_CONFIG_PATH missing in gateway test environment");
      }
