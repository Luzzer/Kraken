...
export function resolveGatewayReloadSettings(cfg: UAGENTConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: UAGENTConfig;
  initialInternalWriteHash?: string | null;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: UAGENTConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: UAGENTConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: UAGENTConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: UAGENTConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: UAGENTConfig) => {
