...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("uagent doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with UAGENT_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with UAGENT_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.UAGENT_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://uagent-gateway.ai:18789",
      onConnectError,
    });
