...
const loadConfig = vi.hoisted(() => vi.fn(() => ({}) as UAGENTConfig));
const resolveDefaultAgentId = vi.hoisted(() => vi.fn(() => "main"));
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg: UAGENTConfig, _agentId: string) => "/tmp/uagent"),
);
const resolveMemorySearchConfig = vi.hoisted(() =>
  vi.fn<(_cfg: UAGENTConfig, _agentId: string) => { enabled: boolean } | null>(() => ({
    enabled: true,
  })),
);
...
describe("doctor.memory.status", () => {
  beforeEach(() => {
    ...
    resolveAgentWorkspaceDir.mockReset().mockReturnValue("/tmp/uagent");
  ...
  it("includes dreaming counts and managed cron status when workspace data is available", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((cfg: UAGENTConfig, agentId: string) => {
      if (agentId === "alpha") {
    ...
    const cronList = vi.fn(async () => [
      {
        ...
        payload: {
          kind: "systemEvent",
          text: "__uagent_memory_core_short_term_promotion_dream__",
        },
  ...
  it("merges workspace store errors when multiple workspace stores are unreadable", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg: UAGENTConfig, agentId: string) =>
