...
export function resolveHooksConfig(cfg: UAGENTConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: UAGENTConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-uagent-token"] === "string"
      ? req.headers["x-uagent-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-uagent-idempotency-key"]) ||
