...
function loadGatewayConfig(): UAGENTConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: UAGENTConfig, env?: NodeJS.ProcessEnv): number {
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__UAGENT_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
