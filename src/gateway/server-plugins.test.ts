...
const loadUAGENTPlugins = vi.hoisted(() => vi.fn());
...
async function createSubagentRuntime(
  ...
  loadUAGENTPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadUAGENTPlugins.mock.calls.at(-1)?.[0] as
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadUAGENTPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("pins the initial startup channel registry against later active-registry churn", async () => {
    ...
    loadUAGENTPlugins.mockReturnValue(startupRegistry);
  ...
  test("treats an empty startup scope as no plugin load instead of an unscoped load", async () => {
    ...
    expect(loadUAGENTPlugins).not.toHaveBeenCalled();
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.uagent.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", async () => {
    ...
    loadUAGENTPlugins.mockReturnValue(pluginRegistry);
