...
beforeAll(async () => {
  sharedTempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-sessions-config-"));
...
describe("gateway config methods", () => {
  it("rejects config.set when SecretRef resolution fails", async () => {
    const missingEnvVar = `UAGENT_MISSING_SECRETREF_${Date.now()}`;
  ...
  it("does not reject config.set for unresolved auth-profile refs outside submitted config", async () => {
    const missingEnvVar = `UAGENT_MISSING_AUTH_PROFILE_REF_${Date.now()}`;
    delete process.env[missingEnvVar];

    const authStorePath = path.join(resolveUAGENTAgentDir(), AUTH_PROFILE_FILENAME);
  ...
  it("rejects config.patch when merged SecretRefs cannot resolve", async () => {
    const missingEnvVar = `UAGENT_MISSING_SECRETREF_PATCH_${Date.now()}`;
    delete process.env[missingEnvVar];
