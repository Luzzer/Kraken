...
import { ensureUAGENTCliOnPath } from "../infra/path-env.js";
...
function pruneSkippedStartupSecretSurfaces(config: UAGENTConfig): UAGENTConfig {
  const skipChannels =
    isTruthyEnvValue(process.env.UAGENT_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.UAGENT_SKIP_PROVIDERS);
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("uagent doctor --fix")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.UAGENT_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.UAGENT_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "UAGENT_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "UAGENT_RAW_STREAM_PATH",
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `uagent config set gateway.auth.mode token` and `uagent config set gateway.auth.token <token>`.",
      );
    }
  }
    ...
    transcriptUnsub = minimalTestGateway
      ...
      : onSessionTranscriptUpdate((update) => {
          ...
          const sessionSnapshot = sessionRow
            ...
            : {};
          const message = attachUAGENTTranscriptMeta(update.message, {
