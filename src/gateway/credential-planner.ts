...
type GatewaySecretDefaults = NonNullable<UAGENTConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${UAGENT_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.UAGENT_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.UAGENT_GATEWAY_PASSWORD);
}
