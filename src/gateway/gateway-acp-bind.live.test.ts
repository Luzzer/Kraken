...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.UAGENT_LIVE_ACP_BIND);
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.UAGENT_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.UAGENT_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.UAGENT_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "uagent.json");
      ...
      process.env.UAGENT_GATEWAY_PORT = String(port);
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.UAGENT_GATEWAY_PORT = previous.port;
        }
