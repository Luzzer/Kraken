...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-session-message-"));
...
describe("session.message websocket events", () => {
  test("includes spawned session ownership metadata on lifecycle sessions.changed events", async () => {
    const previousMinimalGateway = process.env.UAGENT_TEST_MINIMAL_GATEWAY;
    delete process.env.UAGENT_TEST_MINIMAL_GATEWAY;
    try {
      ...
      if (previousMinimalGateway === undefined) {
        ...
        process.env.UAGENT_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
      }
    }
  });
