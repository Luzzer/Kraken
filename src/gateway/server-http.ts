...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-UAGENT-Token header (query parameters are not allowed).",
      );
      return true;
    }
        ...
        wss.handleUpgrade(req, socket, head, (ws) => {
          (
            ws as unknown as import("ws").WebSocket & {
              __uagentPreauthBudgetClaimed?: boolean;
              __uagentPreauthBudgetKey?: string;
            }
          ).__uagentPreauthBudgetKey = preauthBudgetKey;
