...
const READ_SCOPE_HEADER = { "x-uagent-scopes": "operator.read" };
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-session-history-"));
...
describe("session history HTTP endpoints", () => {
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __uagent?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __uagent?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message.__uagent?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __uagent?: { seq?: number } }>;
        messages?: Array<{ __uagent?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message.__uagent?.seq)).toEqual([1]);
  ...
  test("streams bounded history windows over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(suppressedData.messages?.[0]?.__uagent?.seq).toBe(3);
      ...
      expect(nextData.messages?.[0]?.__uagent?.seq).toBe(4);
  ...
  test("sanitizes phased assistant history entries before returning them", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(body.messages?.[0]?.__uagent).toMatchObject({
        id: visibleMessageId,
        seq: 2,
      });
    });
  ...
  test("streams session history updates over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(
        (
          messageEvent.data as {
            message?: { __uagent?: { id?: string; seq?: number } };
          }
        ).message?.__uagent,
      ).toMatchObject({
