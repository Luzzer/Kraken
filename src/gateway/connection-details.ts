...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: UAGENTConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.UAGENT_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.UAGENT_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set UAGENT_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: uagent doctor --fix",
