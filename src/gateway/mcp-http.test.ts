  ...
  it("passes session, account, and message channel headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-uagent-account-id": "work",
        "x-uagent-message-channel": "telegram",
      },
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.uagent?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.uagent?.headers?.Authorization).toBe(
      "Bearer ${UAGENT_MCP_TOKEN}",
    );
    expect(config.mcpServers?.uagent?.headers?.["x-uagent-message-channel"]).toBe(
      "${UAGENT_MCP_MESSAGE_CHANNEL}",
    );
  });
});
