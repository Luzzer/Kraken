...
function cfg(input: Partial<UAGENTConfig>): UAGENTConfig {
  return input as UAGENTConfig;
}
...
const DEFAULT_ENV = {
  UAGENT_GATEWAY_TOKEN: "env-token",
  UAGENT_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when UAGENT env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when UAGENT env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when UAGENT env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
