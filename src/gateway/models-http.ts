...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([UAGENT_MODEL_ID, UAGENT_DEFAULT_MODEL_ID]);
  ids.add(`uagent/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`uagent/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== UAGENT_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
