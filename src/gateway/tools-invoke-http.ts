...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-uagent-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-uagent-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-uagent-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-uagent-thread-id")?.trim() || undefined;
