...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "UAGENT_SKIP_BROWSER_CONTROL_SERVER",
  "UAGENT_SKIP_GMAIL_WATCHER",
  "UAGENT_SKIP_CANVAS_HOST",
  "UAGENT_BUNDLED_PLUGINS_DIR",
  "UAGENT_SKIP_CHANNELS",
  "UAGENT_SKIP_PROVIDERS",
  "UAGENT_SKIP_CRON",
  "UAGENT_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.UAGENT_CONFIG_PATH) {
    configPaths.add(process.env.UAGENT_CONFIG_PATH);
  }
  if (process.env.UAGENT_STATE_DIR) {
    configPaths.add(path.join(process.env.UAGENT_STATE_DIR, "uagent.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.UAGENT_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.UAGENT_SKIP_GMAIL_WATCHER = "1";
  process.env.UAGENT_SKIP_CANVAS_HOST = "1";
  process.env.UAGENT_SKIP_CHANNELS = "1";
  process.env.UAGENT_SKIP_PROVIDERS = "1";
  process.env.UAGENT_SKIP_CRON = "1";
  process.env.UAGENT_TEST_MINIMAL_GATEWAY = "1";
  process.env.UAGENT_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "uagent-test-no-bundled-extensions")
    : "uagent-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  const stateDir = process.env.UAGENT_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".uagent-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".uagent-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".uagent-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>uagent-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__uagentTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__uagentTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["UAGENT_GATEWAY_TOKEN"]);
  const prev = process.env.UAGENT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.UAGENT_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.UAGENT_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
