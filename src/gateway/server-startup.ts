...
import type { loadUAGENTPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureUAGENTModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadUAGENTPlugins>;
  ...
  // Tests can opt out via UAGENT_SKIP_CHANNELS (or legacy UAGENT_SKIP_PROVIDERS).
  const skipChannels =
  ...
  if (!skipChannels) {
    ...
    params.logChannels.info(
      "skipping channel start (UAGENT_SKIP_CHANNELS=1 or UAGENT_SKIP_PROVIDERS=1)",
    );
  }
