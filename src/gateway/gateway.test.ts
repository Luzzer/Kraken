...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".uagent", "extensions", params.id);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "uagent");
      await fs.mkdir(workspaceDir, { recursive: true });

      const configDir = path.join(tempHome, ".uagent");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "uagent.json");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-gw-http-tools-home-"));
      process.env.HOME = tempHome;
      process.env.UAGENT_STATE_DIR = path.join(tempHome, ".uagent");
      delete process.env.UAGENT_CONFIG_PATH;
      process.env.UAGENT_SKIP_CHANNELS = "1";
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-wizard-home-"));
      process.env.HOME = tempHome;
