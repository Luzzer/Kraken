...
export function attachUAGENTTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__uagent && typeof record.__uagent === "object" && !Array.isArray(record.__uagent)
      ? (record.__uagent as Record<string, unknown>)
      : {};
  return {
    ...record,
    __uagent: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachUAGENTTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "uagent" && model === "delivery-mirror";
