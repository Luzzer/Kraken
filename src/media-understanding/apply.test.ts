...
const TEMP_MEDIA_PREFIX = "uagent-media-";
...
function createGroqAudioConfig(): UAGENTConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): UAGENTConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): UAGENTConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: UAGENTConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredUAGENTTmpDir();
