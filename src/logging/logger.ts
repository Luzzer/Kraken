...
import {
  POSIX_UAGENT_TMP_DIR,
  resolvePreferredUAGENTTmpDir,
} from "../infra/tmp-uagent-dir.js";
...
function canUseSilentVitestFileLogFastPath(envLevel: LogLevel | undefined): boolean {
  return (
    ...
    process.env.UAGENT_TEST_FILE_LOG !== "1" &&
    !envLevel &&
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: UAGENTConfig["logging"] | undefined =
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.UAGENT_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[uagent] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
