...
let originalUAGENTLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.UAGENT_TEST_FILE_LOG;
  originalUAGENTLogLevel = process.env.UAGENT_LOG_LEVEL;
  delete process.env.UAGENT_TEST_FILE_LOG;
  delete process.env.UAGENT_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.UAGENT_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalUAGENTLogLevel === undefined) {
    delete process.env.UAGENT_LOG_LEVEL;
  } else {
    process.env.UAGENT_LOG_LEVEL = originalUAGENTLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
