...
describe("resolveInitialTuiAgentId", () => {
  const cfg: UAGENTConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/uagent" },
        { id: "ops", workspace: "/tmp/uagent/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/uagent/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run uagent devices list");
    expect(state.pairingHint).toContain("uagent devices list");
  });
