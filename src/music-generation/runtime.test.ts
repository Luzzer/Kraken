...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    getMusicGenerationProvider: vi.fn<
      (providerId: string, config?: UAGENTConfig) => MusicGenerationProvider | undefined
    >(() => undefined),
    ...
    listMusicGenerationProviders: vi.fn<(config?: UAGENTConfig) => MusicGenerationProvider[]>(
      () => [],
    ),
...
describe("music-generation runtime", () => {
  ...
  it("lists runtime music-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeMusicGenerationProviders({ config: {} as UAGENTConfig })).toEqual(
      providers,
    );
    expect(mocks.listMusicGenerationProviders).toHaveBeenCalledWith({} as UAGENTConfig);
  });
