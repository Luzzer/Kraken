...
describe("test-projects args", () => {
  ...
  it("routes boundary targets to the boundary config", () => {
    expect(buildVitestRunPlans(["src/infra/uagent-root.test.ts"])).toEqual([
      {
        config: "vitest.boundary.config.ts",
        forwardedArgs: [],
        includePatterns: ["src/infra/uagent-root.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("writes scoped include files for routed extension runs", () => {
    ...
    expect(spec?.includeFilePath).toContain("uagent-vitest-include-");
    expect(spec?.env.UAGENT_VITEST_INCLUDE_FILE).toBe(spec?.includeFilePath);
  });
