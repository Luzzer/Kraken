...
function isCurrentUAGENTClient(clientId: string | undefined): boolean {
  const normalized = clientId?.trim().toLowerCase() ?? "";
  return normalized.startsWith("uagent-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentUAGENTClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentUAGENTClient(node.clientId)) {
    score += 10;
