...
import { loadConfig, type UAGENTConfig } from "../config/config.js";
...
const REQUIRE_PROFILE_KEYS =
  isLiveProfileKeyModeEnabled() || isTruthyEnvValue(process.env.UAGENT_LIVE_REQUIRE_PROFILE_KEYS);
const describeLive = LIVE ? describe : describe.skip;
const providerFilter = parseCsvFilter(process.env.UAGENT_LIVE_IMAGE_GENERATION_PROVIDERS);
const caseFilter = parseCaseFilter(process.env.UAGENT_LIVE_IMAGE_GENERATION_CASES);
const envModelMap = parseProviderModelMap(process.env.UAGENT_LIVE_IMAGE_GENERATION_MODELS);
...
function withPluginsEnabled(cfg: UAGENTConfig): UAGENTConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,
