...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    getImageGenerationProvider: vi.fn<
      (providerId: string, config?: UAGENTConfig) => ImageGenerationProvider | undefined
    >(() => undefined),
    ...
    listImageGenerationProviders: vi.fn<(config?: UAGENTConfig) => ImageGenerationProvider[]>(
      () => [],
    ),
...
describe("image-generation runtime", () => {
  ...
  it("lists runtime image-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeImageGenerationProviders({ config: {} as UAGENTConfig })).toEqual(
      providers,
    );
    expect(mocks.listImageGenerationProviders).toHaveBeenCalledWith({} as UAGENTConfig);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    const promise = generateImage({ cfg: {} as UAGENTConfig, prompt: "draw a cat" });
