...
const discoverUAGENTPluginsMock = vi.fn();
const loadPluginManifestMock = vi.fn();

vi.mock("./discovery.js", () => ({
  discoverUAGENTPlugins: (...args: unknown[]) => discoverUAGENTPluginsMock(...args),
}));
...
function setBundledDiscoveryCandidates(candidates: unknown[]) {
  discoverUAGENTPluginsMock.mockReturnValue({
    candidates,
    diagnostics: [],
  });
}
  ...
  loadPluginManifestMock.mockImplementation((rootDir: string) =>
    rootDir in manifestIds
      ...
      : {
          ...
          manifestPath: `${rootDir}/uagent.plugin.json`,
        },
  );
}
...
function createResolvedBundledSource(params: {
  ...
  return {
    ...
    npmSpec: params.npmSpec ?? `@uagent/${params.pluginId}`,
  };
}
...
describe("bundled plugin sources", () => {
  beforeEach(() => {
    discoverUAGENTPluginsMock.mockReset();
  ...
  it("forwards an explicit env to bundled discovery helpers", () => {
    ...
    const env = { HOME: "/tmp/uagent-home" } as NodeJS.ProcessEnv;
    ...
    expect(discoverUAGENTPluginsMock).toHaveBeenNthCalledWith(1, {
      workspaceDir: "/workspace",
      env,
    });
    expect(discoverUAGENTPluginsMock).toHaveBeenNthCalledWith(2, {
      workspaceDir: "/workspace",
      env,
    });
  });
  ...
  it("reuses a pre-resolved bundled map for repeated lookups", () => {
    ...
    expect(
      findBundledPluginSourceInMap({
        bundled,
        lookup: { kind: "npmSpec", value: "@uagent/feishu" },
      })?.pluginId,
    ).toBe("feishu");
  });
});
