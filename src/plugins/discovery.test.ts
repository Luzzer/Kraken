...
import { clearPluginDiscoveryCache, discoverUAGENTPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("uagent-plugins", tempDirs);
}
...
function buildCachedDiscoveryEnv(
  ...
  return {
    ...
    UAGENT_PLUGIN_DISCOVERY_CACHE_MS: "5000",
    ...overrides,
  };
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverUAGENTPlugins>[0],
) {
  return discoverUAGENTPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}

function discoverWithCachedEnv(params: Parameters<typeof discoverUAGENTPlugins>[0]) {
  return discoverUAGENTPlugins(params);
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      uagent: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
function writePluginManifest(params: { pluginDir: string; id: string }) {
  fs.writeFileSync(
    path.join(params.pluginDir, "uagent.plugin.json"),
    JSON.stringify({
      id: params.id,
...
function expectCandidatePresence(
  result: Awaited<ReturnType<typeof discoverUAGENTPlugins>>,
...
describe("discoverUAGENTPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".uagent", "extensions");
  ...
  it("does not recurse arbitrary workspace directories for plugin auto-discovery", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@uagent/workspace-plugin",
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@uagent/stray-workspace-plugin",
    });

    const result = discoverUAGENTPlugins({
      workspaceDir,
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".uagent", "extensions");
    mkdirSafe(workspaceExt);
  ...
  it("does not treat repo-level live or test files as plugin entrypoints", () => {
    ...
    const { candidates, diagnostics } = discoverUAGENTPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    const stateDir = makeTempDir();
    const pluginDir = path.join(stateDir, "extensions", "opik-uagent");
    ...
    writePluginPackageManifest({
      ...
      packageName: "@opik/opik-uagent",
      extensions: ["./src/index.ts"],
    });
    writePluginManifest({ pluginDir, id: "opik-uagent" });
    fs.writeFileSync(
    ...
    writePluginPackageManifest({
      packageDir: path.join(pluginDir, "node_modules", "uagent"),
      packageName: "uagent",
    ...
    const { candidates } = await discoverWithStateDir(stateDir, {});
    expectCandidateOrder(candidates, ["opik-uagent"]);
  });

  it("skips dependency and build directories while scanning workspace roots", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const workspaceRoot = path.join(workspaceDir, ".uagent", "extensions");
    const workspacePluginDir = path.join(workspaceRoot, "workspace-plugin");
    const nestedNodeModulesDir = path.join(workspaceRoot, "node_modules", "uagent");
    ...
    const { candidates } = discoverUAGENTPlugins({
      workspaceDir,
      env: buildDiscoveryEnv(stateDir),
    });
  ...
  it.each([
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@uagent/ollama-provider",
          pluginId: "ollama",
          entryPath: "src/index.ts",
        });
        return {};
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        for (const [dirName, packageName, pluginId] of [
          ["elevenlabs-speech-pack", "@uagent/elevenlabs-speech", "elevenlabs"],
          ["microsoft-speech-pack", "@uagent/microsoft-speech", "microsoft"],
        ] as const) {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@uagent/demo-plugin-dir",
          entryPath: "index.js",
        });
  ...
  it.each([
    ...
    expect(legacy?.format).toBe("uagent");
  ...
  it.each([
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@uagent/escape-pack",
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@uagent/missing-entry-pack",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const result = discoverUAGENTPlugins({
        env: {
          ...process.env,
          UAGENT_DISABLE_BUNDLED_PLUGINS: undefined,
          UAGENT_STATE_DIR: stateDir,
          UAGENT_BUNDLED_PLUGINS_DIR: bundledDir,
        },
      });
