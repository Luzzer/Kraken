...
import { normalizeUAGENTVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeUAGENTVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeUAGENTVersionBase(plugin.version) ??
    plugin.version
  );
}
