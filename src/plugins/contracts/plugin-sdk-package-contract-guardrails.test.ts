...
const PLUGIN_SDK_SUBPATH_PATTERN = /uagent\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
function packUAGENTToTempDir(packDir: string): string {
...
async function readPackedRootPackageJson(archivePath: string): Promise<{
  ...
  const extractDir = makeTrackedTempDir("uagent-packed-root-package-json", tempDirs);
  await tar.x({
    file: archivePath,
    cwd: extractDir,
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references uagent/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("mirrors matrix runtime deps needed by the bundled host graph", () => {
    ...
    const allowlist = matrixPackageJson.uagent?.releaseChecks?.rootDependencyMirrorAllowlist;
  ...
  it("mirrors Bedrock runtime deps needed by the bundled host graph", () => {
    ...
    const allowlist = bedrockPackageJson.uagent?.releaseChecks?.rootDependencyMirrorAllowlist;
  ...
  it("keeps matrix crypto WASM in the packed artifact manifest", async () => {
    const tempRoot = makeTrackedTempDir("uagent-matrix-wasm-pack", tempDirs);
    const packDir = join(tempRoot, "pack");
    mkdirSync(packDir, { recursive: true });

    const archivePath = packUAGENTToTempDir(packDir);
