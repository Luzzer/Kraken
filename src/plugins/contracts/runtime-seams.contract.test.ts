...
function createInstalledRuntimePluginDir(
  ...
  const bundledDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `uagent-runtime-contract-bundled-${pluginId}-`),
  );
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `uagent-runtime-contract-state-${pluginId}-`),
  );
  ...
  fs.writeFileSync(
    path.join(pluginRoot, "uagent.plugin.json"),
    JSON.stringify({
      id: pluginId,
      channels: [pluginId],
