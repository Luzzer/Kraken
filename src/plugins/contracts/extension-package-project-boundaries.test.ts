...
describe("opt-in extension package boundaries", () => {
  it("keeps path aliases in a dedicated shared config", () => {
    ...
    expect(pathsConfig.compilerOptions?.paths).toEqual({
      "uagent/extension-api": ["../src/extensionAPI.ts"],
      "uagent/plugin-sdk": ["../packages/plugin-sdk/dist/src/plugin-sdk/index.d.ts"],
      "uagent/plugin-sdk/*": ["../packages/plugin-sdk/dist/src/plugin-sdk/*.d.ts"],
      "uagent/plugin-sdk/account-id": ["../src/plugin-sdk/account-id.ts"],
      "@uagent/*": ["../packages/plugin-sdk/dist/extensions/*", "../extensions/*"],
      "@uagent/plugin-sdk/*": ["../packages/plugin-sdk/dist/src/plugin-sdk/*.d.ts"],
    });
  ...
  it("keeps every opt-in extension rooted inside its package and on the package sdk", () => {
    ...
    for (const extensionName of optInExtensions) {
      ...
      expect(packageJson.devDependencies?.["@uagent/plugin-sdk"]).toBe("workspace:*");
    }
  });
  ...
  it("keeps plugin-sdk package types generated from the package build, not a hand-maintained types bridge", () => {
    ...
    expect(packageJson.name).toBe("@uagent/plugin-sdk");
