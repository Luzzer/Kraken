...
const CORE_PLUGIN_ENTRY_IMPORT_RE =
  /import\s*\{[^}]*\bdefinePluginEntry\b[^}]*\}\s*from\s*"uagent\/plugin-sdk\/core"/;
    ...
    for (const plugin of listBundledPluginRoots()) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          uagent?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.uagent?.extensions) ? pkg.uagent.extensions : [];
        if (
          extensions.some(
