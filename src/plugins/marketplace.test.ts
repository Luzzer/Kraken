...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("uagent-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("uagent-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("uagent-marketplace-test-", async (homeDir) => {
      const openClawHome = path.join(homeDir, "uagent-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, UAGENT_HOME: openClawHome },
