 ...
 */
export type UAGENTPluginConfigSchema = {
...
export type UAGENTPluginToolContext = {
  config?: UAGENTConfig;
...
export type UAGENTPluginToolFactory = (
  ctx: UAGENTPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type UAGENTPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type UAGENTPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<UAGENTConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). UAGENT uses this to expose
 ...
 *
 * Runs after UAGENT resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common UAGENT auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and UAGENT wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before UAGENT builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. UAGENT no
...
/**
 * Provider-owned stream wrapper hook after UAGENT applies its generic
 ...
 *
 * Return `true` or `false` to override UAGENT's built-in provider cache TTL
 ...
 *
 * Runs only after UAGENT exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that UAGENT should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after UAGENT loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, UAGENT falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * UAGENT calls this only from async model resolution paths. After it
   ...
   *
   * Runs after UAGENT performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before UAGENT registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic UAGENT wrappers.
   *
   ...
   *
   * Called after UAGENT resolves the raw configured credential but before the
   ...
   * hint than UAGENT's generic auth-store guidance.
   */
   ...
   * UAGENT's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when UAGENT should surface a provider-specific hint for
   ...
   * merging. UAGENT deduplicates by `provider/id`, so plugins only need to
   ...
   * without replacing the full UAGENT-owned system prompt.
   */
   ...
   *
   * UAGENT uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `uagent doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `uagent doctor` should offer.
   *
   ...
   *
   * UAGENT calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep UAGENT's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * UAGENT falls back to that stored profile.
   */
...
export type WebSearchProviderPlugin = {
   ...
   * when UAGENT has no config-aware runtime context yet.
   *
  ...
  getConfiguredCredentialValue?: (config?: UAGENTConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: UAGENTConfig, value: unknown) => void;
  applySelectionConfig?: (config: UAGENTConfig) => UAGENTConfig;
  runSetup?: (ctx: WebSearchProviderSetupContext) => UAGENTConfig | Promise<UAGENTConfig>;
...
export type UAGENTPluginGatewayMethod = {
  method: string;
 ...
 */
export type UAGENTPluginCommandDefinition = {
...
export type UAGENTPluginHttpRouteAuth = "gateway" | "plugin";
export type UAGENTPluginHttpRouteMatch = "exact" | "prefix";

export type UAGENTPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type UAGENTPluginHttpRouteParams = {
  path: string;
  handler: UAGENTPluginHttpRouteHandler;
  auth: UAGENTPluginHttpRouteAuth;
  match?: UAGENTPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type UAGENTPluginCliContext = {
  program: Command;
  config: UAGENTConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type UAGENTPluginCliRegistrar = (ctx: UAGENTPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want UAGENT to keep a plugin command lazy-loaded while still
 ...
 */
export type UAGENTPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type UAGENTPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type UAGENTPluginNodeHostCommand = {
  command: string;
  cap?: string;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type UAGENTPluginSecurityAuditContext = {
  config: UAGENTConfig;
...
export type UAGENTPluginSecurityAuditCollector = (
  ctx: UAGENTPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

/** Context passed to long-lived plugin services. */
export type UAGENTPluginServiceContext = {
  config: UAGENTConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type UAGENTPluginService = {
  id: string;
  start: (ctx: UAGENTPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: UAGENTPluginServiceContext) => void | Promise<void>;
};
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether UAGENT should inject bundle MCP config for this backend.
   *
...
export type UAGENTPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type UAGENTPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  kind?: PluginKind | PluginKind[];
  configSchema?: UAGENTPluginConfigSchema;
  reload?: UAGENTPluginReloadRegistration;
  nodeHostCommands?: UAGENTPluginNodeHostCommand[];
  securityAuditCollectors?: UAGENTPluginSecurityAuditCollector[];
  register?: (api: UAGENTPluginApi) => void | Promise<void>;
  activate?: (api: UAGENTPluginApi) => void | Promise<void>;
};

export type UAGENTPluginModule =
  | UAGENTPluginDefinition
  | ((api: UAGENTPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

export type PluginConfigMigration = (config: UAGENTConfig) =>
  | {
      config: UAGENTConfig;
      changes: string[];
    }
...
export type UAGENTPluginApi = {
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | UAGENTPluginToolFactory,
    opts?: UAGENTPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: UAGENTPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: UAGENTPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: UAGENTPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: UAGENTPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, UAGENT can keep
       * the plugin registrar lazy in the normal root CLI path. Command-only
       * registrations stay on the eager compatibility path.
       */
      descriptors?: UAGENTPluginCliCommandDescriptor[];
    },
  ) => void;
  registerReload: (registration: UAGENTPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: UAGENTPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: UAGENTPluginSecurityAuditCollector) => void;
  registerService: (service: UAGENTPluginService) => void;
   ...
   */
  registerCommand: (command: UAGENTPluginCommandDefinition) => void;
...
export type PluginFormat = "uagent" | "bundle";
...
export type PluginHookBeforeToolCallResult = {
  ...
  requireApproval?: {
     ...
     * UAGENT does not await this callback before allowing or denying the tool call.
     */
...
export type PluginHookBeforeInstallPlugin = {
  /** Canonical plugin id UAGENT will install under. */
  pluginId: string;
...
export type PluginHookBeforeInstallContext = {
  ...
  /** Normalized origin of the install target (e.g. "uagent-bundled", "plugin-package"). */
  origin?: string;
};
