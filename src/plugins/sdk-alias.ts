...
import { resolveUAGENTPackageRootSync } from "../infra/uagent-root.js";
...
function windowsAbsolutePathToFileUrl(specifier: string): string {
...
function hasTrustedUAGENTRootIndicator(params: {
  packageRoot: string;
  ...
  const hasUAGENTBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("uagent")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.uagent === "string");
  const hasUAGENTEntrypoint = fs.existsSync(path.join(params.packageRoot, "uagent.mjs"));
  return hasCliEntryExport || hasUAGENTBin || hasUAGENTEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedUAGENTRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedUAGENTRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveUAGENTPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedUAGENTRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveUAGENTPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedUAGENTRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["uagent/plugin-sdk", "@uagent/plugin-sdk"] as const;
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias
      ? { "uagent/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
    ...(pluginSdkAlias
      ? Object.fromEntries(
