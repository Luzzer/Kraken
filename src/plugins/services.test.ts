...
import type { UAGENTPluginService, UAGENTPluginServiceContext } from "./types.js";
...
function createRegistry(services: UAGENTPluginService[]) {
...
function expectServiceContext(
  ctx: UAGENTPluginServiceContext,
...
function expectServiceLogger(ctx: UAGENTPluginServiceContext) {
...
function expectServiceContexts(
  contexts: UAGENTPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: UAGENTPluginServiceContext[] = [];
