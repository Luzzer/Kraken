...
import type { UAGENTPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("uagent-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "uagent.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  format?: "uagent" | "bundle";
  bundleFormat?: "codex" | "claude" | "cursor";
  packageManifest?: UAGENTPackageManifest;
  packageDir?: string;
...
function hermeticEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  return {
    UAGENT_BUNDLED_PLUGINS_DIR: undefined,
    UAGENT_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
    UAGENT_VERSION: undefined,
    VITEST: "true",
    ...overrides,
  };
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "uagent.plugin.json");
  const linkedManifest = path.join(rootDir, "uagent.plugin.json");
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "uagent.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it.each([
    {
      ...
      env: { UAGENT_VERSION: "2026.3.21" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires UAGENT >=2026.3.22, but this host is 2026.3.21",
      expectWarn: false,
    },
    {
      name: "rejects invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedMessage: "plugin manifest invalid | uagent.install.minHostVersion must use",
      expectWarn: false,
    },
    {
      name: "warns distinctly when host version cannot be determined",
      minHostVersion: ">=2026.3.22",
      env: { UAGENT_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        ...
        UAGENT_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: hermeticEnv({
        HOME: homeB,
        UAGENT_HOME: undefined,
        UAGENT_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
