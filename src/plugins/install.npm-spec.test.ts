...
const suiteTempRootTracker = createSuiteTempRootTracker("uagent-plugin-install-npm-spec");
...
describe("installPluginFromNpmSpec", () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@uagent/voice-call@0.0.1",
              name: "@uagent/voice-call",
              version: "0.0.1",
              filename: packedName,
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@uagent/voice-call@0.0.2-beta.1",
      name: "@uagent/voice-call",
...

    {
      ...
      if (!result.ok) {
        ...
        expect(result.error).toContain('"@uagent/voice-call@beta"');
      }
    }
...

    {
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@uagent/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls as Array<[unknown, unknown]>,
        expectedSpec: "@uagent/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });
