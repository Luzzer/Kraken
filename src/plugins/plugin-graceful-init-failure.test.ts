...
const fixtureRoot = makeTrackedTempDir("uagent-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "uagent.plugin.json");
...
async function loadPlugins(pluginPaths: string[], warnings?: string[]) {
  const { loadUAGENTPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("uagent plugins list");
  });
});
