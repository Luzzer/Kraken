...
import { discoverUAGENTPlugins } from "./discovery.js";
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadUAGENTPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadUAGENTPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: UAGENTPluginDefinition;
  register?: UAGENTPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as UAGENTPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as UAGENTPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.UAGENT_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverUAGENTPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverUAGENTPlugins>["candidates"][number];
  right: ReturnType<typeof discoverUAGENTPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadUAGENTPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadUAGENTPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  try {
    ...
    const discovery = discoverUAGENTPlugins({
      workspaceDir: options.workspaceDir,
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into UAGENT yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      try {
        ...
        mod = getJiti(safeSource)(safeImportSource) as UAGENTPluginModule;
      } catch (err) {
        recordPluginError({
          logger,
          registry,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'uagent plugins list' for details.`,
        );
      }
    }
...
export async function loadUAGENTPluginCliRegistry(
  options: PluginLoadOptions = {},
