...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "uagent", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".uagent", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".uagent", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "uagent-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".uagent", "extensions"),
      },
    });
  });
});
