...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-uagent",
    targetDir: params?.targetDir ?? "/tmp/opik-uagent",
...
function createCodexAppServerInstallConfig(params: {
  ...
  return {
    plugins: {
      installs: {
        "uagent-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/uagent-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@uagent/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["uagent-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/uagent-codex-app-server",
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-uagent",
        spec: "@opik/opik-uagent",
        integrity: "sha512-old",
        installPath: "/tmp/opik-uagent",
      }),
      pluginIds: ["opik-uagent"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-uagent",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "uagent-codex-app-server",
        spec: "uagent-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/uagent-codex-app-server",
      }),
      pluginIds: ["uagent-codex-app-server"],
      specOverrides: {
        "uagent-codex-app-server": "uagent-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "uagent-codex-app-server",
        targetDir: "/tmp/uagent-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "uagent-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @uagent/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@uagent/missing",
      ...
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @uagent/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "uagent-codex-app-server@beta",
        resolvedName: "uagent-codex-app-server",
        resolvedSpec: "uagent-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "uagent-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "uagent-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "uagent-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "uagent-codex-app-server",
      }),
      specOverrides: {
        "uagent-codex-app-server": "uagent-codex-app-server@beta",
      },
      expectedSpec: "uagent-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "uagent-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "uagent-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@uagent/voice-call",
      targetDir: "/tmp/uagent-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@uagent/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@uagent/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@uagent/voice-call");
    expect(result.config.plugins?.entries?.["@uagent/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@uagent/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@uagent/voice-call",
      installPath: "/tmp/uagent-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/uagent-home";
    mockBundledSources(
      createBundledSource({
