...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "uagent-empty-bundled-plugins");

function bundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const raw = env.UAGENT_DISABLE_BUNDLED_PLUGINS?.trim().toLowerCase();
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "uagent.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.UAGENT_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveUAGENTPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveUAGENTPackageRootSync({ argv1: process.argv[1] }),
      resolveUAGENTPackageRootSync({ cwd: process.cwd() }),
      resolveUAGENTPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
