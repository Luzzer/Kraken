...
const APPROVALS_PATH = "~/.uagent/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "uagent-app-server:thread:",
  "uagent-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("uagent.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("uagent.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to UAGENT instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
