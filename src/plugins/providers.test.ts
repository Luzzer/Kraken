...
type LoadUAGENTPlugins = typeof import("./loader.js").loadUAGENTPlugins;
...
const loadUAGENTPluginsMock = vi.fn<LoadUAGENTPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/uagent.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadUAGENTPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: UAGENTConfig = {
    plugins: {},
  };
  const autoEnabledConfig: UAGENTConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadUAGENTPlugins: (...args: Parameters<LoadUAGENTPlugins>) =>
        loadUAGENTPluginsMock(...args),
  ...
  beforeEach(() => {
    ...
    loadUAGENTPluginsMock.mockReset();
