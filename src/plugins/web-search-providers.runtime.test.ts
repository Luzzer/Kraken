...
let loadUAGENTPluginsMock: ReturnType<typeof vi.fn>;
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/uagent.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/uagent.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
function expectRuntimeProviderResolution(
  ...
  expect(loadUAGENTPluginsMock).not.toHaveBeenCalled();
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadUAGENTPluginsMock = vi
      .spyOn(loaderModule, "loadUAGENTPlugins")
  ...
  it("retains the snapshot cache when config contents change in place", () => {
    ...
    const env = createWebSearchEnv({ UAGENT_HOME: "/tmp/uagent-home-a" });
  ...
  it("invalidates the snapshot cache when env contents change in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.UAGENT_HOME = "/tmp/uagent-home-b";
      },
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        UAGENT_PLUGIN_DISCOVERY_CACHE_MS: "0",
      },
    },
  ])("$title", ({ env }) => {
    expectSnapshotMemoization({
  ...
  it("does not leak host Vitest env into an explicit non-Vitest cache key", () => {
    ...
    expect(loadUAGENTPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      UAGENT_PLUGIN_DISCOVERY_CACHE_MS: "5",
      UAGENT_PLUGIN_MANIFEST_CACHE_MS: "20",
    });
    ...
    expect(loadUAGENTPluginsMock).toHaveBeenCalledTimes(2);
  });

  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      UAGENT_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    });
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.UAGENT_PLUGIN_DISCOVERY_CACHE_MS = "5";
      },
      expectedLoaderCalls: 2,
    });
  });
