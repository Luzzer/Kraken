...
type PluginConfig = NonNullable<UAGENTConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: UAGENTConfig["channels"];
}): UAGENTConfig {
...
function createSinglePluginWithEmptySlotsConfig(): UAGENTConfig {
...
function createSingleNpmInstallConfig(installPath: string): UAGENTConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as UAGENTConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "uagent-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-uagent-plugin-install/my-plugin",
      },
      extensionsDir,
    });
