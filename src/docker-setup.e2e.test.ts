...
const sandboxRootTracker = createSuiteTempRootTracker({ prefix: "uagent-docker-setup-" });
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    UAGENT_GATEWAY_TOKEN: "test-token",
    UAGENT_CONFIG_DIR: join(sandbox.rootDir, "config"),
    UAGENT_WORKSPACE_DIR: join(sandbox.rootDir, "uagent"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("uagent-gateway");
}
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    UAGENT_GATEWAY_TOKEN: undefined,
    UAGENT_CONFIG_DIR: configDir,
    UAGENT_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      UAGENT_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      UAGENT_EXTRA_MOUNTS: undefined,
      UAGENT_HOME_VOLUME: "uagent-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("UAGENT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("UAGENT_EXTRA_MOUNTS=");
    expect(envFile).toContain("UAGENT_HOME_VOLUME=uagent-home"); // pragma: allowlist secret
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("uagent-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("uagent-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg UAGENT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node uagent-gateway dist/index.js onboard --mode local --no-install-daemon",
    );
    expect(log).toContain(
      'run --rm --no-deps --entrypoint node uagent-gateway dist/index.js config set --batch-json [{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]',
    );
    expect(log).not.toContain("run --rm uagent-cli onboard --mode local --no-install-daemon");
  });

  it("avoids shared-network uagent-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(prestartLines.some((line) => /\bcompose\b.*\brun\b.*\buagent-cli\b/.test(line))).toBe(
      false,
    );
  });
  ...
  it("writes UAGENT_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      UAGENT_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("UAGENT_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh uagent-gateway -c");
  });

  it("reuses existing config token when UAGENT_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("UAGENT_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when UAGENT_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "UAGENT_GATEWAY_TOKEN=dotenv-token-123\nUAGENT_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("UAGENT_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "UAGENT_GATEWAY_TOKEN=first-token",
        "UAGENT_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("UAGENT_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("UAGENT_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats UAGENT_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("UAGENT_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg UAGENT_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg UAGENT_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      UAGENT_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker uagent-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        UAGENT_SANDBOX: "1",
        UAGENT_DOCKER_SOCKET: socketPath,
      ...
      expect(log).toContain(
        "run --rm --no-deps uagent-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate uagent-gateway"));
  ...
  it("rejects injected multiline UAGENT_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      UAGENT_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("UAGENT_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid UAGENT_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      UAGENT_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid UAGENT_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      UAGENT_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("UAGENT_HOME_VOLUME must match");
  });

  it("rejects UAGENT_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("UAGENT_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:uagent-gateway"');
    expect(compose).toContain("depends_on:\n      - uagent-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/UAGENT_GATEWAY_TOKEN: \$\{UAGENT_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{UAGENT_TZ:-UTC\}/g)).toHaveLength(2);
  });
});
