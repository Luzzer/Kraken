...
```bash
export UAGENT_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.uagent/uagent.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env UAGENT_PARALLELS_DISCORD_TOKEN \
...
- Use the `uagent` wrapper for guest `message send/read`; `node uagent.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/uagent-parallels-smoke.*`
...
- host inbound nonce appears in `uagent message read` output
