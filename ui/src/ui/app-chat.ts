...
export async function handleAbortChat(host: ChatHost) {
  ...
  host.chatMessage = "";
  await abortChatRun(host as unknown as UAGENTApp);
}
...
async function sendChatMessageNow(
  ...
  const runId = await sendChatMessage(host as unknown as UAGENTApp, message, opts?.attachments);
...
async function clearChatHistory(host: ChatHost) {
  ...
  try {
    ...
    host.chatRunId = null;
    await loadChatHistory(host as unknown as UAGENTApp);
  } catch (err) {
    host.lastError = String(err);
  }
  scheduleChatScroll(host as unknown as Parameters<typeof scheduleChatScroll>[0]);
...
export async function refreshChat(host: ChatHost, opts?: { scheduleScroll?: boolean }) {
  await Promise.all([
    loadChatHistory(host as unknown as UAGENTApp),
    loadSessions(host as unknown as UAGENTApp, {
      activeMinutes: 0,
      limit: 0,
      includeGlobal: true,
      includeUnknown: true,
