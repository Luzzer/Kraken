...
import type { UAGENTApp } from "./app.ts";
...
export function applySettingsFromUrl(host: SettingsHost) {
  ...
  if (tokenRaw != null) {
    if (queryToken != null) {
      warnQueryToken = true;
      console.warn(
        "[uagent] Auth token passed as query parameter (?token=). Use URL fragment instead: #token=<token>. Query parameters may appear in server logs.",
      );
    }
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as UAGENTApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as UAGENTApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as UAGENTApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as UAGENTApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as UAGENTApp);
    await loadConfig(host as unknown as UAGENTApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as UAGENTApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as UAGENTApp, agentId);
      if (host.agentsPanel === "files") {
        void loadAgentFiles(host as unknown as UAGENTApp, agentId);
      }
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as UAGENTApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as UAGENTApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as UAGENTApp);
    await loadDevices(host as unknown as UAGENTApp);
    await loadConfig(host as unknown as UAGENTApp);
    await loadExecApprovals(host as unknown as UAGENTApp);
  }
  if (host.tab === "dreams") {
    await loadConfig(host as unknown as UAGENTApp);
    await Promise.all([
      loadDreamingStatus(host as unknown as UAGENTApp),
      loadDreamDiary(host as unknown as UAGENTApp),
    ]);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as UAGENTApp);
    await loadConfig(host as unknown as UAGENTApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as UAGENTApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as UAGENTApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__UAGENT_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as UAGENTApp;
...
async function loadOverviewLogs(host: UAGENTApp) {
...
function buildAttentionItems(host: UAGENTApp) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.uagent.ai/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as UAGENTApp, true),
    loadConfigSchema(host as unknown as UAGENTApp),
    loadConfig(host as unknown as UAGENTApp),
  ]);
}
