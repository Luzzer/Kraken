  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __UAGENT_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__UAGENT_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__UAGENT_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /uagent/ ");

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/uagent"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/uagent/chat",
    });

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/uagent"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    sessionStorage.setItem("uagent.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "uagent.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/uagent",
    ...
    const scopedKey = "uagent.control.settings.v1:wss://gateway.example:8443/uagent";
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      ...
      borderRadius: 50,
      sessionsByGateway: {
        "wss://gateway.example:8443/uagent": {
          sessionKey: "agent",
  ...
  it("does not persist gateway tokens when saving settings", async () => {
    ...
    const scopedKey = `uagent.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "claw",
      themeMode: "system",
  ...
  it("caps persisted session scopes to the most recent gateways", async () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `uagent.control.settings.v1:wss://gateway.example:8443`;
