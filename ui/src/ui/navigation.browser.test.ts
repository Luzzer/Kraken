...
function expectConfirmedGatewayChange(app: ReturnType<typeof mountApp>) {
  expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/uagent");
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/uagent/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/uagent");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/uagent/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__UAGENT_CONTROL_UI_BASE_PATH__ = "/uagent";
    const app = mountApp("/uagent/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/uagent");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/uagent/sessions");
  });
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("uagent.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "uagent.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/uagent" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("uagent.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/uagent",
    });
    expect(JSON.parse(localStorage.getItem("uagent.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/uagent";
  ...
  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/uagent#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/uagent");
    expect(app.settings.token).toBe("");
