...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://uagent.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://uagent.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://uagent.ai/abc-123", baseHref)).toBe(
      "blob:https://uagent.ai/abc-123",
    );
  });
