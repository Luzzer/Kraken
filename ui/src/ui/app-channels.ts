...
export async function handleWhatsAppStart(host: UAGENTApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: UAGENTApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: UAGENTApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: UAGENTApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: UAGENTApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: UAGENTApp): string {
...
function resolveGatewayHttpAuthHeader(host: UAGENTApp): string | null {
...
function buildGatewayHttpHeaders(host: UAGENTApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: UAGENTApp) {
...
export function handleNostrProfileToggleAdvanced(host: UAGENTApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: UAGENTApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: UAGENTApp) {
  const state = host.nostrProfileFormState;
