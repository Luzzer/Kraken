...
describe("applySettingsFromUrl", () => {
  ...
  it("hydrates query token params and strips them from the URL", () => {
    ...
    host.settings.gatewayUrl = "wss://control.example/uagent";
  ...
  it("keeps query token params pending when a gatewayUrl confirmation is required", () => {
    setTestWindowUrl(
      "https://control.example/ui/overview?gatewayUrl=wss://other-gateway.example/uagent&token=abc123",
    );
    const host = createHost("overview");
    host.settings.gatewayUrl = "wss://control.example/uagent";
    ...
    expect(host.pendingGatewayUrl).toBe("wss://other-gateway.example/uagent");
