...
function createTempHome(): string {
  return makeTempDir(tempDirs, "uagent-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-uagent.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.uagent/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.uagent/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".uagent", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".uagent", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.UAGENT_LIVE_TEST_QUIET = "1";
    process.env.UAGENT_CONFIG_PATH = "~/custom-uagent.json5";
    process.env.UAGENT_TEST_HOME = priorIsolatedHome;
    process.env.UAGENT_STATE_DIR = path.join(priorIsolatedHome, ".uagent");
    ...
    expect(process.env.UAGENT_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".uagent", "uagent.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".uagent", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".uagent", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.UAGENT_LIVE_TEST = "1";
    process.env.UAGENT_LIVE_USE_REAL_HOME = "1";
    process.env.UAGENT_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.UAGENT_LIVE_TEST;
    delete process.env.UAGENT_LIVE_GATEWAY;
    delete process.env.UAGENT_LIVE_USE_REAL_HOME;
    delete process.env.UAGENT_LIVE_TEST_QUIET;
