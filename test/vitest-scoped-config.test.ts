...
describe("resolveVitestIsolation", () => {
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ UAGENT_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ UAGENT_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ UAGENT_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
});
...
describe("createScopedVitestConfig", () => {
  it("applies the non-isolated runner by default", () => {
    ...
    expect(config.test?.setupFiles).toEqual(["test/setup.ts", "test/setup-uagent-runtime.ts"]);
  });
  ...
  it("loads scoped include overrides from UAGENT_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-vitest-scoped-"));
    try {
      ...
      const config = createScopedVitestConfig(["src/utils/**/*.test.ts"], {
        dir: "src",
        env: {
          UAGENT_VITEST_INCLUDE_FILE: includeFile,
        },
      });
...
describe("scoped vitest configs", () => {
  ...
  it("keeps the process lane off the uagent runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the uagent runtime setup", () => {
  ...
  it("loads channel include overrides from UAGENT_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "uagent-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
