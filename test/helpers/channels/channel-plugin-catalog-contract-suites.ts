    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-catalog-"));
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@uagent/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(os.tmpdir(), "uagent-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-catalog-state-"));
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-catalog-home-"));
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@uagent/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              UAGENT_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              UAGENT_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@uagent/default-env-demo",
