...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked UAGENT_HOME.
  delete process.env.UAGENT_HOME;
  process.env.UAGENT_STATE_DIR = path.join(base, ".uagent");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "uagent-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".uagent", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
