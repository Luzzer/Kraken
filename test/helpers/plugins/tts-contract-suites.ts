...
function asLegacyTtsConfig(value: unknown): UAGENTConfig {
  return value as UAGENTConfig;
}

function asLegacyUAGENTConfig(value: Record<string, unknown>): UAGENTConfig {
  return value as unknown as UAGENTConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): UAGENTConfig {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    const baseCfg: UAGENTConfig = asLegacyUAGENTConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.UAGENT_TTS_PREFS;
      process.env.UAGENT_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.UAGENT_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: UAGENTConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
