...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `uagent-e2e-${name}-`));
  const configDir = path.join(homeDir, ".uagent");
