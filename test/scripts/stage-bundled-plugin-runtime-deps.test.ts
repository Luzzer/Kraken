...
describe("stageBundledPluginRuntimeDeps", () => {
  function createBundledPluginFixture(params: {
    ...
    const repoRoot = createTempDir("uagent-runtime-deps-");
  ...
  it("skips restaging when runtime deps stamp matches the sanitized manifest", () => {
    const { pluginDir, repoRoot } = createBundledPluginFixture({
      packageJson: {
        name: "@uagent/fixture-plugin",
        version: "1.0.0",
        dependencies: { "left-pad": "1.3.0" },
        peerDependencies: { uagent: "^1.0.0" },
        peerDependenciesMeta: { uagent: { optional: true } },
        devDependencies: { uagent: "^1.0.0" },
        uagent: { bundle: { stageRuntimeDependencies: true } },
      },
    });
    ...
    stageBundledPluginRuntimeDeps({
      ...
      installPluginRuntimeDepsImpl: ({ fingerprint }: { fingerprint: string }) => {
        installCount += 1;
        fs.writeFileSync(
          path.join(pluginDir, ".uagent-runtime-deps-stamp.json"),
  ...
  it("stages runtime deps from the root node_modules when already installed", () => {
    ...
    expect(fs.existsSync(path.join(pluginDir, ".uagent-runtime-deps-stamp.json"))).toBe(true);
  });
