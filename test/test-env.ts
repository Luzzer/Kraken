...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.UAGENT_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "UAGENT_TEST_FAST", value: process.env.UAGENT_TEST_FAST },
    {
      key: "UAGENT_STRICT_FAST_REPLY_CONFIG",
      value: process.env.UAGENT_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "UAGENT_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.UAGENT_ALLOW_SLOW_REPLY_TESTS,
    },
    ...
    { key: "UAGENT_STATE_DIR", value: process.env.UAGENT_STATE_DIR },
    { key: "UAGENT_CONFIG_PATH", value: process.env.UAGENT_CONFIG_PATH },
    { key: "UAGENT_GATEWAY_PORT", value: process.env.UAGENT_GATEWAY_PORT },
    { key: "UAGENT_BRIDGE_ENABLED", value: process.env.UAGENT_BRIDGE_ENABLED },
    { key: "UAGENT_BRIDGE_HOST", value: process.env.UAGENT_BRIDGE_HOST },
    { key: "UAGENT_BRIDGE_PORT", value: process.env.UAGENT_BRIDGE_PORT },
    { key: "UAGENT_CANVAS_HOST_PORT", value: process.env.UAGENT_CANVAS_HOST_PORT },
    { key: "UAGENT_TEST_HOME", value: process.env.UAGENT_TEST_HOME },
    { key: "UAGENT_AGENT_DIR", value: process.env.UAGENT_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.UAGENT_TEST_HOME = tempHome;
  process.env.UAGENT_TEST_FAST = "1";
  process.env.UAGENT_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.UAGENT_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.UAGENT_BRIDGE_ENABLED;
  delete process.env.UAGENT_BRIDGE_HOST;
  delete process.env.UAGENT_BRIDGE_PORT;
  delete process.env.UAGENT_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.UAGENT_STATE_DIR = path.join(tempHome, ".uagent");
  }
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.UAGENT_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".uagent");
  const priorIsolatedHome = params.env.UAGENT_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".uagent")
  ) {
    realStateDir = path.join(params.realHome, ".uagent");
  }
  const tempStateDir = path.join(params.tempHome, ".uagent");
  fs.mkdirSync(tempStateDir, { recursive: true });

  const realConfigPath = params.env.UAGENT_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.UAGENT_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "uagent.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "uagent.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.UAGENT_LIVE_TEST === "1" ||
    process.env.UAGENT_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.UAGENT_LIVE_USE_REAL_HOME);
