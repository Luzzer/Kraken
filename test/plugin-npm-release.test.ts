...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @uagent/zalo, @uagent/feishu  @uagent/zalo "),
    ).toEqual(["@uagent/feishu", "@uagent/zalo"]);
  });
});
...
describe("collectPublishablePluginPackages", () => {
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "uagent-plugin-npm-release-");
    ...
    writeJsonFile(join(repoDir, "extensions", "demo-plugin", "package.json"), {
      name: "@uagent/demo-plugin",
      version: "2026.4.6",
      uagent: {
        extensions: ["./index.ts"],
        install: {
          npmSpec: "@uagent/demo-plugin",
        },
        release: {
          publishToNpm: true,
        },
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        packageName: "@uagent/demo-plugin",
        version: "2026.4.6",
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@uagent/demo-plugin",
      },
    ]);
  });
});
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@uagent/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @uagent/missing.");
  });
});
