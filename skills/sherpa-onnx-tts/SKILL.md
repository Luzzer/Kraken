...
1. Download the runtime for your OS (extracts into `$UAGENT_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.uagent/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$UAGENT_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.uagent/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active UAGENT config file (`$UAGENT_CONFIG_PATH`, default `~/.uagent/uagent.json`):
