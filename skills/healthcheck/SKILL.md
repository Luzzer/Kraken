---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for UAGENT deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, UAGENT cron scheduling for periodic checks, or version status checks on a machine running UAGENT (laptop, workstation, Pi, VPS).
---

# UAGENT Host Hardening

## Overview

Assess and harden the host running UAGENT, then align it to a user-defined risk tolerance without breaking access. Use UAGENT security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim UAGENT changes the host firewall, SSH, or OS updates; it does not.
...
5. UAGENT gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if UAGENT can access sensitive data.
...
### 2) Run UAGENT security audits (read-only)

As part of the default read-only checks, run `uagent security audit --deep`. Only offer alternatives if the user requests them:

1. `uagent security audit` (faster, non-probing)
2. `uagent security audit --json` (structured output)

Offer to apply UAGENT safe defaults (numbered):

1. `uagent security audit --fix`

Be explicit that `--fix` only tightens UAGENT defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check UAGENT version/update status (read-only)

As part of the default read-only checks, run `uagent update status`.
...
- Credential hygiene notes (location of UAGENT creds, prefer disk encryption)
...
After UAGENT install or first hardening pass, run at least one baseline audit and version check:

- `uagent security audit`
- `uagent security audit --deep`
- `uagent update status`

Ongoing monitoring is recommended. Use the UAGENT cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `uagent cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `uagent update status`
...
Before creating, `uagent cron list` and match on exact `name`. If found, `uagent cron edit <id> ...`.
If not found, `uagent cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `uagent update status` (preferred for source checkouts and channels)
2. `npm view uagent version` (published npm version)

## UAGENT command accuracy

Use only supported commands and flags:

- `uagent security audit [--deep] [--fix] [--json]`
- `uagent status` / `uagent status --deep`
- `uagent health --json`
- `uagent update status`
- `uagent cron add|list|runs|run`

Do not invent CLI flags or imply UAGENT enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by UAGENT compaction:
