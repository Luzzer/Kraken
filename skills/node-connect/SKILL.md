---
name: node-connect
description: Diagnose UAGENT node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify UAGENT is advertising that route, then fix pairing/auth.
...
- whether `uagent devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `uagent qr --json`. It uses the same setup-code payload Android scans.

```bash
uagent config get gateway.mode
uagent config get gateway.bind
uagent config get gateway.tailscale.mode
uagent config get gateway.remote.url
uagent config get gateway.auth.mode
uagent config get gateway.auth.allowTailscale
uagent config get plugins.entries.device-pair.config.publicUrl
uagent qr --json
uagent devices list
uagent nodes status
```

If this UAGENT instance is pointed at a remote gateway, also run:

```bash
uagent qr --remote --json
```
...
If `uagent qr --json` says `Gateway is only bound to loopback`:
...
- `uagent devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run uagent qr again, rescan, then approve the pending device pairing.`
