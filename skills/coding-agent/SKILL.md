...
**⚠️ CRITICAL: Never review PRs in UAGENT's own project folder!**
...
8. **NEVER start Codex inside your UAGENT state directory** (`$UAGENT_STATE_DIR`, default `~/.uagent`) - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/uagent/** - that's the LIVE UAGENT instance!
...
For long-running background tasks, append a wake trigger to your prompt so UAGENT gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
uagent system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: uagent system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
