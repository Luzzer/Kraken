...
function isSystemThrottleDisabled(env: Record<string, string | undefined>): boolean {
  const normalized = env.UAGENT_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export type UAGENTVitestPool = "forks" | "threads";
...
export function resolveLocalVitestMaxWorkers(
  ...
  pool: UAGENTVitestPool = resolveDefaultVitestPool(env),
): number {
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.UAGENT_VITEST_MAX_WORKERS ?? env.UAGENT_TEST_WORKERS);
  if (override !== null) {
...
export const sharedVitestConfig = {
  resolve: {
    alias: [
      {
        find: "uagent/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `uagent/plugin-sdk/${subpath}`,
