...
export function createScopedVitestConfig(
  ...
  options?: {
    ...
    exclude?: string[];
    argv?: string[];
    includeUAGENTRuntimeSetup?: boolean;
    isolate?: boolean;
    name?: string;
    pool?: "forks" | "threads";
  ...
  const includeFromEnv = loadPatternListFromEnv("UAGENT_VITEST_INCLUDE_FILE", env);
  ...
  const setupFiles = [
    ...new Set([
      ...(baseTest.setupFiles ?? []),
      ...(options?.setupFiles ?? []),
      ...(options?.includeUAGENTRuntimeSetup === false ? [] : ["test/setup-uagent-runtime.ts"]),
    ]),
  ];
