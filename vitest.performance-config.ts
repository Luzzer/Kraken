...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.UAGENT_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.UAGENT_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.UAGENT_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.UAGENT_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.UAGENT_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.UAGENT_VITEST_PRINT_IMPORT_BREAKDOWN)) {
