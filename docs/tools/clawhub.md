---
summary: "ClawHub guide: public registry, native UAGENT install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **UAGENT skills and plugins**.

- Use native `uagent` commands to search/install/update skills and install
...
```bash
uagent plugins install clawhub:<package>
...
Native `uagent` commands install into your active workspace and persist source
...
`uagent plugins install clawhub:...` only accepts installable plugin families.
If a ClawHub package is actually a skill, UAGENT stops and points you at
`uagent skills install <slug>` instead.

## What ClawHub is

- A public registry for UAGENT skills and plugins.
...
4. Users browse, download, and install skills in UAGENT.
...
If you want to add new capabilities to your UAGENT agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `uagent skills install <skill-slug>`
3. Start a new UAGENT session so it picks up the new skill.
...
## How it fits into UAGENT

Native `uagent skills install` installs into the active workspace `skills/`
directory. `uagent plugins install clawhub:...` records a normal managed
...
Community or other non-official channels can still install, but UAGENT warns
...
current working directory. If an UAGENT workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). UAGENT loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.uagent/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches UAGENT how to perform a
...
Interested in becoming a moderator? Ask in the UAGENT Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to UAGENT workspace).
...
Code plugins must include the required UAGENT metadata in `package.json`:

```json
{
  "name": "@myorg/uagent-my-plugin",
  "version": "1.0.0",
  "type": "module",
  "uagent": {
    "extensions": ["./index.ts"],
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/uagent/skills` and `~/.uagent/skills`). This is designed to find older skill installs without extra flags.
