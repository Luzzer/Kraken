...
UAGENT also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "UAGENT plugin SDK" });
    ```
...
Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of UAGENT's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, UAGENT keeps the normal managed `web_search` behavior.
...
If no `provider` is set, UAGENT checks providers in this order and uses the
...
<Note>
  ...
  UAGENT resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
- or omit that field and let UAGENT auto-detect the first ready web-fetch
  provider from available credentials
- today the bundled web-fetch provider is Firecrawl, configured under
  `plugins.entries.firecrawl.config.webFetch.*`

When you choose **Kimi** during `uagent onboard` or
`uagent configure --section web`, UAGENT can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `uagent doctor --fix`.
When you choose Grok during `uagent onboard` or `uagent configure --section web`,
UAGENT can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, UAGENT does not
...
<Tabs>
  <Tab title="Config file">
    Run `uagent configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.uagent/.env`.
...
optional structured filters. UAGENT only enables the built-in xAI `x_search`
