...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `uagent sandbox explain`.
...
Legacy `agent.*` configs are migrated by `uagent doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=uagent-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${UAGENT_STATE_DIR:-$HOME/.uagent}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
