...
`uagent agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    uagent agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    uagent agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    uagent agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    uagent agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    uagent agent --agent ops --message "Generate report" \
...
uagent agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
