...
UAGENT can convert outbound replies into audio using ElevenLabs, Microsoft, MiniMax, or OpenAI.
It works anywhere UAGENT can send audio.
...
When `messages.tts.provider` is unset, UAGENT picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `uagent.json`.
...
- If `provider` is **unset**, UAGENT uses the first configured speech provider in registry auto-select order.
...
`~/.uagent/settings/tts.json`, override with `UAGENT_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, UAGENT retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so UAGENT registers
