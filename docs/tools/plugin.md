---
summary: "Install, configure, and manage UAGENT plugins"
read_when:
...
Plugins extend UAGENT with new capabilities: channels, model providers,
...
search, and more. Some plugins are **core** (shipped with UAGENT), others
...
<Steps>
  ...
  <Step title="Install a plugin">
    ```bash
    # From npm
    uagent plugins install @uagent/voice-call

    # From a local directory or archive
    uagent plugins install ./my-plugin
    uagent plugins install ./my-plugin.tgz
    ```
...
```text
/plugin install clawhub:@uagent/voice-call
...
`uagent doctor --fix`. The only recovery exception is a narrow bundled-plugin
reinstall path for plugins that opt into
`uagent.install.allowInvalidConfigRecovery`.

## Plugin types

UAGENT recognizes two plugin formats:

| Format     | How it works                                                       | Examples                                               |
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `uagent.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to UAGENT features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `uagent plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
| Plugin          | Package                | Docs                                 |
| --------------- | ---------------------- | ------------------------------------ |
| Matrix          | `@uagent/matrix`     | [Matrix](/channels/matrix)           |
| Microsoft Teams | `@uagent/msteams`    | [Microsoft Teams](/channels/msteams) |
| Nostr           | `@uagent/nostr`      | [Nostr](/channels/nostr)             |
| Voice Call      | `@uagent/voice-call` | [Voice Call](/plugins/voice-call)    |
| Zalo            | `@uagent/zalo`       | [Zalo](/channels/zalo)               |
| Zalo Personal   | `@uagent/zalouser`   | [Zalo Personal](/plugins/zalouser)   |

### Core (shipped with UAGENT)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` — bundled browser plugin for the browser tool, `uagent browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
watch + in-process restart enabled (the default `uagent gateway` path), that
...
UAGENT scans for plugins in this order (first match wins):
...
<Steps>
  ...
  <Step title="Workspace extensions">
    `\<workspace\>/.uagent/<plugin-root>/*.ts` and `\<workspace\>/.uagent/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global extensions">
    `~/.uagent/<plugin-root>/*.ts` and `~/.uagent/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with UAGENT. Many are enabled by default (model providers, speech).
...
```bash
uagent plugins list                       # compact inventory
uagent plugins list --enabled            # only loaded plugins
uagent plugins list --verbose            # per-plugin detail lines
uagent plugins list --json               # machine-readable inventory
uagent plugins inspect <id>              # deep detail
uagent plugins inspect <id> --json       # machine-readable
uagent plugins inspect --all             # fleet-wide table
uagent plugins info <id>                 # inspect alias
uagent plugins doctor                    # diagnostics

uagent plugins install <package>         # install (ClawHub first, then npm)
uagent plugins install clawhub:<pkg>     # install from ClawHub only
uagent plugins install <spec> --force    # overwrite existing install
uagent plugins install <path>            # install from local path
uagent plugins install -l <path>         # link (no copy) for dev
uagent plugins install <plugin> --marketplace <source>
uagent plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
uagent plugins install <spec> --pin      # record exact resolved npm spec
uagent plugins install <spec> --dangerously-force-unsafe-install
uagent plugins update <id>             # update one plugin
uagent plugins update <id> --dangerously-force-unsafe-install
uagent plugins update --all            # update all
uagent plugins uninstall <id>          # remove config/install records
uagent plugins uninstall <id> --keep-files
uagent plugins marketplace list <source>
uagent plugins marketplace list <source> --json

uagent plugins enable <id>
uagent plugins disable <id>
```

Bundled plugins ship with UAGENT. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `uagent plugins enable <id>`.
...
override instead, while `uagent skills install` remains the separate ClawHub
...
`uagent plugins inspect <id>` also reports detected bundle capabilities plus
...
See [`uagent plugins` CLI reference](/cli/plugins) for full details.
...
UAGENT loads the entry object and calls `register(api)` during plugin
