...
read_when:
  - Running UAGENT Gateway in WSL2 while Chrome lives on Windows
...
- UAGENT Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/uagent/uagent/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- UAGENT and Chrome are on the same machine
...
- UAGENT points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, UAGENT is not the problem yet.
...
Fix that before touching UAGENT config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point UAGENT at the address that is reachable from WSL2:
...
- use HTTP(S) when you want UAGENT to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting UAGENT to succeed
...
```bash
uagent browser open https://example.com --browser-profile remote
uagent browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `uagent browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `uagent browser stop --browser-profile remote`
...
3. UAGENT config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug UAGENT config or Control UI auth
