...
UAGENT supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), UAGENT reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.uagent/.env`.

If you omit `baseUrl`, UAGENT defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, UAGENT defaults to `kimi-k2.5`.
