...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.uagent/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so UAGENT picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    uagent agent --message "give me a greeting"
    ```
...
| `metadata.uagent.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.uagent.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.uagent.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `uagent agent --message "..."` to test before sharing
...
| `~/.uagent/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with UAGENT) | Low        | Global                |
