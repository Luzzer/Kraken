...
read_when:
  ...
  - Debugging why uagent is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (UAGENT-managed)"
---

# Browser (uagent-managed)

UAGENT can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `uagent` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **uagent** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`uagent`, `work`, `remote`, ...).
...
```bash
uagent browser --browser-profile uagent status
...
If `uagent browser` is missing entirely, or the agent says the browser tool
...
If you turn off only the plugin, the bundled browser CLI (`uagent browser`),
...
If `uagent browser` suddenly becomes an unknown command after an upgrade, or
...
Set `browser.defaultProfile: "uagent"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.uagent/uagent.json`.
...
- If you override the Gateway port (`gateway.port` or `UAGENT_GATEWAY_PORT`),
...
- Default profile is `uagent` (UAGENT-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `uagent` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
UAGENT uses it automatically. Set `browser.executablePath` to override
auto-detection:

CLI example:

```bash
uagent config set browser.executablePath "/usr/bin/google-chrome"
```
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, UAGENT will not launch a local browser.

Stopping behavior differs by profile mode:

- local managed profiles: `uagent browser stop` stops the browser process that
  UAGENT launched
- attach-only and remote CDP profiles: `uagent browser stop` closes the active
  ...
  though no browser process was launched by UAGENT
...
UAGENT preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, UAGENT can
...
- If you set `nodeHost.browserProxy.allowProfiles`, UAGENT treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. UAGENT can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let UAGENT
  discover `/json/version`.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). UAGENT supports both:

- **HTTP(S) endpoints** — UAGENT calls `/json/version` to discover the
  WebSocket debugger URL, then connects.
- **WebSocket endpoints** (`ws://` / `wss://`) — UAGENT connects directly,
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-uagent-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, UAGENT
  auto-generates `gateway.auth.token` on startup and persists it to config.
- UAGENT does **not** auto-generate that token when `gateway.auth.mode` is
...
UAGENT supports multiple named profiles (routing configs). Profiles can be:

- **uagent-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `uagent` profile is auto-created if missing.
...
UAGENT can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when UAGENT attaches.
...
```bash
uagent browser --browser-profile user start
uagent browser --browser-profile user status
uagent browser --browser-profile user tabs
uagent browser --browser-profile user snapshot --format ai
```
...
- `uagent doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `uagent` profile because it can
  act inside your signed-in browser session.
- UAGENT does not launch the browser for this driver; it attaches to an
  existing session only.
- UAGENT uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, UAGENT passes it through to target that explicit
...
When launching locally, UAGENT picks the first available:
...
- `x-uagent-password: <gateway password>` or HTTP Basic auth with that password
...
- ARIA snapshots
- Page screenshots for the managed `uagent` browser when a per-tab CDP
...
```bash
docker compose run --rm uagent-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

To persist browser downloads, set `PLAYWRIGHT_BROWSERS_PATH` (for example,
`/home/node/.cache/ms-playwright`) and make sure `/home/node` is persisted via
`UAGENT_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `uagent browser snapshot --interactive --compact --depth 6`
- `uagent browser snapshot --efficient`
- `uagent browser snapshot --labels`
- `uagent browser snapshot --selector "#main" --interactive`
- `uagent browser snapshot --frame "iframe#main" --interactive`
- `uagent browser console --level error`

Lifecycle note:

- For attach-only and remote CDP profiles, `uagent browser stop` is still the
...
- `uagent browser requests --filter api --clear`
- `uagent browser pdf`
- `uagent browser responsebody "**/api" --max-chars 5000`
...
- `uagent browser storage local set theme dark`
- `uagent browser storage session clear`
- `uagent browser set offline on`
- `uagent browser set headers --headers-json '{"X-Debug":"1"}'`
- `uagent browser set credentials user pass`
- `uagent browser set credentials --clear`
- `uagent browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `uagent browser set geo --clear`
- `uagent browser set media dark`
- `uagent browser set timezone America/New_York`
- `uagent browser set locale en-US`
...
- Download and trace output paths are constrained to UAGENT temp roots:
  - traces: `/tmp/uagent` (fallback: `${os.tmpdir()}/uagent`)
  - downloads: `/tmp/uagent/downloads` (fallback: `${os.tmpdir()}/uagent/downloads`)
- Upload paths are constrained to an UAGENT temp uploads root:
  - uploads: `/tmp/uagent/uploads` (fallback: `${os.tmpdir()}/uagent/uploads`)
...
UAGENT supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `uagent browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `uagent browser click 12`, `uagent browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `uagent browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `uagent browser click e12`, `uagent browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `uagent browser wait --url "**/dash"`
- Wait for load state:
  - `uagent browser wait --load networkidle`
- Wait for a JS predicate:
  - `uagent browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `uagent browser wait "#main"`
...
1. `uagent browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `uagent browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `uagent browser trace start`
   - reproduce the issue
   - `uagent browser trace stop` (prints `TRACE:<path>`)
...
```bash
uagent browser status --json
uagent browser snapshot --interactive --json
uagent browser requests --filter api --json
uagent browser cookies --json
```
...
- The uagent browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `uagent browser evaluate` and `wait --fn`
...
- `browser` accepts:
  - `profile` to choose a named browser profile (uagent, chrome, or remote CDP).
