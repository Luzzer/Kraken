...
`ask: "always"` in `~/.uagent/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `uagent approvals get`, `uagent approvals get --gateway`, or
`uagent approvals get --node <id|name|ip>` to inspect the requested policy,
...
- **gateway host** → `uagent` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, UAGENT also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.uagent/exec-approvals.sock",
...
- requested exec policy in UAGENT config (`tools.exec.*`)
- host-local approvals policy in `~/.uagent/exec-approvals.json`
...
- In YOLO mode, UAGENT does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
```bash
uagent config set tools.exec.host gateway
uagent config set tools.exec.security full
uagent config set tools.exec.ask off
...
```bash
uagent approvals set --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
```bash
uagent approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
When `tools.exec.strictInlineEval=true`, UAGENT treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.uagent/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `uagent approvals allowlist ...`).
- `uagent security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `uagent doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, UAGENT still rejects the `env` builtin in safe-bin
...
`~/.uagent/exec-approvals.json` directly.

CLI: `uagent approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, UAGENT sends a followup `agent` turn to the same session.
...
topics, UAGENT preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, UAGENT prevents the agent from reusing
