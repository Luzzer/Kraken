...
```bash
uagent tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `uagent tui --deliver`
...
- Local shell commands receive `UAGENT_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `UAGENT_THEME=light` before launching `uagent tui`.
- To force the original dark palette instead, set `UAGENT_THEME=dark`.
...
- Check the Gateway logs: `uagent logs --follow`.
- Confirm the agent can run: `uagent status` and `uagent models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `uagent agents list` and your routing config.
