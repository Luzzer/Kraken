...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/uagent`)
...
If the page fails to load, start the Gateway first: `uagent gateway`.
...
created. Re-run `uagent devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `uagent devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.uagent/uagent.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop uagent`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. UAGENT
...
```bash
uagent gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
UAGENT **blocks** Control UI connections without device identity.
...
```bash
UAGENT_CONTROL_UI_BASE_PATH=/uagent/ pnpm ui:build
```
