...
read_when:
  - Setting up UAGENT for the first time
...
UAGENT reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.uagent/uagent.json`.

If the file is missing, UAGENT uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `uagent onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    uagent onboard       # full onboarding flow
    uagent configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    uagent config get agents.defaults.workspace
    uagent config set agents.defaults.heartbeat.every "2h"
    uagent config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.uagent/uagent.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
UAGENT only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

Schema tooling notes:

- `uagent config schema` prints the same JSON Schema family used by Control UI
...
- Only diagnostic commands work (`uagent doctor`, `uagent logs`, `uagent health`, `uagent status`)
- Run `uagent doctor` to see exact issues
- Run `uagent doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `uagent.json`.
    ...
    ```bash
    uagent config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `UAGENT_APNS_RELAY_BASE_URL` and `UAGENT_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `UAGENT_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-uagent-token`); query-string tokens are rejected.
...
The Gateway watches `~/.uagent/uagent.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `uagent config set` for single keys.
    </Warning>
    ...
    ```bash
    uagent gateway call config.get --params '{}'  # capture payload.hash
    uagent gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.uagent/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    uagent gateway call config.patch --params '{
...
UAGENT reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.uagent/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, UAGENT runs your login shell and imports only the missing keys:
...
Env var equivalent: `UAGENT_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${UAGENT_GATEWAY_TOKEN}" } },
