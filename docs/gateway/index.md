...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `UAGENT_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `UAGENT_GATEWAY_TOKEN` / `UAGENT_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

UAGENT’s highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `uagent`, `uagent/default`, and `uagent/<agentId>`.
- `uagent/default` is the stable alias that always maps to the configured default agent.
- Use `x-uagent-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `UAGENT_GATEWAY_PORT` → `gateway.port` → `18789` |
...
```bash
uagent gateway status
uagent gateway status --deep   # adds a system-level service scan
uagent gateway status --json
uagent gateway install
uagent gateway restart
uagent gateway stop
uagent secrets reload
uagent logs --follow
uagent doctor
```
...
LaunchAgent labels are `ai.uagent.gateway` (default) or `ai.uagent.<profile>` (named profile). `uagent doctor` audits and repairs service config drift.
...
```bash
uagent gateway install
systemctl --user enable --now uagent-gateway[-<profile>].service
uagent gateway status
```
...
```ini
[Unit]
Description=UAGENT Gateway
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/uagent gateway --port 18789
Restart=always
RestartSec=5
TimeoutStopSec=30
TimeoutStartSec=30
...
Native Windows managed startup uses a Scheduled Task named `UAGENT Gateway`
(or `UAGENT Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, UAGENT falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now uagent-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/uagent-gateway[-<profile>].service` and adjust
`ExecStart=` if your `uagent` binary lives elsewhere.
...
- Unique `gateway.port`
- Unique `UAGENT_CONFIG_PATH`
- Unique `UAGENT_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
UAGENT_CONFIG_PATH=~/.uagent/a.json UAGENT_STATE_DIR=~/.uagent-a uagent gateway --port 19001
UAGENT_CONFIG_PATH=~/.uagent/b.json UAGENT_STATE_DIR=~/.uagent-b uagent gateway --port 19002
```
...
```bash
uagent --dev setup
uagent --dev gateway --allow-unconfigured
uagent --dev status
```
