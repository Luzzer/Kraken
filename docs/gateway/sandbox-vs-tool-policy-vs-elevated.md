...
UAGENT has three related (but different) controls:
...
Use the inspector to see what UAGENT is _actually_ doing:
...
- UAGENT validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:uagent`: all built-in UAGENT tools (excludes provider plugins)
