...
UAGENT supports OAuth and API keys for model providers. For always-on gateway
...
setup, but UAGENT also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `uagent gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.uagent/.env` so the daemon can read it:
...
API keys for daemon use: `uagent onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.uagent/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in UAGENT as a supported token
path. Anthropic staff has since told us that UAGENT-style Claude CLI usage is
...
```bash
uagent models auth paste-token --provider openrouter
```
...
- If auth exists but UAGENT cannot resolve a probeable model candidate for
...
- Anthropic staff told us this UAGENT integration path is allowed again.
- UAGENT therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `UAGENT_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- UAGENT retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `uagent models status --probe` shows omitted
...
Run `uagent models status` to confirm which profile is expiring. If an
