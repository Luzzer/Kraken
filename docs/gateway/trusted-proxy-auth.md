...
read_when:
  - Running UAGENT behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of UAGENT
...
- You run UAGENT behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. UAGENT checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. UAGENT extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that UAGENT can verify.
...
- UAGENT can stay on loopback HTTP behind the proxy.
...
If UAGENT itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://uagent.example.com
    to: http://uagent-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
UAGENT rejects ambiguous configurations where both a `gateway.auth.token` (or `UAGENT_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-uagent-scopes`.

Examples:

- `x-uagent-scopes: operator.read`
- `x-uagent-scopes: operator.read,operator.write`
- `x-uagent-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, UAGENT honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-uagent-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-uagent-scopes` explicitly when you want a trusted-proxy request to
...
`uagent security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
UAGENT rejected a loopback-source trusted-proxy request.
...
3. Update UAGENT config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `uagent security audit` and review findings
