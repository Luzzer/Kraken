...
UAGENT can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. UAGENT requires a shared password.
...
- `token` (default when `UAGENT_GATEWAY_TOKEN` is set)
- `password` (shared secret via `UAGENT_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. UAGENT verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
UAGENT only treats a request as Serve when it arrives from loopback with
...
Prefer `UAGENT_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
uagent gateway --tailscale serve
uagent gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want UAGENT to undo `tailscale serve`
