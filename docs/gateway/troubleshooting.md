...
- `uagent gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `uagent doctor` reports no blocking config/service issues.
- `uagent channels status --probe` shows live per-account transport status and,
...
uagent config get agents.defaults.models
```
...
uagent status
uagent channels status --probe
uagent pairing list --channel <channel> [--account <id>]
uagent config get channels
uagent logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `uagent config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `uagent devices list` then `uagent devices approve <requestId>`.                                                                                                                                                                                            |
...
If `uagent devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the
  caller also has `operator.admin`
- `uagent devices rotate --scope ...` can only request operator scopes that
...
uagent logs --follow
uagent doctor
uagent gateway status --deep   # also scan system-level services
```
...
- `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `uagent onboard --mode local` / `uagent setup` to restamp the expected local-mode config. If you are running UAGENT via Podman, the default config path is `~/.uagent/uagent.json`.
...
Use this when `uagent gateway probe` reaches something, but still prints a warning block.

```bash
uagent gateway probe
uagent gateway probe --json
uagent gateway probe --ssh user@gateway-host
```
...
uagent cron status
uagent cron list
uagent cron runs --id <jobId> --limit 20
uagent system heartbeat last
uagent logs --follow
```
...
- `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so UAGENT skips the model call.
...
uagent approvals get --node <idOrNameOrIp>
uagent logs --follow
uagent status
```
...
uagent browser start --browser-profile uagent
...
- stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `uagent browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
```bash
uagent gateway status
uagent config get gateway.mode
uagent config get gateway.remote.url
uagent config get gateway.auth.mode
```
...
```bash
uagent config get gateway.bind
uagent config get gateway.auth.mode
uagent config get gateway.auth.token
uagent gateway status
uagent logs --follow
```
