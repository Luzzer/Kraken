---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for UAGENT agents"
read_when:
...
OpenShell is a managed sandbox backend for UAGENT. Instead of running Docker
containers locally, UAGENT delegates sandbox lifecycle to the `openshell` CLI,
...
- UAGENT Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, UAGENT creates an OpenShell
...
- You edit files locally outside UAGENT and want those changes visible in the
...
- When the sandbox is first created, UAGENT seeds the remote workspace from
...
- UAGENT does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside UAGENT after the initial seed,
the remote sandbox does **not** see those changes. Use
`uagent sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"uagent"`  | Sandbox source for first-time create                  |
...
1. UAGENT calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. UAGENT calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `uagent sandbox` commands
