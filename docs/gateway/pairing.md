...
uagent nodes status
uagent nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.uagent`):

- `~/.uagent/nodes/paired.json`
- `~/.uagent/nodes/pending.json`

If you override `UAGENT_STATE_DIR`, the `nodes/` folder moves with it.
