...
UAGENT supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `UAGENT_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, UAGENT runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). UAGENT validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/UAGENT QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/uagent"],
...
- UAGENT resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__UAGENT_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `uagent.json` refs)
...
When reload-time activation fails after a healthy state, UAGENT enters degraded secrets state.
...
- Strict command paths (for example `uagent memory` remote-memory paths and `uagent qr --remote` when it needs remote shared-secret refs) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `uagent status`, `uagent status --all`, `uagent channels status`, `uagent channels resolve`, `uagent security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `uagent secrets reload`.
...
```bash
uagent secrets audit --check
uagent secrets configure
uagent secrets audit --check
```
...
- plaintext values at rest (`uagent.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `uagent.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `uagent secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `uagent.json` plus `auth-profiles.json` for one agent scope
...
- `uagent secrets configure --providers-only`
- `uagent secrets configure --skip-provider-setup`
- `uagent secrets configure --agent <id>`
...
```bash
uagent secrets apply --from /tmp/uagent-secrets-plan.json
uagent secrets apply --from /tmp/uagent-secrets-plan.json --allow-exec
uagent secrets apply --from /tmp/uagent-secrets-plan.json --dry-run
uagent secrets apply --from /tmp/uagent-secrets-plan.json --dry-run --allow-exec
```
...
UAGENT intentionally does not write rollback backups containing historical plaintext secret values.
