---
summary: "Run UAGENT on local LLMs (LM Studio, vLLM, LiteLLM, custom OpenAI endpoints)"
read_when:
...
Local is doable, but UAGENT expects large context + strong defenses against prompt injection. Small cards truncate context and leak safety. Aim high: **≥2 maxed-out Mac Studios or equivalent GPU rig (~$30k+)**. A single **24 GB** GPU works only for lighter prompts with higher latency. Use the **largest / full-size model variant you can run**; aggressively quantized or “small” checkpoints raise prompt-injection risk (see [Security](/gateway/security)).

If you want the lowest-friction local setup, start with [Ollama](/providers/ollama) and `uagent onboard`. This page is the opinionated guide for higher-end local stacks and custom OpenAI-compatible local servers.
...
- UAGENT treats these as proxy-style OpenAI-compatible routes, not native
  OpenAI endpoints
