...
`uagent doctor` is the repair + migration tool for UAGENT. It fixes stale
...
```bash
uagent doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
uagent doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
uagent doctor --non-interactive
```
...
- Extra workspace dir detection (`~/uagent`).
...
you to run `uagent doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.uagent/uagent.json` with the updated schema.

The Gateway also auto-runs doctor migrations on startup when it detects a
legacy config format, so stale configs are repaired without manual intervention.
Cron job store migrations are handled by `uagent doctor --fix`.
...
- Sessions store + transcripts:
  - from `~/.uagent/sessions/` to `~/.uagent/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.uagent/agent/` to `~/.uagent/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.uagent/credentials/*.json` (except `oauth.json`)
  - to `~/.uagent/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `uagent doctor`. Talk provider/provider-map normalization now
...
Doctor also checks the cron job store (`~/.uagent/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.uagent` folders exist across
  home directories or when `UAGENT_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.uagent/uagent.json` is
...
Discord plugin runtime packages) are present in the UAGENT install root.
If any are missing, doctor reports the packages and installs them in
`uagent doctor --fix` / `uagent doctor --repair` mode.

### 8) Gateway service migrations and cleanup hints

Doctor detects legacy gateway services (launchd/systemd/schtasks) and
offers to remove them and install the UAGENT service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named UAGENT gateway services are considered first-class and are not
flagged as "extra."
...
startup continues. In read-only mode (`uagent doctor` without `--fix`) this check
is skipped entirely.
...
- **Legacy workspace dirs**: warns when `~/uagent` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(uagent completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
Run `uagent completion --write-state` to regenerate the cache manually.
...
- `uagent doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `uagent doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
Use `uagent memory status --deep` to verify embedding readiness at runtime.
...
- `uagent doctor` prompts before rewriting supervisor config.
- `uagent doctor --yes` accepts the default repair prompts.
- `uagent doctor --repair` applies recommended fixes without prompts.
- `uagent doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `uagent gateway install --force`.
