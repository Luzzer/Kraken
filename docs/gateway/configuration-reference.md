...
summary: "Complete reference for every UAGENT config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.uagent/uagent.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — UAGENT uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.uagent/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `uagent doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `uagent doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["uagent-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-uagent",
          requireMention: false,
...
- UAGENT additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `uagent plugins install @uagent/mattermost`.
...
- `commands.callbackUrl` must resolve to the UAGENT gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, UAGENT rejects callbacks with
...
UAGENT spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `uagent channels add` (or channel onboarding) while still on a single-account top-level channel config, UAGENT promotes account-scoped top-level single-account values into the channel account map first so the original account keeps working. Most channels move them into `channels.<channel>.accounts.default`; Matrix can preserve an existing matching named/default target instead.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `uagent doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into the promoted account chosen for that channel. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@uagent", "uagent"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@uagent"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `uagent.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, UAGENT auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/uagent" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `openai/gpt-5.4`). If you omit the provider, UAGENT tries an alias first, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider (deprecated compatibility behavior, so prefer explicit `provider/model`). If that provider no longer exposes the configured default model, UAGENT falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before UAGENT aborts it. Default: `900`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.uagent/sandboxes",
        docker: {
          ...
          apparmorProfile: "uagent-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/uagent-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "uagent-sandbox-browser:bookworm-slim",
          network: "uagent-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that UAGENT materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.uagent/sandboxes`
...
In `remote` mode, host-local edits made outside UAGENT are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `uagent.json`.
noVNC observer access uses VNC auth by default and UAGENT emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `uagent-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from UAGENT_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `UAGENT_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `UAGENT_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `UAGENT_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.uagent/workspace",
        agentDir: "~/.uagent/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@uagent"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.uagent/workspace-home" },
      { id: "work", workspace: "~/.uagent/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, UAGENT resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.uagent/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.uagent/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.uagent/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.uagent/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, UAGENT starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.uagent/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, UAGENT treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:uagent`   | All built-in tools (excludes provider plugins)                                                                          |
...
- Files are materialized into the child workspace at `.uagent/attachments/<uuid>/` with a `.manifest.json`.
...
UAGENT uses the built-in model catalog. Add custom providers via `models.providers` in config or `~/.uagent/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `UAGENT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), UAGENT forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `uagent onboard --auth-choice opencode-zen` or `uagent onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `uagent onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `uagent onboard --auth-choice moonshot-api-key-cn`.

Native Moonshot endpoints advertise streaming usage compatibility on the shared
`openai-completions` transport, and UAGENT now keys that off endpoint
...
Anthropic-compatible, built-in provider. Shortcut: `uagent onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `uagent onboard --auth-choice synthetic-api-key`.
...
`uagent onboard --auth-choice minimax-global-api` or
`uagent onboard --auth-choice minimax-cn-api`.
The model catalog now defaults to M2.7 only.
On the Anthropic-compatible streaming path, UAGENT disables MiniMax thinking
...
- `install.nodeManager`: node installer preference for `metadata.uagent.install`
...
- Loaded from `~/.uagent/extensions`, `<workspace>/.uagent/extensions`, plus `plugins.load.paths`.
- Discovery accepts native UAGENT plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native UAGENT plugin schema when available).
...
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; UAGENT applies those as sanitized agent settings, not as raw UAGENT config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `uagent plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      uagent: { cdpPort: 18800, color: "#FF4500" },
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want UAGENT to discover `/json/version`; use WS(S)
...
- Local managed `uagent` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or UAGENT_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `UAGENT_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `UAGENT_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `UAGENT_APNS_RELAY_BASE_URL` / `UAGENT_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `UAGENT_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
UAGENT_CONFIG_PATH=~/.uagent/a.json \
UAGENT_STATE_DIR=~/.uagent-a \
uagent gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.uagent-dev` + port `19001`), `--profile <name>` (uses `~/.uagent-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/uagent/tls/server.crt",
      keyPath: "/etc/uagent/tls/server.key",
      caPath: "/etc/uagent/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.uagent/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-uagent-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `UAGENT_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.uagent/workspace/canvas",
    liveReload: true,
    // enabled: false, // or UAGENT_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__uagent__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__uagent__/a2ui/`
...
- Also serves A2UI at `/__uagent__/a2ui/`.
...
- Hostname defaults to `uagent`. Override with `UAGENT_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.uagent/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `uagent dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.uagent/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${UAGENT_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `uagent.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.uagent/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/uagent-vault-resolver",
...
- Legacy OAuth imports from `~/.uagent/credentials/oauth.json`.
...
- Default log file: `/tmp/uagent/uagent-YYYY-MM-DD.log`.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.uagent/logs/cache-trace.jsonl",
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$UAGENT_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one UAGENT.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `UAGENT_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `uagent doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `uagent.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
