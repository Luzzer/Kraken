...
UAGENT can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **UAGENT tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
uagent agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, UAGENT now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), UAGENT will
...
2. **Builds a system prompt** using the same UAGENT prompt + workspace context.
...
told us UAGENT-style Claude CLI usage is allowed again, so UAGENT treats
...
- UAGENT drops stored CLI session reuse when the backend auth state changes, including relogin, token rotation, or a changed auth profile credential.
...
UAGENT will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, UAGENT appends the
...
- For Gemini CLI JSON output, UAGENT reads reply text from `response` and
...
- `stats.cached` is normalized into UAGENT `cacheRead`.
- If `stats.input` is missing, UAGENT derives input tokens from
...
CLI backends do **not** receive UAGENT tool calls directly, but a backend can
...
When bundle MCP is enabled, UAGENT:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`UAGENT_MCP_TOKEN`)
...
If no MCP servers are enabled, UAGENT still injects a strict config when a
backend opts into bundle MCP so background runs stay isolated.

## Limitations

- **No direct UAGENT tool calls.** UAGENT does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. UAGENT sessions still work
  normally.
