...
UAGENT’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`uagent agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-uagent-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-uagent-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "uagent"`, `model: "uagent/default"`, `model: "uagent/<agentId>"`, or `x-uagent-agent-id`
- use `x-uagent-model` when you want to override the selected agent's backend model
- use `x-uagent-session-key` for explicit session routing
- use `x-uagent-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: UAGENT reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
UAGENT normalizes common OpenAI-style aliases before those counters reach
