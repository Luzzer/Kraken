...
UAGENT supports **camera capture** for agent workflows:
...
```bash
uagent nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
uagent nodes camera snap --node <id> --facing front
uagent nodes camera clip --node <id> --duration 3000
uagent nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`uagent.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `uagent` CLI to invoke camera commands on the macOS node.

Examples:

```bash
uagent nodes camera list --node <id>            # list camera ids
uagent nodes camera snap --node <id>            # prints MEDIA:<path>
uagent nodes camera snap --node <id> --max-width 1280
uagent nodes camera snap --node <id> --delay-ms 2000
uagent nodes camera snap --node <id> --device-id <id>
uagent nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
uagent nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
uagent nodes camera clip --node <id> --device-id <id>
uagent nodes camera clip --node <id> --no-audio
```

Notes:

- `uagent nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
uagent nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
