...
```bash
uagent nodes describe --node <idOrNameOrIp>
uagent nodes canvas snapshot --node <idOrNameOrIp>
uagent logs --follow
```
...
uagent nodes status
uagent approvals get --node <idOrNameOrIp>
uagent approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`uagent approvals get --node ...`), not in the gateway pairing record.
