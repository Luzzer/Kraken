...
UAGENT supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, UAGENT also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `uagent configure --section web`. It stores the key in
`~/.uagent/uagent.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.uagent/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
