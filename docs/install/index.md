---
summary: "Install UAGENT — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs UAGENT, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://uagent.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want UAGENT and Node kept under a local prefix such as
`~/.uagent`, without depending on a system-wide Node install:

```bash
curl -fsSL https://uagent.ai/install-cli.sh | bash
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g uagent@latest
```
...
Or skip the link and use `pnpm uagent ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:uagent/uagent#main
```
...
```bash
uagent --version      # confirm the CLI is available
uagent doctor         # check for config issues
uagent gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `uagent onboard --install-daemon` or `uagent gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy UAGENT on a cloud server or VPS:
...
<CardGroup cols={3}>
  <Card title="Updating" href="/install/updating" icon="refresh-cw">
    Keep UAGENT up to date.
  </Card>
  <Card title="Migrating" href="/install/migrating" icon="arrow-right">
    Move to a new machine.
  </Card>
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove UAGENT completely.
  </Card>
</CardGroup>

## Troubleshooting: `uagent` not found

If the install succeeded but `uagent` is not found in your terminal:
