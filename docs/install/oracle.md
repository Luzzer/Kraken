---
summary: "Host UAGENT on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up UAGENT on Oracle Cloud
  - Looking for free VPS hosting for UAGENT
...
Run a persistent UAGENT Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@uagent`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=uagent --reset` to re-authenticate.

**Gateway will not start** -- Run `uagent doctor --non-interactive` and check logs with `journalctl --user -u uagent-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep UAGENT up to date
