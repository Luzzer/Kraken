---
summary: "Move (migrate) an UAGENT install from one machine to another"
read_when:
  - You are moving UAGENT to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating UAGENT to a New Machine

This guide moves an UAGENT gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.uagent/` by default) and your **workspace**, you preserve:

- **Config** -- `uagent.json` and all gateway settings
...
<Tip>
Run `uagent status` on the old machine to confirm your state directory path.
Custom profiles use `~/.uagent-<profile>/` or a path set via `UAGENT_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    uagent gateway stop
    cd ~
    tar -czf uagent-state.tgz .uagent
    ```

    If you use multiple profiles (e.g. `~/.uagent-work`), archive each separately.

  </Step>

  <Step title="Install UAGENT on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.uagent/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `UAGENT_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `uagent doctor`.
  </Accordion>

  <Accordion title="Copying only uagent.json">
...
- [ ] `uagent status` shows the gateway running
