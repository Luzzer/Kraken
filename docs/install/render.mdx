---
summary: "Deploy UAGENT on Render with Infrastructure-as-Code"
read_when:
  - Deploying UAGENT to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy UAGENT on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/uagent/uagent)
...
repository configures everything needed to run UAGENT:
...
services:
  - type: web
    ...
    envVars:
      - key: UAGENT_GATEWAY_PORT
        value: "8080"
      - key: UAGENT_STATE_DIR
        value: /data/.uagent
      - key: UAGENT_WORKSPACE_DIR
        value: /data/workspace
      - key: UAGENT_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means UAGENT state
...
`UAGENT_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original UAGENT repository, Render will not auto-deploy your UAGENT. To update it, run a manual Blueprint sync from the dashboard.
...
For UAGENT, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with UAGENT state plus any configured
...
- Missing `UAGENT_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `UAGENT_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `uagent backup create` in the Render shell.
