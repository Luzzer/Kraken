---
summary: "Run UAGENT Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want UAGENT running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# UAGENT on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent UAGENT Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "UAGENT 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the UAGENT Gateway in Docker
- Persist `~/.uagent` + `~/.uagent/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.uagent` state includes `uagent.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-uagent-project --name="UAGENT Gateway"
    gcloud config set project my-uagent-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create uagent-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh uagent-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the UAGENT repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    UAGENT_IMAGE=uagent:latest
    UAGENT_GATEWAY_TOKEN=change-me-now
    UAGENT_GATEWAY_BIND=lan
    UAGENT_GATEWAY_PORT=18789

    UAGENT_CONFIG_DIR=/home/$USER/.uagent
    UAGENT_WORKSPACE_DIR=/home/$USER/.uagent/workspace

    GOG_KEYRING_PASSWORD=change-me-now
    XDG_CONFIG_HOME=/home/node/.uagent
    ```
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `UAGENT_GATEWAY_TOKEN`.
    ...
    services:
      uagent-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - UAGENT_GATEWAY_BIND=${UAGENT_GATEWAY_BIND}
          - UAGENT_GATEWAY_PORT=${UAGENT_GATEWAY_PORT}
          - UAGENT_GATEWAY_TOKEN=${UAGENT_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${UAGENT_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`UAGENT_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm uagent-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh uagent-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop uagent-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type uagent-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start uagent-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create uagent-deploy \
     --display-name="UAGENT Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-uagent-project \
     --member="serviceAccount:uagent-deploy@my-uagent-project.iam.gserviceaccount.com" \
