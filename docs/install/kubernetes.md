---
summary: "Deploy UAGENT Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run UAGENT on a Kubernetes cluster
  - You want to test UAGENT in a Kubernetes environment
title: "Kubernetes"
---

# UAGENT on Kubernetes

A minimal starting point for running UAGENT on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

UAGENT is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/uagent 18789:18789 -n uagent
...
```bash
kubectl get secret uagent-secrets -n uagent -o jsonpath='{.data.UAGENT_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: uagent (configurable via UAGENT_NAMESPACE)
├── Deployment/uagent        # Single pod, init container + gateway
├── Service/uagent           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/uagent-config  # uagent.json + AGENTS.md
└── Secret/uagent-secrets    # Gateway token + API keys
```
...
Edit `uagent.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret uagent-secrets -n uagent \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/uagent -n uagent
```

### Custom namespace

```bash
UAGENT_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/uagent/uagent:latest # or pin to a specific version from https://github.com/uagent/uagent/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # uagent.json + AGENTS.md
