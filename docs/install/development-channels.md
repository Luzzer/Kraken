...
UAGENT ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/uagent`, override with
  `UAGENT_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
uagent update --tag 2026.4.1-beta.1
...
uagent update --tag uagent@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `uagent update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  UAGENT prompts for confirmation (skip with `--yes`).
...
Preview what `uagent update` would do without making changes:

```bash
uagent update --dry-run
uagent update --channel beta --dry-run
uagent update --tag 2026.4.1-beta.1 --dry-run
uagent update --dry-run --json
```
...
When you switch channels with `uagent update`, UAGENT also syncs plugin
sources:
