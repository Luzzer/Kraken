---
summary: "ClawDock shell helpers for Docker-based UAGENT installs"
read_when:
  - You run UAGENT with Docker often and want shorter day-to-day commands
...
ClawDock is a small shell-helper layer for Docker-based UAGENT installs.
...
| `clawdock-cli <command>`  | Run UAGENT CLI commands in Docker           |
...
- `~/.uagent/.env` for env-backed provider keys and bot tokens
- `~/.uagent/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.uagent/uagent.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `uagent.json` quickly. It redacts `.env` values in its printed output.
