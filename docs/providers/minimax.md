---
summary: "Use MiniMax models in UAGENT"
read_when:
  - You want MiniMax models in UAGENT
...
UAGENT's MiniMax provider defaults to **MiniMax M2.7**.
...
entries, UAGENT materializes `MiniMax-M2.7` and
...
```bash
uagent onboard --auth-choice minimax-global-oauth
# or
uagent onboard --auth-choice minimax-cn-oauth
```
...
See the MiniMax plugin package README in the UAGENT repo for details.
...
```bash
uagent onboard --auth-choice minimax-global-api
# or
...
On the Anthropic-compatible streaming path, UAGENT now disables MiniMax
...
- On `api: "anthropic-messages"`, UAGENT injects
...
- UAGENT normalizes MiniMax coding-plan usage to the same `% left` display
  used by other providers. MiniMax's raw `usage_percent` / `usagePercent`
  fields are remaining quota, not consumed quota, so UAGENT inverts them.
  Count-based fields win when present. When the API returns `model_remains`,
  UAGENT prefers the chat-model entry, derives the window label from
...
- Use `uagent models list` to confirm the current provider id, then switch with
  `uagent models set minimax/MiniMax-M2.7` or
  `uagent models set minimax-portal/MiniMax-M2.7`.
...
- Running `uagent configure` and selecting a **MiniMax** auth option, or
