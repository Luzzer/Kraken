---
summary: "Use Qwen Cloud via UAGENT's bundled qwen provider"
read_when:
  - You want to use Qwen with UAGENT
...
See [Issue #49557](https://github.com/uagent/uagent/issues/49557) for
background.
...
UAGENT now treats Qwen as a first-class bundled provider with canonical id
...
uagent onboard --auth-choice qwen-api-key

# China Coding Plan endpoint
uagent onboard --auth-choice qwen-api-key-cn

# Global Standard (pay-as-you-go) endpoint
uagent onboard --auth-choice qwen-standard-api-key

# China Standard (pay-as-you-go) endpoint
uagent onboard --auth-choice qwen-standard-api-key-cn
```
...
shared `openai-completions` transport. UAGENT keys that off endpoint
...
UAGENT currently ships this bundled Qwen catalog:
...
For video generation, UAGENT maps the configured Qwen region to the matching
...
available to that process (for example, in `~/.uagent/.env` or via
`env.shellEnv`).
