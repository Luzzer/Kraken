---
summary: "Use OpenAI via API keys or Codex subscription in UAGENT"
read_when:
  - You want to use OpenAI models in UAGENT
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like UAGENT.

## Default interaction style

UAGENT can add a small OpenAI-specific prompt overlay for both `openai/*` and
...
without replacing the base UAGENT system prompt. The friendly overlay also
...
- `"off"`: disable the overlay and use the base UAGENT prompt only.
...
If you want the unmodified base UAGENT prompt, set the overlay to `"off"`:
...
```bash
uagent config set plugins.entries.openai.config.personality off
```

UAGENT normalizes this setting case-insensitively at runtime, so values like
...
- In UAGENT, ChatGPT/Codex sign-in is routed through `openai-codex/*`, not `openai/*`

### CLI setup

```bash
uagent onboard --auth-choice openai-api-key
# or non-interactive
uagent onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. UAGENT forwards both through the `openai/*` Responses path.
UAGENT intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

UAGENT does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in UAGENT.
...
- Supports `size`
- Current OpenAI-specific caveat: UAGENT does not forward `aspectRatio` or
...
- Current OpenAI-specific caveat: UAGENT currently only forwards `size`
...
uagent onboard --auth-choice openai-codex

# Or run OAuth directly
uagent models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. UAGENT
...
managed by Codex CLI. On expiry, UAGENT re-reads the external Codex source
first and, when the provider can refresh it, writes the refreshed credential
back to Codex storage instead of taking ownership in a separate UAGENT-only
copy.

If your Codex account is entitled to Codex Spark, UAGENT also supports:

- `openai-codex/gpt-5.3-codex-spark`

UAGENT treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

UAGENT also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
UAGENT treats the Codex model metadata and the runtime context cap as separate
values.
...
UAGENT uses `pi-ai` for model streaming. For both `openai/*` and
`openai-codex/*`, default transport is `"auto"` (WebSocket-first, then SSE
fallback).

In `"auto"` mode, UAGENT also retries one early, retryable WebSocket failure
...
After a connect or early-turn WebSocket failure in `"auto"` mode, UAGENT marks
...
OpenAI Responses), UAGENT also attaches stable session and turn identity state
...
UAGENT also normalizes OpenAI usage counters across transport variants before
...
`prompt_tokens` / `completion_tokens`; UAGENT treats those as the same input
and output counters for `/status`, `/usage`, and session logs. When native
WebSocket traffic omits `total_tokens` (or reports `0`), UAGENT falls back to
...
For `openai/*` (Responses API), UAGENT also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. UAGENT enables it by default for
...
UAGENT, set `agents.defaults.models["<provider>/<model>"].params.serviceTier`
...
UAGENT forwards `params.serviceTier` to both direct `openai/*` Responses
...
- if you route either provider through another base URL or proxy, UAGENT leaves `service_tier` untouched

### OpenAI fast mode

UAGENT exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, UAGENT maps it to OpenAI priority processing:
...
UAGENT treats direct OpenAI, Codex, and Azure OpenAI endpoints differently
...
- hidden UAGENT attribution headers (`originator`, `version`, and
...
`baseUrl` on `api.openai.com`), UAGENT now auto-enables OpenAI server-side
