---
summary: "Run UAGENT with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run UAGENT against a local SGLang server
...
UAGENT can connect to SGLang using the `openai-completions` API.

UAGENT can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, UAGENT will query:
