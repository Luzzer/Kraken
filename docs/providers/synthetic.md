---
summary: "Use Synthetic's Anthropic-compatible API in UAGENT"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. UAGENT registers it as the
...
```bash
uagent onboard --auth-choice synthetic-api-key
```
...
Note: UAGENT's Anthropic client appends `/v1` to the base URL, so use
