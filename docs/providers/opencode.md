---
summary: "Use OpenCode Zen and Go catalogs with UAGENT"
read_when:
...
OpenCode exposes two hosted catalogs in UAGENT:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. UAGENT keeps the runtime provider ids
...
uagent onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
uagent onboard --auth-choice opencode-go
uagent onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
...
- Gemini-backed OpenCode refs stay on the proxy-Gemini path, so UAGENT keeps
