---
title: "ComfyUI"
summary: "ComfyUI workflow image, video, and music generation setup in UAGENT"
read_when:
  - You want to use local ComfyUI workflows with UAGENT
...
UAGENT ships a bundled `comfy` plugin for workflow-driven ComfyUI runs.
...
The bundled plugin is workflow-driven, so UAGENT does not try to map generic
...
UAGENT treats that legacy shape as the image workflow config.
...
the configured graph. UAGENT does not pass input videos into Comfy workflows.
