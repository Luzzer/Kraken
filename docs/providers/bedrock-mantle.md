---
summary: "Use Amazon Bedrock Mantle (OpenAI-compatible) models with UAGENT"
read_when:
  - You want to use Bedrock Mantle hosted OSS models with UAGENT
...
UAGENT includes a bundled **Amazon Bedrock Mantle** provider that connects to
...
When `AWS_BEARER_TOKEN_BEDROCK` is set, UAGENT uses it directly. Otherwise,
UAGENT attempts to generate a Mantle bearer token from the AWS default
...
- UAGENT can mint the Mantle bearer token for you from AWS SDK-compatible
