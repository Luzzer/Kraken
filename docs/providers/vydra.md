---
summary: "Use Vydra image, video, and speech in UAGENT"
read_when:
  - You want Vydra media generation in UAGENT
...
UAGENT uses the same `VYDRA_API_KEY` for all three capabilities.
...
```bash
uagent onboard --auth-choice vydra-api-key
```
...
Current bundled support is text-to-image only. Vydra's hosted edit routes expect remote image URLs, and UAGENT does not add a Vydra-specific upload bridge in the bundled plugin yet.
...
```bash
export UAGENT_LIVE_VYDRA_KLING_IMAGE_URL="https://example.com/reference.png"
```
