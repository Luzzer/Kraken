...
read_when:
  - You want to use Google Gemini models with UAGENT
...
```bash
uagent onboard --auth-choice gemini-api-key
```
...
```bash
uagent models auth login --provider google-gemini-cli --set-default
```

Environment variables:

- `UAGENT_GEMINI_OAUTH_CLIENT_ID`
- `UAGENT_GEMINI_OAUTH_CLIENT_SECRET`
...
command is installed and on `PATH`. UAGENT supports both Homebrew installs
...
For direct Gemini API runs (`api: "google-generative-ai"`), UAGENT now
...
- Gemini cache-hit usage is normalized into UAGENT `cacheRead` from
