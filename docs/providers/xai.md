---
summary: "Use xAI Grok models in UAGENT"
read_when:
  - You want to use Grok models in UAGENT
...
UAGENT ships a bundled `xai` provider plugin for Grok models.
...
```bash
uagent onboard --auth-choice xai-api-key
```
...
UAGENT now uses the xAI Responses API as the bundled xAI transport. The same
...
UAGENT now includes these xAI model families out of the box:
...
```bash
uagent config set tools.web.search.provider grok
```
...
- Auth is API-key only today. There is no xAI OAuth/device-code flow in UAGENT yet.
- `grok-4.20-multi-agent-experimental-beta-0304` is not supported on the normal xAI provider path because it requires a different upstream API surface than the standard UAGENT xAI transport.

## Notes

- UAGENT applies xAI-specific tool-schema and tool-call compatibility fixes automatically on the shared runner path.
...
- `web_search`, `x_search`, and `code_execution` are exposed as UAGENT tools. UAGENT enables the specific xAI built-in it needs inside each tool request instead of attaching all native tools to every chat turn.
