...
read_when:
  - You want to use Chutes with UAGENT
...
OpenAI-compatible API. UAGENT supports both browser OAuth and direct API-key
...
- Auth:
  - OAuth via `uagent onboard --auth-choice chutes`
  - API key via `uagent onboard --auth-choice chutes-api-key`
...
```bash
uagent onboard --auth-choice chutes
```

UAGENT launches the browser flow locally, or shows a URL + redirect-paste
flow on remote/headless hosts. OAuth tokens auto-refresh through UAGENT auth
profiles.
...
```bash
uagent onboard --auth-choice chutes-api-key
```
...
When Chutes auth is available, UAGENT queries the Chutes catalog with that
credential and uses the discovered models. If discovery fails, UAGENT falls
back to a bundled static catalog so onboarding and startup still work.

## Default aliases

UAGENT also registers three convenience aliases for the bundled Chutes
catalog:
