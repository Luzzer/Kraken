---
title: "Runway"
summary: "Runway video generation setup in UAGENT"
read_when:
  - You want to use Runway video generation in UAGENT
...
UAGENT ships a bundled `runway` provider for hosted video generation.
...
```bash
uagent onboard --auth-choice runway-api-key
```

2. Set Runway as the default video provider:

```bash
uagent config set agents.defaults.videoGenerationModel.primary "runway/gen4.5"
```
