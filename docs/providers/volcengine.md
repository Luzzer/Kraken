...
read_when:
  - You want to use Volcano Engine or Doubao models with UAGENT
...
```bash
uagent onboard --auth-choice volcengine-api-key
```
...
`uagent onboard --auth-choice volcengine-api-key` currently sets
...
loaded yet, UAGENT falls back to the unfiltered catalog instead of showing an
...
`~/.uagent/.env` or via `env.shellEnv`).
