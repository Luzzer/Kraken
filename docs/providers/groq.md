...
read_when:
  - You want to use Groq with UAGENT
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. UAGENT connects
...
media-understanding provider, UAGENT uses Groq's `whisper-large-v3-turbo`
...
Groq's model catalog changes frequently. Run `uagent models list | grep groq`
