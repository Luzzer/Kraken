---
summary: "Use Venice AI privacy-focused models in UAGENT"
read_when:
  - You want privacy-focused inference in UAGENT
...
```bash
uagent onboard --auth-choice venice-api-key
```
...
```bash
uagent agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, UAGENT shows all available Venice models. Pick based on your needs:
...
```bash
uagent models set venice/kimi-k2-5
uagent models set venice/claude-opus-4-6
```

List all available models:

```bash
uagent models list | grep venice
```

## Configure via `uagent configure`

1. Run `uagent configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
UAGENT automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
uagent agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
uagent agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
uagent agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
uagent agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
uagent agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `uagent models list` to see currently available models. Some models may be temporarily offline.
