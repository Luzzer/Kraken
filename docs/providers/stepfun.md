---
summary: "Use StepFun models with UAGENT"
read_when:
  - You want StepFun models in UAGENT
...
UAGENT includes a bundled StepFun provider plugin with two provider ids:
...
```bash
uagent onboard --auth-choice stepfun-standard-api-key-intl --stepfun-api-key "$STEPFUN_API_KEY"
uagent onboard --auth-choice stepfun-plan-api-key-intl --stepfun-api-key "$STEPFUN_API_KEY"
```
...
- The provider is bundled with UAGENT, so there is no separate plugin install step.
...
- Use `uagent models list` and `uagent models set <provider/model>` to inspect or switch models.
