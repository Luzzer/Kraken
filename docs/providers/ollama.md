---
summary: "Run UAGENT with Ollama (cloud and local models)"
read_when:
  - You want to run UAGENT with cloud or local models via Ollama
...
Ollama is a local LLM runtime that makes it easy to run open-source models on your machine. UAGENT integrates with Ollama's native API (`/api/chat`), supports streaming and tool calling, and can auto-discover local Ollama models when you opt in with `OLLAMA_API_KEY` (or an auth profile) and do not define an explicit `models.providers.ollama` entry.

<Warning>
**Remote Ollama users**: Do not use the `/v1` OpenAI-compatible URL (`http://host:11434/v1`) with UAGENT. This breaks tool calling and models may output raw tool JSON as plain text. Use the native Ollama API URL instead: `baseUrl: "http://host:11434"` (no `/v1`).
</Warning>
...
```bash
uagent onboard --non-interactive \
  --auth-choice ollama \
  --accept-risk
```
...
UAGENT currently suggests:

- local default: `glm-4.7-flash`
- cloud defaults: `kimi-k2.5:cloud`, `minimax-m2.5:cloud`, `glm-5:cloud`

5. If you prefer manual setup, enable Ollama for UAGENT directly (any value works; Ollama doesn't require a real key):
...
uagent config set models.providers.ollama.apiKey "ollama-local"
```
...
```bash
uagent models list
uagent models set ollama/glm-4.7-flash
```
...
When you set `OLLAMA_API_KEY` (or an auth profile) and **do not** define `models.providers.ollama`, UAGENT discovers models from the local Ollama instance at `http://127.0.0.1:11434`:
...
- Sets `maxTokens` to the default Ollama max-token cap used by UAGENT
- Sets all costs to `0`
...
If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry and UAGENT will fill it for availability checks.
...
UAGENT also supports **Ollama Web Search** as a bundled `web_search`
provider.
...
Choose **Ollama Web Search** during `uagent onboard` or
`uagent configure --section web`, or set:
...
UAGENT treats models with names such as `deepseek-r1`, `reasoning`, or `think` as reasoning-capable by default:
...
UAGENT's Ollama integration uses the **native Ollama API** (`/api/chat`) by default, which fully supports streaming and tool calling simultaneously. No special configuration is needed.
...
When `api: "openai-completions"` is used with Ollama, UAGENT injects `options.num_ctx` by default so Ollama does not silently fall back to a 4096 context window. If your proxy/upstream rejects unknown `options` fields, disable this behavior:
...
For auto-discovered models, UAGENT uses the context window reported by Ollama when available, otherwise it falls back to the default Ollama context window used by UAGENT. You can override `contextWindow` and `maxTokens` in explicit provider config.
