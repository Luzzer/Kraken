---
summary: "Use Amazon Bedrock (Converse API) models with UAGENT"
read_when:
  - You want to use Amazon Bedrock models with UAGENT
...
UAGENT can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
UAGENT can automatically discover Bedrock models that support **streaming**
...
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is `true`,
  UAGENT will try discovery even when no AWS env marker is present.
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is unset,
  UAGENT only auto-adds the
...
- `enabled` defaults to auto mode. In auto mode, UAGENT only enables the
...
- For explicit `models.providers["amazon-bedrock"]` entries, UAGENT can still
...
When running UAGENT on an EC2 instance with an IAM role attached, the AWS SDK
can use the instance metadata service (IMDS) for authentication. For Bedrock
model discovery, UAGENT only auto-enables the implicit provider from AWS env
...
uagent config set plugins.entries.amazon-bedrock.config.discovery.enabled true
uagent config set plugins.entries.amazon-bedrock.config.discovery.region us-east-1
...
UAGENT discovers **regional and global inference profiles** alongside
...
foundation models in `uagent models list`.
...
- If you rely on auto mode, set one of the supported AWS auth env markers on the
  ...
  `plugins.entries.amazon-bedrock.config.discovery.enabled: true`.
- UAGENT surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
