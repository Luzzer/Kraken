...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in UAGENT
...
Running UAGENT via Node with `tsx` fails at startup with:

```
[uagent] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `uagent status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch uagent.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node uagent.mjs status` works on Node 25.
