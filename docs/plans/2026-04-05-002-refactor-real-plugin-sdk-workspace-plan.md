...
| ----------- | ------------------------ | ------------------------------------ | -------------------------------------------- |
| Legacy mode | `uagent/plugin-sdk/*`  | all existing non-opted-in extensions | current permissive behavior remains          |
| Opt-in mode | `@uagent/plugin-sdk/*` | first-wave extensions only           | package-local `rootDir` + project references |
...
- root `tsconfig.json` maps `uagent/plugin-sdk/*` directly to
  `src/plugin-sdk/*.ts`
...
- moves the SDK toward a workspace package named `@uagent/plugin-sdk`
...
- R2. Name the new package `@uagent/plugin-sdk`.
- R3. Give the new SDK package its own `package.json` and `tsconfig.json`.
- R4. Keep legacy `uagent/plugin-sdk/*` imports working for non-opted-in
...
- Root `tsconfig.json` currently maps:
  - `uagent/plugin-sdk` -> `src/plugin-sdk/index.ts`
  - `uagent/plugin-sdk/*` -> `src/plugin-sdk/*.ts`
...
The initial `@uagent/plugin-sdk` package only needs to cover these:
...
- Introduce `@uagent/plugin-sdk` as a new workspace package while keeping the
  legacy root `uagent/plugin-sdk/*` surface alive during migration.
...
- Keep `@uagent/plugin-sdk` private in the first wave.
...
- Whether the first-wave package can point directly at package-local `src/*.ts`
  ...
  still required for the `@uagent/plugin-sdk` package.
  This is an implementation-owned TS graph validation question.

- Whether the root `uagent` package should proxy first-wave SDK subpaths to
...
- [ ] **Unit 1: Introduce the real `@uagent/plugin-sdk` workspace package**
...
- Add a new workspace package named `@uagent/plugin-sdk`.
...
- The repo contains a valid `@uagent/plugin-sdk` workspace package with a
...
- [ ] **Unit 3: Migrate the first-wave extensions onto `@uagent/plugin-sdk`**
...
- Modify: production and test imports under each of the 10 extension roots that
  currently reference `uagent/plugin-sdk/*`

**Approach:**

- Add `@uagent/plugin-sdk: workspace:*` to the first-wave extension
  `devDependencies`.
- Replace `uagent/plugin-sdk/*` imports in those packages with
  `@uagent/plugin-sdk/*`.
...
- Package dependency shape used by other `@uagent/*` workspace packages
...
- Integration: migrated extensions do not require root `uagent/plugin-sdk/*`
  aliases for typechecking.

**Verification:**

- The first-wave extensions build and test against `@uagent/plugin-sdk`
...
- Keep root `uagent/plugin-sdk/*` as the compatibility surface for legacy
...
- **API surface parity:** first-wave subpaths must remain semantically identical
  through both `uagent/plugin-sdk/*` and `@uagent/plugin-sdk/*` during the
  transition.
