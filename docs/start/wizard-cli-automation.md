---
summary: "Scripted onboarding and agent setup for the UAGENT CLI"
read_when:
...
Use `--non-interactive` to automate `uagent onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but UAGENT now prefers Claude CLI reuse when available.
