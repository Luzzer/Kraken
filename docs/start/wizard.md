...
CLI onboarding is the **recommended** way to set up UAGENT on macOS,
...
`uagent dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`uagent configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.uagent/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `uagent doctor` first.
</Note>
...
Use `uagent agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.uagent/workspace-<agentId>`.
...
- CLI command reference: [`uagent onboard`](/cli/onboard)
