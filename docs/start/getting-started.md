---
summary: "Get UAGENT installed and run your first chat in minutes."
read_when:
...
Install UAGENT, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.uagent/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.uagent/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run UAGENT as a service account or want custom paths:

- `UAGENT_HOME` — home directory for internal path resolution
- `UAGENT_STATE_DIR` — override the state directory
- `UAGENT_CONFIG_PATH` — override the config file path
