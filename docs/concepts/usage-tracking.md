...
- `/usage cost` in chats: local cost summary aggregated from UAGENT session logs.
- CLI: `uagent status --usage` prints a full per-provider breakdown.
- CLI: `uagent channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. UAGENT treats
  ...
  quota, so UAGENT inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, UAGENT prefers the
...
can supply plugin-specific usage auth logic; otherwise UAGENT falls back to
