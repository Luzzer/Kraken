...
UAGENT selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models UAGENT can use (plus aliases).
...
- If you omit the provider, UAGENT resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, UAGENT
...
uagent models set <provider/model>
uagent models set-image <provider/model>

uagent models aliases list
uagent models aliases add <alias> <provider/model>
uagent models aliases remove <alias>

uagent models fallbacks list
uagent models fallbacks add <provider/model>
uagent models fallbacks remove <provider/model>
uagent models fallbacks clear

uagent models image-fallbacks list
uagent models image-fallbacks add <provider/model>
uagent models image-fallbacks remove <provider/model>
uagent models image-fallbacks clear
```

`uagent models` (no subcommand) is a shortcut for `models status`.
...
`uagent models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.uagent/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: UAGENT writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever UAGENT regenerates `models.json`, including command-driven paths like `uagent agent`.
