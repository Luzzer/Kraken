...
read_when:
  - You want to understand how UAGENT assembles model context
...
A **context engine** controls how UAGENT builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

UAGENT ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.uagent/uagent.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other UAGENT plugin. Install
...
uagent plugins install -l ./my-context-engine
```
...
Every time UAGENT runs a model prompt, the context engine participates at
...
UAGENT currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. UAGENT
...
The built-in `legacy` engine preserves UAGENT's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. UAGENT uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. UAGENT disables Pi's built-in
...
`ownsCompaction: false` does **not** mean UAGENT automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `uagent/plugin-sdk/core` to use
  UAGENT's built-in compaction behavior.
...
UAGENT resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to UAGENT's built-in summarization. Plugin engines can implement
...
- Use `uagent doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, UAGENT
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `uagent plugins install -l ./my-engine` to link a
