---
summary: "What the UAGENT system prompt contains and how it is assembled"
read_when:
...
UAGENT builds a custom system prompt for every agent run. The prompt is **UAGENT-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by UAGENT and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full UAGENT-owned prompt. The provider runtime can:
...
- **UAGENT Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to UAGENT docs (repo or npm package) and when to read them.
...
UAGENT can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **UAGENT
...
occurs, UAGENT can inject a warning block in Project Context; control this with
...
When eligible skills exist, UAGENT injects a compact **available skills list**
...
local UAGENT docs directory (either `docs/` in the repo workspace or the bundled npm
...
for UAGENT behavior, commands, configuration, or architecture, and to run
`uagent status` itself when possible (asking the user only when it lacks access).
