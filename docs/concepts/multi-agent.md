...
- **Session store** (chat history + routing state) under `~/.uagent/agents/<agentId>/sessions`.
...
`~/.uagent/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.uagent` (or `UAGENT_STATE_DIR`)
- Workspace: `~/.uagent/workspace` (or `~/.uagent/workspace-<agentId>`)
- Agent dir: `~/.uagent/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.uagent/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, UAGENT runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.uagent/workspace` (or `~/.uagent/workspace-<profile>` when `UAGENT_PROFILE` is set).
- State defaults to `~/.uagent/agents/main/agent`.
...
```bash
uagent agents add coding
uagent agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.uagent/agents/<agentId>`.
...
```bash
uagent channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.uagent/workspace-alex" },
      { id: "mia", workspace: "~/.uagent/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, UAGENT upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, UAGENT falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.uagent/workspace-main" },
      { id: "coding", workspace: "~/.uagent/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.uagent/workspace-main" },
      { id: "alerts", workspace: "~/.uagent/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
uagent channels login --channel whatsapp --account personal
uagent channels login --channel whatsapp --account biz
```

`~/.uagent/uagent.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.uagent/workspace-home",
        agentDir: "~/.uagent/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.uagent/workspace-work",
        agentDir: "~/.uagent/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.uagent/credentials/whatsapp/personal
          // authDir: "~/.uagent/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.uagent/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.uagent/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.uagent/workspace-opus",
