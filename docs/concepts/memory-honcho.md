...
[Honcho](https://honcho.dev) adds AI-native memory to UAGENT. It persists
...
```bash
uagent plugins install @honcho-ai/uagent-honcho
uagent honcho setup
...
Settings live under `plugins.entries["uagent-honcho"].config`:
    ...
    entries: {
      "uagent-honcho": {
        config: {
          ...
          workspaceId: "uagent", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `uagent honcho setup` detects and
...
```bash
uagent honcho setup                        # Configure API key and migrate files
uagent honcho status                       # Check connection status
uagent honcho ask <question>               # Query Honcho about the user
uagent honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/uagent-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho UAGENT integration guide](https://docs.honcho.dev/v3/guides/integrations/uagent)
- [Memory](/concepts/memory) -- UAGENT memory overview
