---
summary: "How UAGENT manages conversation sessions"
read_when:
...
UAGENT organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `uagent security audit`.
...
UAGENT automatically bounds session storage over time. By default, it runs
...
Preview with `uagent sessions cleanup --dry-run`.

## Inspecting sessions

- `uagent status` -- session store path and recent activity.
- `uagent sessions --json` -- all sessions (filter with `--active <minutes>`).
