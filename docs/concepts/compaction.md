---
summary: "How UAGENT summarizes long conversations to stay within model limits"
read_when:
...
When a conversation approaches that limit, UAGENT **compacts** older messages
...
When UAGENT splits history into compaction chunks, it keeps assistant tool
calls paired with their matching `toolResult` entries. If a split point lands
inside a tool block, UAGENT moves the boundary so the pair stays together and
...
UAGENT compacts and retries). Typical overflow signatures include
...
<Info>
Before compacting, UAGENT automatically reminds the agent to save important
