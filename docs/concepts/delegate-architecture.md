---
summary: "Delegate architecture: running UAGENT as a named agent on behalf of an organization"
...
Goal: run UAGENT as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an UAGENT agent that:
...
UAGENT's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of UAGENT's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.uagent/workspace-delegate",
  tools: {
...
- Cron run history: `~/.uagent/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.uagent/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through UAGENT's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
uagent agents add delegate
```

This creates:

- Workspace: `~/.uagent/workspace-delegate`
- State: `~/.uagent/agents/delegate/agent`
- Sessions: `~/.uagent/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.uagent/workspace" },
      {
        id: "delegate",
        workspace: "~/.uagent/workspace-delegate",
        tools: {
...
~/.uagent/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.uagent/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.uagent/workspace-org",
        agentDir: "~/.uagent/agents/org-assistant/agent",
...
recall view. UAGENT redacts credential/token-like text, truncates long
