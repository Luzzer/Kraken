...
summary: "How UAGENT remembers things across sessions"
read_when:
...
UAGENT remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.uagent/workspace`).
...
<Info>
UAGENT auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, UAGENT
...
```bash
uagent memory status          # Check index status and provider
uagent memory search "query"  # Search from the command line
uagent memory index --force   # Rebuild the index
```
