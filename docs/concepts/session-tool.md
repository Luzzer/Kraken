...
UAGENT gives agents tools to work across sessions, inspect status, and
...
After the target responds, UAGENT can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned UAGENT
...
the completion origin only identifies a channel UAGENT can still reuse the
