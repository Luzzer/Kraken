---
summary: "CLI reference for `uagent cron` (schedule and run background jobs)"
read_when:
...
Tip: run `uagent cron --help` for the full command surface.
...
Note: `uagent cron run` now returns as soon as the manual run is queued for execution. Successful responses include `{ ok: true, enqueued: true, runId }`; use `uagent cron runs --id <job-id>` to follow the eventual outcome.

Note: `uagent cron run <job-id>` force-runs by default. Use `--due` to keep the
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.uagent/cron/runs/<jobId>.jsonl`.

Upgrade note: if you have older cron jobs from before the current delivery/store format, run
`uagent doctor --fix`. Doctor now normalizes legacy cron fields (`jobId`, `schedule.cron`,
...
```bash
uagent cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
uagent cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
uagent cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
uagent cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```
...
```bash
uagent cron run <job-id>
uagent cron run <job-id> --due
uagent cron runs --id <job-id> --limit 50
```

Agent/session retargeting:

```bash
uagent cron edit <job-id> --agent ops
uagent cron edit <job-id> --clear-agent
uagent cron edit <job-id> --session current
uagent cron edit <job-id> --session "session:daily-brief"
```
...
```bash
uagent cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
uagent cron edit <job-id> --best-effort-deliver
uagent cron edit <job-id> --no-best-effort-deliver
uagent cron edit <job-id> --no-deliver
```
