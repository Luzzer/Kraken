---
summary: "CLI reference for `uagent logs` (tail gateway logs via RPC)"
read_when:
...
`uagent logs` also accepts the standard Gateway client flags:
...
uagent logs
uagent logs --follow
uagent logs --follow --interval 2000
uagent logs --limit 500 --max-bytes 500000
uagent logs --json
uagent logs --plain
uagent logs --no-color
uagent logs --limit 500
uagent logs --local-time
uagent logs --follow --local-time
uagent logs --url ws://127.0.0.1:18789 --token "$UAGENT_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the local loopback Gateway asks for pairing, `uagent logs` falls back to the configured local log file automatically. Explicit `--url` targets do not use this fallback.
