---
summary: "UAGENT CLI reference for `uagent` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.uagent-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.uagent-<name>`.
- `--container <name>`: target a named container for execution.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `uagent update` (source installs only).
...
UAGENT uses a lobster palette for CLI output.
...
```
uagent [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `uagent voicecall`).

## Security

- `uagent security audit` — audit config + local state for common security foot-guns.
- `uagent security audit --deep` — best-effort live Gateway probe.
- `uagent security audit --fix` — tighten safe defaults and state/config permissions.
...
- `uagent plugins list` — discover plugins (use `--json` for machine output).
- `uagent plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `uagent plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`; use `--force` to overwrite an existing install target).
- `uagent plugins marketplace list <marketplace>` — list marketplace entries before install.
- `uagent plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `uagent plugins doctor` — report plugin load errors.
...
- `uagent memory status` — show index stats; use `--deep` for vector + embedding readiness checks or `--fix` to repair stale recall/promotion artifacts.
- `uagent memory index` — reindex memory files.
- `uagent memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
- `uagent memory promote` — rank short-term recalls and optionally append top entries into `MEMORY.md`.
...
- `--install` writes an `UAGENT Completion` block into your shell profile and points it at the cached script under the UAGENT state directory.
...
- `--workspace <dir>`: agent workspace path (default `~/.uagent/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `uagent config` with no
...
- `config set --dry-run`: validate assignments without writing `uagent.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `uagent.json`, including propagated field `title` / `description` docs metadata across nested object, wildcard, array-item, and composition branches, plus best-effort live plugin/channel schema metadata.
...
- `uagent --update` rewrites to `uagent update`.

### `backup`

Create and verify local backup archives for UAGENT state.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs live per-account probe/audit checks when the gateway is reachable; if not, it falls back to config-only channel summaries. Use `uagent health` or `uagent status --deep` for broader gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `uagent doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, UAGENT promotes account-scoped values into the channel account map before writing the new account. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```bash
uagent channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
uagent channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
uagent channels remove --channel discord --account work --delete
...
Look up self, peer, and group IDs for channels that expose a directory surface. See [`uagent directory`](/cli/directory).
...
Tip: use `uagent skills search`, `uagent skills install`, and `uagent skills update` for ClawHub-backed skills.
...
Generate a mobile pairing QR and setup code from the current Gateway config. See [`uagent qr`](/cli/qr).
...
- After scanning, approve the request with `uagent devices list` / `uagent devices approve <requestId>`.

### `clawbot`

Legacy alias namespace. Currently supports `uagent clawbot qr`, which maps to [`uagent qr`](/cli/qr).
...
- `hooks enable <name>`
- `hooks disable <name>`
- `hooks install <path-or-spec>` (deprecated alias for `uagent plugins install`)
- `hooks update [id]` (deprecated alias for `uagent plugins update`)
...
- Plugin-managed hooks cannot be enabled or disabled through `uagent hooks`; enable or disable the owning plugin instead.
...
- `setup` configures the Gmail watch plus the UAGENT-facing push path.
...
- Without `--apply`, this is a planning helper that prints the recommended UAGENT + Tailscale DNS config.
...
- `uagent message send --target +15555550123 --message "Hi"`
- `uagent message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Running `uagent agents` with no subcommand is equivalent to `uagent agents list`.
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, UAGENT may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Manage saved MCP server definitions and expose UAGENT channels over MCP stdio.

#### `mcp serve`

Expose routed UAGENT channel conversations over MCP stdio.
...
- `--gateway`
- `--json`
- node RPC options from `uagent nodes`
...
UAGENT can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `uagent status --usage` (prints full provider breakdown)
...
- MiniMax note: raw `usage_percent` / `usagePercent` means remaining quota, so UAGENT inverts it before display; count-based fields still win when present. `model_remains` responses prefer the chat-model entry, derive the window label from timestamps when needed, and include the model name in the plan label.
- Usage auth comes from provider-specific hooks when available; otherwise UAGENT falls back to matching OAuth/API-key credentials from auth profiles, env, or config. If none resolve, usage is hidden.
...
Legacy docs shortcut. Flow commands live under `uagent tasks flow`:
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named UAGENT services are treated as first-class and aren't flagged as "extra".
...
```bash
uagent logs --follow
uagent logs --limit 200
uagent logs --plain
uagent logs --json
uagent logs --no-color
```
...
Anthropic note: Anthropic staff told us UAGENT-style Claude CLI usage is
allowed again, so UAGENT treats Claude CLI reuse and `claude -p` usage as
...
Anthropic setup-token remains available as a supported token-auth path, but UAGENT now prefers Claude CLI reuse and `claude -p` when available.

### `models` (root)

`uagent models` is an alias for `models status`.
...
- Anthropic `setup-token` / `paste-token` remain available as a supported UAGENT token path, but UAGENT now prefers Claude CLI reuse and `claude -p` when available.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `UAGENT_GATEWAY_TOKEN` / `UAGENT_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Node-host auth resolution only honors `UAGENT_GATEWAY_*` env vars.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`uagent browser`](/cli/browser) and the [Browser tool](/tools/browser).
...
Plugin-provided voice-call utilities. Only appears when the voice-call plugin is installed and enabled. See [`uagent voicecall`](/cli/voicecall).
...
Search the live UAGENT docs index.
