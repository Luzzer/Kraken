---
summary: "CLI reference for `uagent health` (gateway health snapshot via RPC)"
read_when:
...
uagent health
uagent health --json
uagent health --timeout 2500
uagent health --verbose
uagent health --debug
```

Notes:

- Default `uagent health` asks the running gateway for its health snapshot. When the
