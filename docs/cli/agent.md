---
summary: "CLI reference for `uagent agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
uagent agent --to +15555550123 --message "status update" --deliver
uagent agent --agent ops --message "Summarize logs"
...
uagent agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: UAGENT persists markers from the active source config snapshot, not from resolved runtime secret values.
