---
summary: "CLI reference for `uagent config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `uagent.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `uagent configure`).

Root options:

- `--section <section>`: repeatable guided-setup section filter when you run `uagent config` without a subcommand
...
```bash
uagent config file
uagent config --section model
uagent config --section gateway --section daemon
uagent config schema
uagent config get browser.executablePath
...
uagent config set secrets.providers.vaultfile --provider-source file --provider-path /etc/uagent/secrets.json --provider-mode json
...
uagent config validate
uagent config validate --json
```

### `config schema`

Print the generated JSON schema for `uagent.json` to stdout as JSON.
...
```bash
uagent config schema > uagent.schema.json
```
...
uagent config get agents.list[0].id
```
...
uagent config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
uagent config set agents.defaults.heartbeat.every "0m"
uagent config set gateway.port 19001 --strict-json
uagent config set channels.whatsapp.groups '["*"]' --strict-json
```
...
`uagent config set` supports four assignment styles:

1. Value mode: `uagent config set <path> <value>`
2. SecretRef builder mode:

```bash
uagent config set channels.discord.token \
...
```bash
uagent config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/uagent-vault \
  --provider-arg read \
...
```bash
uagent config set --batch-file ./config-set.batch.json --dry-run
```
...
uagent config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/uagent/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `uagent.json`.
...
- `config file`: Print the active config file path (resolved from `UAGENT_CONFIG_PATH` or default location).
