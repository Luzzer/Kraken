---
summary: "CLI reference for `uagent models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
uagent models list
uagent models set <model-or-alias>
uagent models scan
```

`uagent models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise UAGENT falls back to matching OAuth/API-key
...
the command uses `UAGENT_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, UAGENT resolves the input as an alias first, then
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `UAGENT_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but UAGENT could not resolve a probeable
...
```bash
uagent models auth add
uagent models auth login --provider <id>
uagent models auth setup-token --provider <id>
uagent models auth paste-token
```
...
`uagent plugins list` to see which providers are installed.

Examples:

```bash
uagent models auth login --provider openai-codex --set-default
```
