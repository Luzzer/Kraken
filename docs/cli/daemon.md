---
summary: "CLI reference for `uagent daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `uagent daemon ...` in scripts
...
`uagent daemon ...` maps to the same service control surface as `uagent gateway ...` service commands.

## Usage

```bash
uagent daemon status
uagent daemon install
uagent daemon start
uagent daemon stop
uagent daemon restart
uagent daemon uninstall
```
...
Use [`uagent gateway`](/cli/gateway) for current docs and examples.
