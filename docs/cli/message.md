---
summary: "CLI reference for `uagent message` (send + channel actions)"
read_when:
...
```
uagent message <subcommand> [flags]
```
...
- On cache miss, UAGENT will attempt a live directory lookup when the provider supports it.
...
- `uagent message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, UAGENT does not force a `default` account SecretRef scope
...
```
uagent message send --channel discord \
...
```bash
uagent message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
uagent message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
uagent message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
uagent message poll --channel msteams \
...
```
uagent message react --channel slack \
...
```
uagent message react --channel signal \
...
```
uagent message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
uagent message send --channel telegram --target @mychat \
