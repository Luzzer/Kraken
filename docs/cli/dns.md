---
summary: "CLI reference for `uagent dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: uagent.internal)
title: "dns"
---
...
```bash
uagent dns setup
uagent dns setup --domain uagent.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `uagent.internal`)
...
- zone file path
- current tailnet IPs
- recommended `uagent.json` discovery config
...
- If `--domain` is omitted, UAGENT uses `discovery.wideArea.domain` from config.
