---
summary: "CLI reference for `uagent onboard` (interactive onboarding)"
read_when:
...
```bash
uagent onboard
uagent onboard --flow quickstart
uagent onboard --flow manual
uagent onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`UAGENT_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `uagent gateway run`.
...
- Fastest first chat: `uagent dashboard` (Control UI, no channel setup).
