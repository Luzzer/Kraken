---
summary: "CLI reference for `uagent approvals` (exec approvals for gateway or node hosts)"
read_when:
...
```bash
uagent approvals get
uagent approvals get --node <id|name|ip>
uagent approvals get --gateway
```

`uagent approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
uagent approvals set --file ./exec-approvals.json
uagent approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
uagent approvals set --node <id|name|ip> --file ./exec-approvals.json
uagent approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested UAGENT policy aligned, also set:
...
```bash
uagent approvals allowlist add "~/Projects/**/bin/rg"
uagent approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
uagent approvals allowlist add --agent "*" "/usr/bin/uname"

uagent approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `uagent nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.uagent/exec-approvals.json`.
