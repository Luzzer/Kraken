---
summary: "CLI reference for `uagent qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
uagent qr
uagent qr --setup-code-only
uagent qr --json
uagent qr --remote
uagent qr --url wss://gateway.example/ws
```
...
- With `--remote`, UAGENT requires either `gateway.remote.url` or
