---
summary: "Expose UAGENT channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to UAGENT-backed channels
  - Running `uagent mcp serve`
  - Managing UAGENT-saved MCP server definitions
title: "mcp"
---

# mcp

`uagent mcp` has two jobs:

- run UAGENT as an MCP server with `uagent mcp serve`
- manage UAGENT-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is UAGENT acting as an MCP server
- `list` / `show` / `set` / `unset` is UAGENT acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`uagent acp`](/cli/acp) when UAGENT should host a coding harness
session itself and route that runtime through ACP.

## UAGENT as an MCP server

This is the `uagent mcp serve` path.

## When to use `serve`

Use `uagent mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  UAGENT-backed channel conversations
- you already have a local or remote UAGENT Gateway with routed sessions
- you want one MCP server that works across UAGENT's channel backends instead
  of running separate per-channel bridges

Use [`uagent acp`](/cli/acp) instead when UAGENT should host the coding
runtime itself and keep the agent session inside UAGENT.

## How it works

`uagent mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote UAGENT Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `uagent mcp serve`
...
conversations. A conversation appears when UAGENT already has session state
...
# Remote Gateway
uagent mcp serve --url wss://gateway-host:18789 --token-file ~/.uagent/gateway.token

# Remote Gateway with password auth
uagent mcp serve --url wss://gateway-host:18789 --password-file ~/.uagent/gateway.password

# Enable verbose bridge logs
uagent mcp serve --verbose

# Disable Claude-specific push notifications
uagent mcp serve --claude-channel-mode off
```
...
UAGENT equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying UAGENT channel configuration
...
UAGENT ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `uagent mcp serve`
...
## UAGENT as an MCP client registry

This is the `uagent mcp list`, `show`, `set`, and `unset` path.

These commands do not expose UAGENT over MCP. They manage UAGENT-owned MCP
server definitions under `mcp.servers` in UAGENT config.

Those saved definitions are for runtimes that UAGENT launches or configures
later, such as embedded Pi and other runtime adapters. UAGENT stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write UAGENT config
...
UAGENT also stores a lightweight MCP server registry in config for surfaces
that want UAGENT-managed MCP definitions.

Commands:

- `uagent mcp list`
- `uagent mcp show [name]`
- `uagent mcp set <name> <json>`
- `uagent mcp unset <name>`
...
```bash
uagent mcp list
uagent mcp show context7 --json
uagent mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
uagent mcp set docs '{"url":"https://mcp.example.com"}'
uagent mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, UAGENT uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |
