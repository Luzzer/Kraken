---
summary: "CLI reference for `uagent memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
uagent memory status
uagent memory status --deep
uagent memory status --fix
uagent memory index --force
uagent memory search "meeting notes"
uagent memory search --query "deployment" --max-results 20
uagent memory promote --limit 10 --min-score 0.75
uagent memory promote --apply
uagent memory promote --json --min-recall-count 0 --min-unique-queries 0
uagent memory promote-explain "router vlan"
uagent memory promote-explain "router vlan" --json
uagent memory rem-harness
uagent memory rem-harness --json
uagent memory status --json
uagent memory status --deep --index
uagent memory status --deep --index --verbose
uagent memory status --agent main
uagent memory index --agent main --verbose
```
...
```bash
uagent memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `uagent cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
uagent memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
uagent memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
