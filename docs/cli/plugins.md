---
summary: "CLI reference for `uagent plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
```bash
uagent plugins list
uagent plugins list --enabled
uagent plugins list --verbose
uagent plugins list --json
uagent plugins install <path-or-spec>
uagent plugins inspect <id>
uagent plugins inspect <id> --json
uagent plugins inspect --all
uagent plugins info <id>
uagent plugins enable <id>
uagent plugins disable <id>
uagent plugins uninstall <id>
uagent plugins doctor
uagent plugins update <id>
uagent plugins update --all
uagent plugins marketplace list <marketplace>
uagent plugins marketplace list <marketplace> --json
```

Bundled plugins ship with UAGENT. Some are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin); others require `plugins enable`.

Native UAGENT plugins must ship `uagent.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: uagent` or `Format: bundle`. Verbose list/info
...
uagent plugins install clawhub:<package>              # ClawHub only
uagent plugins install <package> --force              # overwrite existing install
uagent plugins install <package> --pin                # pin version
uagent plugins install <package> --dangerously-force-unsafe-install
uagent plugins install <path>                         # local path
uagent plugins install <plugin>@<marketplace>         # marketplace
uagent plugins install <plugin> --marketplace <name>  # marketplace (explicit)
...
run `uagent doctor --fix` first. The only documented exception is a narrow
...
override, while `uagent skills install` remains a separate ClawHub skill
download/install flow.

`plugins install` is also the install surface for hook packs that expose
`uagent.hooks` in `package.json`. Use `uagent hooks` for filtered hook
...
If a bare install spec matches a bundled plugin id (for example `diffs`), UAGENT
...
```bash
uagent plugins install clawhub:uagent-codex-app-server
uagent plugins install clawhub:uagent-codex-app-server@1.2.3
```

UAGENT now also prefers ClawHub for bare npm-safe plugin specs. It only falls
...
UAGENT downloads the package archive from ClawHub, checks the advertised
...
```bash
uagent plugins marketplace list <marketplace-name>
uagent plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
uagent plugins install <plugin-name> --marketplace <marketplace-name>
uagent plugins install <plugin-name> --marketplace <owner/repo>
uagent plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
uagent plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. UAGENT accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, UAGENT auto-detects:

- native UAGENT plugins (`uagent.plugin.json`)
...
```bash
uagent plugins install -l ./my-plugin
```
...
```bash
uagent plugins uninstall <id>
uagent plugins uninstall <id> --dry-run
...
```bash
uagent plugins update <id-or-npm-spec>
uagent plugins update --all
uagent plugins update <id-or-npm-spec> --dry-run
uagent plugins update @uagent/voice-call@beta
uagent plugins update uagent-codex-app-server --dangerously-force-unsafe-install
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, UAGENT reuses the recorded install spec for that
...
or exact version. UAGENT resolves that package name back to the tracked plugin
