---
summary: "CLI reference for `uagent secrets` (reload, audit, configure, apply)"
read_when:
...
Use `uagent secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
uagent secrets reload
uagent secrets reload --json
uagent secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan UAGENT state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `uagent.json` refs)
...
```bash
uagent secrets audit
uagent secrets audit --check
uagent secrets audit --json
uagent secrets audit --allow-exec
```
...
```bash
uagent secrets configure
uagent secrets configure --plan-out /tmp/uagent-secrets-plan.json
uagent secrets configure --apply --yes
uagent secrets configure --providers-only
uagent secrets configure --skip-provider-setup
uagent secrets configure --agent ops
uagent secrets configure --json
```
...
- `configure` targets secret-bearing fields in `uagent.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, UAGENT fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
uagent secrets apply --from /tmp/uagent-secrets-plan.json --json
```
...
- `uagent.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.uagent/.env` known secret keys whose values were migrated
