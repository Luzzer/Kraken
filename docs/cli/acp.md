...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an UAGENT Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`uagent acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to UAGENT channel
conversations instead of hosting an ACP harness session, use
[`uagent mcp serve`](/cli/mcp) instead.
...
- UAGENT acts as an ACP server
- an IDE or ACP client connects to UAGENT
- UAGENT forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where UAGENT runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to UAGENT: use `uagent acp`
- UAGENT should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the UAGENT gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
uagent acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
uagent acp --url wss://gateway-host:18789 --token-file ~/.uagent/gateway.token

# Attach to an existing session key
uagent acp --session agent:main:main

# Attach by label (must already exist)
uagent acp --session-label "support inbox"

# Reset the session key before the first prompt
uagent acp --session agent:main:main --reset-session
```
...
uagent acp client --server-args --url wss://gateway-host:18789 --token-file ~/.uagent/gateway.token

# Override the server command (default: uagent)
uagent acp client --server "node" --server-args uagent.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run UAGENT through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an UAGENT Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `uagent acp` over stdio.

Example config (persisted):

```bash
uagent config set gateway.remote.url wss://gateway-host:18789
uagent config set gateway.remote.token <token>
```
...
```bash
uagent acp --session agent:main:main
uagent acp --session agent:design:main
uagent acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see UAGENT plugin tools, enable the
...
UAGENT bot over ACP, use `acpx` with its built-in `uagent` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx uagent` at `uagent acp`.
3. Target the UAGENT session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default UAGENT ACP session
acpx uagent exec "Summarize the active UAGENT session state."

# Persistent named session for follow-up turns
acpx uagent sessions ensure --name codex-bridge
acpx uagent -s codex-bridge --cwd /path/to/repo \
  "Ask my UAGENT work agent for recent context relevant to this repo."
```

If you want `acpx uagent` to target a specific Gateway and session key every
time, override the `uagent` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "uagent": {
      "command": "env UAGENT_HIDE_BANNER=1 UAGENT_SUPPRESS_NOTES=1 uagent acp --url ws://127.0.0.1:18789 --token-file ~/.uagent/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local UAGENT checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env UAGENT_HIDE_BANNER=1 UAGENT_SUPPRESS_NOTES=1 node uagent.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an UAGENT agent without scraping a terminal.
...
In Zed, open the Agent panel and select “UAGENT ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`UAGENT_GATEWAY_TOKEN`, `UAGENT_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`UAGENT_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `UAGENT_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `uagent acp client` sets `UAGENT_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `uagent`).
