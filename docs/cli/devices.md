---
summary: "CLI reference for `uagent devices` (device pairing + token rotation/revocation)"
read_when:
...
```
uagent devices remove <deviceId>
uagent devices remove <deviceId> --json
```

### `uagent devices clear --yes [--pending]`

Clear paired devices in bulk.

```
uagent devices clear --yes
uagent devices clear --yes --pending
uagent devices clear --yes --pending --json
```

### `uagent devices approve [requestId] [--latest]`

Approve a pending device pairing request. If `requestId` is omitted, UAGENT
automatically approves the most recent pending request.

Note: if a device retries pairing with changed auth details (role/scopes/public
key), UAGENT supersedes the previous pending entry and issues a new
`requestId`. Run `uagent devices list` right before approval to use the
current ID.

```
uagent devices approve
...
### `uagent devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
uagent devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```

Returns the new token payload as JSON.

### `uagent devices revoke --device <id> --role <role>`
...
```
uagent devices revoke --device <deviceId> --role node
```
...
```bash
uagent devices rotate --device <deviceId> --role operator
```
