---
summary: "CLI reference for `uagent configure` (interactive configuration prompts)"
read_when:
...
Tip: `uagent config` without a subcommand opens the same wizard. Use
`uagent config get|set|unset` for non-interactive edits.

For web search, `uagent configure --section web` lets you choose a provider
...
uagent configure --section model --section channels
uagent configure --section gateway --section daemon
```
