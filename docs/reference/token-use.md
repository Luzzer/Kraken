---
summary: "How UAGENT builds prompt context and reports token usage + costs"
read_when:
...
UAGENT tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

UAGENT assembles its own system prompt on every run. It includes:
...
For images, UAGENT downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from UAGENT session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `uagent status --usage` and `uagent channels list` show
...
available; otherwise UAGENT falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, UAGENT shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. UAGENT can
...
Anthropic's 1M context window is currently beta-gated. UAGENT can inject the
...
UAGENT skips the `context-1m-*` beta header because Anthropic currently
