...
This page lists every configuration knob for UAGENT memory search. For
...
`uagent.json` unless noted otherwise.
...
When `provider` is not set, UAGENT selects the first available:
...
If UAGENT runs on EC2 with a Bedrock-enabled instance role, just set the
provider and model:
...
When sqlite-vec is unavailable, UAGENT falls back to in-process cosine
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.uagent/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
...
UAGENT prefers the current QMD collection and MCP query shapes, but keeps
older QMD releases working by falling back to legacy `--mask` collection flags
and older MCP tool names when needed.

QMD model overrides stay on the QMD side, not UAGENT config. If you need to
