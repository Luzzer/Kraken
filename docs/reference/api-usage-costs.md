...
UAGENT features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, UAGENT reads usage from
...
Anthropic still does not expose a per-message dollar estimate that UAGENT can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `uagent status --usage` and `uagent channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so UAGENT inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, UAGENT prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise UAGENT falls back to matching OAuth/API-key
...
UAGENT can pick up credentials from:
...
UAGENT's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's UAGENT Claude-login path with **Extra Usage** enabled.
...
`uagent models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
