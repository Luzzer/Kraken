...
UAGENT integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- UAGENT owns lifecycle when `channels.signal.autoStart=true`.
...
- UAGENT spawns `imsg rpc` as a child process (legacy iMessage integration).
