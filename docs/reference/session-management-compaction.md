...
This document explains how UAGENT manages sessions end-to-end:
...
UAGENT is designed around a single **Gateway process** that owns session state.
...
UAGENT persists sessions in two layers:
...
- Transcripts: `~/.uagent/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

UAGENT resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, UAGENT reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
uagent sessions cleanup --dry-run
uagent sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.uagent/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
UAGENT intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When UAGENT splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, UAGENT
...
- If a trailing tool-result block would otherwise push the chunk over target,
  UAGENT preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (UAGENT consumes the events, but Pi decides when to compact).
...
UAGENT also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, UAGENT bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, UAGENT leaves it alone.
...
UAGENT supports “silent” turns for background tasks where the user should not see intermediate output.
...
- UAGENT strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, UAGENT also suppresses **draft/typing streaming** when a
...
UAGENT uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but UAGENT’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `uagent status`.
