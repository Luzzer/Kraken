...
read_when:
  - You want to place an outbound voice call from UAGENT
...
Voice calls for UAGENT via a plugin. Supports outbound notifications and
...
- Use `uagent voicecall ...` or the `voice_call` tool
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `uagent doctor --fix` to rewrite them.
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `uagent doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `uagent doctor --fix`:
...
```bash
uagent voicecall call --to "+15555550123" --message "Hello from UAGENT"
uagent voicecall start --to "+15555550123"   # alias for call
uagent voicecall continue --call-id <id> --message "Any questions?"
uagent voicecall speak --call-id <id> --message "One moment"
uagent voicecall end --call-id <id>
uagent voicecall status --call-id <id>
uagent voicecall tail
uagent voicecall latency                     # summarize turn latency from logs
