...
read_when:
  - You are building an UAGENT plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (uagent.plugin.json)

This page is for the **native UAGENT plugin manifest** only.
...
UAGENT auto-detects those bundle layouts too, but they are not validated
against the `uagent.plugin.json` schema described here.

For compatible bundles, UAGENT currently reads bundle metadata plus declared
...
UAGENT runtime expectations.

Every native UAGENT plugin **must** ship a `uagent.plugin.json` file in the
**plugin root**. UAGENT uses this manifest to validate configuration
...
`uagent.plugin.json` is the metadata UAGENT reads before it loads your
plugin code.
...
| `providerAuthEnvVars`               | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that UAGENT can inspect without loading plugin code.                                                                                                                      |
| `channelEnvVars`                    | No       | `Record<string, string[]>`       | Cheap channel env metadata that UAGENT can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                     |
...
UAGENT reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, UAGENT falls back to `choiceId`.                                        |
...
Use `contracts` only for static capability ownership metadata that UAGENT can
...
Use `modelSupport` when UAGENT should infer your provider plugin from
...
UAGENT applies this precedence:
...
Legacy top-level capability keys are deprecated. Use `uagent doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `uagent.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `uagent` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if UAGENT must know it before loading plugin code, put it in `uagent.plugin.json`
...
`uagent` block instead of `uagent.plugin.json`.

Important examples:

| Field                                                             | What it means                                                                                                                                |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `uagent.extensions`                                             | Declares native plugin entrypoints.                                                                                                          |
| `uagent.setupEntry`                                             | Lightweight setup-only entrypoint used during onboarding and deferred channel startup.                                                       |
| `uagent.channel`                                                | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                         |
| `uagent.channel.configuredState`                                | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime. |
| `uagent.channel.persistedAuthState`                             | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.       |
| `uagent.install.npmSpec` / `uagent.install.localPath`         | Install/update hints for bundled and externally published plugins.                                                                           |
| `uagent.install.defaultChoice`                                  | Preferred install path when multiple install sources are available.                                                                          |
| `uagent.install.minHostVersion`                                 | Minimum supported UAGENT host version, using a semver floor like `>=2026.3.22`.                                                            |
| `uagent.install.allowInvalidConfigRecovery`                     | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                               |
| `uagent.startup.deferConfiguredChannelFullLoadUntilAfterListen` | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                         |

`uagent.install.minHostVersion` is enforced during install and manifest
registry loading. Invalid values are rejected; newer-but-valid values skip the
plugin on older hosts.

`uagent.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `uagent doctor --fix`.

`uagent.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`uagent.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
- The manifest is **required for native UAGENT plugins**, including local filesystem loads.
