...
**Import:** `uagent/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `UAGENTPluginConfigSchema \| () => UAGENTPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: UAGENTPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `uagent.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- UAGENT resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `uagent/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: UAGENTPluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: UAGENTPluginApi) => void`                               | No       | —                   |
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and UAGENT
...
UAGENT loads this instead of the full entry when a channel is disabled,
...
- `uagent/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `uagent/plugin-sdk/channel-setup` for optional-install setup surfaces
- `uagent/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
- use `descriptors` when the registrar owns one or more root commands and you
  want UAGENT to lazy-load the real CLI module on first invocation
...
UAGENT classifies loaded plugins by their registration behavior:
...
Use `uagent plugins inspect <id>` to see a plugin's shape.
