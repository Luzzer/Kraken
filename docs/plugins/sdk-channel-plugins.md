...
summary: "Step-by-step guide to building a messaging channel plugin for UAGENT"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect UAGENT to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects UAGENT to a
...
<Info>
  If you have not built any UAGENT plugin before, read
...
Channel plugins do not need their own send/edit/react tools. UAGENT keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `uagent/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization and transport hooks. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, `createApproverRestrictedNativeApprovalCapability`, and `createChannelNativeApprovalRuntime` from `uagent/plugin-sdk/approval-runtime` so core owns request filtering, routing, dedupe, expiry, and gateway subscription.
...
- `uagent/plugin-sdk/approval-auth-runtime`
- `uagent/plugin-sdk/approval-client-runtime`
- `uagent/plugin-sdk/approval-delivery-runtime`
- `uagent/plugin-sdk/approval-native-runtime`
- `uagent/plugin-sdk/approval-reply-runtime`

Likewise, prefer `uagent/plugin-sdk/setup-runtime`,
`uagent/plugin-sdk/setup-adapter-runtime`,
`uagent/plugin-sdk/reply-runtime`,
`uagent/plugin-sdk/reply-dispatch-runtime`,
`uagent/plugin-sdk/reply-reference`, and
`uagent/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `uagent/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `uagent/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `uagent/plugin-sdk/channel-setup` covers the optional-install setup
...
- use the broader `uagent/plugin-sdk/setup` seam only when you also need the
...
- `uagent/plugin-sdk/account-core`,
  `uagent/plugin-sdk/account-id`,
  `uagent/plugin-sdk/account-resolution`, and
  `uagent/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `uagent/plugin-sdk/inbound-envelope` and
  `uagent/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `uagent/plugin-sdk/messaging-targets` for target parsing/matching
- `uagent/plugin-sdk/outbound-media` and
  `uagent/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates
- `uagent/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `uagent/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `uagent/plugin-sdk/telegram-command-config` for Telegram custom-command
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#uagentchannel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/uagent-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "uagent": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect UAGENT to Acme Chat."
        }
      }
    }
    ```

    ```json uagent.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { UAGENTConfig } from "uagent/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "uagent/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so UAGENT
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "uagent/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    UAGENT loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    UAGENT. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to UAGENT.
...
├── package.json              # uagent.channel metadata
├── uagent.plugin.json      # Manifest with config schema
