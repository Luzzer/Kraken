...
read_when:
  - Building or debugging native UAGENT plugins
...
This page covers the internal architecture of the UAGENT plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside UAGENT. Every
native UAGENT plugin registers against one or more capability types:
...
UAGENT classifies every loaded plugin into a shape based on its actual
...
Use `uagent plugins inspect <id>` to see a plugin's shape and capability
...
When you run `uagent doctor` or `uagent plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `uagent status --all` and `uagent plugins doctor`.

## Architecture overview

UAGENT's plugin system has four layers:

1. **Manifest + discovery**
   UAGENT finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `uagent.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native UAGENT plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of UAGENT reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting UAGENT
...
That split lets UAGENT validate config, explain missing/disabled plugins, and
...
normal chat actions. UAGENT keeps one shared `message` tool in core, and
...
UAGENT treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's UAGENT-facing
  surfaces
...
So if UAGENT adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If UAGENT has shared
...
```ts
import type { UAGENTPluginDefinition } from "uagent/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "uagent/plugin-sdk/media-understanding";

const plugin: UAGENTPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
UAGENT already treats image/audio/video understanding as one shared
...
`UAGENTPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   UAGENT can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that UAGENT knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `UAGENTPluginApi` or
  `api.runtime`
...
Native UAGENT plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because UAGENT currently treats them
...
name: `@uagent/<id>` by default, or an approved typed suffix such as
...
UAGENT exports capabilities, not implementation convenience.
...
At startup, UAGENT does roughly this:
...
The manifest is the control-plane source of truth. UAGENT uses it to:
...
UAGENT keeps short in-process caches for:
...
- Set `UAGENT_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `UAGENT_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `UAGENT_PLUGIN_DISCOVERY_CACHE_MS` and
  `UAGENT_PLUGIN_MANIFEST_CACHE_MS`.
...
UAGENT still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, UAGENT calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | UAGENT tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                       |
...
that still runs on UAGENT's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before UAGENT's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as UAGENT adds those
...
- UAGENT only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "UAGENT plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-uagent-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-uagent-scopes` only when the header is explicitly present
  - if `x-uagent-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-uagent-scopes` header contract.

## Plugin SDK import paths

Use SDK subpaths instead of the monolithic `uagent/plugin-sdk` import when
authoring plugins:

- `uagent/plugin-sdk/plugin-entry` for plugin registration primitives.
- `uagent/plugin-sdk/core` for the generic shared plugin-facing contract.
- `uagent/plugin-sdk/config-schema` for the root `uagent.json` Zod schema
  export (`UAGENTSchema`).
- Stable channel primitives such as `uagent/plugin-sdk/channel-setup`,
  `uagent/plugin-sdk/setup-runtime`,
  `uagent/plugin-sdk/setup-adapter-runtime`,
  `uagent/plugin-sdk/setup-tools`,
  `uagent/plugin-sdk/channel-pairing`,
  `uagent/plugin-sdk/channel-contract`,
  `uagent/plugin-sdk/channel-feedback`,
  `uagent/plugin-sdk/channel-inbound`,
  `uagent/plugin-sdk/channel-lifecycle`,
  `uagent/plugin-sdk/channel-reply-pipeline`,
  `uagent/plugin-sdk/command-auth`,
  `uagent/plugin-sdk/secret-input`, and
  `uagent/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  ...
  `CONFIG_DIR`).
- Domain subpaths such as `uagent/plugin-sdk/channel-config-helpers`,
  `uagent/plugin-sdk/allow-from`,
  `uagent/plugin-sdk/channel-config-schema`,
  `uagent/plugin-sdk/telegram-command-config`,
  `uagent/plugin-sdk/channel-policy`,
  `uagent/plugin-sdk/approval-runtime`,
  `uagent/plugin-sdk/config-runtime`,
  `uagent/plugin-sdk/infra-runtime`,
  `uagent/plugin-sdk/agent-runtime`,
  `uagent/plugin-sdk/lazy-runtime`,
  `uagent/plugin-sdk/reply-history`,
  `uagent/plugin-sdk/routing`,
  `uagent/plugin-sdk/status-helpers`,
  `uagent/plugin-sdk/text-runtime`,
  `uagent/plugin-sdk/runtime-store`, and
  `uagent/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `uagent/plugin-sdk/channel-runtime` is deprecated and remains only as a
...
- Bundled extension internals remain private. External plugins should use only
  `uagent/plugin-sdk/*` subpaths. UAGENT core/test code may use the repo
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  UAGENT is not yet serving a runtime snapshot.
...
- Avoid the root `uagent/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `uagent/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `uagent/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `uagent/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `uagent/plugin-sdk/channel-targets`; channel-specific
...
`uagent/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape UAGENT writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to UAGENT's
...
- if both `catalog` and `discovery` are registered, UAGENT uses `catalog`
...
- Read-only command paths such as `uagent status`, `uagent status --all`,
  `uagent channels status`, `uagent channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `uagent.extensions`:
...
Security guardrail: every `uagent.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `uagent plugins install` installs plugin dependencies with
...
Optional: `uagent.setupEntry` can point at a lightweight setup-only module.
When UAGENT needs setup surfaces for a disabled channel plugin, or
...
Optional: `uagent.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let UAGENT load the
...
Channel plugins can advertise setup/discovery metadata via `uagent.channel` and
install hints via `uagent.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@uagent/nextcloud-talk",
  "uagent": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Useful `uagent.channel` fields beyond the minimal example:
...
UAGENT can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.uagent/mpm/plugins.json`
- `~/.uagent/mpm/catalog.json`
- `~/.uagent/plugins/catalog.json`

Or point `UAGENT_PLUGIN_CATALOG_PATHS` (or `UAGENT_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "uagent": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { delegateCompactionToRuntime } from "uagent/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `UAGENTPluginApi` and/or `api.runtime` with the smallest useful
...
This is how UAGENT stays opinionated without becoming hardcoded to one
