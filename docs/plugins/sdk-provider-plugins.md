...
summary: "Step-by-step guide to building a model provider plugin for UAGENT"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to UAGENT
...
(LLM) to UAGENT. By the end you will have a provider with a model catalog,
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so UAGENT can detect
    credentials without loading your plugin runtime. `modelSupport` is optional
    and lets UAGENT auto-load your provider plugin from shorthand model ids
    like `acme-large` before runtime hooks exist. If you publish the
    provider on ClawHub, those `uagent.compat` and `uagent.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "uagent/plugin-sdk/provider-auth";
    ...
    `uagent onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "uagent/plugin-sdk/provider-entry";
    ...
    `uagent/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `uagent/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    ```typescript
    import { buildProviderReplayFamilyHooks } from "uagent/plugin-sdk/provider-model-shared";
    import { buildProviderStreamFamilyHooks } from "uagent/plugin-sdk/provider-stream";
    import { buildProviderToolCompatFamilyHooks } from "uagent/plugin-sdk/provider-tools";
    ...
    `uagent/plugin-sdk/provider-model-shared` also exports the replay-family
    ...
    `uagent/plugin-sdk/provider-stream` exposes both the family builder and
    ...
    example: `@uagent/anthropic-provider` exports
    ...
    `uagent/plugin-sdk/provider-tools` currently exposes one shared
    ...
    - `@uagent/openai-provider`: `api.ts` exports provider builders,
      default-model helpers, and realtime provider builders
    - `@uagent/openrouter-provider`: `api.ts` exports the provider builder
    ...
    <Accordion title="All available provider hooks">
      UAGENT calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    UAGENT classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # uagent.providers metadata
├── uagent.plugin.json      # Manifest with providerAuthEnvVars
