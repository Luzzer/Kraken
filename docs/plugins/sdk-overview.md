...
read_when:
  ...
  - You want a reference for all registration methods on UAGENTPluginApi
...
prefer `uagent/plugin-sdk/channel-core`; keep `uagent/plugin-sdk/core` for
...
`uagent/plugin-sdk/slack`, `uagent/plugin-sdk/discord`,
`uagent/plugin-sdk/signal`, `uagent/plugin-sdk/whatsapp`, or
...
| `plugin-sdk/config-schema`  | `UAGENTSchema`                                                                                                                       |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `uagent.json` Zod schema export (`UAGENTSchema`) |
...
The `register(api)` callback receives an `UAGENTPluginApi` object with these
methods:
...
- User config still wins. UAGENT merges `agents.defaults.cliBackends.<id>` over the
...
| `api.config`             | `UAGENTConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `uagent/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when UAGENT is already running. If no runtime
...
- `@uagent/openrouter-provider`: `api.ts` exports the provider builder plus
  onboarding/config helpers

<Warning>
  Extension production code should also avoid `uagent/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `uagent/plugin-sdk/speech`, `.../provider-model-shared`, or another
