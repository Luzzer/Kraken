...
read_when:
  - You see the UAGENT_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the UAGENT_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external UAGENT plugin
---

# Plugin SDK Migration

UAGENT has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`uagent/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`uagent/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`uagent/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `uagent/plugin-sdk/slack`, `uagent/plugin-sdk/discord`,
`uagent/plugin-sdk/signal`, `uagent/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`uagent/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
<Steps>
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `uagent/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "uagent/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    import { resolveControlCommandGate } from "uagent/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "uagent/extension-api";
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `UAGENTSchema` |
...
```bash
UAGENT_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 uagent gateway run
UAGENT_SUPPRESS_EXTENSION_API_WARNING=1 uagent gateway run
```
