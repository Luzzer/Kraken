...
summary: "Create your first UAGENT plugin in minutes"
read_when:
  - You want to create a new UAGENT plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to UAGENT
---
...
You do not need to add your plugin to the UAGENT repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`uagent plugins install <package-name>`. UAGENT tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect UAGENT to a messaging platform (Discord, IRC, etc.)
  </Card>
...
`uagent/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json uagent.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to UAGENT",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Write the entry point">
    ...
    ```typescript
    // index.ts
    import { definePluginEntry } from "uagent/plugin-sdk/plugin-entry";
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    uagent plugins install clawhub:@myorg/uagent-my-plugin
    ```

    UAGENT also checks ClawHub before npm for bare package specs like
    `@myorg/uagent-my-plugin`.
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, UAGENT retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `uagent/plugin-sdk/error-runtime`
...
Always import from focused `uagent/plugin-sdk/<subpath>` paths:
...
package-root seam instead of promoting it into `uagent/plugin-sdk/*`.

Some generated `uagent/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `uagent` metadata</Check>
<Check>**uagent.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [uagent/uagent](https://github.com/uagent/uagent/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official UAGENT X account [@uagent](https://x.com/uagent) for release announcements.
