...
read_when:
  ...
  - You are defining plugin config schemas or package.json uagent metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`uagent.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `uagent` field that tells the plugin system what
...
```json uagent-clawhub-package.json
{
...
`uagent.channel` is cheap package metadata for channel discovery and setup
...
`uagent.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported UAGENT version in the form `>=x.y.z`.                        |
...
still fails closed and tells the operator to run `uagent doctor --fix`.
...
When enabled, UAGENT loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `uagent.plugin.json` in the package root.
UAGENT uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to UAGENT",
  "configSchema": {
    "type": "object",
...
UAGENT loads when it only needs setup surfaces (onboarding, config repair,
...
**When UAGENT uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `uagent/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that UAGENT validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "uagent/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `uagent onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "uagent/plugin-sdk/channel-setup";
...
helpers from `uagent/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`uagent/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `uagent/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "uagent/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/uagent-my-channel",
  docsPath: "/channels/my-channel",
});
...
```bash
uagent plugins install @myorg/uagent-my-plugin
```

UAGENT tries ClawHub first and falls back to npm automatically. You can also
force ClawHub explicitly:

```bash
uagent plugins install clawhub:@myorg/uagent-my-plugin   # ClawHub only
```
...
<Info>
  For npm-sourced installs, `uagent plugins install` runs
