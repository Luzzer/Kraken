...
Bind a Task Flow runtime to an existing UAGENT session key or trusted tool
...
trusted UAGENT session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "UAGENT plugin SDK", count: 5 },
});
```
