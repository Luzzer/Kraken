---
summary: "Symptom first troubleshooting hub for UAGENT"
read_when:
  - UAGENT is not working and you need the fastest path to a fix
...
- `uagent status` → shows configured channels and no obvious auth errors.
- `uagent status --all` → full report is present and shareable.
- `uagent gateway probe` → expected gateway target is reachable (`Reachable: yes`). `RPC: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `uagent gateway status` → `Runtime: running` and `RPC probe: ok`.
- `uagent doctor` → no blocking config/service errors.
- `uagent channels status --probe` → reachable gateway returns live per-account
...
- `uagent logs --follow` → steady activity, no repeating fatal errors.
...
## Plugin install fails with missing uagent extensions

If install fails with `package.json missing uagent.extensions`, the plugin package
is using an old shape that UAGENT no longer accepts.

Fix in the plugin package:

1. Add `uagent.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `uagent plugins install <package>` again.

Example:

```json
{
  "name": "@uagent/my-plugin",
  "version": "1.2.3",
  "uagent": {
...
```mermaid
flowchart TD
  A[UAGENT is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `uagent gateway status`
    - `RPC probe: ok`
...
- `requests-in-flight` → main lane busy; heartbeat wake was deferred. - `unknown accountId` → heartbeat delivery target account does not exist.
    ...
    <Accordion title="Exec suddenly asks for approval">
      ```bash
      uagent config get tools.exec.host
      uagent config get tools.exec.security
      uagent config get tools.exec.ask
    ...
    <Accordion title="Browser tool fails">
      ...
      - `uagent` starts, or `user` can see local Chrome tabs.
      ...
      - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `uagent browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
