...
This guide summarizes a sane workflow for working on the pi integration in UAGENT.
...
```bash
UAGENT_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm uagent agent --message "Hello" --thinking low`
...
State lives under the UAGENT state directory. Default is `~/.uagent`. If `UAGENT_STATE_DIR` is set, use that directory instead.

To reset everything:

- `uagent.json` for config
