...
read_when:
  - You want to connect UAGENT to QQ
...
QQ Bot connects to UAGENT via the official QQ Bot API (WebSocket gateway). The
...
Current UAGENT releases bundle QQ Bot, so normal packaged builds do not need
a separate `uagent plugins install` step.
...
```bash
uagent channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `uagent channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single UAGENT instance:
...
```bash
uagent channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
