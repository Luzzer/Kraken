...
Tlon is a decentralized messenger built on Urbit. UAGENT connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current UAGENT releases, so normal packaged
...
```bash
uagent plugins install @uagent/tlon
```

Local checkout (when running from a git repo):

```bash
uagent plugins install ./path/to/local/tlon-plugin
```
...
By default, UAGENT blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `uagent message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, UAGENT replies in-thread.
