...
read_when:
  - Setting up Zalo Personal for UAGENT
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside UAGENT.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current UAGENT releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `uagent plugins install @uagent/zalouser`
- Or from a source checkout: `uagent plugins install ./path/to/local/zalouser-plugin`
...
- `uagent pairing list zalouser`
- `uagent pairing approve zalouser <code>`
...
- On startup, UAGENT resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, UAGENT stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in UAGENT state. Example:
...
- UAGENT sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, UAGENT sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `uagent channels logout --channel zalouser && uagent channels login --channel zalouser`
...
- The channel now runs fully in UAGENT without external CLI binaries.
