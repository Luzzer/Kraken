...
UAGENT routes replies **back to the channel where a message came from**. The
...
UAGENT infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, UAGENT still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when UAGENT would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.uagent/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.uagent`):

- `~/.uagent/agents/<agentId>/sessions/sessions.json`
