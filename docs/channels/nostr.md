...
read_when:
  - You want UAGENT to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables UAGENT to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current UAGENT releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`uagent onboard`) and `uagent channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
uagent plugins install @uagent/nostr
```

Use a local checkout (dev workflows):

```bash
uagent plugins install --link <path-to-local-nostr-plugin>
```
...
uagent channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "uagent@example.com",
        lud16: "uagent@example.com",
      },
    },
  },
}
