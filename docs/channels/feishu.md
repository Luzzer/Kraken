...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects UAGENT to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current UAGENT releases, so no separate plugin install
is required.
...
```bash
uagent plugins install @uagent/feishu
```
...
If you just installed UAGENT, run onboarding:
...
2. Configuring app credentials in UAGENT
...
1. You already ran `uagent channels add` for Feishu
2. The gateway is running (`uagent gateway status`)
...
```bash
uagent pairing approve feishu <CODE>
```
...
2. Run `uagent logs --follow` and look for `chat_id`
...
2. Run `uagent logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `uagent gateway status`  | Show gateway status           |
| `uagent gateway install` | Install/start gateway service |
| `uagent gateway stop`    | Stop gateway service          |
| `uagent gateway restart` | Restart gateway service       |
| `uagent logs --follow`   | Tail gateway logs             |
...
4. Check logs: `uagent logs --follow`
...
5. Ensure the gateway is running: `uagent gateway status`
6. Check logs: `uagent logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.uagent/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.uagent/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
