...
read_when:
  - You want to connect UAGENT to LINE
...
LINE connects to UAGENT via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current UAGENT releases, so normal
...
```bash
uagent plugins install @uagent/line
```

Local checkout (when running from a git repo):

```bash
uagent plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so UAGENT applies strict pre-auth body limits and timeout before verification.
- UAGENT processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
uagent pairing list line
uagent pairing approve line <CODE>
```
