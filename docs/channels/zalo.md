...
Zalo ships as a bundled plugin in current UAGENT releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `uagent plugins install @uagent/zalo`
- Or from a source checkout: `uagent plugins install ./path/to/local/zalo-plugin`
...
This page reflects current UAGENT behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `uagent pairing list zalo`
  - `uagent pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in UAGENT.
...
- Example: `uagent message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `uagent channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `uagent logs --follow`
