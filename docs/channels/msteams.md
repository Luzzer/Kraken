...
Microsoft Teams ships as a bundled plugin in current UAGENT releases, so no
...
```bash
uagent plugins install @uagent/msteams
```

Local checkout (when running from a git repo):

```bash
uagent plugins install ./path/to/local/msteams-plugin
```
...
3. Configure UAGENT with those credentials.
...
- Talk to UAGENT via Teams DMs, group chats, or channels.
...
- On startup, UAGENT resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.uagent/uagent.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring UAGENT, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `uagent-msteams` (must be unique) |
...
1. **Ensure the Microsoft Teams plugin is available**
   ...
   - Older/custom installs can add it manually:
     - From npm: `uagent plugins install @uagent/msteams`
     - From a local checkout: `uagent plugins install ./path/to/local/msteams-plugin`
...
UAGENT exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "UAGENT in Teams", full: "UAGENT in Teams" },
...
UAGENT handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, UAGENT only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/UAGENTShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

UAGENT sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `uagent message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.uagent/msteams-polls.json`.
...
```bash
uagent message send --channel msteams \
...
uagent message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
uagent message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
uagent message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
uagent message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
