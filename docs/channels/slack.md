...
```json
{
  "display_information": {
    "name": "UAGENT",
    "description": "Slack connector for UAGENT"
  },
  "features": {
    "bot_user": {
      "display_name": "UAGENT",
    ...
    "slash_commands": [
      {
        "command": "/uagent",
        "description": "Send a message to UAGENT",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `uagent pairing approve slack <code>`.
...
- If streaming fails mid-reply, UAGENT falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while UAGENT is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are UAGENT-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, UAGENT falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `uagent channels status --probe --json` shows `botTokenStatus` or
