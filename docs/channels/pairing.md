...
read_when:
  ...
  - Reviewing UAGENT security posture
title: "Pairing"
---

# Pairing

“Pairing” is UAGENT’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `uagent-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.uagent/credentials/`:
...
3. On your phone, open the UAGENT iOS app → Settings → Gateway.
...
- The legacy `node.pair.*` API (CLI: `uagent nodes pending|approve|reject|rename`) is a
