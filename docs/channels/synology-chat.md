---
summary: "Synology Chat webhook setup and UAGENT config"
read_when:
  - Setting up Synology Chat with UAGENT
...
Synology Chat ships as a bundled plugin in current UAGENT releases, so normal
...
```bash
uagent plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `uagent onboard` now shows Synology Chat in the same channel setup list as `uagent channels add`.
   - Non-interactive setup: `uagent channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your UAGENT gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in UAGENT.
   - Guided: `uagent onboard`
   - Direct: `uagent channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- UAGENT accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `uagent pairing list synology-chat`
  - `uagent pairing approve synology-chat <CODE>`
...
```bash
uagent message send --channel synology-chat --target 123456 --text "Hello from UAGENT"
uagent message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. UAGENT now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached UAGENT
- `Rate limit exceeded`:
