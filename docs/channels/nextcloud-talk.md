...
Nextcloud Talk ships as a bundled plugin in current UAGENT releases, so
...
```bash
uagent plugins install @uagent/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
uagent plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "UAGENT" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `uagent pairing list nextcloud-talk`
  - `uagent pairing approve nextcloud-talk <CODE>`
