...
read_when:
  - Setting up Twitch chat integration for UAGENT
title: "Twitch"
---

# Twitch

Twitch chat support via IRC connection. UAGENT connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

Twitch ships as a bundled plugin in current UAGENT releases, so normal
...
```bash
uagent plugins install @uagent/twitch
```

Local checkout (when running from a git repo):

```bash
uagent plugins install ./path/to/local/twitch-plugin
```
...
5. Configure the token:
   - Env: `UAGENT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "uagent", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use UAGENT_TWITCH_ACCESS_TOKEN env var)
...
```bash
UAGENT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
