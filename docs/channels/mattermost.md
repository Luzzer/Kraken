---
summary: "Mattermost bot setup and UAGENT config"
read_when:
...
Mattermost ships as a bundled plugin in current UAGENT releases, so normal
...
```bash
uagent plugins install @uagent/mattermost
```

Local checkout (when running from a git repo):

```bash
uagent plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure UAGENT and start the gateway.
...
Native slash commands are opt-in. When enabled, UAGENT registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, UAGENT derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when UAGENT registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as UAGENT.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to UAGENT.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from UAGENT, not `404`.
...
- Approve via:
  - `uagent pairing list mattermost`
  - `uagent pairing approve mattermost <CODE>`
...
Use these target formats with `uagent message send` or cron/webhooks:
...
UAGENT resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), UAGENT sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When UAGENT sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, UAGENT derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and UAGENT run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="uagent-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"uagent-mattermost-interactions",
...
`uagent message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: UAGENT
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as UAGENT. Set an
