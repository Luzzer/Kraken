---
summary: "UAGENT on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up UAGENT on a Raspberry Pi
  - Running UAGENT on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# UAGENT on Raspberry Pi

## Goal

Run a persistent, always-on UAGENT Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl --user -u uagent-gateway.service -f
```

## 9) Access the UAGENT Dashboard
...
```bash
ssh user@gateway-host 'uagent dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/uagent-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/uagent-compile-cache
mkdir -p /var/tmp/uagent-compile-cache
export UAGENT_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `UAGENT_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running UAGENT, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit uagent-gateway.service
```

```ini
[Service]
Environment=UAGENT_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/uagent-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep UAGENT state/cache on SSD-backed storage to avoid SD-card
...
Most UAGENT features work on ARM64, but some external binaries may need ARM builds:
...
systemctl --user is-enabled uagent-gateway.service

# Enable if not
systemctl --user enable uagent-gateway.service
...
```bash
# Check logs
journalctl --user -u uagent-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/uagent  # if using hackable install
npm run build
systemctl --user restart uagent-gateway.service
```
