...
UAGENT supports both **native Windows** and **WSL2**. WSL2 is the more
...
- local CLI use such as `uagent --version`, `uagent doctor`, and `uagent plugins list --json`
- embedded local-agent/provider smoke such as:

```powershell
uagent agent --local --agent main --thinking low -m "Reply with exactly WINDOWS-HATCH-OK."
```

Current caveats:

- `uagent onboard --non-interactive` still expects a reachable local gateway unless you pass `--skip-health`
- `uagent onboard --non-interactive --install-daemon` and `uagent gateway install` try Windows Scheduled Tasks first
- if Scheduled Task creation is denied, UAGENT falls back to a per-user Startup-folder login item and starts the gateway immediately
...
```powershell
uagent onboard --non-interactive --skip-health
uagent gateway run
```
...
### 2) Install the UAGENT gateway user service
...
systemctl --user status uagent-gateway.service --no-pager
```
