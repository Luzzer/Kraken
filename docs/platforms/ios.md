...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `uagent.internal.`), **or**
...
Run `uagent devices list` again before approval.

4. Verify connection:

```bash
uagent nodes status
uagent gateway call node.list --params "{}"
```
...
- `UAGENT_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine UAGENT iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight UAGENT builds.
...
```bash
export UAGENT_APNS_TEAM_ID="TEAMID"
export UAGENT_APNS_KEY_ID="KEYID"
export UAGENT_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.uagent/credentials/apns
chmod 700 ~/.uagent/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.uagent/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.uagent/credentials/apns/AuthKey_KEYID.p8
export UAGENT_APNS_PRIVATE_KEY_PATH="$HOME/.uagent/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_uagent-gw._tcp` on `local.` and, when configured, the same
...
`uagent.internal.`) and Tailscale split DNS.
...
```bash
uagent nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__uagent__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__uagent__/canvas/` and `/__uagent__/a2ui/`.
...
```bash
uagent nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__uagent; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
uagent nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `uagent devices list` and approve manually.
