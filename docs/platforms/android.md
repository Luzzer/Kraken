...
> **Note:** The Android app has not been publicly released yet. The source code is available in the [UAGENT repository](https://github.com/uagent/uagent) under `apps/android`. You can build it yourself using Java 17 and the Android SDK (`./gradlew :app:assemblePlayDebug`). See [apps/android/README.md](https://github.com/uagent/uagent/blob/main/apps/android/README.md) for build instructions.
...
- You can run the CLI (`uagent`) on the gateway machine (or via SSH).
...
```bash
dns-sd -B _uagent-gw._tcp local.
```
...
1. Set up a DNS-SD zone (example `uagent.internal.`) on the gateway host and publish `_uagent-gw._tcp` records.
...
1. Create `~/.uagent/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
uagent nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__uagent__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__uagent__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__uagent__/a2ui/`.
...
Android supports launching UAGENT from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
UAGENT..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set UAGENT as the default assistant app.
</Note>
