---
summary: "UAGENT on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up UAGENT on DigitalOcean
  - Looking for cheap VPS hosting for UAGENT
title: "DigitalOcean (Platform)"
---

# UAGENT on DigitalOcean

## Goal

Run a persistent UAGENT Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
uagent config set gateway.bind tailnet
uagent gateway restart
```
...
```bash
uagent channels login whatsapp
# Scan QR code
```
...
- `~/.uagent/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u uagent-gateway.service --no-pager -n 50
```
