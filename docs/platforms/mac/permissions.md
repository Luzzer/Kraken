...
- Same path: run the app from a fixed location (for UAGENT, `dist/UAGENT.app`).
...
```bash
sudo tccutil reset Accessibility ai.uagent.mac
sudo tccutil reset ScreenCapture ai.uagent.mac
...
Workaround: move files into the UAGENT workspace (`~/.uagent/workspace`) if you want to avoid per-folder grants.
