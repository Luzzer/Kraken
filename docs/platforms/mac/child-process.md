...
service via the external `uagent` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.uagent.gateway`
  (or `ai.uagent.<profile>` when using `--profile`/`UAGENT_PROFILE`; legacy `com.uagent.*` is supported).
