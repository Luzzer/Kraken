...
- `uagent tasks list` shows all tasks; `uagent tasks audit` surfaces issues.
...
uagent tasks list --runtime acp
uagent tasks list --status running

# Show details for a specific task (by ID, run ID, or session key)
uagent tasks show <lookup>

# Cancel a running task (kills the child session)
uagent tasks cancel <lookup>

# Change notification policy for a task
uagent tasks notify <lookup> state_changes

# Run a health audit
uagent tasks audit

# Preview or apply maintenance
uagent tasks maintenance
uagent tasks maintenance --apply

# Inspect TaskFlow state
uagent tasks flow list
...
| CLI operations         | `cli`        | `uagent agent` commands that run through the gateway | `silent`              |
...
| `cancelled` | Stopped by the operator via `uagent tasks cancel`                        |
...
When a task reaches a terminal state, UAGENT notifies you. There are two delivery paths:

**Direct delivery** — if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Telegram, Discord, Slack, etc.). For subagent completions, UAGENT also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
```bash
uagent tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
uagent tasks notify <lookup> <done_only|state_changes|silent>
```

### `tasks audit`

```bash
uagent tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `uagent status` when issues are detected.
...
```bash
uagent tasks maintenance [--json]
uagent tasks maintenance --apply [--json]
```
...
```bash
uagent tasks flow list [--status <status>] [--json]
uagent tasks flow show <lookup> [--json]
uagent tasks flow cancel <lookup>
```
...
For the full operator ledger, use the CLI: `uagent tasks list`.

## Status integration (task pressure)

`uagent status` includes an at-a-glance task summary:
...
```
$UAGENT_STATE_DIR/tasks/runs.sqlite
```
...
[Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `uagent tasks` to inspect individual task records and `uagent tasks flow` to inspect the orchestrating flow.

See [Task Flow](/automation/taskflow) for details.

### Tasks and cron

A cron job **definition** lives in `~/.uagent/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
