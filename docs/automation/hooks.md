...
Hooks are small scripts that run when something happens inside the Gateway. They are automatically discovered from directories and can be inspected with `uagent hooks`.

There are two kinds of hooks in UAGENT:

- **Internal hooks** (this page): run inside the Gateway when agent events fire, like `/new`, `/reset`, `/stop`, or lifecycle events.
- **Webhooks**: external HTTP endpoints that let other systems trigger work in UAGENT. See [Webhooks](/automation/cron-jobs#webhooks).

Hooks can also be bundled inside plugins. `uagent hooks list` shows both standalone hooks and plugin-managed hooks.
...
metadata:
  { "uagent": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
**Metadata fields** (`metadata.uagent`):
...
1. **Bundled hooks**: shipped with UAGENT
2. **Plugin hooks**: hooks bundled inside installed plugins
3. **Managed hooks**: `~/.uagent/hooks/` (user-installed, shared across workspaces). Extra directories from `hooks.internal.load.extraDirs` share this precedence.
...
Hook packs are npm packages that export hooks via `uagent.hooks` in `package.json`. Install with:
...
| command-logger        | `command`                      | Logs all commands to `~/.uagent/logs/commands.log`  |
| boot-md               | `gateway:startup`              | Runs `BOOT.md` when the gateway starts                |

Enable any bundled hook:

```bash
uagent hooks enable <hook-name>
```
...
# Enable/disable
uagent hooks enable <hook-name>
uagent hooks disable <hook-name>
```
...
1. Verify the hook is enabled: `uagent hooks list`
