...
read_when:
  ...
  - Wiring external triggers (webhooks, Gmail) into UAGENT
...
# See run history
uagent cron runs --id <job-id>
```

## How cron works

- Cron runs **inside the Gateway** process (not inside the model).
- Jobs persist at `~/.uagent/cron/jobs.json` so restarts do not lose schedules.
...
together`, and similar hints) and no descendant subagent run is still
  responsible for the final answer, UAGENT re-prompts once for the actual
...
running, UAGENT suppresses that partial parent update instead of announcing it.
...
Wire Gmail inbox triggers to UAGENT via Google PubSub.

**Prerequisites**: `gcloud` CLI, `gog` (gogcli), UAGENT hooks enabled, Tailscale for the public HTTPS endpoint.

### Wizard setup (recommended)

```bash
uagent webhooks gmail setup --account uagent@gmail.com
```
...
When `hooks.enabled=true` and `hooks.gmail.account` is set, the Gateway starts `gog gmail watch serve` on boot and auto-renews the watch. Set `UAGENT_SKIP_GMAIL_WATCHER=1` to opt out.
...
# Edit a job
uagent cron edit <jobId> --message "Updated prompt" --model "opus"

# Force run a job now
uagent cron run <jobId>

# Run only if due
uagent cron run <jobId> --due

# View run history
uagent cron runs --id <jobId> --limit 50

# Delete a job
uagent cron remove <jobId>

# Agent selection (multi-agent setups)
uagent cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops
uagent cron edit <jobId> --clear-agent
```

Model override note:

- `uagent cron add|edit --model ...` changes the job's selected model.
...
Disable cron: `cron.enabled: false` or `UAGENT_SKIP_CRON=1`.
...
- Check `cron.enabled` and `UAGENT_SKIP_CRON` env var.
- Confirm the Gateway is running continuously.
- For `cron` schedules, verify timezone (`--tz`) vs the host timezone.
- `reason: not-due` in run output means manual run was checked with `uagent cron run <jobId> --due` and the job was not due yet.
...
- If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`),
  UAGENT suppresses direct outbound delivery and also suppresses the fallback
