...
metadata: { "uagent": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == UAGENT `sessions_spawn`
- **File I/O** == UAGENT `read`/`write`
- **Remote fetch** == UAGENT `web_fetch` (or `exec` with curl when POST is required)
