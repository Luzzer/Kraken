...
import { normalizeChannelId, type ChannelId } from "uagent/plugin-sdk/channel-targets";
import type {
  UAGENTConfig,
  TtsAutoMode,
  TtsConfig,
...
import { redactSensitiveText } from "uagent/plugin-sdk/logging-core";
import { resolveSendableOutboundReplyParts } from "uagent/plugin-sdk/reply-payload";
import type { ReplyPayload } from "uagent/plugin-sdk/reply-runtime";
import { isVerbose, logVerbose } from "uagent/plugin-sdk/runtime-env";
import { resolvePreferredUAGENTTmpDir } from "uagent/plugin-sdk/sandbox";
import { CONFIG_DIR, resolveUserPath, stripMarkdown } from "uagent/plugin-sdk/text-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.UAGENT_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: UAGENTConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: UAGENTConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: UAGENTConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: UAGENTConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: UAGENTConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: UAGENTConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredUAGENTTmpDir();
