...
function normalizeDenyList(cfg: UAGENTPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: UAGENTPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: UAGENTPluginApi) {
    ...
    const timerService: UAGENTPluginService = {
