...
import { resolveAgentDir } from "uagent/plugin-sdk/agent-runtime";
import { agentCommandFromIngress } from "uagent/plugin-sdk/agent-runtime";
import { resolveTtsConfig, type ResolvedTtsConfig } from "uagent/plugin-sdk/agent-runtime";
import type { UAGENTConfig } from "uagent/plugin-sdk/config-runtime";
import type { DiscordAccountConfig, TtsConfig } from "uagent/plugin-sdk/config-runtime";
import { resolveAgentRoute } from "uagent/plugin-sdk/routing";
import { logVerbose, shouldLogVerbose } from "uagent/plugin-sdk/runtime-env";
import { createSubsystemLogger } from "uagent/plugin-sdk/runtime-env";
import type { RuntimeEnv } from "uagent/plugin-sdk/runtime-env";
import { parseTtsDirectives } from "uagent/plugin-sdk/speech";
import { formatErrorMessage } from "uagent/plugin-sdk/ssrf-runtime";
import { resolvePreferredUAGENTTmpDir } from "uagent/plugin-sdk/temp-path";
...
function resolveVoiceTtsConfig(params: { cfg: UAGENTConfig; override?: TtsConfig }): {
  cfg: UAGENTConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredUAGENTTmpDir(), "discord-voice-"));
