...
const { STORE_PATH, mockSessionStoreEntries } = vi.hoisted(() => ({
  STORE_PATH: "/tmp/uagent-exec-approvals-test.json",
...
beforeEach(() => {
  ...
  mockResolveGatewayConnectionAuth.mockReset().mockImplementation(
    async (params: {
      ...
      const envToken = params.env.UAGENT_GATEWAY_TOKEN;
      const envPassword = params.env.UAGENT_GATEWAY_PASSWORD;
...
vi.mock("../../../../src/gateway/operator-approvals-client.js", () => ({
  createOperatorApprovalsGatewayClient: async (params: {
    ...
    const envUrl = process.env.UAGENT_GATEWAY_URL?.trim();
...
vi.mock("uagent/plugin-sdk/text-runtime", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/text-runtime")>(
...
describe("DiscordExecApprovalHandler gateway auth", () => {
  ...
  it("prefers UAGENT_GATEWAY_TOKEN when config token is missing", async () => {
    vi.stubEnv("UAGENT_GATEWAY_TOKEN", "env-gateway-token");
    const handler = new DiscordExecApprovalHandler({
      token: "discord-bot-token",
      accountId: "default",
      config: { enabled: true, approvers: ["123"] },
...
describe("DiscordExecApprovalHandler gateway auth resolution", () => {
  ...
  it("passes env URL overrides to shared gateway auth resolver", async () => {
    const previousGatewayUrl = process.env.UAGENT_GATEWAY_URL;
    try {
      process.env.UAGENT_GATEWAY_URL = "wss://gateway-from-env.example/ws";
      const handler = new DiscordExecApprovalHandler({
        token: "test-token",
        accountId: "default",
        config: { enabled: true, approvers: ["123"] },
      ...
      if (typeof previousGatewayUrl === "string") {
        process.env.UAGENT_GATEWAY_URL = previousGatewayUrl;
      } else {
