...
import type { loadConfig } from "uagent/plugin-sdk/config-runtime";
import { clearSessionStoreCacheForTest } from "uagent/plugin-sdk/config-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { createNoopThreadBindingManager } from "./thread-bindings.js";

type ConversationRuntimeModule = typeof import("uagent/plugin-sdk/conversation-binding-runtime");
...
vi.mock("uagent/plugin-sdk/conversation-binding-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("uagent/plugin-sdk/conversation-binding-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("uagent/plugin-sdk/conversation-binding-runtime")>(
        "uagent/plugin-sdk/conversation-binding-runtime",
      ),
  );
});

vi.mock("uagent/plugin-sdk/agent-runtime", () => ({
...
vi.mock("uagent/plugin-sdk/models-provider-runtime", () => ({
  buildModelsProviderData: buildModelsProviderDataMock,
}));

const STORE_PATH = path.join(
  os.tmpdir(),
  `uagent-discord-think-autocomplete-${process.pid}.json`,
);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("uagent/plugin-sdk/command-auth").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("uagent/plugin-sdk/command-auth").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("uagent/plugin-sdk/command-auth");
