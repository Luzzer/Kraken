import { DEFAULT_EMOJIS } from "uagent/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/uagent-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("uagent/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("uagent/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("uagent/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/uagent-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/uagent-discord-process-test-sessions.json" },
      },
    });
