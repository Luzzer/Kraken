...
import type { RuntimeEnv } from "uagent/plugin-sdk/runtime-env";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: UAGENTConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): UAGENTConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("uagent/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/acp-runtime")>(
    "uagent/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("uagent/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/command-auth")>(
...
vi.mock("uagent/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/config-runtime")>(
...
vi.mock("uagent/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/runtime-env")>(
    "uagent/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("uagent/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/infra-runtime")>(
