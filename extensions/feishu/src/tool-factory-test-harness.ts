import type { AnyAgentTool, UAGENTPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: UAGENTPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<UAGENTPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
