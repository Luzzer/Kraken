import { hasConfiguredSecretInput } from "uagent/plugin-sdk/secret-input";
import {
  ...
  type UAGENTConfig,
} from "uagent/plugin-sdk/setup-runtime";
import { formatDocsLink } from "uagent/plugin-sdk/setup-tools";
...
function enableSlackAccount(cfg: UAGENTConfig, accountId: string): UAGENTConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: UAGENTConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: UAGENTConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: UAGENTConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: UAGENTConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: UAGENTConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: UAGENTConfig; accountId: string }) =>
    ...
    disable: (cfg: UAGENTConfig) => setSetupChannelEnabled(cfg, channel, false),
