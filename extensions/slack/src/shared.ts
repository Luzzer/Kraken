...
import { patchChannelConfigForAccount } from "uagent/plugin-sdk/setup-runtime";
...
import { getChatChannelMeta, type ChannelPlugin, type UAGENTConfig } from "./channel-api.js";
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "UAGENT";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for UAGENT`,
    },
    features: {
      bot_user: {
...
export function buildSlackSetupLines(botName = "UAGENT"): string[] {
  return [
