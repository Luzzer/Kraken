...
// Prefix for UAGENT-generated action IDs to scope our handler
const UAGENT_ACTION_PREFIX = "uagent:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${UAGENT_ACTION_PREFIX}`);

  // Handle UAGENT modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
