...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-slack-thread-"));
  });
  ...
  function createThreadSlackCtx(params: { cfg: UAGENTConfig; replies: unknown }) {
...
describe("prepareSlackMessage sender prefix", () => {
  function createSenderPrefixCtx(params: {
    ...
    return {
      cfg: {
        agents: { defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/uagent" } },
  ...
  it("prefixes channel bodies with sender label", async () => {
    const ctx = createSenderPrefixCtx({
      channels: {},
      slashCommand: { command: "/uagent", enabled: true },
    });
...
describe("slack thread.requireExplicitMention", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-slack-explicit-mention-"));
  });
  ...
  it("drops thread reply without explicit mention when requireExplicitMention is true", async () => {
    ...
    const { storePath } = makeTmpStorePath();
    vi.spyOn(
      await import("uagent/plugin-sdk/config-runtime"),
      "resolveStorePath",
