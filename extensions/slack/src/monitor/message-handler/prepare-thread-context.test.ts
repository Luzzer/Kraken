...
import { resolveEnvelopeFormatOptions } from "uagent/plugin-sdk/channel-inbound";
...
describe("resolveSlackThreadContextData", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-slack-thread-context-"));
  });
  ...
  it("omits non-allowlisted starter text and thread history messages", async () => {
    ...
    const result = await resolveSlackThreadContextData({
      ...
      envelopeOptions: resolveEnvelopeFormatOptions({} as UAGENTConfig),
