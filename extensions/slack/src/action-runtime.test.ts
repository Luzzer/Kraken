...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): UAGENTConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as UAGENTConfig;
  ...
  async function resolveReadToken(cfg: UAGENTConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: UAGENTConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
