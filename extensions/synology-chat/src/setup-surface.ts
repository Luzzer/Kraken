...
function getChannelConfig(cfg: UAGENTConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: UAGENTConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: UAGENTConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: UAGENTConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL UAGENT uses to send replies back to Chat.",
      ],
