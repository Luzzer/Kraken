/**
 * Synology Chat Channel Plugin for UAGENT.
 *
 * Implements the ChannelPlugin interface following the LINE pattern.
 */

import { DEFAULT_ACCOUNT_ID } from "uagent/plugin-sdk/account-id";
import type { UAGENTConfig } from "uagent/plugin-sdk/account-resolution";
import {
...
import { waitUntilAbort } from "uagent/plugin-sdk/channel-lifecycle";
import {
...
import { createEmptyChannelDirectoryAdapter } from "uagent/plugin-sdk/directory-runtime";
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "uagent pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: UAGENTConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: UAGENTConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to UAGENT",
        order: 90,
      },
      capabilities: {
    ...
    pairing: {
      text: {
        ...
        message: "UAGENT: your access has been approved.",
