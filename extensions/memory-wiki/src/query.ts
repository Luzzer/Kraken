...
import { resolveDefaultAgentId } from "uagent/plugin-sdk/config-runtime";
import type { MemorySearchResult } from "uagent/plugin-sdk/memory-host-files";
import { getActiveMemorySearchManager } from "uagent/plugin-sdk/memory-host-search";
...
async function resolveActiveMemoryManager(appConfig?: UAGENTConfig) {
  if (!appConfig) {
    return null;
  }
  try {
