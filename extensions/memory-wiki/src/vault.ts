...
This vault is maintained by the UAGENT memory-wiki plugin.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(
    path.join(rootDir, ".uagent-wiki", "state.json"),
    withTrailingNewline(
      JSON.stringify(
        {
          version: 1,
  ...
  await writeFileIfMissing(path.join(rootDir, ".uagent-wiki", "log.jsonl"), "", createdFiles);
