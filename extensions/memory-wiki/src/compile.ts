...
import {
  ...
  withTrailingNewline,
} from "uagent/plugin-sdk/memory-host-markdown";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- uagent:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- uagent:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- uagent:wiki:index:start -->",
      endMarker: "<!-- uagent:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- uagent:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- uagent:wiki:${group.dir}:index:end -->`,
