...
- Start from `uagent wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `uagent wiki obsidian status` before shelling out, then prefer the dedicated helpers like `uagent wiki obsidian search`, `uagent wiki obsidian open`, `uagent wiki obsidian command`, and `uagent wiki obsidian daily`.
