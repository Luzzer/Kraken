import { MAX_IMAGE_BYTES } from "uagent/plugin-sdk/media-runtime";
...
describe("tlon monitor media", () => {
  async function loadMediaModule() {
    const mediaRuntime = await import("uagent/plugin-sdk/media-runtime");
  ...
  it("stores fetched media through the shared inbound media store with the image cap", async () => {
    ...
    saveMediaBuffer.mockResolvedValue({
      id: "photo---uuid.png",
      path: "/tmp/uagent/media/inbound/photo---uuid.png",
    ...
    expect(result).toEqual({
      localPath: "/tmp/uagent/media/inbound/photo---uuid.png",
      contentType: "image/png",
