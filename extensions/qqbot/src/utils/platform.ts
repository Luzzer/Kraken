...
/**
 * Return a path under `~/.uagent/qqbot`, creating it on demand.
 */
export function getQQBotDataDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".uagent", "qqbot", ...subPaths);
...
/**
 * Return a path under `~/.uagent/media/qqbot`, creating it on demand.
 *
 * Unlike `getQQBotDataDir`, this lives under UAGENT's core media allowlist so
...
export function getQQBotMediaDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".uagent", "media", "qqbot", ...subPaths);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  return dir;
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".uagent", "workspace", "qqbot");
