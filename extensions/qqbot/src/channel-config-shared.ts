...
import type { ChannelSetupInput } from "uagent/plugin-sdk/setup";
import {
  DEFAULT_ACCOUNT_ID,
...
export const qqbotConfigAdapter = {
  listAccountIds: (cfg: UAGENTConfig) => listQQBotAccountIds(cfg),
  resolveAccount: (cfg: UAGENTConfig, accountId?: string | null) =>
    resolveQQBotAccount(cfg, accountId, { allowUnresolvedSecretRef: true }),
  defaultAccountId: (cfg: UAGENTConfig) => resolveDefaultQQBotAccountId(cfg),
  setAccountEnabled: ({
    cfg,
    accountId,
    enabled,
  ...
  deleteAccount: ({ cfg, accountId }: { cfg: UAGENTConfig; accountId: string }) =>
    deleteAccountFromConfigSection({
      cfg,
      sectionKey: "qqbot",
      accountId,
...
export const qqbotSetupAdapterShared = {
  resolveAccountId: ({ cfg, accountId }: { cfg: UAGENTConfig; accountId?: string | null }) =>
