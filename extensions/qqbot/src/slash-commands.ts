...
import { resolveRuntimeServiceVersion } from "uagent/plugin-sdk/cli-runtime";
...
const QQBOT_PLUGIN_GITHUB_URL = "https://github.com/uagent/uagent/tree/main/extensions/qqbot";
const QQBOT_UPGRADE_GUIDE_URL = "https://q.qq.com/qqbot/uagent/upgrade.html";
...
/**
 * /bot-ping — test current network latency between UAGENT and QQ.
 */
registerCommand({
  name: "bot-ping",
  description: "测试 UAGENT 与 QQ 之间的网络延迟",
  usage: [
    `/bot-ping`,
    ``,
    `测试当前 UAGENT 宿主机与 QQ 服务器之间的网络延迟。`,
    `返回网络传输耗时和插件处理耗时。`,
  ].join("\n"),
  handler: (ctx) => {
    const now = Date.now();
...
/**
 * /bot-version — show the UAGENT framework version.
 */
registerCommand({
  name: "bot-version",
  description: "查看 UAGENT 框架版本",
  usage: [`/bot-version`, ``, `查看当前 UAGENT 框架版本。`].join("\n"),
  handler: async () => {
    const frameworkVersion = resolveRuntimeServiceVersion();
    const lines = [`🦞 UAGENT 版本：${frameworkVersion}`];
...
function getConfiguredLogFiles(): string[] {
  ...
  const files: string[] = [];
  for (const cli of ["uagent", "clawdbot", "moltbot"]) {
    try {
...
function collectCandidateLogDirs(): string[] {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (/STATE_DIR$/i.test(key) && /(UAGENT|CLAWDBOT|MOLTBOT)/i.test(key)) {
      pushStateDir(value);
    }
  }

  for (const name of [".uagent", ".clawdbot", ".moltbot", "uagent", "clawdbot", "moltbot"]) {
  ...
  for (const root of searchRoots) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(uagent|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
  ...
  if (!isWindows()) {
    for (const name of ["uagent", "clawdbot", "moltbot"]) {
...
function collectRecentLogFiles(logDirs: string[]): LogCandidate[] {
  ...
  for (const dir of logDirs) {
    ...
    pushFile(path.join(dir, "uagent.log"), dir);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(gateway|uagent|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
