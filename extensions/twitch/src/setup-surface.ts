...
function resolveSetupAccountId(cfg: UAGENTConfig): string {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: UAGENT_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: UAGENTConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var UAGENT_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
function resolveTwitchGroupPolicy(cfg: UAGENTConfig): "open" | "allowlist" | "disabled" {
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.UAGENT_TWITCH_ACCESS_TOKEN?.trim();
