import { resolveDefaultModelForAgent } from "uagent/plugin-sdk/agent-runtime";
...
type LoadConfigFn = typeof import("uagent/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("uagent/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("uagent/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("uagent/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/uagent-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("uagent/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("uagent/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
function createModelsProviderDataFromConfig(cfg: UAGENTConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: UAGENTConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: UAGENTConfig) => {
