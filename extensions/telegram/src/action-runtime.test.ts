...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): UAGENTConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as UAGENTConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): UAGENTConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as UAGENTConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: UAGENTConfig, accountId = "media") {
    await handleTelegramAction(
