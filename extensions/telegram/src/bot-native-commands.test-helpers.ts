...
import type { TelegramAccountConfig } from "uagent/plugin-sdk/config-runtime";
...
vi.mock("uagent/plugin-sdk/plugin-runtime", () => ({
...
vi.mock("uagent/plugin-sdk/reply-dispatch-runtime", () => ({
  dispatchReplyWithBufferedBlockDispatcher:
    replyPipelineMocks.dispatchReplyWithBufferedBlockDispatcher,
}));
vi.mock("uagent/plugin-sdk/conversation-runtime", () => ({
...
export function createNativeCommandsHarness(params?: {
  ...
  const telegramDeps = {
    loadConfig: vi.fn(() => params?.cfg ?? ({} as UAGENTConfig)),
    readChannelAllowFromStore: vi.fn(async () => []),
    dispatchReplyWithBufferedBlockDispatcher:
      replyPipelineMocks.dispatchReplyWithBufferedBlockDispatcher,
    getPluginCommandSpecs: pluginCommandMocks.getPluginCommandSpecs,
