...
vi.mock("uagent/plugin-sdk/runtime", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/runtime")>(
    "uagent/plugin-sdk/runtime",
  );
  return {
    ...actual,
...
describe("telegram doctor", () => {
  ...
  it("formats username repair warnings", () => {
    ...
    expect(warnings[1]).toContain("uagent doctor --fix");
  });
});
