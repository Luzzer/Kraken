...
} from "uagent/plugin-sdk/command-auth-native";
import {
...
import type { ChannelGroupPolicy } from "uagent/plugin-sdk/config-runtime";
import type {
  ReplyToMode,
...
import { getRuntimeConfigSnapshot } from "uagent/plugin-sdk/runtime-config-snapshot";
import { danger, logVerbose } from "uagent/plugin-sdk/runtime-env";
...
type TelegramChunkMode = ReturnType<
  typeof import("uagent/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload = import("uagent/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): UAGENTConfig => telegramDeps.loadConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: UAGENTConfig): TelegramAccountConfig => {
    try {
