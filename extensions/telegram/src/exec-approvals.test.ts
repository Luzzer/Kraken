...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<UAGENTConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<UAGENTConfig["channels"]>["telegram"]>>,
): UAGENTConfig {
  return {
    channels: {
      telegram: {
