import { note } from "uagent/plugin-sdk/browser-setup-tools";
import {
...
function collectChromeMcpProfiles(cfg: UAGENTConfig): ExistingSessionProfile[] {
...
export async function noteChromeMcpBrowserReadiness(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time UAGENT connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. UAGENT does not bundle Chrome.`,
