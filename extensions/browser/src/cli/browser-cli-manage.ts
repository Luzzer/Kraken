...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "uagent";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "uagent" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (uagent|existing-session). Default: uagent")
    .action(
      async (
        opts: {
          name: string;
