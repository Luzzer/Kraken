import type { UAGENTPluginSecurityAuditContext } from "uagent/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "uagent/plugin-sdk/secret-input";
import { formatCliCommand } from "uagent/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "uagent/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: UAGENTPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("uagent security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.UAGENT_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.UAGENT_GATEWAY_PASSWORD) ||
