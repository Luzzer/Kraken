...
import { resolveUAGENTUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveUAGENTUserDataDir: vi.fn(() => "/tmp/uagent-test/uagent/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "uagent-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveUAGENTUserDataDir).mockReturnValue(userDataDir);
