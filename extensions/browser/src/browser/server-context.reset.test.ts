...
function localUAGENTProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "uagent",
...
function createLocalUAGENTResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localUAGENTProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveUAGENTUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-reset-"));
    const profileDir = path.join(tempRoot, "uagent");
    ...
    const ops = createLocalUAGENTResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveUAGENTUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-reset-no-own-"));
    const profileDir = path.join(tempRoot, "uagent");
    fs.mkdirSync(profileDir, { recursive: true });
