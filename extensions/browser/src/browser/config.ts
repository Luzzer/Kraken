...
import {
  ...
  DEFAULT_UAGENT_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_UAGENT_BROWSER_COLOR;
  }
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_UAGENT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_UAGENT_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: UAGENTConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_UAGENT_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_UAGENT_BROWSER_PROFILE_NAME]
        ? DEFAULT_UAGENT_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "uagent";
