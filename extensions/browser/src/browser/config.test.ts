...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("uagent");
    expect(profile?.driver).toBe("uagent");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const uagent = resolveProfile(resolved, "uagent");
    expect(uagent?.driver).toBe("uagent");
    expect(uagent?.cdpPort).toBe(18800);
    expect(uagent?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from UAGENT_GATEWAY_PORT when unset", () => {
    withEnv({ UAGENT_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const uagent = resolveProfile(resolved, "uagent");
      expect(uagent?.cdpPort).toBe(19012);
      expect(uagent?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ UAGENT_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const uagent = resolveProfile(resolved, "uagent");
      expect(uagent?.cdpPort).toBe(19022);
      expect(uagent?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(uagent?.cdpPort).toBe(19000);
    expect(uagent?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "uagent");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "uagent")!;
  ...
  describe("default profile preference", () => {
    it("defaults to uagent profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("uagent");
    });

    it("keeps uagent default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("uagent");
    ...
    it("keeps uagent default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("uagent");
    ...
    it("keeps uagent default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
