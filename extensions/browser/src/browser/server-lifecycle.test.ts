...
const { stopUAGENTChromeMock } = vi.hoisted(() => ({
  stopUAGENTChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopUAGENTChrome: stopUAGENTChromeMock,
}));
...
beforeEach(() => {
  ...
  stopUAGENTChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["uagent", "user"]);
    ...
    expect(stopMap.uagent).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopUAGENTChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("uagent browser stop failed: Error: oops");
  });
});
