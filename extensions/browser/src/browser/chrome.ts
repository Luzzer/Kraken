...
export function resolveUAGENTUserDataDir(profileName = DEFAULT_UAGENT_BROWSER_PROFILE_NAME) {
...
export function buildUAGENTChromeLaunchArgs(params: {
...
export async function launchUAGENTChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveUAGENTUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_UAGENT_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildUAGENTChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateUAGENTProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 uagent browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`uagent browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`uagent browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 uagent browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopUAGENTChrome(
