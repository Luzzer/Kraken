...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".uagent-profile-decorated");
}
 ...
 */
export function decorateUAGENTProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_UAGENT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_UAGENT_BROWSER_COLOR).toUpperCase();
