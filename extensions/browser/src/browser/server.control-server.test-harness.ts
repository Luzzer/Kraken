...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "uagent" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.UAGENT_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "uagent",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/uagent" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    uagent: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchUAGENTChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "uagent";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.UAGENT_GATEWAY_PORT;
  process.env.UAGENT_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.UAGENT_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.UAGENT_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.UAGENT_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.UAGENT_GATEWAY_PASSWORD;
  } else {
    process.env.UAGENT_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
