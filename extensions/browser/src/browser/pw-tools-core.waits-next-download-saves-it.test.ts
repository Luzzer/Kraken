...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredUAGENTTmpDir: vi.fn(() => "/tmp/uagent"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-uagent-dir.js");

describe("pw-tools-core", () => {
  beforeAll(async () => {
    vi.doMock("./pw-session.js", () => sessionMocks);
    vi.doMock("./chrome.js", () => chromeMocks);
    tmpDirModule = await import("../infra/tmp-uagent-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredUAGENTTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredUAGENTTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredUAGENTTmpDir.mockReturnValue("/tmp/uagent");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".uagent-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredUAGENTTmpDir.mockReturnValue("/tmp/uagent-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "uagent-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "uagent-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredUAGENTTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "uagent-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "uagent-preferred", "downloads")}${path.sep}`),
    );
  });
