  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          uagent: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.UAGENT_GATEWAY_PORT;
    process.env.UAGENT_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.UAGENT_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.UAGENT_GATEWAY_PASSWORD;
