...
- Extension production code should import from `uagent/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `uagent.plugin.json` and the package
  `uagent` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `uagent/plugin-sdk/*`.
