...
`uagent.invoke --each`) without adding new UAGENT code per workflow.
...
This extension depends on UAGENT internal modules (the embedded agent runner).
It is intended to ship as a **bundled** UAGENT extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.uagent/extensions` as a standalone plugin directory.
