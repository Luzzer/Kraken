...
import { resolveAccountEntry } from "uagent/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: UAGENTConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: UAGENTConfig): string {
