...
import { createDangerousNameMatchingMutableAllowlistWarningCollector } from "uagent/plugin-sdk/channel-policy";
import type { UAGENTConfig } from "uagent/plugin-sdk/config-runtime";
import { isZalouserMutableGroupEntry } from "./security-audit.js";

type ZalouserChannelsConfig = NonNullable<UAGENTConfig["channels"]>;
...
function normalizeZalouserCompatibilityConfig(cfg: UAGENTConfig): ChannelDoctorConfigMutation {
...
const ZALOUSER_LEGACY_CONFIG_RULES: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      'channels.zalouser.groups.<id>.allow is legacy; use channels.zalouser.groups.<id>.enabled instead. Run "uagent doctor --fix".',
    match: hasLegacyZalouserGroupAllowAliases,
  },
  {
    path: ["channels", "zalouser", "accounts"],
    message:
      'channels.zalouser.accounts.<id>.groups.<id>.allow is legacy; use channels.zalouser.accounts.<id>.groups.<id>.enabled instead. Run "uagent doctor --fix".',
