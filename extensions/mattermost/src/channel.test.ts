...
vi.mock("uagent/plugin-sdk/ssrf-runtime", async () => {
  const original = (await vi.importActual("uagent/plugin-sdk/ssrf-runtime")) as Record<
    string,
    unknown
  >;
...
function getDescribedActions(cfg: UAGENTConfig, accountId?: string): string[] {
