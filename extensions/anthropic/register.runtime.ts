import { formatCliCommand, parseDurationMs } from "uagent/plugin-sdk/cli-runtime";
import type {
  UAGENTPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type UAGENTConfig as ProviderAuthConfig,
  type ProviderAuthResult,
  suggestOAuthProfileIdForLegacyDefault,
  upsertAuthProfile,
  validateAnthropicSetupToken,
} from "uagent/plugin-sdk/provider-auth";
import { cloneFirstTemplateModel } from "uagent/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "uagent/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("uagent models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("uagent models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("uagent doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: UAGENTPluginApi): void {
