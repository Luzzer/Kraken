...
Read-only diff viewer plugin for **UAGENT** agents.
...
Set plugin-wide defaults in `~/.uagent/uagent.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/uagent",
        },
      },
    },
  },
...
Before:
UAGENT supports plugins.

After:
UAGENT supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/uagent-diffs`).
