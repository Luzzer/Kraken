...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-uagent-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-uagent-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.uagentDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.uagentDiffsError = "true";
