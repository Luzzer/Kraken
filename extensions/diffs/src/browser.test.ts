...
import type { UAGENTPluginApi, UAGENTPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("uagent-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<UAGENTPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: UAGENTPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<UAGENTPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as UAGENTPluginApi);
