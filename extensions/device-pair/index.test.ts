...
import type { UAGENTPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredUAGENTTmpDir: vi.fn(() => path.join(os.tmpdir(), "uagent-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredUAGENTTmpDir: pluginApiMocks.resolvePreferredUAGENTTmpDir,
...
function createApi(params?: {
  runtime?: UAGENTPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as UAGENTPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: UAGENTPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as UAGENTPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredUAGENTTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredUAGENTTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the UAGENT iOS app:");
    expect(text).toContain("![UAGENT pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the UAGENT iOS app:");
