...
vi.mock("uagent/plugin-sdk/config-runtime", async () => {
  const original = (await vi.importActual("uagent/plugin-sdk/config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("uagent/plugin-sdk/text-runtime", async () => {
  const original = (await vi.importActual("uagent/plugin-sdk/text-runtime")) as Record<
    string,
    unknown
  >;
  return {
