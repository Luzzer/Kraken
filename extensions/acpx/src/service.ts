...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: UAGENTPluginServiceContext): Promise<void> {
      if (process.env.UAGENT_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (UAGENT_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (process.env.UAGENT_SKIP_ACPX_RUNTIME_PROBE === "1") {
        return;
      }
    ...
    async stop(_ctx: UAGENTPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
