...
import type { z } from "uagent/plugin-sdk/zod";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "uagent-plugin-tools";

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "uagent.plugin.json")) &&
...
function resolveAcpxPluginRootFromUAGENTLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common UAGENT layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromUAGENTLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveUAGENTRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const openClawRoot = resolveUAGENTRoot(pluginRoot);
