...
The bundled ACPX extension is a thin UAGENT wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `uagent/acpx`, not in this extension.
...
Use this flow when UAGENT needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `uagent/acpx` repo first.
2. In UAGENT, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild UAGENT and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for UAGENT ACP validation.
...
If a change feels like shared ACP runtime behavior instead of UAGENT-specific glue, move it to `uagent/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
