...
import {
  ...
  resolveEnvApiKey,
} from "uagent/plugin-sdk/provider-auth-runtime";
import { resolveAgentModelPrimaryValue } from "uagent/plugin-sdk/provider-onboard";
...
describe("Kilo Gateway provider config", () => {
  ...
  describe("env var resolution", () => {
    ...
    it("resolves the kilocode api key via resolveApiKeyForProvider", async () => {
      const agentDir = mkdtempSync(join(tmpdir(), "uagent-test-"));
      const envSnapshot = captureEnv(["KILOCODE_API_KEY"]);
