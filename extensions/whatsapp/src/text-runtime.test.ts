...
describe("jidToE164", () => {
  it("maps @lid using reverse mapping file", async () => {
    await withTempDir("uagent-state-", async (stateDir) => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("uagent-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("uagent-lid-a-", async (first) => {
      await withTempDir("uagent-lid-b-", (second) => {
