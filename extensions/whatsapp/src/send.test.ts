...
import { redactIdentifier } from "uagent/plugin-sdk/logging-core";
...
let resetLogger: typeof import("uagent/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("uagent/plugin-sdk/runtime-env").setLoggerOverride;
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("uagent/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `uagent-outbound-${crypto.randomUUID()}.log`);
