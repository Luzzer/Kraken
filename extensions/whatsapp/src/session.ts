...
import { VERSION } from "uagent/plugin-sdk/cli-runtime";
import { resolveAmbientNodeProxyAgent } from "uagent/plugin-sdk/extension-shared";
import { danger, success } from "uagent/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "uagent/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "uagent/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("uagent channels login")}`,
              ),
            );
          }
        }
