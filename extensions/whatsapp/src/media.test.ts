...
import { optimizeImageToPng } from "uagent/plugin-sdk/media-runtime";
import { resolveStateDir } from "uagent/plugin-sdk/state-paths";
import { resolvePreferredUAGENTTmpDir } from "uagent/plugin-sdk/temp-path";
import { captureEnv } from "uagent/plugin-sdk/testing";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredUAGENTTmpDir(), "uagent-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub UAGENT_STATE_DIR.
    // Also keep it outside the UAGENT temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["UAGENT_STATE_DIR"]);
    process.env.UAGENT_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "uagent-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredUAGENTTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default UAGENT state workspace and sandbox roots", async () => {
  ...
  it("rejects default UAGENT state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
