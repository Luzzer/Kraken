import { resolveInboundDebounceMs } from "uagent/plugin-sdk/channel-inbound";
import { formatCliCommand } from "uagent/plugin-sdk/cli-runtime";
import { waitForever } from "uagent/plugin-sdk/cli-runtime";
import { hasControlCommand } from "uagent/plugin-sdk/command-detection";
import { enqueueSystemEvent } from "uagent/plugin-sdk/infra-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "uagent/plugin-sdk/reply-history";
import { resolveAgentRoute } from "uagent/plugin-sdk/routing";
import { logVerbose } from "uagent/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "uagent/plugin-sdk/runtime-env";
import { getChildLogger } from "uagent/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "uagent/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("uagent channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("uagent channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
