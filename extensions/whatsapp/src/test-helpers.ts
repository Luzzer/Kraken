...
const CONFIG_KEY = Symbol.for("uagent:testConfigMock");
...
vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/uagent-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("uagent:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("uagent:lastSocket")];
  if (typeof getter === "function") {
