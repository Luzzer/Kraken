...
vi.mock("uagent/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/media-runtime")>(
    "uagent/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `uagent-inbound-media-${crypto.randomUUID()}`);
