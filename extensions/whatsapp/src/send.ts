...
import { generateSecureUuid } from "uagent/plugin-sdk/core";
import { normalizePollInput, type PollInput } from "uagent/plugin-sdk/media-runtime";
import { createSubsystemLogger } from "uagent/plugin-sdk/runtime-env";
import { getChildLogger } from "uagent/plugin-sdk/text-runtime";
import { redactIdentifier } from "uagent/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: UAGENTConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const cfg = options.cfg ?? loadConfig();
