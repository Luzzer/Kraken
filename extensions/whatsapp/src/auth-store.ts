...
import { info, success } from "uagent/plugin-sdk/runtime-env";
import { getChildLogger } from "uagent/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "uagent/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("uagent channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
