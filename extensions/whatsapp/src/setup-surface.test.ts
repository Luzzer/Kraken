...
import { DEFAULT_ACCOUNT_ID, type UAGENTConfig } from "uagent/plugin-sdk/setup";
...
const hoisted = vi.hoisted(() => ({
  detectWhatsAppLinked: vi.fn<(cfg: UAGENTConfig, accountId: string) => Promise<boolean>>(
    async () => false,
  ),
  loginWeb: vi.fn(async () => {}),
  pathExists: vi.fn(async () => false),
  resolveWhatsAppAuthDir: vi.fn(() => ({
    authDir: "/tmp/uagent-whatsapp-test",
  })),
}));
...
vi.mock("uagent/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/setup")>(
    "uagent/plugin-sdk/setup",
  );
  return {
    ...actual,
...
function expectFinalizeResult(result: Awaited<ReturnType<typeof runFinalizeWithHarness>>): {
  ...
  return result as { cfg: UAGENTConfig };
}
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/uagent-whatsapp-test" });
  });
  ...
  it("uses configured defaultAccount for omitted-account setup status", async () => {
    hoisted.detectWhatsAppLinked.mockImplementation(
      async (_cfg: UAGENTConfig, accountId: string) => accountId === "work",
    );
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("uagent channels login"),
      "WhatsApp",
    );
  });
