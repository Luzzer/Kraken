...
import type { ChannelSetupWizard } from "uagent/plugin-sdk/setup";
...
type WhatsAppConfig = NonNullable<NonNullable<UAGENTConfig["channels"]>["whatsapp"]>;
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so UAGENT can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for UAGENT" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("uagent channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
