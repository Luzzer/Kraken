...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        UAGENT_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      ...
      UAGENT_SKIP_BROWSER_CONTROL_SERVER: "1"
...
${imageBlock}    pull_policy: never
    ...
    environment:
      UAGENT_CONFIG_PATH: /tmp/uagent/uagent.json
      ...
      UAGENT_PROFILE: ""
    volumes:
      - ./state:/opt/uagent-scaffold:ro
      - ${repoMount}:/opt/uagent-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/uagent/workspace /tmp/uagent/state && cp /opt/uagent-scaffold/uagent.json /tmp/uagent/uagent.json && cp -R /opt/uagent-scaffold/seed-workspace/. /tmp/uagent/workspace/ && ln -snf /opt/uagent-repo /tmp/uagent/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
UAGENT_GATEWAY_TOKEN=${params.gatewayToken}
...
1. Build the prebaked image once:
   - \`docker build -t uagent:qa-local-prebaked --build-arg UAGENT_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "uagent:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "uagent.json"),
  ];
