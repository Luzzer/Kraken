...
function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    HOME: params.homeDir,
    UAGENT_HOME: params.homeDir,
    UAGENT_CONFIG_PATH: params.configPath,
    UAGENT_STATE_DIR: params.stateDir,
    UAGENT_OAUTH_DIR: path.join(params.stateDir, "credentials"),
...
export async function startQaGatewayChild(params: {
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-qa-suite-"));
  ...
  const configPath = path.join(tempRoot, "uagent.json");
  ...
  const keepTemp = process.env.UAGENT_QA_KEEP_TEMP === "1";
