...
import { buildAgentSessionKey } from "uagent/plugin-sdk/routing";
...
function createQaActionConfig(baseUrl: string): UAGENTConfig {
  return {
    channels: {
      "qa-channel": {
        enabled: true,
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.UAGENT_QA_DEBUG === "1") {
...
async function runQaCli(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      child.kill("SIGKILL");
      reject(new Error(`qa cli timed out: uagent ${args.join(" ")}`));
...
async function callPluginToolsMcp(params: {
  ...
  const client = new Client({ name: "uagent-qa-suite", version: "0.0.0" }, {});
  try {
          ...
          },
          {
            ...
            run: async () => {
              state.addInboundMessage({
                ...
                senderId: "alice",
                senderName: "Alice",
                text: "@uagent explain the QA lab",
              });
        ...
        await runScenario("Threaded follow-up", [
          {
            ...
            run: async () => {
              ...
              state.addInboundMessage({
                ...
                senderId: "alice",
                senderName: "Alice",
                text: "@uagent reply in one short sentence inside this thread only. Do not use ACP or any external runtime. Confirm you stayed in-thread.",
                threadId,
        ...
        await runScenario("Memory tools in channel context", [
          {
            ...
            run: async () => {
              ...
              const prompt =
                "@uagent Memory tools check: what is the hidden project codename stored only in memory? Use memory tools first.";
              state.addInboundMessage({
                conversation: { id: "qa-room", kind: "channel", title: "QA Room" },
                senderId: "alice",
                senderName: "Alice",
...
export async function runQaSuite(params?: {
  ...
  try {
    ...
    const report = renderQaMarkdownReport({
      title: "UAGENT QA Scenario Suite",
      startedAt,
      finishedAt,
      checks: [],
    ...
    const keepTemp = process.env.UAGENT_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
    });
    await mock?.stop();
