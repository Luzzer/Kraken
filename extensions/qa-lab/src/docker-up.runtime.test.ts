...
describe("runQaDockerUp", () => {
  it("builds the QA UI, writes the harness, starts compose, and waits for health", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        "pnpm qa:lab:build @/repo/uagent",
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/uagent`,
        expect.stringContaining(
          `docker compose -f ${outputDir}/docker-compose.qa.yml up --build -d @/repo/uagent`,
        ),
      ]);
  ...
  it("skips UI build and compose --build for prebuilt images", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/uagent`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/uagent`,
      ]);
    } finally {
      await rm(outputDir, { recursive: true, force: true });
    }
