...
import type { UAGENTConfig, UAGENTPluginApi } from "uagent/plugin-sdk/memory-core";
import type { MemorySearchResult } from "uagent/plugin-sdk/memory-core-host-runtime-files";
import {
...
} from "uagent/plugin-sdk/memory-core-host-status";
...
type Logger = Pick<UAGENTPluginApi["logger"], "info" | "warn" | "error">;
...
const LIGHT_SLEEP_EVENT_TEXT = "__uagent_memory_core_light_sleep__";

const REM_SLEEP_CRON_NAME = "Memory REM Dreaming";
const REM_SLEEP_CRON_TAG = "[managed-by=memory-core.dreaming.rem]";
const REM_SLEEP_EVENT_TEXT = "__uagent_memory_core_rem_sleep__";
...
const MANAGED_DAILY_DREAMING_BLOCKS = [
  {
    heading: "## Light Sleep",
    startMarker: "<!-- uagent:dreaming:light:start -->",
    endMarker: "<!-- uagent:dreaming:light:end -->",
  },
  {
    heading: "## REM Sleep",
    startMarker: "<!-- uagent:dreaming:rem:start -->",
    endMarker: "<!-- uagent:dreaming:rem:end -->",
  },
] as const;
...
export function registerMemoryDreamingPhases(api: UAGENTPluginApi): void {
  api.registerHook(
    "gateway:startup",
    async (event: unknown) => {
