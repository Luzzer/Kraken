...
vi.mock("./cli.host.runtime.js", async () => {
  const [runtimeCli, runtimeCore, runtimeFiles] = await Promise.all([
    import("uagent/plugin-sdk/memory-core-host-runtime-cli"),
    import("uagent/plugin-sdk/memory-core-host-runtime-core"),
    import("uagent/plugin-sdk/memory-core-host-runtime-files"),
  ]);
  return {
...
let defaultRuntime: typeof import("uagent/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("uagent/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("uagent/plugin-sdk/memory-core-host-runtime-cli").setVerbose;
let fixtureRoot = "";
...
beforeAll(async () => {
  ...
  ({ defaultRuntime, isVerbose, setVerbose } =
    await import("uagent/plugin-sdk/memory-core-host-runtime-cli"));
...
describe("memory cli", () => {
  ...
  it("shows the fix hint only before --fix has been run", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await runMemoryCli(["status"]);
      expect(log).toHaveBeenCalledWith(
        expect.stringContaining("Fix: uagent memory status --fix --agent main"),
      );
  ...
  it("applies top promote candidates into MEMORY.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(memoryText).toContain("uagent-memory-promotion:");
