...
import type { UAGENTConfig } from "uagent/plugin-sdk/memory-core";
...
describe("short-term dreaming cron reconciliation", () => {
  ...
  it("prunes legacy light/rem dreaming cron jobs during reconciliation", async () => {
    ...
    const legacyLightJob: CronJobLike = {
      ...
      sessionTarget: "main",
      wakeMode: "next-heartbeat",
      payload: { kind: "systemEvent", text: "__uagent_memory_core_light_sleep__" },
      createdAtMs: 8,
    };
    const legacyRemJob: CronJobLike = {
      ...
      sessionTarget: "main",
      wakeMode: "next-heartbeat",
      payload: { kind: "systemEvent", text: "__uagent_memory_core_rem_sleep__" },
      createdAtMs: 9,
    };
