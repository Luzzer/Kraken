...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("uagent-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- uagent:dreaming:diary:start -->");
    expect(content).toContain("<!-- uagent:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- uagent:dreaming:diary:start -->");
    const end = content.indexOf("<!-- uagent:dreaming:diary:end -->");
