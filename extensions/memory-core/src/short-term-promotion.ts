...
import { formatMemoryDreamingDay } from "uagent/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "uagent/plugin-sdk/memory-host-events";
import {
  deriveConceptTags,
  MAX_CONCEPT_TAGS,
...
const PROMOTION_MARKER_PREFIX = "uagent-memory-promotion:";
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*uagent-memory-promotion:([^\n]+?)\s*-->/gi);
