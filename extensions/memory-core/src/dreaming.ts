...
const DREAMING_SYSTEM_EVENT_TEXT = "__uagent_memory_core_short_term_promotion_dream__";
...
const LEGACY_LIGHT_SLEEP_EVENT_TEXT = "__uagent_memory_core_light_sleep__";
const LEGACY_REM_SLEEP_CRON_NAME = "Memory REM Dreaming";
const LEGACY_REM_SLEEP_CRON_TAG = "[managed-by=memory-core.dreaming.rem]";
const LEGACY_REM_SLEEP_EVENT_TEXT = "__uagent_memory_core_rem_sleep__";
...
export function registerShortTermPromotionDreaming(api: UAGENTPluginApi): void {
  api.registerHook(
    "gateway:startup",
    async (event: unknown) => {
      try {
