...
import {
  ...
  formatHelpExamples,
  theme,
} from "uagent/plugin-sdk/memory-core-host-runtime-cli";
import type {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["uagent memory status", "Show index and provider status."],
          [
            "uagent memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["uagent memory status --deep", "Probe embedding provider readiness."],
          ["uagent memory index --force", "Force a full reindex."],
          ['uagent memory search "meeting notes"', "Quick search using positional query."],
          [
            'uagent memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `uagent memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          ["uagent memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.uagent.ai/cli/memory")}\n`,
    );
