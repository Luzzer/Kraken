...
import { resolveMemoryDreamingConfig } from "uagent/plugin-sdk/memory-core-host-status";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: UAGENTConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: UAGENTConfig, enabled: boolean): UAGENTConfig {
...
function formatStatus(cfg: UAGENTConfig): string {
...
export function registerDreamingCommand(api: UAGENTPluginApi): void {
  api.registerCommand({
    name: "dreaming",
