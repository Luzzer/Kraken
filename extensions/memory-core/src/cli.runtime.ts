...
import { resolveMemoryRemDreamingConfig } from "uagent/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: UAGENTConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function formatDreamingSummary(cfg: UAGENTConfig): string {
...
function resolveAgent(cfg: UAGENTConfig, agent?: string) {
...
function resolveAgentIds(cfg: UAGENTConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: uagent memory status --fix --agent ${agentId}`)}`);
      }
    }
