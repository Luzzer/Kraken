...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): UAGENTConfig {
    const defaults: NonNullable<NonNullable<UAGENTConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: UAGENTConfig) {
