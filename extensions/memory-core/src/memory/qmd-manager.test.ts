...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("uagent.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("uagent.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("uagent.qmdEmbedQueueTail");
...
vi.mock("uagent/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("uagent/plugin-sdk/memory-core-host-engine-foundation")
  >("uagent/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("uagent/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/file-lock")>(
    "uagent/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { UAGENTConfig } from "uagent/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
  resolveMemoryBackendConfig,
} from "uagent/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: UAGENTConfig }) {
