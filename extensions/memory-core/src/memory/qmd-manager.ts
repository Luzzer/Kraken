...
import { withFileLock } from "uagent/plugin-sdk/file-lock";
import {
...
} from "uagent/plugin-sdk/memory-core-host-engine-foundation";
import {
  buildSessionEntry,
...
} from "uagent/plugin-sdk/memory-core-host-engine-qmd";
import {
  isFileMissingError,
  requireNodeSqlite,
  statRegularFile,
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so UAGENT search semantics match direct qmd search.
  return trimmed;
}
...
export class QmdMemoryManager implements MemorySearchManager {
  ...
  private readonly cfg: UAGENTConfig;
