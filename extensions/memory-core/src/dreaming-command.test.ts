...
function resolveStoredDreaming(config: UAGENTConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: UAGENTConfig = {}) {
  let command: UAGENTPluginCommandDefinition | undefined;
  let runtimeConfig: UAGENTConfig = initialConfig;

  const runtime = {
    config: {
      loadConfig: vi.fn(() => runtimeConfig),
      writeConfigFile: vi.fn(async (nextConfig: UAGENTConfig) => {
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: UAGENTPluginCommandDefinition) => {
      command = definition;
    }),
  } as unknown as UAGENTPluginApi;
