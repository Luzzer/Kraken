...
const LIGHT_DREAMING_TEST_CONFIG: UAGENTConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
function createHarness(config: UAGENTConfig, workspaceDir?: string) {
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("uagent-dreaming-phases-");
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__uagent_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
  ...
  it("records light/rem signals that reinforce deep promotion ranking", async () => {
    ...
    await beforeAgentReply(
      { cleanedBody: "__uagent_memory_core_rem_sleep__" },
      { trigger: "heartbeat", workspaceDir },
    );
