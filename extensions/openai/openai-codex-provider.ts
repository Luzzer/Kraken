...
import { buildOauthProviderAuthResult } from "uagent/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "uagent/plugin-sdk/provider-auth-login";
import {
  DEFAULT_CONTEXT_TOKENS,
  normalizeModelCompat,
  normalizeProviderId,
  type ProviderPlugin,
} from "uagent/plugin-sdk/provider-model-shared";
import { buildProviderStreamFamilyHooks } from "uagent/plugin-sdk/provider-stream-family";
import { fetchCodexUsage } from "uagent/plugin-sdk/provider-usage";
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `uagent models auth login --provider openai-codex` or `uagent configure`.";
}
