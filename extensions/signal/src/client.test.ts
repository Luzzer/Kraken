...
vi.mock("uagent/plugin-sdk/fetch-runtime", () => ({
  resolveFetch: (...args: unknown[]) => resolveFetchMock(...args),
}));

vi.mock("uagent/plugin-sdk/core", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/core")>(
    "uagent/plugin-sdk/core",
  );
  return {
    ...actual,
...
vi.mock("uagent/plugin-sdk/text-runtime", () => ({
