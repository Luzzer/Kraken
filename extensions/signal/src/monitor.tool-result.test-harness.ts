import type { MockFn } from "uagent/plugin-sdk/testing";
...
vi.mock("uagent/plugin-sdk/config-runtime", async () => {
  ...
  return {
    ...actual,
    loadConfig: () => config,
    resolveStorePath: vi.fn(() => "/tmp/uagent-sessions.json"),
...
vi.mock("uagent/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/conversation-runtime")>(
    "uagent/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("uagent/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("uagent/plugin-sdk/security-runtime")>(
    "uagent/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("uagent/plugin-sdk/reply-runtime"),
      import("uagent/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
