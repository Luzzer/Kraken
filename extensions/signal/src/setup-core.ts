...
import { formatCliCommand, formatDocsLink } from "uagent/plugin-sdk/setup-tools";
...
export const signalDmPolicy = {
  ...
  resolveConfigKeys: (cfg: UAGENTConfig, accountId?: string) =>
  ...
  getCurrent: (cfg: UAGENTConfig, accountId?: string) =>
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "UAGENT"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("uagent gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
