...
import { runPluginCommandWithTimeout } from "uagent/plugin-sdk/run-command";
import type { RuntimeEnv } from "uagent/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "uagent/plugin-sdk/setup-tools";
...
async function installSignalCliFromRelease(runtime: RuntimeEnv): Promise<SignalInstallResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-signal-"));
