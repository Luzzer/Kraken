...
function resolveQianfanPreset(cfg: UAGENTConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: UAGENTConfig) => {
...
export function applyQianfanProviderConfig(cfg: UAGENTConfig): UAGENTConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: UAGENTConfig): UAGENTConfig {
