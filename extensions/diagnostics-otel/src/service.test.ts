...
import type { UAGENTPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/uagent-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): UAGENTPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("uagent.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("uagent.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("uagent.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("uagent.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("uagent.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("uagent.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("uagent.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("uagent.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("uagent.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("uagent.webhook.processed");
    expect(spanNames).toContain("uagent.message.processed");
    expect(spanNames).toContain("uagent.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["uagent.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("uagent.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "uagent.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["uagent.reason"]).toBe("string");
    expect(String(attrs?.["uagent.reason"])).not.toContain(
