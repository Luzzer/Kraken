...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["uagentPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["uagent", "pollId"]) ??
    readNestedString(value, ["uagent", "poll", "id"]) ??
    readNestedString(value, ["data", "uagentPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "uagent", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "uagent poll vote",
          displayText: "Vote recorded",
          value: { uagentPollId: pollId, pollId },
        },
      },
    ],
  };
