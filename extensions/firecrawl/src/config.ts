...
type WebSearchConfig = NonNullable<UAGENTConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<UAGENTConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: UAGENTConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: UAGENTConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: UAGENTConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: UAGENTConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: UAGENTConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: UAGENTConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: UAGENTConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: UAGENTConfig, override?: number): number {
