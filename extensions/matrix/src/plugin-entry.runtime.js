...
const UAGENT_PLUGIN_SDK_PREFIX = ["uagent", "plugin-sdk"].join("/");
...
function findUAGENTPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "uagent" && pkg.exports?.["./plugin-sdk"]) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findUAGENTPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    aliasMap[UAGENT_PLUGIN_SDK_PREFIX] = rootAlias;
  }
  ...
  for (const exportKey of Object.keys(packageJson.exports ?? {})) {
    ...
    if (resolvedPath) {
      aliasMap[`${UAGENT_PLUGIN_SDK_PREFIX}/${subpath}`] = resolvedPath;
    }
  }
  ...
  if (extensionApi) {
    aliasMap["uagent/extension-api"] = extensionApi;
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findUAGENTPackageRoot(moduleDir);
  if (location) {
