...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<UAGENTConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<UAGENTConfig["channels"]>["matrix"]>>,
): UAGENTConfig {
  return {
    channels: {
      matrix: {
