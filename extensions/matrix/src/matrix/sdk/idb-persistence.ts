...
import { withFileLock } from "uagent/plugin-sdk/infra-runtime";
...
// `uagent matrix verify bootstrap`) can corrupt each other's state.
...
function resolveDefaultIdbSnapshotPath(): string {
  const stateDir =
    process.env.UAGENT_STATE_DIR || path.join(process.env.HOME || "/tmp", ".uagent");
