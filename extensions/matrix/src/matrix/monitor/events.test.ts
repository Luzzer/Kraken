...
describe("registerMatrixMonitorEvents verification routing", () => {
  ...
  it("adds self-device guidance when decrypt failures come from the same Matrix user", async () => {
    ...
    expect(logger.warn).toHaveBeenNthCalledWith(
      2,
      "matrix: failed to decrypt a message from this same Matrix user. This usually means another Matrix device did not share the room key, or another UAGENT runtime is using the same account. Check 'uagent matrix verify status --verbose --account ops' and 'uagent matrix devices list --account ops'.",
      {
        roomId: "!room:example.org",
        eventId: "$enc-self",
        sender: "@gumadeiras:matrix.example.org",
