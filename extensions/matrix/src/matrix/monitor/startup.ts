...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleUAGENTDevices.length > 0) {
      params.logger.warn(
        `matrix: stale UAGENT devices detected for ${params.auth.userId}: ${deviceHealth.staleUAGENTDevices.map((device) => device.deviceId).join(", ")}. Run 'uagent matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
