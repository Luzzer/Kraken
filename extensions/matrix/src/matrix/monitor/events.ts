...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another UAGENT runtime is using the same account. " +
    `Check 'uagent matrix verify status --verbose --account ${accountId}' and 'uagent matrix devices list --account ${accountId}'.`
  );
}
