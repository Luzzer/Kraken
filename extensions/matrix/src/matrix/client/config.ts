import { formatErrorMessage, type PinnedDispatcherPolicy } from "uagent/plugin-sdk/infra-runtime";
import { coerceSecretRef } from "uagent/plugin-sdk/provider-auth";
import { retryAsync } from "uagent/plugin-sdk/retry-runtime";
...
export async function resolveMatrixAuth(params?: {
  ...
  const login = (await retryMatrixAuthRequest("matrix auth login", async () => {
    return (await loginClient.doRequest("POST", "/_matrix/client/v3/login", undefined, {
      ...
      initial_device_display_name: resolved.deviceName ?? "UAGENT Gateway",
    })) as {
      access_token?: string;
      user_id?: string;
      device_id?: string;
