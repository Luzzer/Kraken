...
import { writeJsonFileAtomically } from "uagent/plugin-sdk/json-store";
import { resolveRequiredHomeDir } from "uagent/plugin-sdk/provider-auth";
...
const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "uagent-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  env?: NodeJS.ProcessEnv;
  outputDir?: string;
  createBackupArchive?: typeof import("uagent/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("uagent/plugin-sdk/runtime")).createBackupArchive;
