...
function fakeApi(): UAGENTPluginApi {
  return {
    config: {},
  } as UAGENTPluginApi;
}
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.uagent.ai", "", "uagent.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.uagent.ai", "uagent.ai"],
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as UAGENTConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
