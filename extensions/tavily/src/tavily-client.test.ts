...
vi.mock("uagent/plugin-sdk/provider-web-search", () => ({
...
vi.mock("uagent/plugin-sdk/security-runtime", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: uagent", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "uagent" });
  });

  it("runTavilyExtract sends X-Client-Source: uagent", async () => {
