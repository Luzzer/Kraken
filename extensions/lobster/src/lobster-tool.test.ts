...
import type { UAGENTPluginApi, UAGENTPluginToolContext } from "../runtime-api.js";
...
function fakeApi(overrides: Partial<UAGENTPluginApi> = {}): UAGENTPluginApi {
...
function fakeCtx(overrides: Partial<UAGENTPluginToolContext> = {}): UAGENTPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    ...
    const api = fakeApi();
    const factoryTool = (ctx: UAGENTPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
