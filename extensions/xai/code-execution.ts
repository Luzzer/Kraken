import { getRuntimeConfigSnapshot, type UAGENTConfig } from "@uagent/plugin-sdk/config-runtime";
import { jsonResult, readStringParam } from "@uagent/plugin-sdk/provider-web-search";
...
type _XaiPluginConfig = NonNullable<
  NonNullable<UAGENTConfig["plugins"]>["entries"]
>["xai"] extends {
  config?: infer Config;
}
  ? Config
...
function readPluginCodeExecutionConfig(cfg?: UAGENTConfig): CodeExecutionConfig | undefined {
...
export function createCodeExecutionTool(options?: {
  config?: UAGENTConfig;
  runtimeConfig?: UAGENTConfig | null;
}) {
    ...
    execute: async (_toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!apiKey) {
        return jsonResult({
          ...
          docs: "https://docs.uagent.ai/tools/code-execution",
        });
      }
