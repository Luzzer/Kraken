...
const UAGENT_OPENSHELL_E2E = process.env.UAGENT_E2E_OPENSHELL === "1";
const UAGENT_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const UAGENT_OPENSHELL_COMMAND =
  process.env.UAGENT_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "uagent-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && UAGENT_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: UAGENT_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(UAGENT_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "uagent-openshell-e2e-"));
      ...
      const gatewayName = `uagent-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `uagent-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/uagent-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: UAGENT_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("uagent-openshell-e2e");
