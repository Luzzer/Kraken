...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@uagent/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "uagent.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "uagent.plugin.json");
    ...
    if (packageJson.uagent && "extensions" in packageJson.uagent) {
      packageJson.uagent = {
        ...packageJson.uagent,
        extensions: rewritePackageExtensions(packageJson.uagent.extensions),
        ...(typeof packageJson.uagent.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.uagent.setupEntry) }
          : {}),
      };
    }
