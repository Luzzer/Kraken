...
APP_BUNDLE="${1:-dist/UAGENT.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
DISABLE_LIBRARY_VALIDATION="${DISABLE_LIBRARY_VALIDATION:-0}"
SKIP_TEAM_ID_CHECK="${SKIP_TEAM_ID_CHECK:-0}"
ENT_TMP_BASE=$(mktemp -t uagent-entitlements-base.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t uagent-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t uagent-entitlements-runtime.XXXXXX)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/UAGENT" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/UAGENT" "$APP_ENTITLEMENTS"
fi
