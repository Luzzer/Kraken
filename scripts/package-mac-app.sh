...
# Build and bundle UAGENT into a minimal .app we can open.
# Outputs to dist/UAGENT.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/UAGENT.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="UAGENT"
BUNDLE_ID="${BUNDLE_ID:-ai.uagent.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/uagent/uagent/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/UAGENT/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :UAGENTBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :UAGENTGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/UAGENT"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/UAGENT"
fi
chmod +x "$APP_ROOT/Contents/MacOS/UAGENT"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/UAGENT" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/UAGENT/Resources/UAGENT.icns" "$APP_ROOT/Contents/Resources/UAGENT.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/UAGENT/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying UAGENTKit resources"
UAGENTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/UAGENTKit_UAGENTKit.bundle"
if [ -d "$UAGENTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/UAGENTKit_UAGENTKit.bundle"
  cp -R "$UAGENTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/UAGENTKit_UAGENTKit.bundle"
else
  echo "WARN: UAGENTKit resource bundle not found at $UAGENTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running UAGENT"
killall -q UAGENT 2>/dev/null || true
