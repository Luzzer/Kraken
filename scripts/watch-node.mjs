...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[uagent] ${message}\n`);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.UAGENT_WATCH_MODE = "1";
  childEnv.UAGENT_WATCH_SESSION = watchSession;
  ...
  childEnv.UAGENT_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.UAGENT_WATCH_COMMAND = deps.args.join(" ");
  }
