...
# UAGENT Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://uagent.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one UAGENT."
...
GUM_VERSION="${UAGENT_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "UAGENT Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://uagent.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$UAGENT_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.uagent.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_uagent_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.uagent-* "$npm_root"/uagent 2>/dev/null || true
}

extract_uagent_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_uagent_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/uagent" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/uagent"|"/usr/local/bin/uagent")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/uagent/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale uagent symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing uagent binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing UAGENT package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_uagent_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "UAGENT npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*uagent" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_uagent_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_uagent_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "UAGENT npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an uagent binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES+=("Automation with intent: minimal fuss, maximal throughput.")
...
TAGLINES+=("Let's ship something useful, stable, and mildly responsible.")
TAGLINES+=("I'll smooth out your workflow until the rough edges stop mattering.")
...
TAGLINES+=("The assistant in your contacts you actually want to hear from.")
...
pick_tagline() {
    ...
    if [[ -n "${UAGENT_TAGLINE_INDEX:-}" ]]; then
        if [[ "${UAGENT_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((UAGENT_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${UAGENT_NO_ONBOARD:-0}
NO_PROMPT=${UAGENT_NO_PROMPT:-0}
DRY_RUN=${UAGENT_DRY_RUN:-0}
INSTALL_METHOD=${UAGENT_INSTALL_METHOD:-}
UAGENT_VERSION=${UAGENT_VERSION:-latest}
USE_BETA=${UAGENT_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/uagent"
GIT_DIR=${UAGENT_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${UAGENT_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${UAGENT_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${UAGENT_VERBOSE:-0}"
VERIFY_INSTALL="${UAGENT_VERIFY_INSTALL:-0}"
UAGENT_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
UAGENT installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://uagent.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/uagent)
...
Environment variables:
  UAGENT_INSTALL_METHOD=git|npm
  UAGENT_VERSION=latest|next|main|<semver>|<spec>
  UAGENT_BETA=0|1
  UAGENT_GIT_DIR=...
  UAGENT_GIT_UPDATE=0|1
  UAGENT_NO_PROMPT=1
  UAGENT_VERIFY_INSTALL=1
  UAGENT_DRY_RUN=1
  UAGENT_NO_ONBOARD=1
  UAGENT_VERBOSE=1
  UAGENT_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://uagent.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://uagent.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://uagent.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://uagent.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://uagent.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected UAGENT checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a UAGENT source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_uagent_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"uagent"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://uagent.ai/install.sh | bash"
}
...
ensure_uagent_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/uagent" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/uagent" ]]; then
        ln -sf "$npm_root/uagent/dist/entry.js" "${npm_bin}/uagent"
        ui_info "Created uagent bin link at ${npm_bin}/uagent"
    fi
    return 0
}

# Check for existing UAGENT installation
check_existing_uagent() {
    if [[ -n "$(type -P uagent 2>/dev/null || true)" ]]; then
        ui_info "Existing UAGENT installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make uagent show as \"command not found\" in new terminals."
...
warn_uagent_not_found() {
    ui_warn "Installed, but uagent is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t uagent 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named uagent; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_uagent_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P uagent 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/uagent" ]]; then
        echo "${npm_bin}/uagent"
        return 0
    fi
...
install_uagent_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/uagent/uagent.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing UAGENT from git checkout: ${repo_dir}"
    else
        ui_info "Installing UAGENT from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning UAGENT" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building UAGENT" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/uagent" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/uagent"
    ui_success "UAGENT wrapper installed to \$HOME/.local/bin/uagent"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view uagent dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:uagent/uagent#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_uagent() {
    local package_name="uagent"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            UAGENT_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="uagent"
        else
            UAGENT_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${UAGENT_VERSION}" ]]; then
        UAGENT_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${UAGENT_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${UAGENT_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing UAGENT v${resolved_version}"
    else
        ui_info "Installing UAGENT (${UAGENT_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${UAGENT_VERSION}")"

    if ! install_uagent_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_uagent_paths
        install_uagent_npm "${install_spec}"
    fi

    if [[ "${UAGENT_VERSION}" == "latest" && "${package_name}" == "uagent" ]]; then
        if ! resolve_uagent_bin &> /dev/null; then
            ui_warn "npm install uagent@latest failed; retrying uagent@next"
            cleanup_npm_uagent_paths
            install_uagent_npm "uagent@next"
        fi
    fi

    ensure_uagent_bin_link || true

    ui_success "UAGENT installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${UAGENT_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_uagent_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (uagent not on PATH yet)"
        warn_uagent_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${UAGENT_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.uagent/workspace-${profile}"
    else
        echo "${HOME}/.uagent/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${UAGENT_CONFIG_PATH:-$HOME/.uagent/uagent.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run uagent onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but uagent not on PATH; skipping onboarding"
        warn_uagent_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run uagent onboard to retry"
        return
    }
}
...
resolve_uagent_version() {
    local version=""
    local raw_version_output=""
    local claw="${UAGENT_BIN:-}"
    if [[ -z "$claw" ]] && command -v uagent &> /dev/null; then
        claw="$(command -v uagent)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_uagent_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/uagent/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/uagent/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: uagent not on PATH yet"
        warn_uagent_not_found
        return 1
    fi

    run_quiet_step "Checking UAGENT version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: uagent gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_uagent_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found UAGENT checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set UAGENT_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_uagent_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/uagent" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/uagent"
    ...
    UAGENT_BIN="$(resolve_uagent_bin || true)"
    ...
    installed_version=$(resolve_uagent_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "UAGENT installed successfully (${installed_version})!"
    else
        ui_celebrate "UAGENT installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/uagent"
        ui_kv "Update command" "uagent update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://uagent.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running uagent doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                UAGENT_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                UAGENT_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                UAGENT_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run uagent doctor and uagent plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run uagent onboard later"
        else
            local config_path="${UAGENT_CONFIG_PATH:-$HOME/.uagent/uagent.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (uagent not on PATH yet)"
                    warn_uagent_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run uagent onboard to finish setup"
            return 0
        fi
    fi

    if command -v uagent &> /dev/null; then
        local claw="${UAGENT_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_uagent_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (uagent daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if UAGENT_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: uagent daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${UAGENT_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
