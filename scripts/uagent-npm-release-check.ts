...
const EXPECTED_REPOSITORY_URL = "https://github.com/uagent/uagent";
...
const skipPackValidationEnv = "UAGENT_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "uagent") {
    errors.push(`package.json name must be "uagent"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.uagent !== "uagent.mjs") {
    errors.push(
      `package.json bin.uagent must be "uagent.mjs"; found "${pkg.bin?.uagent ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`uagent-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `uagent-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
