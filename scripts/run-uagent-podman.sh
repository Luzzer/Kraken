#!/usr/bin/env bash
# Rootless UAGENT in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-uagent-podman.sh launch        # Start gateway
#   ./scripts/run-uagent-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   uagent --container uagent dashboard --no-open
#   uagent --container uagent channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      UAGENT_GATEWAY_TOKEN|UAGENT_PODMAN_CONTAINER|UAGENT_PODMAN_IMAGE|UAGENT_IMAGE|UAGENT_PODMAN_PULL|UAGENT_PODMAN_GATEWAY_HOST_PORT|UAGENT_GATEWAY_PORT|UAGENT_PODMAN_BRIDGE_HOST_PORT|UAGENT_BRIDGE_PORT|UAGENT_GATEWAY_BIND|UAGENT_PODMAN_USERNS|UAGENT_BIND_MOUNT_OPTIONS|UAGENT_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-uagent-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${UAGENT_CONFIG_DIR:-$EFFECTIVE_HOME/.uagent}"
ENV_FILE="${UAGENT_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${UAGENT_CONFIG_DIR:-$EFFECTIVE_HOME/.uagent}"
ENV_FILE="${UAGENT_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${UAGENT_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${UAGENT_PODMAN_CONTAINER:-uagent}"
UAGENT_IMAGE="${UAGENT_PODMAN_IMAGE:-${UAGENT_IMAGE:-uagent:local}}"
PODMAN_PULL="${UAGENT_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${UAGENT_PODMAN_GATEWAY_HOST_PORT:-${UAGENT_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${UAGENT_PODMAN_BRIDGE_HOST_PORT:-${UAGENT_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${UAGENT_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$UAGENT_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v uagent >/dev/null 2>&1; then
    return 0
  fi
  UAGENT_CONTAINER="" UAGENT_CONFIG_DIR="$config_dir" \
    uagent config get gateway.bind 2>/dev/null || true
}
...
# UAGENT_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${UAGENT_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v uagent >/dev/null 2>&1; then
    echo "Warning: uagent not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    UAGENT_CONTAINER="" UAGENT_CONFIG_DIR="$config_dir" \
      uagent config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    UAGENT_CONTAINER="" UAGENT_CONFIG_DIR="$config_dir" \
      uagent config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  UAGENT_CONTAINER="" UAGENT_CONFIG_DIR="$config_dir" \
    uagent config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${UAGENT_GATEWAY_TOKEN:-}" ]]; then
  export UAGENT_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "UAGENT_GATEWAY_TOKEN" "$UAGENT_GATEWAY_TOKEN"
  echo "Generated UAGENT_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/uagent.json"
...
PODMAN_USERNS="${UAGENT_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported UAGENT_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (UAGENT_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${UAGENT_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${UAGENT_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$UAGENT_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.uagent/.npm \
    -e UAGENT_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.uagent:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.uagent/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$UAGENT_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME uagent dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME uagent devices approve --latest  # if pairing required"
