...
BASE_IMAGE="${BASE_IMAGE:-uagent-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-uagent-sandbox-common:bookworm-slim}"
...
UAGENT_DOCKER_BUILD_USE_BUILDX="${UAGENT_DOCKER_BUILD_USE_BUILDX:-0}"
UAGENT_DOCKER_BUILD_CACHE_FROM="${UAGENT_DOCKER_BUILD_CACHE_FROM:-}"
UAGENT_DOCKER_BUILD_CACHE_TO="${UAGENT_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${UAGENT_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${UAGENT_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${UAGENT_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${UAGENT_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${UAGENT_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=uagent.sandbox=1)
NOTE
