...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/uagent/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "UAGENT Re-Auth" -c "After getting key, run: ssh $SERVER '~/uagent/scripts/mobile-reauth.sh'" --id uagent-auth
        ;;
    *)
