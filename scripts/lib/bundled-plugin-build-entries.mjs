...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.uagent?.extensions)
    ? packageJson.uagent.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.uagent?.setupEntry === "string" &&
    packageJson.uagent.setupEntry.trim().length > 0
      ? packageJson.uagent.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "uagent.plugin.json"));
    }
    if (hasPackageJson) {
