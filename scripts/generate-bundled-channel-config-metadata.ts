...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const uagentMeta =
    ...
    "uagent" in source.packageJson
      ? (source.packageJson.uagent as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    uagentMeta &&
    typeof uagentMeta.channel === "object" &&
    uagentMeta.channel &&
    !Array.isArray(uagentMeta.channel)
      ? (uagentMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
