...
run_protocol_ci_mirror() {
  local targets=(
    ...
    "apps/macos/Sources/UAGENTProtocol/GatewayModels.swift"
    "apps/shared/UAGENTKit/Sources/UAGENTProtocol/GatewayModels.swift"
  )
  local before after
...
has_native_swift_changes() {
  if git rev-parse --verify --quiet origin/main >/dev/null; then
    if git diff --name-only --relative origin/main...HEAD -- apps/macos apps/ios apps/shared/UAGENTKit | rg -q .; then
      return 0
    fi
  fi

  if git rev-parse --verify --quiet HEAD^ >/dev/null; then
    git diff --name-only --relative HEAD^..HEAD -- apps/macos apps/ios apps/shared/UAGENTKit | rg -q .
    return $?
  fi

  git show --name-only --relative --pretty='' HEAD -- apps/macos apps/ios apps/shared/UAGENTKit | rg -q .
}
...
run_linux_ci_mirror() {
  ...
  log_step "UAGENT_VITEST_MAX_WORKERS=${UAGENT_VITEST_MAX_WORKERS:-1} NODE_OPTIONS=${NODE_OPTIONS:---max-old-space-size=6144} pnpm test"
  UAGENT_VITEST_MAX_WORKERS="${UAGENT_VITEST_MAX_WORKERS:-1}" \
  NODE_OPTIONS="${NODE_OPTIONS:---max-old-space-size=6144}" \
    pnpm test
}

run_macos_ci_mirror() {
  if [[ "${UAGENT_PREPUSH_SKIP_MACOS:-0}" == "1" ]]; then
    log_step "Skipping macOS mirror because UAGENT_PREPUSH_SKIP_MACOS=1"
    return 0
  fi
