#!/bin/bash
# Setup UAGENT Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'uagent-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  UAGENT message: Send warning via UAGENT itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/uagent-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/uagent-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/uagent-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now uagent-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status uagent-auth-monitor.timer"
echo ""
echo "Quick commands:"
