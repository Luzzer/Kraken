...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.uagent) ? packageJson.uagent : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.uagent?.channel?.id) || left.name;
    const rightId = trimString(right.uagent?.channel?.id) || right.name;
