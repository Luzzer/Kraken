...
const warningFilterKey = Symbol.for("uagent.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadUAGENTPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadUAGENTPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@uagent/build-smoke-plugin",
      type: "module",
      uagent: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "uagent.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'uagent/plugin-sdk';",
...
const registry = loadUAGENTPlugins({
  ...
  env: {
    ...process.env,
    UAGENT_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
