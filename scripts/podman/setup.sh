#!/usr/bin/env bash
# One-time host setup for rootless UAGENT in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.uagent by default, and uses the repo-local
# launch script at ./scripts/run-uagent-podman.sh.
#
...
#   Or set UAGENT_PODMAN_QUADLET=1 (or 0) to choose without a flag.
#
# After this, start the gateway manually:
#   ./scripts/run-uagent-podman.sh launch
#   ./scripts/run-uagent-podman.sh launch setup
# Or, if you used --quadlet:
#   systemctl --user start uagent.service
set -euo pipefail

REPO_PATH="${UAGENT_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-uagent-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/uagent.container.in"
UAGENT_USER="$(id -un)"
UAGENT_HOME="${HOME:-}"
UAGENT_CONFIG_DIR="${UAGENT_CONFIG_DIR:-}"
UAGENT_WORKSPACE_DIR="${UAGENT_WORKSPACE_DIR:-}"
UAGENT_IMAGE="${UAGENT_PODMAN_IMAGE:-${UAGENT_IMAGE:-uagent:local}}"
UAGENT_CONTAINER_NAME="${UAGENT_PODMAN_CONTAINER:-uagent}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${UAGENT_PODMAN_GATEWAY_HOST_PORT:-${UAGENT_GATEWAY_PORT:-18789}}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate UAGENT_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -n "${UAGENT_PODMAN_QUADLET:-}" ]]; then
  case "${UAGENT_PODMAN_QUADLET,,}" in
...
if [[ "$UAGENT_IMAGE" == "uagent:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set UAGENT_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$UAGENT_HOME" ]]; then
  UAGENT_HOME="$(resolve_user_home "$UAGENT_USER")"
fi
if [[ -z "$UAGENT_HOME" ]]; then
  echo "Unable to resolve HOME for user $UAGENT_USER." >&2
  exit 1
fi
if [[ -z "$UAGENT_CONFIG_DIR" ]]; then
  UAGENT_CONFIG_DIR="$UAGENT_HOME/.uagent"
fi
if [[ -z "$UAGENT_WORKSPACE_DIR" ]]; then
  UAGENT_WORKSPACE_DIR="$UAGENT_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$UAGENT_HOME"
validate_mount_source_path "config directory" "$UAGENT_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$UAGENT_WORKSPACE_DIR"
validate_container_name "$UAGENT_CONTAINER_NAME"
validate_image_name "$UAGENT_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$UAGENT_CONFIG_DIR" "$UAGENT_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$UAGENT_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$UAGENT_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${UAGENT_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "UAGENT_DOCKER_APT_PACKAGES=${UAGENT_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${UAGENT_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "UAGENT_EXTENSIONS=${UAGENT_EXTENSIONS}")
fi

if [[ "$UAGENT_IMAGE" == "uagent:local" ]]; then
  echo "Building image $UAGENT_IMAGE ..."
  podman build -t "$UAGENT_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$UAGENT_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $UAGENT_IMAGE"
  else
    echo "Pulling image $UAGENT_IMAGE ..."
    podman pull "$UAGENT_IMAGE"
  fi
fi

ENV_FILE="$UAGENT_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
  TOKEN="$(generate_token_hex_32)"
  (
    umask 077
    write_file_atomically "$ENV_FILE" 600 <<EOF
UAGENT_GATEWAY_TOKEN=$TOKEN
EOF
  )
  echo "Generated UAGENT_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "UAGENT_PODMAN_CONTAINER" "$UAGENT_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "UAGENT_PODMAN_IMAGE" "$UAGENT_IMAGE"

CONFIG_JSON="$UAGENT_CONFIG_DIR/uagent.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  QUADLET_DIR="$UAGENT_HOME/.config/containers/systemd"
  QUADLET_DST="$QUADLET_DIR/uagent.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  UAGENT_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$UAGENT_HOME")"
  UAGENT_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$UAGENT_CONFIG_DIR")"
  UAGENT_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$UAGENT_WORKSPACE_DIR")"
  UAGENT_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$UAGENT_IMAGE")"
  UAGENT_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$UAGENT_CONTAINER_NAME")"
  sed \
    -e "s|{{UAGENT_HOME}}|$UAGENT_HOME_ESCAPED|g" \
    -e "s|{{UAGENT_CONFIG_DIR}}|$UAGENT_CONFIG_ESCAPED|g" \
    -e "s|{{UAGENT_WORKSPACE_DIR}}|$UAGENT_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$UAGENT_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$UAGENT_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start uagent.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start uagent.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-uagent-podman.sh launch"
echo "  ./scripts/run-uagent-podman.sh launch setup"
echo "  uagent --container $UAGENT_CONTAINER_NAME dashboard --no-open"
