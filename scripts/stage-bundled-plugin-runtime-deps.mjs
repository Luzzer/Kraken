...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.uagent) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies.uagent;
  ...
  if (packageJson.peerDependenciesMeta?.uagent) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta.uagent;
  ...
  if (packageJson.devDependencies?.uagent) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies.uagent;
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".uagent-runtime-deps-stamp.json");
}
...
function stageInstalledRootRuntimeDeps(params) {
  ...
  const stagedNodeModulesDir = path.join(
    makeTempDir(
      os.tmpdir(),
      `uagent-runtime-deps-${sanitizeTempPrefixSegment(path.basename(pluginDir))}-`,
    ),
    "node_modules",
  );
...
function installPluginRuntimeDeps(params) {
  ...
  const tempInstallDir = makeTempDir(
    os.tmpdir(),
    `uagent-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}-`,
  );
