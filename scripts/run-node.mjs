...
const extensionRestartMetadataFiles = new Set(["uagent.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.UAGENT_FORCE_BUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by UAGENT_FORCE_BUILD",
...
const logRunner = (message, deps) => {
  if (deps.env.UAGENT_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[uagent] ${message}\n`);
};
...
const runUAGENT = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["uagent.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
const shouldSkipCleanWatchRuntimeSync = (deps) => deps.env.UAGENT_WATCH_MODE === "1";
