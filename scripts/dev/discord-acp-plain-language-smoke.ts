...
type DriverMode = "token" | "webhook" | "uagent";
...
function resolveStateDir(): string {
  const override = process.env.UAGENT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.UAGENT_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".uagent");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver uagent] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) UAGENT spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|uagent> Driver transport mode (default: token)\n" +
    ...
    "  --uagent-bin <path>        UAGENT CLI binary for driver=uagent (default: uagent)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  UAGENT_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  UAGENT_DISCORD_SMOKE_DRIVER\n" +
    "  UAGENT_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  UAGENT_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  UAGENT_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  UAGENT_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  UAGENT_DISCORD_SMOKE_AGENT\n" +
    "  UAGENT_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  UAGENT_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  UAGENT_DISCORD_SMOKE_POLL_MS\n" +
    "  UAGENT_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  UAGENT_DISCORD_SMOKE_UAGENT_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.UAGENT_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.UAGENT_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "uagent"
        ? "uagent"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.UAGENT_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.UAGENT_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.UAGENT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.UAGENT_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.UAGENT_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.UAGENT_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.UAGENT_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.UAGENT_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.UAGENT_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.UAGENT_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const uagentBin =
    resolveArg("--uagent-bin") || process.env.UAGENT_DISCORD_SMOKE_UAGENT_BIN || "uagent";
...
async function uagentCliJson<T>(params: { uagentBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.uagentBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`uagent ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await uagentCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    uagentBin: params.uagentBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "uagent") {
    return await readMessagesWithOpenclaw({
      uagentBin: params.args.uagentBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `uagent-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await uagentCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("uagent message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from UAGENT.`,
        diagnostics: {
          bindingCandidates: [
            {
