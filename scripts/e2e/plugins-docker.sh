...
if [ -f dist/index.mjs ]; then
  UAGENT_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  UAGENT_ENTRY="dist/index.js"
else
...
home_dir=$(mktemp -d "/tmp/uagent-plugins-e2e.XXXXXX")
export HOME="$home_dir"
BUNDLED_PLUGIN_ROOT_DIR="extensions"
UAGENT_PLUGIN_HOME="$HOME/.uagent/$BUNDLED_PLUGIN_ROOT_DIR"
...
const configPath = path.join(process.env.HOME, ".uagent", "uagent.json");
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$UAGENT_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$UAGENT_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$UAGENT_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/uagent.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/uagent.plugin.json" "$id"
const fs = require("node:fs");
...
demo_plugin_root="$UAGENT_PLUGIN_HOME/$demo_plugin_id"
...
JS
cat > "$demo_plugin_root/uagent.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$UAGENT_ENTRY" plugins list --json > /tmp/plugins.json
node "$UAGENT_ENTRY" plugins inspect demo-plugin --json > /tmp/plugins-inspect.json
...
pack_dir="$(mktemp -d "/tmp/uagent-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@uagent/demo-plugin-tgz",
  "version": "0.0.1",
  "uagent": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/uagent.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$UAGENT_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$UAGENT_ENTRY" plugins list --json > /tmp/plugins2.json
node "$UAGENT_ENTRY" plugins inspect demo-plugin-tgz --json > /tmp/plugins2-inspect.json
...
dir_plugin="$(mktemp -d "/tmp/uagent-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@uagent/demo-plugin-dir",
  "version": "0.0.1",
  "uagent": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/uagent.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$UAGENT_ENTRY" plugins install "$dir_plugin"
node "$UAGENT_ENTRY" plugins list --json > /tmp/plugins3.json
node "$UAGENT_ENTRY" plugins inspect demo-plugin-dir --json > /tmp/plugins3-inspect.json
...
file_pack_dir="$(mktemp -d "/tmp/uagent-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@uagent/demo-plugin-file",
  "version": "0.0.1",
  "uagent": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/uagent.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$UAGENT_ENTRY" plugins install "file:$file_pack_dir/package"
node "$UAGENT_ENTRY" plugins list --json > /tmp/plugins4.json
node "$UAGENT_ENTRY" plugins inspect demo-plugin-file --json > /tmp/plugins4-inspect.json
...
bundle_root="$UAGENT_PLUGIN_HOME/$bundle_plugin_id"
...
gateway_log="/tmp/uagent-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/uagent-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@uagent/slash-install-plugin",
  "version": "0.0.1",
  "uagent": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/uagent.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "uagent/fixture-marketplace"
    }
  }
}
JSON

node "$UAGENT_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$UAGENT_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$UAGENT_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$UAGENT_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
node "$UAGENT_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-shortcut-inspect.json
node "$UAGENT_ENTRY" plugins inspect marketplace-direct --json > /tmp/plugins-marketplace-direct-inspect.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$UAGENT_ENTRY" plugins update marketplace-shortcut --dry-run
node "$UAGENT_ENTRY" plugins update marketplace-shortcut
node "$UAGENT_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
node "$UAGENT_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-updated-inspect.json
