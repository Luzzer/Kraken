...
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://uagent.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_UAGENT_BIN="/opt/homebrew/bin/uagent"
GUEST_UAGENT_ENTRY="/opt/homebrew/lib/node_modules/uagent/uagent.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/uagent-parallels-smoke.XXXXXX)"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://uagent.ai/install.sh
...
resolve_guest_git_uagent_entry() {
  local guest_home
  guest_home="$(resolve_guest_current_user_home)"
  printf '%s/uagent/uagent.mjs\n' "$guest_home"
}
...
spawn {*}$cmd
send -- "printf '__UAGENT_READY__\\n'\r"
expect "__UAGENT_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/uagent-prl.sh <<'__UAGENT_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__UAGENT_SCRIPT__\r"
send -- "/bin/bash /tmp/uagent-prl.sh; rc=\$?; rm -f /tmp/uagent-prl.sh; printf '__UAGENT_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__UAGENT_RC__:(-?[0-9]+)} {
...
export UAGENT_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/uagent-install.sh
bash /tmp/uagent-install.sh${version_arg_q}
$GUEST_UAGENT_BIN --version
EOF
)"
}
...
ensure_guest_pnpm_for_dev_update() {
  ...
  bootstrap_root="/tmp/uagent-smoke-pnpm-bootstrap"
...
run_dev_channel_update() {
  ...
  bootstrap_bin="/tmp/uagent-smoke-pnpm-bootstrap/node_modules/.bin"
  guest_home="$(resolve_guest_current_user_home)"
  update_root="$guest_home/uagent"
  update_entry="$update_root/uagent.mjs"
  ensure_guest_pnpm_for_dev_update
  printf 'update-dev: run\n'
  guest_current_user_sh "$(cat <<EOF
rm -rf $(shell_quote "$update_root")
export PATH=$(shell_quote "$bootstrap_bin:$GUEST_EXEC_PATH")
$GUEST_NODE_BIN $GUEST_UAGENT_ENTRY update --channel dev --yes --json
EOF
)"
...
verify_dev_channel_update() {
  ...
  update_entry="$(resolve_guest_git_uagent_entry)"
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_UAGENT_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
start_server() {
  ...
  ) >/tmp/uagent-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/uagent"; check_path "\$root/uagent/extensions"; if [ -d "\$root/uagent/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/uagent/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
verify_gateway() {
  guest_current_user_cli "$GUEST_UAGENT_BIN" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_cli "$GUEST_UAGENT_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_cli "$GUEST_UAGENT_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_cli "$GUEST_UAGENT_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_cli "$GUEST_UAGENT_BIN" models set "$MODEL_ID"
  guest_current_user_cli \
    /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" \
    "$GUEST_UAGENT_BIN" agent \
    --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_UAGENT_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from uagent dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL "\$dashboard_http_url" >/tmp/uagent-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>UAGENT Control</title>' /tmp/uagent-dashboard-smoke.html >/dev/null; then
      if grep -F '<uagent-app></uagent-app>' /tmp/uagent-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>UAGENT Control</title>' /tmp/uagent-dashboard-smoke.html >/dev/null
grep -F '<uagent-app></uagent-app>' /tmp/uagent-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/uagent-discord-token <<'__UAGENT_TOKEN__'
$DISCORD_TOKEN_VALUE
__UAGENT_TOKEN__
cat >/tmp/uagent-discord-guilds.json <<'__UAGENT_GUILDS__'
$guilds_json
__UAGENT_GUILDS__
token="\$(tr -d '\n' </tmp/uagent-discord-token)"
guilds_json="\$(cat /tmp/uagent-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_UAGENT_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_UAGENT_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_UAGENT_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_UAGENT_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_UAGENT_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_UAGENT_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_UAGENT_ENTRY channels status --probe --json
rm -f /tmp/uagent-discord-token /tmp/uagent-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "uagent-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "uagent-main-upgrade.tgz"
