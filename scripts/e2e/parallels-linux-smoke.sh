...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/uagent-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/uagent-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/uagent-install.sh
  guest_exec /usr/bin/env UAGENT_NO_ONBOARD=1 bash /tmp/uagent-install.sh "${version_args[@]}" --no-onboard
  guest_exec uagent --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec uagent --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" uagent onboard \
    --non-interactive \
    --mode local \
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "uagent gateway run" >/dev/null 2>&1 || true
rm -f /tmp/uagent-parallels-linux-gateway.log
setsid sh -lc 'exec env UAGENT_HOME=/root UAGENT_STATE_DIR=/root/.uagent UAGENT_CONFIG_PATH=/root/.uagent/uagent.json ${API_KEY_ENV}=${api_key_value_q} uagent gateway run --bind loopback --port 18789 --force >/tmp/uagent-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec uagent gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec uagent gateway status --deep --require-rpc
    return
  fi
  guest_exec uagent gateway status --deep
}

verify_local_turn() {
  guest_exec uagent models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" uagent agent \
    --local \
    --agent main \
    --message ping \
    --json
