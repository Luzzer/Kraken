...
PACKAGE_SPEC=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/uagent-parallels-npm-update.XXXXXX)"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: uagent@latest
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/uagent-main-$CURRENT_HEAD_SHORT.tgz"
  cp "$MAIN_TGZ_DIR/$pkg" "$MAIN_TGZ_PATH"
}

write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/uagent-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/uagent warnings do not
...
try {
  ...
  $tgz = Join-Path $env:TEMP 'uagent-main-update.tgz'
  ...
  $version = Invoke-CaptureLogged 'uagent --version' { & $uagent --version }
  ...
  Invoke-Logged 'uagent models set' { & $uagent models set $ModelId }
  ...
  Invoke-Logged 'uagent gateway restart' { & $uagent gateway restart }
  Start-Sleep -Seconds 5
  Write-ProgressLog 'update.gateway-status'
  Invoke-Logged 'uagent gateway status' { & $uagent gateway status --deep --require-rpc }
  Write-ProgressLog 'update.agent-turn'
  Invoke-CaptureLogged 'uagent agent' { & $uagent agent --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/uagent-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
matches = [match for match in matches if re.search(r"UAGENT \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="uagent-update-$RANDOM-$RANDOM.ps1"
  log_name="uagent-update-$RANDOM-$RANDOM.log"
  done_name="uagent-update-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/uagent-update-log-state.XXXXXX")"
  : >"$log_state_path"
...
run_macos_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$MACOS_VM" --current-user /usr/bin/tee /tmp/uagent-main-update.sh >/dev/null
set -euo pipefail
...
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/uagent-main-update.tgz
/opt/homebrew/bin/npm install -g /tmp/uagent-main-update.tgz
version="\$(/opt/homebrew/bin/uagent --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
/opt/homebrew/bin/uagent models set "$MODEL_ID"
...
/opt/homebrew/bin/uagent gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if /opt/homebrew/bin/uagent gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
/opt/homebrew/bin/uagent gateway status --deep --require-rpc
/usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /opt/homebrew/bin/uagent agent --agent main --session-id parallels-npm-update-macos-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$MACOS_VM" --current-user /bin/bash /tmp/uagent-main-update.sh
}
...
run_linux_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/uagent-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/uagent-main-update.tgz
npm install -g /tmp/uagent-main-update.tgz --no-fund --no-audit
version="\$(uagent --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
uagent models set "$MODEL_ID"
uagent agent --local --agent main --session-id parallels-npm-update-linux-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/uagent-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="uagent@$LATEST_VERSION"
fi
