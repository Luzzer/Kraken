...
IMAGE_NAME="uagent-openwebui-e2e"
...
MODEL="${UAGENT_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${UAGENT_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${UAGENT_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${UAGENT_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${UAGENT_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${UAGENT_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="uagent-openwebui-e2e-$$"
GW_NAME="uagent-openwebui-gateway-$$"
OW_NAME="uagent-openwebui-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "UAGENT_GATEWAY_TOKEN=$TOKEN" \
  -e "UAGENT_OPENWEBUI_MODEL=$MODEL" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/uagent-openwebui-config.XXXXXX.json)"
    UAGENT_CONFIG_BATCH_PATH="$batch_file" node - <<'"'"'NODE'"'"' "$openai_api_key"
const fs = require("node:fs");

const openaiApiKey = process.argv[2];
const batchPath = process.env.UAGENT_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.UAGENT_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.UAGENT_OPENWEBUI_MODEL },
];
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="UAGENT E2E" \
  -e WEBUI_SECRET_KEY="uagent-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="UAGENT E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="uagent/default" \
...
echo "Running Open WebUI -> UAGENT smoke..."
if ! docker exec \
