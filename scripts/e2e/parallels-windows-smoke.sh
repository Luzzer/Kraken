...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-uagent-native-e2e-2026-03-12"
MODE="both"
PROVIDER="openai"
API_KEY_ENV=""
AUTH_CHOICE=""
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://uagent.ai/install.ps1"
HOST_PORT="18426"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/uagent-parallels-windows.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/uagent-parallels-build.lock"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-uagent-native-e2e-2026-03-12"
  ...
  --install-url <url>        Installer URL for latest release. Default: https://uagent.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run uagent update --channel dev.
  --target-package-spec <npm-spec>
                             ...
                             Example: uagent@2026.3.13-beta.1
...
\$uagent = Join-Path \$env:APPDATA 'npm\uagent.cmd'
\$args = $args_literal
if ('${env_name_q}' -ne '') {
  Set-Item -Path ('Env:' + '${env_name_q}') -Value '${env_value_q}'
}
# uagent.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$uagent @args 2>&1
if (\$null -ne \$output) {
...
resolve_latest_version() {
  ...
  npm view uagent version --userconfig "$(mktemp)"
}
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "uagent-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_powershell "$(cat <<EOF
\$depsRoot = Join-Path \$env:LOCALAPPDATA 'UAGENT\deps'
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/uagent-main-$short_head.tgz"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/uagent-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
write_latest_install_runner_script() {
  ...
  WINDOWS_LATEST_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/uagent-install-latest.ps1"
...
try {
  ...
  & (Join-Path \$env:APPDATA 'npm\uagent.cmd') --version *>&1 | Tee-Object -FilePath \$LogPath -Append | Out-Null
  if (\$LASTEXITCODE -ne 0) {
    throw "uagent --version failed with exit code \$LASTEXITCODE"
  }
...
write_baseline_npm_install_runner_script() {
  WINDOWS_BASELINE_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/uagent-install-baseline-npm.ps1"
...
try {
  $portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'UAGENT\deps') 'portable-git') ''
  $env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
  $uagent = Join-Path $env:APPDATA 'npm\uagent.cmd'

  Write-ProgressLog 'install.start'
  Invoke-Logged 'npm install baseline release' {
    & npm.cmd install -g "uagent@$Version" --no-fund --no-audit --loglevel=error
  }

  Write-ProgressLog 'install.version'
  Invoke-Logged 'uagent --version' { & $uagent --version }
...
install_baseline_npm_release() {
  ...
  runner_name="uagent-install-baseline-$RANDOM-$RANDOM.ps1"
  log_name="uagent-install-baseline-$RANDOM-$RANDOM.log"
  done_name="uagent-install-baseline-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/uagent-install-baseline-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/uagent-install-baseline-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
...
install_latest_release() {
  ...
  runner_name="uagent-install-latest-$RANDOM-$RANDOM.ps1"
  log_name="uagent-install-latest-$RANDOM-$RANDOM.log"
  done_name="uagent-install-latest-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/uagent-install-latest-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
  startup_checked=0
...
install_main_tgz() {
  ...
  runner_name="uagent-install-$RANDOM-$RANDOM.ps1"
  log_name="uagent-install-$RANDOM-$RANDOM.log"
  done_name="uagent-install-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/uagent-install-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/uagent-install-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
...
write_dev_update_runner_script() {
  WINDOWS_DEV_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/uagent-update-dev.ps1"
...
try {
  ...
  $env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
  $uagent = Join-Path $env:APPDATA 'npm\uagent.cmd'
  $gitRoot = Join-Path $env:USERPROFILE 'uagent'
  $gitEntry = Join-Path $gitRoot 'uagent.mjs'
  ...
  Invoke-Logged 'uagent update --channel dev --yes --json' {
    & $uagent update --channel dev --yes --json
  }
  ...
  Invoke-Logged 'git uagent --version' { & node.exe $gitEntry --version }
  Invoke-Logged 'git uagent update status --json' { & node.exe $gitEntry update status --json }
...
run_dev_channel_update() {
  ...
  runner_name="uagent-update-dev-$RANDOM-$RANDOM.ps1"
  log_name="uagent-update-dev-$RANDOM-$RANDOM.log"
  done_name="uagent-update-dev-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/uagent-update-dev-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
  startup_checked=0
...
write_install_runner_script() {
  WINDOWS_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/uagent-install-main.ps1"
...
try {
  $env:PATH = "$env:LOCALAPPDATA\UAGENT\deps\portable-git\cmd;$env:LOCALAPPDATA\UAGENT\deps\portable-git\mingw64\bin;$env:LOCALAPPDATA\UAGENT\deps\portable-git\usr\bin;$env:PATH"
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_uagent "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
write_onboard_runner_script() {
  WINDOWS_ONBOARD_SCRIPT_PATH="$MAIN_TGZ_DIR/uagent-onboard-$PROVIDER.ps1"
...
try {
  \$uagent = Join-Path \$env:APPDATA 'npm\uagent.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice ${AUTH_CHOICE} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$uagent, \$LogPath)
...
run_ref_onboard() {
  ...
  runner_name="uagent-onboard-$RANDOM-$RANDOM.ps1"
  log_name="uagent-onboard-$RANDOM-$RANDOM.log"
  done_name="uagent-onboard-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
verify_gateway() {
  guest_run_uagent "" "" gateway status --deep --require-rpc
}
...
$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'UAGENT\deps') 'portable-git') ''
$env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
$gitEntry = Join-Path (Join-Path $env:USERPROFILE 'uagent') 'uagent.mjs'
if (-not (Test-Path $gitEntry)) {
...
run_gateway_daemon_action() {
  ...
  local runner_name log_name done_name done_status launcher_state
  local poll_rc state_rc log_rc start_seconds poll_deadline startup_checked
  runner_name="uagent-gateway-$action-$RANDOM-$RANDOM.ps1"
  log_name="uagent-gateway-$action-$RANDOM-$RANDOM.log"
  done_name="uagent-gateway-$action-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
try {
  \$uagent = Join-Path \$env:APPDATA 'npm\uagent.cmd'
  & \$uagent gateway $action *>&1 | Tee-Object -FilePath \$log -Append | Out-Null
...
show_gateway_status_compat() {
  if guest_run_uagent "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_uagent "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_uagent "" "" gateway status --deep
}

verify_turn() {
  guest_run_uagent "" "" models set "$MODEL_ID"
  guest_run_uagent "$API_KEY_ENV" "$API_KEY_VALUE" \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "uagent-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-baseline-package" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "uagent-main-upgrade.tgz" || return $?
