...
- regenerates apps/ios/UAGENT.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${UAGENT_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid UAGENT_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid UAGENT_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid UAGENT_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid UAGENT_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing UAGENT_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(UAGENT_URL_SLASH)$(UAGENT_URL_SLASH)#g'
)"
...
UAGENT_CODE_SIGN_STYLE = Automatic
UAGENT_DEVELOPMENT_TEAM = ${TEAM_ID}
UAGENT_IOS_SELECTED_TEAM = ${TEAM_ID}
UAGENT_APP_BUNDLE_ID = ai.uagent.client
UAGENT_SHARE_BUNDLE_ID = ai.uagent.client.share
UAGENT_ACTIVITY_WIDGET_BUNDLE_ID = ai.uagent.client.activitywidget
UAGENT_WATCH_APP_BUNDLE_ID = ai.uagent.client.watchkitapp
UAGENT_WATCH_EXTENSION_BUNDLE_ID = ai.uagent.client.watchkitapp.extension
UAGENT_APP_PROFILE =
UAGENT_SHARE_PROFILE =
UAGENT_PUSH_TRANSPORT = relay
UAGENT_PUSH_DISTRIBUTION = official
UAGENT_URL_SLASH = /
UAGENT_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
UAGENT_PUSH_APNS_ENVIRONMENT = production
EOF
