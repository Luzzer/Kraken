...
const WATCH_GATEWAY_SKIP_ENV = {
  UAGENT_DISABLE_BONJOUR: "1",
  UAGENT_SKIP_ACPX_RUNTIME: "1",
  UAGENT_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$UAGENT_WATCH_PID_FILE"',
    'mkdir -p "$UAGENT_HOME/.uagent"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}}}' > "$UAGENT_HOME/.uagent/uagent.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    UAGENT_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    UAGENT_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "uagent-gateway-watch-"));
