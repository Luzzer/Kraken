...
SMOKE_PREVIOUS_VERSION="${UAGENT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${UAGENT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="uagent"
PACKAGE_NAME="${UAGENT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (UAGENT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${UAGENT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${UAGENT_INSTALL_LATEST_OUT:-}"
fi
