 ...
 * `uagent update`. Use it to verify the completion installation flow
...
\`uagent update\`, \`uagent doctor\`, and \`uagent onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# UAGENT Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`uagent completion --install\` later to enable.`));
    return;
  }
