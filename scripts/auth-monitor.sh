...
#
# Suggested cron: */30 * * * * /home/admin/uagent/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send UAGENT notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., uagent-alerts)
...
STATE_FILE="$HOME/.uagent/auth-monitor-state"
...
send_notification() {
    ...
    # Send via UAGENT if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use uagent
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via UAGENT to $NOTIFY_PHONE..."
            uagent send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! UAGENT is down. Run: ssh l36 '~/uagent/scripts/mobile-reauth.sh'" "urgent"
    exit 1
