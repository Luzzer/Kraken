...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  UAGENT Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one UAGENT.$NC" -Level info
    Write-Host ""
}
...
function Install-UAGENTGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing UAGENT from git..." -Level info
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install UAGENT from git to $GitDir" -Level info
        } else {
            Install-UAGENTGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install UAGENT via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-UAGENTNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'uagent onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "UAGENT installed successfully!" -Level success
}
