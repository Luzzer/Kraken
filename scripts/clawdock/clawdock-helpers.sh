#!/usr/bin/env bash
# ClawDock - Docker helpers for UAGENT
# Inspired by Simon Willison's "Running UAGENT in Docker"
# https://til.simonwillison.net/llms/uagent-docker
#
# Installation:
...
# Common paths to check for UAGENT
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/uagent"
  "${HOME}/workspace/uagent"
  "${HOME}/projects/uagent"
  "${HOME}/dev/uagent"
  "${HOME}/code/uagent"
  "${HOME}/src/uagent"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found UAGENT at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/uagent"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ UAGENT not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/uagent/uagent.git ~/uagent"
    echo "  cd ~/uagent && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^UAGENT_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d uagent-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart uagent-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f uagent-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.uagent"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show uagent.json
  if [[ -f "${config_dir}/uagent.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/uagent.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/uagent.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No uagent.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec uagent-gateway \
    bash -c 'echo "alias uagent=\"./uagent.mjs\"" > /tmp/.bashrc_uagent && bash --rcfile /tmp/.bashrc_uagent'
}
...
clawdock-cli() {
  _clawdock_compose run --rm uagent-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build uagent-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d uagent-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build uagent-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "UAGENT_GATEWAY_TOKEN=$token" uagent-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" uagent-gateway \
    bash -c './uagent.mjs config set gateway.remote.token "$TOKEN" && ./uagent.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec uagent-gateway \
    bash -c "./uagent.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart uagent-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm uagent-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec uagent-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'uagent config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for UAGENT${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (uagent alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to uagent project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.uagent)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'uagent channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'uagent status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.uagent.ai${_CLR_RESET}"
  echo ""
}
