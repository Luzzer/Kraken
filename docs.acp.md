...
This document describes how the UAGENT ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`uagent acp` exposes an ACP agent over stdio and forwards prompts to a running
UAGENT Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing UAGENT Gateway
...
- ACP client spawns `uagent acp` and speaks ACP messages over stdio.
...
`uagent acp` resolves the Gateway URL and auth from CLI flags or config:
