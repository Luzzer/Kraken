...
final class MotionService: MotionServicing {
    func activities(params: UAGENTMotionActivityParams) async throws -> UAGENTMotionActivityPayload {
        ...
        let mapped: [UAGENTMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return UAGENTMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: UAGENTPedometerParams) async throws -> UAGENTPedometerPayload {
        ...
        let payload: UAGENTPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = UAGENTPedometerPayload(
