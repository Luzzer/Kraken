...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [UAGENTCapability.canvas.rawValue, UAGENTCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(UAGENTCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(UAGENTCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = UAGENTLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(UAGENTCapability.location.rawValue) }

        caps.append(UAGENTCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(UAGENTCapability.watch.rawValue)
        }
        caps.append(UAGENTCapability.photos.rawValue)
        caps.append(UAGENTCapability.contacts.rawValue)
        caps.append(UAGENTCapability.calendar.rawValue)
        caps.append(UAGENTCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(UAGENTCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            UAGENTScreenCommand.record.rawValue,
            UAGENTSystemCommand.notify.rawValue,
            UAGENTChatCommand.push.rawValue,
        ...
        if caps.contains(UAGENTCapability.camera.rawValue) {
            commands.append(UAGENTCameraCommand.list.rawValue)
            commands.append(UAGENTCameraCommand.snap.rawValue)
            commands.append(UAGENTCameraCommand.clip.rawValue)
        }
        if caps.contains(UAGENTCapability.location.rawValue) {
            commands.append(UAGENTLocationCommand.get.rawValue)
        }
        if caps.contains(UAGENTCapability.device.rawValue) {
            commands.append(UAGENTDeviceCommand.status.rawValue)
            commands.append(UAGENTDeviceCommand.info.rawValue)
        }
        if caps.contains(UAGENTCapability.watch.rawValue) {
            commands.append(UAGENTWatchCommand.status.rawValue)
            commands.append(UAGENTWatchCommand.notify.rawValue)
        }
        if caps.contains(UAGENTCapability.photos.rawValue) {
            commands.append(UAGENTPhotosCommand.latest.rawValue)
        }
        if caps.contains(UAGENTCapability.contacts.rawValue) {
            commands.append(UAGENTContactsCommand.search.rawValue)
            commands.append(UAGENTContactsCommand.add.rawValue)
        }
        if caps.contains(UAGENTCapability.calendar.rawValue) {
            commands.append(UAGENTCalendarCommand.events.rawValue)
            commands.append(UAGENTCalendarCommand.add.rawValue)
        }
        if caps.contains(UAGENTCapability.reminders.rawValue) {
            commands.append(UAGENTRemindersCommand.list.rawValue)
            commands.append(UAGENTRemindersCommand.add.rawValue)
        }
        if caps.contains(UAGENTCapability.motion.rawValue) {
            commands.append(UAGENTMotionCommand.activity.rawValue)
            commands.append(UAGENTMotionCommand.pedometer.rawValue)
        }
