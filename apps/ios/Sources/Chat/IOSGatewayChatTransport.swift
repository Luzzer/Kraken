...
struct IOSGatewayChatTransport: UAGENTChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.uagent", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> UAGENTChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(UAGENTChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> UAGENTChatHistoryPayload {
        ...
        return try JSONDecoder().decode(UAGENTChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [UAGENTChatAttachmentPayload]) async throws -> UAGENTChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [UAGENTChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(UAGENTChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(UAGENTGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: UAGENTGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: UAGENTChatEventPayload.self)
                        {
