...
final class ContactsService: ContactsServicing {
    ...
    func search(params: UAGENTContactsSearchParams) async throws -> UAGENTContactsSearchPayload {
        ...
        return UAGENTContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: UAGENTContactsAddParams) async throws -> UAGENTContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return UAGENTContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return UAGENTContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> UAGENTContactPayload {
        UAGENTContactPayload(
