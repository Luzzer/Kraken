...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> UAGENTDeviceStatusPayload {
        ...
        return UAGENTDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> UAGENTDeviceInfoPayload {
        ...
        return UAGENTDeviceInfoPayload(
    ...
    private func batteryStatus() -> UAGENTBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: UAGENTBatteryState = switch device.batteryState {
        ...
        return UAGENTBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> UAGENTThermalStatusPayload {
        let state: UAGENTThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return UAGENTThermalStatusPayload(state: state)
    }

    private func storageStatus() -> UAGENTStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return UAGENTStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
