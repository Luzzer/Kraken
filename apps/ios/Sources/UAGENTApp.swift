...
@MainActor
enum UAGENTAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class UAGENTAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.uagent.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.uagent.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.uagent.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? UAGENTAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "uagent.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "uagent.watch.promptId"
    static let sessionKeyKey = "uagent.watch.sessionKey"
    static let actionPrimaryIDKey = "uagent.watch.action.primary.id"
    static let actionPrimaryLabelKey = "uagent.watch.action.primary.label"
    static let actionSecondaryIDKey = "uagent.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "uagent.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "uagent.watch.action.primary"
    static let actionSecondaryIdentifier = "uagent.watch.action.secondary"
    static let actionIdentifierPrefix = "uagent.watch.action."
    static let actionIDKeyPrefix = "uagent.watch.action.id."
    static let actionLabelKeyPrefix = "uagent.watch.action.label."
    static let categoryPrefix = "uagent.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: UAGENTWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> UAGENTWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return UAGENTWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "UAGENT" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [UAGENTWatchAction]) -> [UNNotificationAction] {
...
@main
struct UAGENTApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(UAGENTAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        UAGENTAppModelRegistry.appModel = appModel
...
extension UAGENTApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("UAGENT: installing uncaught exception handler")
