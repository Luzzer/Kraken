...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__UAGENT_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<UAGENTTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.uagent", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = UAGENTTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> UAGENTTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return UAGENTTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> UAGENTTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> UAGENTTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return UAGENTTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> UAGENTTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = UAGENTTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: UAGENTTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: UAGENTAgentEventPayload.self
            ) else {
                continue
            }
