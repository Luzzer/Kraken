...
final class RemindersService: RemindersServicing {
    func list(params: UAGENTRemindersListParams) async throws -> UAGENTRemindersListPayload {
        ...
        let payload: [UAGENTReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return UAGENTRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: UAGENTRemindersAddParams) async throws -> UAGENTRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = UAGENTReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return UAGENTRemindersAddPayload(reminder: payload)
    }
