...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: UAGENTWatchNotifyParams) -> UAGENTWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [UAGENTWatchAction]?,
        kind: String?,
        promptId: String?) -> [UAGENTWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> UAGENTWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return UAGENTWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                UAGENTWatchAction(id: "approve", label: "Approve"),
                UAGENTWatchAction(id: "decline", label: "Decline", style: "destructive"),
                UAGENTWatchAction(id: "open_phone", label: "Open iPhone"),
                UAGENTWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            UAGENTWatchAction(id: "done", label: "Done"),
            UAGENTWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            UAGENTWatchAction(id: "open_phone", label: "Open iPhone"),
            UAGENTWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: UAGENTWatchRisk?,
        priority: UAGENTNotificationPriority?) -> UAGENTWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: UAGENTNotificationPriority?,
        risk: UAGENTWatchRisk?) -> UAGENTNotificationPriority?
    {
