...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.uagent.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to UAGENT", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to UAGENT gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to UAGENT.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "UAGENTShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "UAGENT is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "UAGENT Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("uagent-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
