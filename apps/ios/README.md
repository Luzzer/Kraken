# UAGENT iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an UAGENT Gateway as a `role: node`.
...
- Beta release uses canonical `ai.uagent.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `UAGENTPushTransport=relay`, `UAGENTPushDistribution=official`, and `UAGENTPushAPNsEnvironment=production`.
...
- `UAGENT_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export UAGENT_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
6. Expected outputs after a successful run:
   - `apps/ios/build/beta/UAGENT-<version>.ipa`
   - `apps/ios/build/beta/UAGENT-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/UAGENT.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `UAGENTPushTransport=direct` and `UAGENTPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `UAGENT_APNS_TEAM_ID`, `UAGENT_APNS_KEY_ID`, and either `UAGENT_APNS_PRIVATE_KEY_P8` or `UAGENT_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.uagent/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `UAGENT_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `UAGENTPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `uagent.json`. `UAGENT_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine UAGENT official builds and to ensure a
