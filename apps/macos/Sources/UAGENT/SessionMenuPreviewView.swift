...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.uagent", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> UAGENTSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: UAGENTSessionPreviewEntry,
    ...
    private static func cache(payload: UAGENTSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: UAGENTChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [UAGENTKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [UAGENTKit.AnyCodable]) -> [UAGENTChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(UAGENTChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: UAGENTChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: UAGENTChatMessage) -> Bool {
    ...
    private static func toolNames(for message: UAGENTChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: UAGENTChatMessage) -> String? {
