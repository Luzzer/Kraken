...
enum DebugActions {
    private static let verboseDefaultsKey = "uagent.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "uagent.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = UAGENTPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "UAGENT", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = UAGENTPaths.configURL
        guard
