...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.uagent.mac`) even if the shipped
    ...
    {
        if environment["UAGENT_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "uagent.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "uagent.nixMode") { return true }
