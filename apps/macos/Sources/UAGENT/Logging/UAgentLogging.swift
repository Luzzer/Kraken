...
enum UAGENTLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = UAGENTOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = UAGENTFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        UAGENTLogging.bootstrapIfNeeded()
        let label = UAGENTLogging.makeLabel(subsystem: subsystem, category: category)
...
struct UAGENTOSLogHandler: AppLogLevelBackedHandler {
...
struct UAGENTFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = UAGENTLogging.parseLabel(self.label)
