...
private let webChatSwiftLogger = Logger(subsystem: "ai.uagent", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "uagent.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: UAGENTChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> UAGENTChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(UAGENTChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            UAGENTChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return UAGENTChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> UAGENTChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                UAGENTGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: UAGENTProtocol.ModelChoice) -> UAGENTChatModelChoice {
        UAGENTChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<UAGENTChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any UAGENTChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = UAGENTChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: UAGENTChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<UAGENTChatView>) -> NSViewController
    {
