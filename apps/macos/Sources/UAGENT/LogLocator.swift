...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["UAGENT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/uagent")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("uagent-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("uagent-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/uagent/ (rolling or stdout), or nil if none exist.
