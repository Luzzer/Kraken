...
private let deepLinkLogger = Logger(subsystem: "ai.uagent", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "UAGENT is paused", message: "Unpause UAGENT to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run UAGENT agent?", message: body) else { return }
        }
