...
enum CommandResolver {
    private static let projectRootDefaultsKey = "uagent.gatewayProjectRootPath"
    private static let helperName = "uagent"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let uagentEntry = root.appendingPathComponent("uagent.mjs").path
        if FileManager().isReadableFile(atPath: uagentEntry) { return uagentEntry }
        let binEntry = root.appendingPathComponent("bin/uagent.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/uagent")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let uagentPaths = self.uagentManagedPaths(home: home)
        if !uagentPaths.isEmpty {
            extras.insert(contentsOf: uagentPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + uagentPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func uagentManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".uagent"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func uagentExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectUAGENTExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("uagent.mjs").path,
            root.appendingPathComponent("bin/uagent.js").path,
        ]
    ...
    static func hasAnyUAGENTInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.uagentExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let uagentPath = self.projectUAGENTExecutable(projectRoot: root) {
            return [uagentPath, subcommand] + extraArgs
        }
        if let uagentPath = self.uagentExecutable(searchPaths: searchPaths) {
            return [uagentPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "uagent", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            uagent entrypoint missing (looked for dist/index.js or uagent.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real uagent CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/uagent"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/uagent.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/uagent.mjs"
            node "$PRJ/uagent.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/uagent.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/uagent.js"
            node "$PRJ/bin/uagent.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent uagent"
          pnpm --silent uagent \(quotedArgs);
        else
          echo "uagent CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? UAGENTConfigFile.loadDict()
