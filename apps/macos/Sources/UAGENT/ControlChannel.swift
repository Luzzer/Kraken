...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: UAGENTProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.uagent", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: UAGENTKit.AnyCodable]()) {
                $0[$1.key] = UAGENTKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: UAGENTProtocol.AnyCodable?) -> [String: UAGENTProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: UAGENTProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: UAGENTKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: UAGENTProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("uagent.control.heartbeat")
    static let controlAgentEvent = Notification.Name("uagent.control.agent")
}
