...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.uagent", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["UAGENT_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = UAGENTConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.uagentExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "uagent CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["UAGENT_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "uagent@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "uagent@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "uagent@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "uagent@\(target)"])
            }

        statusHandler("Installing uagent@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed uagent@\(target)")
        } else {
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("uagent ") {
            normalized = String(normalized.dropFirst("uagent ".count))
        }
