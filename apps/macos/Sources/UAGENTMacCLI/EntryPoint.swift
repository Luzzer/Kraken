...
struct UAGENTMacCLI {
    static func main() async {
        ...
        default:
            fputs("uagent-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      uagent-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      uagent-mac discover [--timeout <ms>] [--json] [--include-local]
      uagent-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      uagent-mac connect
      uagent-mac connect --url ws://127.0.0.1:18789 --json
      uagent-mac discover --timeout 3000 --json
      uagent-mac wizard --mode local
    """)
}
