...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: UAGENTProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: UAGENTProtocol.AnyCodable] = [
            "presence": .init([UAGENTProtocol.AnyCodable(entry)]),
        ]
        let payload = UAGENTProtocol.AnyCodable(payloadMap)
