...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey uagent do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["uagent"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "uagent", start: 0.2, duration: 0.1, range: transcript.range(of: "uagent")),
