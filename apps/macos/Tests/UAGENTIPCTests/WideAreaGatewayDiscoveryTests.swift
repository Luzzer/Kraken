...
import Foundation
import Testing
@testable import UAGENTDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("UAGENT_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("UAGENT_WIDE_AREA_DOMAIN", "uagent.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("UAGENT_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("UAGENT_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._uagent-gw._tcp.uagent.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.uagent.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (UAGENT)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/uagent/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (UAGENT)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/uagent/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._uagent-gw._tcp.uagent.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
