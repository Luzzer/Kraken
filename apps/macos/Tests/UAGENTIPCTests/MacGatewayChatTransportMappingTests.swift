...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: UAGENTProtocol.AnyCodable(["ok": UAGENTProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: UAGENTProtocol.AnyCodable(["ok": UAGENTProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = UAGENTProtocol.AnyCodable([
            "runId": UAGENTProtocol.AnyCodable("run-1"),
            "sessionKey": UAGENTProtocol.AnyCodable("main"),
            "state": UAGENTProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: UAGENTProtocol.AnyCodable(["a": UAGENTProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
