...
@Suite(.serialized)
struct UAGENTConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("uagent-config-\(UUID().uuidString)")
            .appendingPathComponent("uagent.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["UAGENT_CONFIG_PATH": override]) {
            #expect(UAGENTConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["UAGENT_CONFIG_PATH": override]) {
            ...
            #expect(UAGENTConfigFile.remoteGatewayPort() == 19999)
            #expect(UAGENTConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(UAGENTConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(UAGENTConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["UAGENT_CONFIG_PATH": override]) {
            ...
            UAGENTConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = UAGENTConfigFile.loadDict()
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["UAGENT_CONFIG_PATH": override]) {
            ...
            UAGENTConfigFile.clearRemoteGatewayUrl()
            let root = UAGENTConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("uagent-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "UAGENT_CONFIG_PATH": nil,
            "UAGENT_STATE_DIR": dir,
        ]) {
            #expect(UAGENTConfigFile.stateDirURL().path == dir)
            #expect(UAGENTConfigFile.url().path == "\(dir)/uagent.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("uagent-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("uagent.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "UAGENT_STATE_DIR": stateDir.path,
            "UAGENT_CONFIG_PATH": configPath.path,
        ]) {
            UAGENTConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-uagent-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let loaded = UAGENTConfigFile.loadDict()
