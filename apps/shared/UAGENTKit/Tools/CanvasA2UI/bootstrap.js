...
import "@a2ui/lit/ui";
import { themeContext } from "@uagent/a2ui-theme-context";
...
class UAGENTA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.uagentA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["uagent:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.uagentCanvasA2UIAction ??
      globalThis.uagentCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.uagentCanvasA2UIAction) {
...
if (!customElements.get("uagent-a2ui-host")) {
  customElements.define("uagent-a2ui-host", UAGENTA2UIHost);
}
