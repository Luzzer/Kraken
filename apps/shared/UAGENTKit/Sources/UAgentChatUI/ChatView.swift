...
@MainActor
public struct UAGENTChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: UAGENTChatViewModel
    ...
    private var visibleMessages: [UAGENTChatMessage] {
        let base: [UAGENTChatMessage]
    ...
    private func mergeToolResults(in messages: [UAGENTChatMessage]) -> [UAGENTChatMessage] {
    ...
    private func isToolResultMessage(_ message: UAGENTChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: UAGENTChatMessage) -> Bool {
    ...
    private func primaryText(in message: UAGENTChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: UAGENTChatMessage) -> Bool {
    ...
    private func toolCalls(in message: UAGENTChatMessage) -> [UAGENTChatMessageContent] {
    ...
    private func inlineToolResults(in message: UAGENTChatMessage) -> [UAGENTChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: UAGENTChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: UAGENTChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(UAGENTChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
