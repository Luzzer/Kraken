...
private let chatUILogger = Logger(subsystem: "ai.uagent", category: "UAGENTChatUI")

@MainActor
@Observable
public final class UAGENTChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [UAGENTChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [UAGENTChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [UAGENTPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [UAGENTChatPendingToolCall] = []
    public private(set) var sessions: [UAGENTChatSessionEntry] = []
    private let transport: any UAGENTChatTransport
    private var sessionDefaults: UAGENTChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: UAGENTChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any UAGENTChatTransport,
    ...
    public var sessionChoices: [UAGENTChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [UAGENTChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: UAGENTPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [UAGENTChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: UAGENTChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: UAGENTChatMessage) -> UAGENTChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> UAGENTChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return UAGENTChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: UAGENTChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: UAGENTChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: UAGENTChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [UAGENTChatMessage],
        incoming: [UAGENTChatMessage]) -> [UAGENTChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [UAGENTChatMessage]) -> [UAGENTChatMessage] {
        var result: [UAGENTChatMessage] = []
    ...
    private static func dedupeKey(for message: UAGENTChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [UAGENTChatMessageContent] = [
            UAGENTChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> UAGENTChatAttachmentPayload in
            UAGENTChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> UAGENTChatSessionEntry {
        UAGENTChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = UAGENTChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: UAGENTChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: UAGENTChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: UAGENTAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = UAGENTChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> UAGENTPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
