...
struct ChatMessageBubble: View {
    let message: UAGENTChatMessage
    let style: UAGENTChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? UAGENTChatTheme.userText : UAGENTChatTheme.assistantText
    ...
    private var inlineAttachments: [UAGENTChatMessageContent] {
    ...
    private var toolCalls: [UAGENTChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [UAGENTChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? UAGENTChatTheme.userBubble
        }
        if self.style == .onboarding {
            return UAGENTChatTheme.onboardingAssistantBubble
        }
        return UAGENTChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return UAGENTChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: UAGENTChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? UAGENTChatTheme.userText : UAGENTChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: UAGENTChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(UAGENTChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [UAGENTChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: UAGENTChatTheme.assistantText)
            }
        }
    }
}
