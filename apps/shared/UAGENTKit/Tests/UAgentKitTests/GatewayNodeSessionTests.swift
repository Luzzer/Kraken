...
struct GatewayNodeSessionTests {
    ...
    func scannedSetupCodePrefersBootstrapAuthOverStoredDeviceToken() async throws {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["UAGENT_STATE_DIR"]
        setenv("UAGENT_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("UAGENT_STATE_DIR", previousStateDir, 1)
            } else {
                unsetenv("UAGENT_STATE_DIR")
            }
    ...
    func normalizeCanvasHostUrlPreservesExplicitSecureCanvasPort() {
        let normalized = canonicalizeCanvasHostUrl(
            raw: "https://canvas.example.com:9443/__uagent__/cap/token",
            activeURL: URL(string: "wss://gateway.example.com")!)

        #expect(normalized == "https://canvas.example.com:9443/__uagent__/cap/token")
    }

    @Test
    func normalizeCanvasHostUrlBackfillsGatewayHostForLoopbackCanvas() {
        let normalized = canonicalizeCanvasHostUrl(
            raw: "http://127.0.0.1:18789/__uagent__/cap/token",
            activeURL: URL(string: "wss://gateway.example.com:7443")!)

        #expect(normalized == "https://gateway.example.com:7443/__uagent__/cap/token")
    }
