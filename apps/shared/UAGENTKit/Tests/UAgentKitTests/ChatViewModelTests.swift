...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> UAGENTChatHistoryPayload
{
    UAGENTChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> UAGENTChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> UAGENTChatSessionEntry
{
    UAGENTChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> UAGENTChatModelChoice {
    UAGENTChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [UAGENTChatHistoryPayload],
    sessionsResponses: [UAGENTChatSessionsListResponse] = [],
    modelResponses: [[UAGENTChatModelChoice]] = [],
    ...
    -> (TestChatTransport, UAGENTChatViewModel)
{
...
private func sendUserMessage(_ vm: UAGENTChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, UAGENTChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [UAGENTChatHistoryPayload]
    private let sessionsResponses: [UAGENTChatSessionsListResponse]
    private let modelResponses: [[UAGENTChatModelChoice]]
    ...
    private let stream: AsyncStream<UAGENTChatTransportEvent>
    private let continuation: AsyncStream<UAGENTChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<UAGENTChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> UAGENTChatHistoryPayload {
        ...
        return self.historyResponses.last ?? UAGENTChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [UAGENTChatAttachmentPayload]) async throws -> UAGENTChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return UAGENTChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> UAGENTChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? UAGENTChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: UAGENTChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = UAGENTChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = UAGENTChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: UAGENTChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = UAGENTChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: UAGENTChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(UAGENTChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == UAGENTChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = UAGENTChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: UAGENTChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = UAGENTChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = UAGENTChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = UAGENTChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { UAGENTChatViewModel(sessionKey: "main", transport: transport) }
