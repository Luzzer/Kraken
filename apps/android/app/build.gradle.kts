...
val androidStoreFile = providers.gradleProperty("UAGENT_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("UAGENT_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("UAGENT_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("UAGENT_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set UAGENT_ANDROID_STORE_FILE, " +
            "UAGENT_ANDROID_STORE_PASSWORD, UAGENT_ANDROID_KEY_ALIAS, and " +
            "UAGENT_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "UAGENT_ENABLE_SMS", "false")
            buildConfigField("boolean", "UAGENT_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "UAGENT_ENABLE_SMS", "true")
            buildConfigField("boolean", "UAGENT_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "uagent-$versionName-$buildType.apk"
                    } else {
                        "uagent-$versionName-$flavorName-$buildType.apk"
                    }
