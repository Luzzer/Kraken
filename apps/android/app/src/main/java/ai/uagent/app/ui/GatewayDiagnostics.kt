...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this UAGENT Android gateway connection failure.
    ...
    - tell me whether `uagent devices list` should show a pending pairing request
    - if more signal is needed, ask for `uagent qr --json`, `uagent devices list`, and `uagent nodes status`
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("UAGENT gateway diagnostics", report))
