...
private const val NOTIFICATION_CHANNEL_BASE_ID = "uagent.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "UAGENT Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "UAGENT Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "UAGENT Active")
      }
